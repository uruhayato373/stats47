...
> **用語**: ここで扱う DB は **ローカルビルド DB (SQLite)** であり、現在は Cloudflare D1 サービスではない（本番 D1 binding は Phase 8 / リモート D1 は 2026-04-29 解約済み、本番 Web app は R2 snapshot のみ読む）。SSOT (Drizzle スキーマ / TS-config / R2) から再生成可能なビルド時派生キャッシュ + 集計エンジン。詳細: 下記「データ管理アーキテクチャ (完全DBレス)」。本 README 内に残る「リモート D1」「`wrangler d1 execute`」「`/pull-remote-d1`」等の記述は解約前の歴史的手順（無効）。
...
- 本番 D1 binding は Phase 8 で削除済み。リモート D1 は 2026-04-29 に解約済み (残数 0)。Phase F (2026-05-30) で
  アプリ実行時の Drizzle query は **0** になり本番デプロイ済。本番 Web app は **R2 snapshot のみ**読む。
...
- **初回 seed（R2 に一度も push していない最初の1回）だけは、実データを持つ環境（旧 miniflare DB が残る Mac）から push する**必要がある。他環境の migrate では空 schema しか作れない:
  ...
  ```
- 要 env: `R2_S3_ENDPOINT` / `R2_ACCESS_KEY_ID` / `R2_SECRET_ACCESS_KEY`（`.env.local` か環境変数）。詳細: 本 README「データ管理アーキテクチャ (完全DBレス)」
