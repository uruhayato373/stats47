...
describe('resolveSurveyTaxonomy', () => {
  ...
  it.each([
    ...
    'SSDS 原典 %s/%s を master survey %s へ正規化する',
    (statsDataId, cdCat01, surveyId) => {
      const result = resolveSurveyTaxonomy(
  ...
  it.each([
    ...
    'registry 外の一次統計 %s も statsDataId override で %s へ解決する',
    (statsDataId, surveyId) => {
      const result = resolveSurveyTaxonomy(
  ...
  it('幽霊 metric と未登録 statsDataId を未解決として保持する', () => {
    ...
    expect(result.unresolvedEstatReferences).toEqual([
  ...
  it.each(['future-population', 'future-population-change-rate-2050'])(
    'IPSS 指標 %s の版付き正式名を既存の調査へ接続する',
    (metricKey) => {
      const result = resolveSurveyTaxonomy(
  ...
  it.each([
    ...
    'metric %s を正式な master survey %s へ解決する',
    (metricKey, surveyId) => {
      const result = resolveSurveyTaxonomy(
        { metricKeys: [metricKey] },
        METRICS_REGISTRY
...
describe('blog chart taxonomy', () => {
  it('複数 rankingKey・nested ref・R2 path を決定的に抽出する', () => {
    ...
    expect(refs.rankingKeys).toEqual([
      'metric-e',
      'metric-f',
      'a',
      'b',
  ...
  it('相関・constituents・secondary key・nested params・市区町村 R2 path も再帰的に抽出する', () => {
    const refs = extractBlogChartSourceReferences({
      correlation: {
        base: 'population',
        pair: 'forest-area',
      ...
      constituents: {
        ...
        denominator: 'habitable-area',
      },
      layers: [{ secondaryRankingKey: 'aging-rate' }],
      request: {
        params: {
          statsDataId: '0000010103',
          cdCat01: '#A03506',
      ...
      citySource: 'r2:app/ranking/population-city-13/values.json',
    });
    expect(refs.rankingKeys).toEqual([
      'aging-rate',
      'population',
    ...
    const result = resolveBlogChartSurveyTaxonomy(
      {
        ...
        source:
          'r2:app/stats/volunteer-activity-international-cooperation-15plus/values.json',
      },
      METRICS_REGISTRY
    );
    expect(result.status).toBe('resolved');
  ...
  it('metric 自体が理由付き対象外なら chart も not-applicable にする', () => {
    const registry = {
      'administrative-report': {
        ...
        surveyScopeReason:
          '行政業務の登録を集計した値で、統計調査を原典としないため',
      },
    } as MetricRegistry;
    const result = resolveBlogChartSurveyTaxonomy(
      { kind: 'ranking', rankingKey: 'administrative-report' },
      registry
    );
  ...
  it('authored は survey 対象外、欠落 manifest は missing-lineage', () => {
    expect(
      resolveBlogChartSurveyTaxonomy({ kind: 'authored' }, METRICS_REGISTRY)
        .status
    ).toBe('not-applicable');
    expect(
      resolveBlogChartSurveyTaxonomy(null, {} as MetricRegistry).status
    ).toBe('missing-lineage');
  });
});
