...
async function main() {
  ...
  if (
    ...
    throw Error('Flood source version/publication policy changed');
  const root = path.resolve('.local/r2');
  const temp = path.join(os.tmpdir(), 'stats47-flood-view');
  mkdirSync(temp, { recursive: true });
  const response = await fetch(
    `${base}/app/geo/population-flood-risk/manifest.json`,
    { signal: AbortSignal.timeout(60_000) }
  );
  if (!response.ok)
    throw Error(`Flood analysis manifest HTTP ${response.status}`);
  ...
  const sources = FLOOD_ARCHIVES.map((source) => {
    const matches = sourceOutputs.filter((output) => output.key === source.key);
    if (
      matches.length !== 1 ||
      !/^[a-f0-9]{64}$/.test(matches[0].sha256) ||
      !Number.isSafeInteger(matches[0].recordCount) ||
      matches[0].recordCount < 0
    )
      throw Error(`Invalid flood source evidence ${source.key}`);
    return matches[0];
  });
  const assets: {
    key: string;
  ...
  async function processSource(
    ...
    // Each ZIP has been verified and fully split; do not accumulate all 201 on the CI disk.
    if (process.env.CI === 'true') unlinkSync(cache);
    console.log(
