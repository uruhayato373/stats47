...
export class EstatStatsListFormatter {
  ...
  static formatStatsListData(
    ...
    logger.debug("Formatter: formatStatsListData 開始");
    ...
    logger.debug(
      {
        ...
        hasTABLE_INF: !!datalist.TABLE_INF,
        hasRESULT_INF: !!datalist.RESULT_INF,
      },
      "Formatter: DATALIST_INF構造"
    );
    ...
    if (!tables) {
      logger.warn(
        {
          NUMBER: datalist.NUMBER,
          TABLE_INF_exists: !!datalist.TABLE_INF,
          RESULT_INF: datalist.RESULT_INF,
          datalistKeys: Object.keys(datalist),
        },
        "Formatter: テーブル情報が見つかりません"
      );
      return {
    ...
    logger.debug({ count: tableArray.length }, `Formatter: テーブル数: ${tableArray.length}`);
    ...
    logger.info(
      { duration, count: formattedTables.length },
      `Formatter: formatStatsListData 完了 (${duration}ms) - ${formattedTables.length}件`
    );
    logger.debug({ result }, "Formatter: 結果");
    return result;
  }
