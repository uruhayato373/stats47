...
export function extractKanjiClaims(text: string): KanjiClaim[] {
  ...
  for (const m of body.matchAll(KANJI_NUMERAL_PATTERN)) {
    ...
    // ★単独のスケール文字は単位語であって漢数字主張ではない (2026-08-12)。
    //   「千円や万円という金額」「労働費は45,596,934千円」の 千 / 万 は単位の一部で、
    //   算用数字への統一という規約に反していない。「三千円」のように前に数詞が付く形は
    //   raw が複数文字になるので、この skip では取りこぼさない。
    if (/^[千万億兆]$/.test(raw)) continue;
