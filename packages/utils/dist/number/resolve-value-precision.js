import { getMaxDecimalPlaces } from "./get-max-decimal-places";
/**
 ...
 */
export function resolveValuePrecision(values, maxDecimals = 2) {
    const finite = values.filter((v) => Number.isFinite(v));
    if (finite.length === 0)
        return 0;
    return Math.min(getMaxDecimalPlaces(finite), maxDecimals);
}
