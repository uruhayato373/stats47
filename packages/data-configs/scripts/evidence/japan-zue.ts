...
async function main(): Promise<void> {
  ...
  if (command === "resolve") {
    ...
    const summary = buildSummary(document, items);
    if (check) {
      const storedItems = await readFile(ITEMS_PATH, "utf8");
      const storedSummary = await readFile(SUMMARY_PATH, "utf8");
      if (storedItems !== generatedItemsSource(items, document.candidatesSha256)) errors.push("generated items drift");
      if (storedSummary !== stableJson(summary)) errors.push("summary drift");
      if (errors.length) throw new Error(errors.join("\n"));
