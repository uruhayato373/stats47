...
function catalog(
  groups: ThemeCatalog['metricGroups'],
  keys: string[]
): ThemeCatalog {
  return {
    key: 'test-theme',
...
function run(c: ThemeCatalog) {
  const errors: string[] = [];
  const warns: string[] = [];
  validateMetricGroups(
    c,
    new Set(c.metrics.map((m) => m.rankingKey)),
    errors,
    warns
  );
  return { errors, warns };
...
describe('validateMetricGroups — error', () => {
        ...
        [
          {
            key: 'g',
            title: 'G',
            rankingKeys: [YEN, PERCENT, TIMES],
            defaultCheckedKeys: [YEN],
          },
        ],
        [YEN, PERCENT, TIMES]
      )
  ...
  it('defaultCheckedKeys が空なら弾く (系列ゼロでカードが開く)', () => {
    const { errors } = run(
      catalog(
        [{ key: 'g', title: 'G', rankingKeys: [YEN], defaultCheckedKeys: [] }],
        [YEN]
      )
    );
  ...
  it('rankingKeys が空なら弾く', () => {
    const { errors } = run(
      catalog(
        [{ key: 'g', title: 'G', rankingKeys: [], defaultCheckedKeys: [YEN] }],
        [YEN]
      )
    );
...
describe('validateMetricGroups — warn', () => {
  it('概況の比較表で読める指標は旧カードの未所属警告から除く', () => {
    const { errors, warns } = run({
      ...catalog(
        [
          {
            key: 'g',
      ...
      overview: {
        introduction: '主要指標の比較',
        headlineRankingKeys: [YEN],
        comparisonRankingKeys: [YEN, PERCENT],
        mapNotes: { [YEN]: '金額', [PERCENT]: '割合' },
      },
    });
    expect(errors).toEqual([]);
  ...
  it('概況があっても比較表と旧カードの両方にない指標は警告する', () => {
    const { warns } = run({
      ...catalog(
        [
          {
    ...
    expect(
      warns.some(
        (warning) =>
          warning.startsWith('[group-orphan]') && warning.includes(PERCENT)
      )
    ).toBe(true);
  });

  it('概況で表示していても旧カードの単位不整合は拒否する', () => {
    const { errors } = run({
      ...catalog(
        [
          {
      ...
      overview: {
        introduction: '主要指標の比較',
        headlineRankingKeys: [YEN],
        comparisonRankingKeys: [YEN, PERCENT, TIMES],
        mapNotes: { [YEN]: '金額', [PERCENT]: '割合', [TIMES]: '倍率' },
      },
    });
    expect(errors.some((error) => error.startsWith('[group-units]'))).toBe(
      true
    );
  });
  ...
  it('初期チェックが 4 件以上なら warn する (初期取得が増える)', () => {
    ...
    const { warns } = run(
      catalog(
        [{ key: 'g', title: 'G', rankingKeys: keys, defaultCheckedKeys: keys }],
        keys
      )
    );
