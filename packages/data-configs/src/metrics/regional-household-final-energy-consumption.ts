...
export const regionalHouseholdFinalEnergyConsumption: MetricConfig = {
  key: 'regional-household-final-energy-consumption',
  title: '家庭部門の最終エネルギー消費量',
  description: '都道府県別エネルギー消費統計の家庭部門における直接利用分。',
  note: '2023年度の暫定値。原表のエネルギー単位表にある直接利用分の合計（TJ）を使用し、非エネルギー利用を含む。電力・熱の寄与損失を配分した総合計や炭素排出量とは異なる。対象は産業、業務他、家庭、運輸のうち家庭乗用車で、営業用輸送等を含む全国全体のエネルギー消費ではない。部門ごとに直接集計と按分推計を組み合わせている。公表一覧の調査年月は2024年度だが、使用する観測値は原表に明記された2023年度。家庭部門（700000）に家庭乗用車の走行分は含めず、運輸として別に扱う。',
  unit: 'TJ',
  category: 'energy',
  source: {
    kind: 'external',
