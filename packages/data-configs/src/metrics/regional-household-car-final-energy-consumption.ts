...
export const regionalHouseholdCarFinalEnergyConsumption: MetricConfig = {
  key: 'regional-household-car-final-energy-consumption',
  title: '家庭乗用車の最終エネルギー消費量',
  description:
    '都道府県別エネルギー消費統計の運輸部門として推計される家庭乗用車の直接利用分。',
  note: '2023年度の暫定値。原表のエネルギー単位表にある直接利用分の合計（TJ）を使用し、非エネルギー利用を含む。電力・熱の寄与損失を配分した総合計や炭素排出量とは異なる。対象は産業、業務他、家庭、運輸のうち家庭乗用車で、営業用輸送等を含む全国全体のエネルギー消費ではない。部門ごとに直接集計と按分推計を組み合わせている。公表一覧の調査年月は2024年度だが、使用する観測値は原表に明記された2023年度。原表の運輸（800000）=旅客（810000）=乗用車（811000）。家計調査のガソリン購入数量による推計で、旅客・貨物輸送全体の消費量ではない。',
  unit: 'TJ',
  category: 'energy',
  source: {
    kind: 'external',
