...
describe('theme chart indicator-hub links', () => {
  ...
  it('全31 data-bound componentがテーマ内指標へ紐付く', () => {
  ...
  it('現行チャートが参照する57指標すべてに個別の説明を必須化する', () => {
    const errors: string[] = [];
    const warns: string[] = [];
    ...
    expect(coverage.totalKeys).toBe(57);
    expect(coverage.missingDescriptionKeys).toHaveLength(0);
    expect(coverage.authoredNoteKeys).toHaveLength(28);
    expect(errors).toEqual([]);
