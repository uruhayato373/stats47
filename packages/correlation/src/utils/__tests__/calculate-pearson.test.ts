...
describe('calculateMatchedPearsonR', () => {
  it('matches the array implementation without allocating scatter data', () => {
    const x = [
      { areaCode: '01', value: 1 },
      { areaCode: '02', value: 2 },
      { areaCode: '03', value: 3 },
      { areaCode: '04', value: 999 },
    ];
    const y = new Map([
      ['03', 9],
      ['01', 3],
    ...
    const result = calculateMatchedPearsonR(x, y);

    expect(result.count).toBe(3);
    expect(result.r).toBeCloseTo(calculatePearsonR([1, 2, 3], [3, 6, 9]).r, 12);
  });

  it('reports the matched count when there are too few points', () => {
    expect(
      calculateMatchedPearsonR(
        [{ areaCode: '01', value: 1 }],
        new Map([['01', 2]])
  ...
  it('preserves buildScatterData semantics when the X side contains duplicate areas', () => {
    const x = [
      { areaCode: '01', value: 1 },
      { areaCode: '01', value: 2 },
      { areaCode: '02', value: 4 },
    ...
    expect(result.r).toBeCloseTo(
      calculatePearsonR([1, 2, 4], [10, 10, 30]).r,
      12
    );
  });
});
