...
| `sns-content-standards.md` | SNS 投稿 (X/IG/YouTube pilot/note) の企画・生成・投稿・計測 (チャネル戦略・頻度リミット・投稿雛形・投稿台帳 posts.json・YouTube 通常動画 pilot・TikTok 撤退・**統合メディアコンソールと R2 素材保持 §5.5** の正典)。**運営を横断管理する管理コンソールは `npm run admin` → http://127.0.0.1:4747/ (10 画面: 制作=/sns・/buzz-map、資産=/assets・/svg、収益=/revenue・/ads、品質運用=/dashboard・/quality・/ops・/todo。書き込みは /sns の X/IG 投稿予約と /buzz-map の素材生成だけで他は読み取り専用。skill `/admin-console`)** |
