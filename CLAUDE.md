...
| 市区町村コロプレスマップ設計 | `docs/01_技術設計/07_市区町村コロプレスマップ設計.md` |
...
| **データ基盤の全景・D1/R2 同期・配信フロー** | **`docs/01_技術設計/11_データ基盤設計.md` ★データ操作時は必ず参照（唯一の真実源）** |
| DB 操作詳細（スキーマ・seed・Drizzle） | `packages/database/README.md` |
...
- **ローカル D1/R2 は `.local/d1/` に統一。** `wrangler.toml` の `persist_to = "../../.local/d1"` と `next.config.ts` の `initOpenNextCloudflareForDev({ persist: { path: "../../.local/d1" } })` で、dev server が同じデータを参照する。**`apps/web/.wrangler/state/` は使わない。**
...
- **新規 PC セットアップ**: ローカル D1 (8.4GB) は git ignore されているため、メイン PC からのファイル転送で取得する（リモート D1 は撤廃済みのため pull できない）

## データ配信フロー

詳細は **`docs/01_技術設計/11_データ基盤設計.md`** を参照（唯一の真実源）。要点:

- **単一 PC 運用前提**。ローカル D1 (8.4 GB) が source of truth、本番配信は R2 snapshot のみ
- **リモート D1 は Phase 10 (2026-04-29) で完全撤廃**（`wrangler d1 list` → `[]`）。`apps/web` (Cloudflare Workers) には D1 binding が無く、R2 snapshot のみを fetch する

```
ローカル D1（編集元）──/export-snapshots──▶ ローカル R2 ──/push-r2──▶ R2 bucket（本番配信）
```

- 作業後は `/export-snapshots` で全 snapshot を出力 → `/push-r2` でリモート R2 に push
- 緊急ロールバック: ローカル D1 から再 export、または `wrangler deployments rollback`
- ローカル D1 は git 管理されない（巨大すぎるため）。複数 PC 運用は非対応（`11_データ基盤設計.md` §6.4 参照）

> **廃止スキル**: `/sync-remote-d1`, `/pull-remote-d1`, `/diff-d1`, `/purge-cache-r2` はリモート D1 撤廃に伴い 2026-05 削除済み。古いドキュメント・agent 内の参照を見つけたら新フローに書き換えること。
...
- **browser-use を使うスキル / スクリプトは終了時に必ず daemon 停止 + Chrome タブクローズすること。** `browser-use ... close` は page を閉じるだけで `browser_use.skill_cli.daemon` を止めない。さらに `--profile "Profile 1"` で起動した場合は **ユーザーの実 Chrome 内にタブを開く** ため daemon kill 後もタブが残る（2026-04-25 検証で daemon 6 個 + note タブ 5 個残存）。bash スクリプトでは 3 段すべて trap 必須:
  ```bash
  trap '
  ...
  ' EXIT INT TERM
  ```
  Node.js orchestrator では `process.on('exit')` 等で同等処理を spawn する。対象スキル: `publish-note` / `fetch-note-metrics` / `post-instagram` / `post-x` / `post-tiktok` / `post-youtube` 等 browser-use 系全般。AppleScript の URL 条件は対象サイトに合わせて調整（note 系: `editor.note.com` `note.com/sitesettings` `note.com/login`）
- **DB データを変更する場合はローカル D1 SQLite を直接操作すること。** 本番反映は `/export-snapshots` で R2 snapshot を生成 → `/push-r2` でリモート R2 に push する（詳細: `docs/01_技術設計/11_データ基盤設計.md`）。
...
# 本番反映（ローカル D1 → R2 snapshot → リモート R2）
/export-snapshots
/push-r2
```
