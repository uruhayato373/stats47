...
- **browser-use を使うスキル / スクリプトは終了時に必ず daemon を停止すること。** `browser-use ... close` は page を閉じるだけで `browser_use.skill_cli.daemon` プロセスを止めない。残ると Chromium インスタンスが累積し、次回実行で 60s timeout や OS 全体が重くなる原因に（2026-04-25 検証で 6 個残存確認）。bash スクリプトでは:
  ```bash
  trap '
  ...
  ' EXIT INT TERM
  ```
  Node.js orchestrator では `process.on('exit')` 等で同等の `pkill` を spawn する。対象スキル: `publish-note` / `fetch-note-metrics` / `post-instagram` / `post-x` / `post-tiktok` / `post-youtube` 等 browser-use 系全般
