...
Agent tool 経由で subagent を呼ぶ際は prompt の **冒頭** に下記 Output Format テンプレ (A/B/C) のいずれかを必ず含める。

- 制約は prompt 末尾ではなく **冒頭** (agent 側で format が最初に固定されないと末尾の指示は無視される)
...
❌ NG (末尾に書いて無視されるパターン):
```
docs/01_技術設計/ の 11 ファイルを KEEP/DELETE/MOVE-TO-D1 に分類して。各ファイルの content / authority / redundancy を考慮。
... (中略) ...
