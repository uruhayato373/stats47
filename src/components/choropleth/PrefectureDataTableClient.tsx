...
import { FormattedValue, SubcategoryData } from '@/types/choropleth';
...
export const PrefectureDataTableClient: React.FC<PrefectureDataTableClientProps> = ({
  data,
  subcategory,
  className = ''
}) => {
  ...
  if (!data || !Array.isArray(data) || data.length === 0) {
    return (
      <div className={`bg-white dark:bg-neutral-800 rounded-lg border border-gray-200 dark:border-neutral-700 ${className}`}>
    ...
    <div className={`bg-white dark:bg-neutral-800 rounded-lg border border-gray-200 dark:border-neutral-700 ${className}`}>
              ...
              {sortedData.map((item, index) => (
                ...
                >
                  <td className="px-3 py-2 whitespace-nowrap">
                    <span className={`text-sm ${getRankStyle(item.rank || index + 1)}`}>
                      {item.rank || index + 1}
                    </span>
                  </td>
                  <td className="px-3 py-2 whitespace-nowrap">
      ...
      <div className="px-4 py-3 bg-gray-50 dark:bg-neutral-700 rounded-b-lg">
        <div className="flex justify-between items-center text-xs text-gray-500 dark:text-neutral-400">
          ...
          <span>
            データ種別: {subcategory.dataType === 'numerical' ? '数値' :
