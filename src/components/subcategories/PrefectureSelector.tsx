...
export const PrefectureSelector: React.FC<PrefectureSelectorProps> = ({
  ...
  const handlePrefectureChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    if (selectedAreaCode === '00000') {
      // 全国を選択した場合は、サブカテゴリページへ遷移
      router.push(`/${category.id}/${subcategory.id}`);
    } else {
      // 都道府県を選択した場合は、都道府県別ページへ遷移
      router.push(`/${category.id}/${subcategory.id}/${selectedAreaCode}`);
    }
  };

  return (
    <div className="flex items-center gap-2">
      <label htmlFor="prefecture-select" className="text-sm text-gray-700 dark:text-gray-300 font-medium whitespace-nowrap">
        地域:
      </label>
      <select
