...
export function transformEstatToFormattedValues(
  ...
  if (
    !(estatResponse.GET_STATS_DATA?.STATISTICAL_DATA?.DATA_INF as any)?.DATA_OBJ
  ) {
    return result;
  }

  const dataObj = (
    estatResponse.GET_STATS_DATA.STATISTICAL_DATA.DATA_INF as any
  ).DATA_OBJ;
  const classTables =
  ...
  if ((classTables as any).CLASS_OBJ_01?.CLASS) {
    (classTables as any).CLASS_OBJ_01.CLASS.forEach((cls: any) => {
    ...
    dataObj.forEach((data: any) => {
      ...
      if (areaCode && areaCode !== "00000" && value) {
        const normalizedAreaCode =
          PREFECTURE_CODE_MAP[areaCode] || areaCode.slice(-2);
        const areaName =
          areaTable.get(areaCode) ||
          PREFECTURE_NAMES[normalizedAreaCode] ||
          "不明";
