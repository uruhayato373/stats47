...
export class EstatMapDataService {
  ...
  private static convertToMapDataset(data: FormattedEstatData): MapDataset {
    ...
    // カテゴリ集計
    const categoryStats = this.aggregateCategories(
      data.categories,
      data.values
    );
   ...
   * カテゴリを集計
   */
  private static aggregateCategories(
    categories: FormattedArea[],
  ...
  private static aggregateYears(years: any[], values: FormattedValue[]) {
    ...
    values.forEach((value) => {
      if (value.yearInfo) {
        yearCount.set(
          value.yearInfo.code,
          (yearCount.get(value.yearInfo.code) || 0) + 1
        );
      }
    });
