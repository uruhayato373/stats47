...
import { EstatIdValidationError } from "@/features/estat-api/core/errors";
 ...
 *
 * @example
 * generateEstatIdRange("0000010101", "0000010103")
 * // => ["0000010101", "0000010102", "0000010103"]
 */
export function generateEstatIdRange(startId: string, endId: string): string[] {
 ...
 * // => "0000010101"
 */
export function formatEstatId(num: number): string {
 ...
 * @returns 妥当な場合true
 */
export function validateEstatId(id: string): boolean {
  // 基本的な形式チェック
  if (!/^\d{10}$/.test(id)) {
    return false;
  }
 ...
 */
export function validateEstatIdFormat(id: string): {
  valid: boolean;
  error?: string;
} {
  if (typeof id !== "string") {
  ...
  if (num > 9999999999) {
    return {
      valid: false,
      error: "IDは9999999999以下である必要があります",
    };
  }
 ...
 */
export function normalizeEstatId(id: string): string {
  const validation = validateEstatIdFormat(id);
  if (!validation.valid) {
