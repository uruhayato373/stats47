...
const CAT01_REGISTERED_FOREIGNER_POPULATION = "A3100"; // 外国人登録人口

interface RegisteredForeignerPopulationCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 外国人登録人口統計カード
 */
export async function RegisteredForeignerPopulationCard({
  areaCode,
  timeCode,
  title = "外国人登録人口",
}: RegisteredForeignerPopulationCardProps) {
  try {
    // e-Stat APIから外国人登録人口データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_REGISTERED_FOREIGNER_POPULATION,
      areaFilter: areaCode,
    ...
    console.error(
      "[RegisteredForeignerPopulationCard] データ取得エラー:",
      error
    );
    return (
      <Card>
