...
const CAT01_RESIDENT_FOREIGNER_POPULATION = "A3200"; // 在留外国人数

interface ResidentForeignerPopulationCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 在留外国人数統計カード
 */
export async function ResidentForeignerPopulationCard({
  areaCode,
  timeCode,
  title = "在留外国人数",
}: ResidentForeignerPopulationCardProps) {
  try {
    // e-Stat APIから在留外国人数データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_RESIDENT_FOREIGNER_POPULATION,
      areaFilter: areaCode,
    ...
    console.error(
      "[ResidentForeignerPopulationCard] データ取得エラー:",
      error
    );
    return (
      <Card>
