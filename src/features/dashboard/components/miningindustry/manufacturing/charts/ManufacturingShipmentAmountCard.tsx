...
const CAT01_MANUFACTURING_SHIPMENT = "C3401"; // 製造品出荷額等

interface ManufacturingShipmentAmountCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 製造品出荷額等統計カード
 */
export async function ManufacturingShipmentAmountCard({
  areaCode,
  timeCode,
  title = "製造品出荷額等",
}: ManufacturingShipmentAmountCardProps) {
  try {
    // e-Stat APIから製造品出荷額等データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_MANUFACTURING_SHIPMENT,
      areaFilter: areaCode,
    ...
    console.error(
      "[ManufacturingShipmentAmountCard] データ取得エラー:",
      error
    );
    return (
      <Card>
