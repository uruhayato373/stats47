...
const CAT01_MAJOR_ROAD_LENGTH = "H7111"; // 道路実延長（主要道路）

interface MajorRoadLengthCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 道路実延長（主要道路）統計カード
 */
export async function MajorRoadLengthCard({
  areaCode,
  timeCode,
  title = "道路実延長（主要道路）",
}: MajorRoadLengthCardProps) {
  try {
    // e-Stat APIから道路実延長（主要道路）データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_MAJOR_ROAD_LENGTH,
      areaFilter: areaCode,
    ...
    console.error("[MajorRoadLengthCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
