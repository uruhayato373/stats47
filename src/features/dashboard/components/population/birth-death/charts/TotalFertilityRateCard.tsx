...
const CAT01_TOTAL_FERTILITY_RATE = "A4103"; // 合計特殊出生率

interface TotalFertilityRateCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 合計特殊出生率統計カード
 */
export async function TotalFertilityRateCard({
  areaCode,
  timeCode,
  title = "合計特殊出生率",
}: TotalFertilityRateCardProps) {
  try {
    // e-Stat APIから合計特殊出生率データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_TOTAL_FERTILITY_RATE,
      areaFilter: areaCode,
    ...
    console.error("[TotalFertilityRateCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
