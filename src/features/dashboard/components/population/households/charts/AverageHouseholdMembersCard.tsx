...
const CAT01_AVERAGE_MEMBERS = "A710211"; // 一般世帯平均人員数

interface AverageHouseholdMembersCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 一般世帯平均人員数統計カード
 */
export async function AverageHouseholdMembersCard({
  areaCode,
  timeCode,
  title = "一般世帯平均人員数",
}: AverageHouseholdMembersCardProps) {
  try {
    // e-Stat APIから一般世帯平均人員数データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_AVERAGE_MEMBERS,
      areaFilter: areaCode,
    ...
    console.error("[AverageHouseholdMembersCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
