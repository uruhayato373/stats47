...
const CAT01_GENERAL_CLINIC_COUNT = "I5102"; // 一般診療所数

interface GeneralClinicCountCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 一般診療所数統計カード
 */
export async function GeneralClinicCountCard({
  areaCode,
  timeCode,
  title = "一般診療所数",
}: GeneralClinicCountCardProps) {
  try {
    // e-Stat APIから一般診療所数データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_GENERAL_CLINIC_COUNT,
      areaFilter: areaCode,
    ...
    console.error("[GeneralClinicCountCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
