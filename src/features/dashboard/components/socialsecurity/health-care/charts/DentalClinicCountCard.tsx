...
const CAT01_DENTAL_CLINIC_COUNT = "I5103"; // 歯科診療所数

interface DentalClinicCountCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 歯科診療所数統計カード
 */
export async function DentalClinicCountCard({
  areaCode,
  timeCode,
  title = "歯科診療所数",
}: DentalClinicCountCardProps) {
  try {
    // e-Stat APIから歯科診療所数データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_DENTAL_CLINIC_COUNT,
      areaFilter: areaCode,
    ...
    console.error("[DentalClinicCountCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
