...
interface HospitalCountCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 病院数統計カード
 */
export async function HospitalCountCard({
  areaCode,
  timeCode,
  title = "病院数",
}: HospitalCountCardProps) {
  try {
    // e-Stat APIから病院数データを取得
    ...
    console.error("[HospitalCountCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
