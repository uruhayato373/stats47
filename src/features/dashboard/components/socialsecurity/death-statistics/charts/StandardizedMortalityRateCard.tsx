...
const CAT01_STANDARDIZED_MORTALITY_RATE = "A4301"; // 標準化死亡率〔日本人〕

interface StandardizedMortalityRateCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 標準化死亡率統計カード
 */
export async function StandardizedMortalityRateCard({
  areaCode,
  timeCode,
  title = "標準化死亡率",
}: StandardizedMortalityRateCardProps) {
  try {
    // e-Stat APIから標準化死亡率データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_STANDARDIZED_MORTALITY_RATE,
      areaFilter: areaCode,
    ...
    console.error(
      "[StandardizedMortalityRateCard] データ取得エラー:",
      error
    );
    return (
      <Card>
