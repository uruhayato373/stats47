...
const CAT01_DEATH_COUNT_65_PLUS = "A4231"; // 死亡数（65歳以上）

interface DeathCount65PlusCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 死亡数（65歳以上）統計カード
 */
export async function DeathCount65PlusCard({
  areaCode,
  timeCode,
  title = "死亡数（65歳以上）",
}: DeathCount65PlusCardProps) {
  try {
    // e-Stat APIから死亡数（65歳以上）データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_DEATH_COUNT_65_PLUS,
      areaFilter: areaCode,
    ...
    console.error("[DeathCount65PlusCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
