/**
 * 年度別死亡数推移チャートコンポーネント（Server Component）
...
import { DeathCountTrendChartClient } from "./DeathCountTrendChartClient";

// e-Stat APIパラメータ定義
const STATS_DATA_ID = "0000010101"; // 都道府県データ 基礎データ
const CAT01_DEATH_COUNT = "A4200"; // 死亡数

interface DeathCountTrendChartProps {
  /** 地域コード */
  areaCode: string;
  /** タイトル */
  title: string;
...
/**
 * 年度別死亡数推移チャート（Server Component）
 */
export async function DeathCountTrendChart({
  areaCode,
  title,
  description,
}: DeathCountTrendChartProps) {
  try {
    // e-Stat APIから死亡数データを取得（全年度）
    ...
    console.error("[DeathCountTrendChart] データ取得エラー:", error);
    return (
      <DeathCountTrendChartClient
        chartData={[]}
        title={title}
