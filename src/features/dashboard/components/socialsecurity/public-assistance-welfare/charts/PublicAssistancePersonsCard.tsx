...
const CAT01_PUBLIC_ASSISTANCE_PERSONS = "J1105"; // 生活保護被保護実人員

interface PublicAssistancePersonsCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 生活保護被保護実人員統計カード
 */
export async function PublicAssistancePersonsCard({
  areaCode,
  timeCode,
  title = "生活保護被保護実人員",
}: PublicAssistancePersonsCardProps) {
  try {
    // e-Stat APIから生活保護被保護実人員データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_PUBLIC_ASSISTANCE_PERSONS,
      areaFilter: areaCode,
    ...
    console.error("[PublicAssistancePersonsCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
