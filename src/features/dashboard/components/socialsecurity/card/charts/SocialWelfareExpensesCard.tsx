...
interface SocialWelfareExpensesCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 社会福祉費（都道府県財政）統計カード
 */
export async function SocialWelfareExpensesCard({
  areaCode,
  timeCode,
  title = "社会福祉費（都道府県財政）",
}: SocialWelfareExpensesCardProps) {
  try {
    // e-Stat APIから社会福祉費データを取得
    ...
    console.error("[SocialWelfareExpensesCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
