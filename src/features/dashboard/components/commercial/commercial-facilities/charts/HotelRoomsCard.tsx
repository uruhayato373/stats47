...
const CAT01_HOTEL_ROOMS = "C3804"; // ホテル営業施設客室数

interface HotelRoomsCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * ホテル営業施設客室数統計カード
 */
export async function HotelRoomsCard({
  areaCode,
  timeCode,
  title = "ホテル営業施設客室数",
}: HotelRoomsCardProps) {
  try {
    // e-Stat APIからホテル営業施設客室数データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_HOTEL_ROOMS,
      areaFilter: areaCode,
    ...
    console.error("[HotelRoomsCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
