...
const CAT01_COMMERCIAL_ESTABLISHMENTS = "C3502"; // 商業事業所数（卸売業＋小売業）

interface CommercialEstablishmentsCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 商業事業所数統計カード
 */
export async function CommercialEstablishmentsCard({
  areaCode,
  timeCode,
  title = "商業事業所数",
}: CommercialEstablishmentsCardProps) {
  try {
    // e-Stat APIから商業事業所数データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_COMMERCIAL_ESTABLISHMENTS,
      areaFilter: areaCode,
    ...
    console.error("[CommercialEstablishmentsCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
