...
const CAT01_BUSINESS_TAX = "D4203"; // 事業税
 ...
 * 事業税統計カード
 */
export async function BusinessTaxCard({
  areaCode,
  timeCode,
  title = "事業税",
}: BusinessTaxCardProps) {
  try {
    // e-Stat APIから事業税データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_BUSINESS_TAX,
      areaFilter: areaCode,
    ...
    console.error("[BusinessTaxCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
