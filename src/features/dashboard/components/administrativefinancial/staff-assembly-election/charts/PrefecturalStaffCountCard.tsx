...
interface PrefecturalStaffCountCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 */
export async function PrefecturalStaffCountCard({
  areaCode,
  timeCode,
  title = "一般行政部門職員数（都道府県）",
}: PrefecturalStaffCountCardProps) {
  try {
    // e-Stat APIから一般行政部門職員数データを取得
    ...
    console.error("[PrefecturalStaffCountCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
