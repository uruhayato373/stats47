...
const CAT01_VOTER_LIST_REGISTRANTS = "D1212"; // 選挙人名簿登録者数

interface VoterListRegistrantsCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 選挙人名簿登録者数統計カード
 */
export async function VoterListRegistrantsCard({
  areaCode,
  timeCode,
  title = "選挙人名簿登録者数",
}: VoterListRegistrantsCardProps) {
  try {
    // e-Stat APIから選挙人名簿登録者数データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_VOTER_LIST_REGISTRANTS,
      areaFilter: areaCode,
    ...
    console.error("[VoterListRegistrantsCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
