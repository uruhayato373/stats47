...
const CAT01_PREFECTURAL_ASSEMBLY_MEMBERS = "D1210"; // 都道府県議会議員数

interface PrefecturalAssemblyMembersCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 都道府県議会議員数統計カード
 */
export async function PrefecturalAssemblyMembersCard({
  areaCode,
  timeCode,
  title = "都道府県議会議員数",
}: PrefecturalAssemblyMembersCardProps) {
  try {
    // e-Stat APIから都道府県議会議員数データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_PREFECTURAL_ASSEMBLY_MEMBERS,
      areaFilter: areaCode,
    ...
    console.error("[PrefecturalAssemblyMembersCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
