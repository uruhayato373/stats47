...
const CAT01_INDEPENDENT_REVENUE = "D3102"; // 自主財源額（都道府県財政）

interface IndependentRevenueCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 自主財源額（都道府県財政）統計カード
 */
export async function IndependentRevenueCard({
  areaCode,
  timeCode,
  title = "自主財源額（都道府県財政）",
}: IndependentRevenueCardProps) {
  try {
    // e-Stat APIから自主財源額データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_INDEPENDENT_REVENUE,
      areaFilter: areaCode,
    ...
    console.error("[IndependentRevenueCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
