...
interface FiscalStrengthIndexCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 財政力指数統計カード
 */
export async function FiscalStrengthIndexCard({
  areaCode,
  timeCode,
  title = "財政力指数",
}: FiscalStrengthIndexCardProps) {
  try {
    // e-Stat APIから財政力指数データを取得
    ...
    console.error("[FiscalStrengthIndexCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
