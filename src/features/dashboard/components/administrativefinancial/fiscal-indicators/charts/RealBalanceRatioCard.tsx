...
const CAT01_REAL_BALANCE_RATIO = "D2102"; // 実質収支比率（都道府県財政）

interface RealBalanceRatioCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 実質収支比率統計カード
 */
export async function RealBalanceRatioCard({
  areaCode,
  timeCode,
  title = "実質収支比率",
}: RealBalanceRatioCardProps) {
  try {
    // e-Stat APIから実質収支比率データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_REAL_BALANCE_RATIO,
      areaFilter: areaCode,
    ...
    console.error("[RealBalanceRatioCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
