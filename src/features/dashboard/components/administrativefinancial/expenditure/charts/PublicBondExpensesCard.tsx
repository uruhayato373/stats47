...
const CAT01_PUBLIC_BOND_EXPENSES = "D310409"; // 公債費（都道府県財政）（性質別歳出内訳）

interface PublicBondExpensesCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 公債費（都道府県財政）統計カード
 */
export async function PublicBondExpensesCard({
  areaCode,
  timeCode,
  title = "公債費（都道府県財政）",
}: PublicBondExpensesCardProps) {
  try {
    // e-Stat APIから公債費データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_PUBLIC_BOND_EXPENSES,
      areaFilter: areaCode,
    ...
    console.error("[PublicBondExpensesCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
