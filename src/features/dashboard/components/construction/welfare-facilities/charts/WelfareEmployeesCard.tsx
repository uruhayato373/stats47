...
const CAT01_WELFARE_EMPLOYEES = "C220850"; // 従業者数（民営）（医療、福祉）

interface WelfareEmployeesCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 福祉従業者数統計カード
 */
export async function WelfareEmployeesCard({
  areaCode,
  timeCode,
  title = "福祉従業者数",
}: WelfareEmployeesCardProps) {
  try {
    // e-Stat APIから福祉従業者数データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_WELFARE_EMPLOYEES,
      areaFilter: areaCode,
    ...
    console.error("[WelfareEmployeesCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
