...
interface WelfareEstablishmentsCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 福祉事業所数統計カード
 */
export async function WelfareEstablishmentsCard({
  areaCode,
  timeCode,
  title = "福祉事業所数",
}: WelfareEstablishmentsCardProps) {
  try {
    // e-Stat APIから福祉事業所数データを取得
    ...
    console.error("[WelfareEstablishmentsCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
