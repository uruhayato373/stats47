...
const CAT01_MANUFACTURING_ESTABLISHMENTS = "C3403"; // 製造業事業所数

interface ManufacturingEstablishmentsCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 製造業事業所数統計カード
 */
export async function ManufacturingEstablishmentsCard({
  areaCode,
  timeCode,
  title = "製造業事業所数",
}: ManufacturingEstablishmentsCardProps) {
  try {
    // e-Stat APIから製造業事業所数データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_MANUFACTURING_ESTABLISHMENTS,
      areaFilter: areaCode,
    ...
    console.error("[ManufacturingEstablishmentsCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
