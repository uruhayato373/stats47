...
const CAT01_CONSTRUCTION_ESTABLISHMENTS = "C210836"; // 事業所数（民営）（建設業）

interface ConstructionEstablishmentsCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 建設業事業所数統計カード
 */
export async function ConstructionEstablishmentsCard({
  areaCode,
  timeCode,
  title = "建設業事業所数",
}: ConstructionEstablishmentsCardProps) {
  try {
    // e-Stat APIから建設業事業所数データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_CONSTRUCTION_ESTABLISHMENTS,
      areaFilter: areaCode,
    ...
    console.error("[ConstructionEstablishmentsCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
