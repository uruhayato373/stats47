...
const CAT01_PRIME_CONTRACTOR_COMPLETED_CONSTRUCTION = "C3304"; // 元請完成工事高

interface PrimeContractorCompletedConstructionCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 元請完成工事高統計カード
 */
export async function PrimeContractorCompletedConstructionCard({
  areaCode,
  timeCode,
  title = "元請完成工事高",
}: PrimeContractorCompletedConstructionCardProps) {
  try {
    // e-Stat APIから元請完成工事高データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_PRIME_CONTRACTOR_COMPLETED_CONSTRUCTION,
      areaFilter: areaCode,
    ...
    console.error(
      "[PrimeContractorCompletedConstructionCard] データ取得エラー:",
      error
    );
    return (
      <Card>
