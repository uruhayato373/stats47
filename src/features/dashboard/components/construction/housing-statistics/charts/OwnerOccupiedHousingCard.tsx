...
const CAT01_OWNER_OCCUPIED_HOUSING = "H1310"; // 持ち家数

interface OwnerOccupiedHousingCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 持ち家数統計カード
 */
export async function OwnerOccupiedHousingCard({
  areaCode,
  timeCode,
  title = "持ち家数",
}: OwnerOccupiedHousingCardProps) {
  try {
    // e-Stat APIから持ち家数データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_OWNER_OCCUPIED_HOUSING,
      areaFilter: areaCode,
    ...
    console.error("[OwnerOccupiedHousingCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
