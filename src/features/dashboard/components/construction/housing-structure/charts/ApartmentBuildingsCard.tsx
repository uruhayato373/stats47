...
interface ApartmentBuildingsCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 共同住宅数統計カード
 */
export async function ApartmentBuildingsCard({
  areaCode,
  timeCode,
  title = "共同住宅数",
}: ApartmentBuildingsCardProps) {
  try {
    // e-Stat APIから共同住宅数データを取得
    ...
    console.error("[ApartmentBuildingsCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
