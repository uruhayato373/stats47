...
interface LaborForcePopulationCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 労働力人口統計カード
 */
export async function LaborForcePopulationCard({
  areaCode,
  timeCode,
  title = "労働力人口",
}: LaborForcePopulationCardProps) {
  try {
    // e-Stat APIから労働力人口データを取得
    ...
    console.error("[LaborForcePopulationCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
