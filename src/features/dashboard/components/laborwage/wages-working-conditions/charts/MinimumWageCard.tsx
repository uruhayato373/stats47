 ...
 * 最低賃金統計カード
 */
export async function MinimumWageCard({
  areaCode,
  timeCode,
  title = "地域別最低賃金",
}: MinimumWageCardProps) {
  try {
    // e-Stat APIから最低賃金データを取得
        ...
        <CardContent>
          <div className="space-y-2">
            <div className="text-3xl font-bold">
              {formatNumber(targetData.value)}円
            </div>
    ...
    console.error("[MinimumWageCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
