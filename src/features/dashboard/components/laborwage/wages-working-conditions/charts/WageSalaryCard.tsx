...
// e-Stat APIパラメータ定義
const STATS_DATA_ID = "0000010103"; // 県民経済計算
const CAT01_WAGE_SALARY = "C122201"; // 賃金・俸給（平成27年基準）

interface WageSalaryCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 賃金・俸給統計カード
 */
export async function WageSalaryCard({
  areaCode,
  timeCode,
  title = "賃金・俸給",
}: WageSalaryCardProps) {
  try {
    // e-Stat APIから賃金・俸給データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_WAGE_SALARY,
      areaFilter: areaCode,
    ...
    console.error("[WageSalaryCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
