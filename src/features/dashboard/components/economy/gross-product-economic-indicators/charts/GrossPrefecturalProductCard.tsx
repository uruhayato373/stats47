...
interface GrossPrefecturalProductCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 県内総生産額統計カード
 */
export async function GrossPrefecturalProductCard({
  areaCode,
  timeCode,
  title = "県内総生産額",
}: GrossPrefecturalProductCardProps) {
  try {
    // e-Stat APIから県内総生産額データを取得
    ...
    console.error("[GrossPrefecturalProductCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
