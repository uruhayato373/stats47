...
const CAT01_CONSUMER_PRICE_DIFFERENCE_INDEX_HOUSING = "C5704"; // 消費者物価地域差指数（住居）

interface ConsumerPriceDifferenceIndexHousingCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 消費者物価地域差指数（住居）統計カード
 */
export async function ConsumerPriceDifferenceIndexHousingCard({
  areaCode,
  timeCode,
  title = "消費者物価地域差指数（住居）",
}: ConsumerPriceDifferenceIndexHousingCardProps) {
  try {
    // e-Stat APIから消費者物価地域差指数（住居）データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_CONSUMER_PRICE_DIFFERENCE_INDEX_HOUSING,
      areaFilter: areaCode,
    ...
    console.error(
      "[ConsumerPriceDifferenceIndexHousingCard] データ取得エラー:",
      error
    );
    return (
      <Card>
