...
const CAT01_CONSUMER_PRICE_DIFFERENCE_INDEX_FOOD = "C5703"; // 消費者物価地域差指数（食料）

interface ConsumerPriceDifferenceIndexFoodCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 消費者物価地域差指数（食料）統計カード
 */
export async function ConsumerPriceDifferenceIndexFoodCard({
  areaCode,
  timeCode,
  title = "消費者物価地域差指数（食料）",
}: ConsumerPriceDifferenceIndexFoodCardProps) {
  try {
    // e-Stat APIから消費者物価地域差指数（食料）データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_CONSUMER_PRICE_DIFFERENCE_INDEX_FOOD,
      areaFilter: areaCode,
    ...
    console.error(
      "[ConsumerPriceDifferenceIndexFoodCard] データ取得エラー:",
      error
    );
    return (
      <Card>
