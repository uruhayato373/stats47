...
interface ConsumerPriceDifferenceIndexOverallCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 消費者物価地域差指数（総合）統計カード
 */
export async function ConsumerPriceDifferenceIndexOverallCard({
  areaCode,
  timeCode,
  title = "消費者物価地域差指数（総合）",
}: ConsumerPriceDifferenceIndexOverallCardProps) {
  try {
    // e-Stat APIから消費者物価地域差指数（総合）データを取得
    ...
    console.error(
      "[ConsumerPriceDifferenceIndexOverallCard] データ取得エラー:",
      error
    );
    return (
      <Card>
