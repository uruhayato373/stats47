...
const CAT01_PRIVATE_ESTABLISHMENTS = "C2102"; // 事業所数（民営）（事業所・企業統計調査結果）

interface PrivateEstablishmentsCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 事業所数（民営）統計カード
 */
export async function PrivateEstablishmentsCard({
  areaCode,
  timeCode,
  title = "事業所数（民営）",
}: PrivateEstablishmentsCardProps) {
  try {
    // e-Stat APIから事業所数（民営）データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_PRIVATE_ESTABLISHMENTS,
      areaFilter: areaCode,
    ...
    console.error("[PrivateEstablishmentsCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
