...
interface NaturalParkAreaCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 自然公園面積統計カード
 */
export async function NaturalParkAreaCard({
  areaCode,
  timeCode,
  title = "自然公園面積",
}: NaturalParkAreaCardProps) {
  try {
    // e-Stat APIから自然公園面積データを取得
    ...
    console.error("[NaturalParkAreaCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
