/**
 * 国立・国定公園面積割合ドーナツチャート表示コンポーネント（Client Component）
 */
...
interface NationalParkRatioDonutChartClientProps {
  /** チャート用データ */
  chartData: Array<{
    name: string;
    value: number;
...
/**
 * 国立・国定公園面積割合ドーナツチャート表示コンポーネント
 */
export function NationalParkRatioDonutChartClient({
  chartData,
  title,
  description,
  timeName,
  totalValue,
  unit = "ｈａ",
}: NationalParkRatioDonutChartClientProps) {
  ...
  // 色定義
  const COLORS = [
    "hsl(221, 83%, 53%)", // 国立公園（青）
    "hsl(142, 76%, 36%)", // 国定公園（緑）
  ];
