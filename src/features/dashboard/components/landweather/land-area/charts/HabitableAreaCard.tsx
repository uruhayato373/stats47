...
const CAT01_HABITABLE_AREA = "B1103"; // 可住地面積

interface HabitableAreaCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 可住地面積統計カード
 */
export async function HabitableAreaCard({
  areaCode,
  timeCode,
  title = "可住地面積",
}: HabitableAreaCardProps) {
  try {
    // e-Stat APIから可住地面積データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_HABITABLE_AREA,
      areaFilter: areaCode,
    ...
    console.error("[HabitableAreaCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
