...
interface UrbanPlanningAreaCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 都市計画区域指定面積統計カード
 */
export async function UrbanPlanningAreaCard({
  areaCode,
  timeCode,
  title = "都市計画区域指定面積",
}: UrbanPlanningAreaCardProps) {
  try {
    // e-Stat APIから都市計画区域指定面積データを取得
    ...
    console.error("[UrbanPlanningAreaCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
