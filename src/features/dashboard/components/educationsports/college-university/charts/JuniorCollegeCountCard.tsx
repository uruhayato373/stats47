...
const CAT01_JUNIOR_COLLEGE_COUNT = "E6101"; // 短期大学数

interface JuniorCollegeCountCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 短期大学数統計カード
 */
export async function JuniorCollegeCountCard({
  areaCode,
  timeCode,
  title = "短期大学数",
}: JuniorCollegeCountCardProps) {
  try {
    // e-Stat APIから短期大学数データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_JUNIOR_COLLEGE_COUNT,
      areaFilter: areaCode,
    ...
    console.error("[JuniorCollegeCountCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
