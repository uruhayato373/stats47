...
interface PublicGymnasiumCountCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 体育館数（公共）統計カード
 */
export async function PublicGymnasiumCountCard({
  areaCode,
  timeCode,
  title = "体育館数（公共）",
}: PublicGymnasiumCountCardProps) {
  try {
    // e-Stat APIから体育館数（公共）データを取得
    ...
    console.error("[PublicGymnasiumCountCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
