...
const CAT01_VOLUNTEER_ACTIVITY_RATE = "G6417"; // ボランティア活動行動者率（10歳以上）

interface VolunteerActivityRateCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * ボランティア活動行動者率統計カード
 */
export async function VolunteerActivityRateCard({
  areaCode,
  timeCode,
  title = "ボランティア活動行動者率",
}: VolunteerActivityRateCardProps) {
  try {
    // e-Stat APIからボランティア活動行動者率データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_VOLUNTEER_ACTIVITY_RATE,
      areaFilter: areaCode,
    ...
    console.error("[VolunteerActivityRateCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
