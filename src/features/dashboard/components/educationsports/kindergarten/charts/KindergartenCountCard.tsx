...
const CAT01_KINDERGARTEN_COUNT = "E1101"; // 幼稚園数

interface KindergartenCountCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 幼稚園数統計カード
 */
export async function KindergartenCountCard({
  areaCode,
  timeCode,
  title = "幼稚園数",
}: KindergartenCountCardProps) {
  try {
    // e-Stat APIから幼稚園数データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_KINDERGARTEN_COUNT,
      areaFilter: areaCode,
    ...
    console.error("[KindergartenCountCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
