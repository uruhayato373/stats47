...
const CAT01_HIGH_SCHOOL_COUNT = "E4101"; // 高等学校数

interface HighSchoolCountCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 高等学校数統計カード
 */
export async function HighSchoolCountCard({
  areaCode,
  timeCode,
  title = "高等学校数",
}: HighSchoolCountCardProps) {
  try {
    // e-Stat APIから高等学校数データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_HIGH_SCHOOL_COUNT,
      areaFilter: areaCode,
    ...
    console.error("[HighSchoolCountCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
