...
const CAT01_FIRE_DEPARTMENT_EMPLOYEES_COUNT = "K1102"; // 消防職員数

interface FireDepartmentEmployeesCountCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 消防職員数統計カード
 */
export async function FireDepartmentEmployeesCountCard({
  areaCode,
  timeCode,
  title = "消防職員数",
}: FireDepartmentEmployeesCountCardProps) {
  try {
    // e-Stat APIから消防職員数データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_FIRE_DEPARTMENT_EMPLOYEES_COUNT,
      areaFilter: areaCode,
    ...
    console.error("[FireDepartmentEmployeesCountCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
