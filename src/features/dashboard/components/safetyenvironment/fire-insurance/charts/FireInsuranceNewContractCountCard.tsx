...
interface FireInsuranceNewContractCountCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 火災保険新契約件数統計カード
 */
export async function FireInsuranceNewContractCountCard({
  areaCode,
  timeCode,
  title = "火災保険新契約件数",
}: FireInsuranceNewContractCountCardProps) {
  try {
    // e-Stat APIから火災保険新契約件数データを取得
    ...
    console.error("[FireInsuranceNewContractCountCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
