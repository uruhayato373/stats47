...
const CAT01_POLICE_OFFICER_COUNT = "K4102"; // 警察官数

interface PoliceOfficerCountCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 警察官数統計カード
 */
export async function PoliceOfficerCountCard({
  areaCode,
  timeCode,
  title = "警察官数",
}: PoliceOfficerCountCardProps) {
  try {
    // e-Stat APIから警察官数データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_POLICE_OFFICER_COUNT,
      areaFilter: areaCode,
    ...
    console.error("[PoliceOfficerCountCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
