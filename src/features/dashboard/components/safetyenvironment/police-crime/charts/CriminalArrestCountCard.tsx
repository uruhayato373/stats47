...
const CAT01_CRIMINAL_ARREST_COUNT = "K4202"; // 刑法犯検挙件数

interface CriminalArrestCountCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 刑法犯検挙件数統計カード
 */
export async function CriminalArrestCountCard({
  areaCode,
  timeCode,
  title = "刑法犯検挙件数",
}: CriminalArrestCountCardProps) {
  try {
    // e-Stat APIから刑法犯検挙件数データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_CRIMINAL_ARREST_COUNT,
      areaFilter: areaCode,
    ...
    console.error("[CriminalArrestCountCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
