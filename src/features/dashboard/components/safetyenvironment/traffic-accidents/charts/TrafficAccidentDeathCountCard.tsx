...
const CAT01_TRAFFIC_ACCIDENT_DEATH_COUNT = "K3103"; // 交通事故死者数

interface TrafficAccidentDeathCountCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 交通事故死者数統計カード
 */
export async function TrafficAccidentDeathCountCard({
  areaCode,
  timeCode,
  title = "交通事故死者数",
}: TrafficAccidentDeathCountCardProps) {
  try {
    // e-Stat APIから交通事故死者数データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_TRAFFIC_ACCIDENT_DEATH_COUNT,
      areaFilter: areaCode,
    ...
    console.error("[TrafficAccidentDeathCountCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
