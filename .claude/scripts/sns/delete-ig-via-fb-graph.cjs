 ...
 *   - Token 取得手順: .claude/skills/sns/post-instagram/reference/facebook-token-setup.md
 *
 * 使い方:
...
if (!TOKEN || !IG_USER_ID) {
  ...
  console.error("   セットアップ: .claude/skills/sns/post-instagram/reference/facebook-token-setup.md");
  process.exit(1);
}
...
async function main() {
  ...
  if (me.error) {
    ...
    console.error("   .claude/skills/sns/post-instagram/reference/facebook-token-setup.md を参照");
    process.exit(1);
  }
