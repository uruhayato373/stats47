...
const snsPostsStore = require("../lib/sns-posts-store.cjs") as {
  query: (p: (r: Record<string, unknown>) => boolean) => Array<Record<string, unknown>>;
...
const SPEC_DIR = join(PROJECT_ROOT, "apps/remotion/src/features/buzz-map/specs");
...
function loadCatalog(): CatalogFile {
  if (!existsSync(STATE_PATH)) {
    throw new Error(`catalog state が無い: ${STATE_PATH} (先に build-buzz-map-catalog.ts)`);
  }
  return JSON.parse(readFileSync(STATE_PATH, "utf8")) as CatalogFile;
}

/** curated 候補の spec を ideaId / aliasesOf から解決 (存在するファイル名を返す)。 */
function resolveSpecId(entry: CuratedEntry): { specId: string | null; exists: boolean } {
  const candidates = [entry.ideaId, ...(entry.aliasesOf ?? [])].filter(
    (x): x is string => Boolean(x),
  );
  for (const id of candidates) {
    if (existsSync(join(SPEC_DIR, `${id}.json`))) return { specId: id, exists: true };
  }
...
async function headR2(key: string): Promise<{ ok: boolean; status: number | null; contentType: string | null }> {
  try {
...
async function main() {
  ...
  const curated = catalog.entries.filter((e) => e.lane === "curated" && e.ideaId);
  ...
  for (let i = 0; i < capped.length; i++) {
    ...
    const alreadyDraft =
      existingDrafts.has(`instagram:${ideaId}`) || existingDrafts.has(`x:${ideaId}`);
    const idem = idempotencyDecision({ requiredKeys, existingR2Keys: existingR2, alreadyDraft });

    if (assetPlan.xVideo || assetPlan.igReel) videoBudget = Math.max(0, videoBudget - 1);
  ...
  if (opts.json) {
    console.log(JSON.stringify({ limit: opts.limit, maxVideos: opts.maxVideos, mode: opts.apply ? "apply" : "dry-run", candidates: report }, null, 2));
  } else {
  ...
  if (!opts.apply) {
    ...
    const notice =
      `[dry-run] 実行はしていません。render / R2 push / draft insert は --apply で実行されます。`;
...
interface ReportRow {
  ...
  assets: { still: boolean; xVideo: boolean; igReel: boolean; videoDeferred: boolean };
...
function printReport(rows: ReportRow[], opts: ReturnType<typeof parseArgs>) {
  ...
  console.log(`選定 ${rows.length} 件 (curated・score 降順 × isEligibleForBatch)\n`);
  for (const r of rows) {
    const flags = [
    ...
    const assetList = [
      ...
      .filter(Boolean)
      .join("+") || "(none)";
    console.log(`     assets: ${assetList} / landing: ${r.landing} ${r.primaryUrl ?? ""}`);
...
function renderStill(compositionId: string, specPath: string, r2Key: string): boolean {
  ...
  if (!ok) console.error(`   render 失敗 (${compositionId} → ${r2Key}): ${(res.stderr ?? "").slice(-200)}`);
  return ok;
}

/** diff-push-r2 --prefix sns/buzz-map を実行 (self-load .env.local)。 */
function pushR2(): boolean {
  const res = spawnSync(
    "npx",
    ["tsx", "packages/r2-storage/src/scripts/diff-push-r2.ts", "--prefix", "sns/buzz-map"],
    { cwd: PROJECT_ROOT, stdio: "pipe", encoding: "utf8", env: { ...process.env, ALLOW_LOCAL_R2_WRITE: "1" } },
  );
  if (res.status !== 0) console.error(`   R2 push 失敗: ${(res.stderr ?? res.stdout ?? "").slice(-300)}`);
  return res.status === 0;
}

/** R2 の key を HEAD で検証 (200 + Content-Type 一致)。 */
async function verifyR2Key(key: string): Promise<{ ok: boolean; status: number | null; ctOk: boolean }> {
  const head = await headR2(key);
  const expected = expectedContentType(key);
  const ctOk = head.contentType != null && expected != null && head.contentType.startsWith(expected);
  return { ok: head.ok && ctOk, status: head.status, ctOk };
}

/**
 * --apply の実行本体 (段階フラグ --to で分離)。
 * spec 確認/生成 → render (compositionsForType) → 決定的レンダ検査 → diff-push-r2 (sns/buzz-map)
 ...
 */
async function applyBatch(rows: ReportRow[], opts: ReturnType<typeof parseArgs>) {
  ...
  const existingDrafts = new Set(
    snsPostsStore
      .query((p) => p.domain === "buzz-map" && (p.status === "draft" || p.status === "scheduled" || p.status === "posted"))
  ...
  for (const r of rows) {
    ...
    if (!r.spec.exists) {
      ...
      console.log(`[apply] ${r.ideaId}: skip (spec 未生成 — 先に build-buzz-map-spec が必要)`);
      continue;
    }
    ...
    if (r.assets.still && comps.still) {
      renderOk = renderStill(comps.still, specPath, r.r2Keys.xStill) &&
    ...
    // 4. R2 push (sns/buzz-map prefix のみ)
    const pushed = pushR2();
    if (!pushed) {
    ...
    const stillKeys = [r.r2Keys.xStill, r.r2Keys.igStill].filter(Boolean) as string[];
    let r2Ok = true;
    for (const key of stillKeys) {
      const v = await verifyR2Key(key);
      if (!v.ok) {
        r2Ok = false;
        console.error(`   HEAD 検証 失敗 ${key}: status=${v.status} ctOk=${v.ctOk}`);
      }
    }
    ...
    const utm = r.primaryUrl
      ? buildUtmUrl({ canonicalUrl: r.primaryUrl, platform: "x", ideaId: r.ideaId, variant })
      : null;
    const caption =
    ...
    if (!postable.postable) {
      console.log(`[apply] ${r.ideaId}: draft 見送り (${postable.reasons.join(", ")})`);
      continue;
    }
    if (stopAfter === "draft" || stopAfter === null) {
      ...
      console.log(`[apply] ${r.ideaId}: draft 登録 id=${row.id} (campaign=${utmCampaignFor(r.ideaId)})`);
    }
  }
