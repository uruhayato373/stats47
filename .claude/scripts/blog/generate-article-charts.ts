...
const prefCodeOf = (name) => PREF_NAME_TO_CODE.get(String(name || "").trim()) || "";
...
function genLineChartSvg(data) {
  ...
  if (!series && Array.isArray(data.data)) series = [{ label: data.label || "値", data: data.data }];
  ...
  const statsData = series.flatMap((s, si) =>
    s.data.map((pt) => ({
      ...
      value: typeof pt.value === "number" ? pt.value : typeof pt.y === "number" ? pt.y : null,
      unit,
    })),
  );
