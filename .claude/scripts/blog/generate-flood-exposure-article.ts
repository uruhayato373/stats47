...
const sourceFor = (name: string, formula: string) => write(`${name}.source.json`, { kind: 'derived', source: `r2:${prefix}/item.json`, inputs: sources, generatedAt: item.generatedAt, generatedBy: '.claude/scripts/blog/generate-flood-exposure-article.ts', formula, license: 'CC BY 4.0; 国土交通省データを加工', dataFile: `${name}.json` });
sourceFor('flood-tile-grid', 'item.rows[].values.floodExposureShare2050; no rank-to-rank join');
...
sourceFor('flood-lineage', 'manifest.quality and exact A31b river-class input inventory');
...
sourceFor('tokyo-mesh-evidence', 'pref/13 meshes bounds, meshes[7]>0 classification; mainland crop only; full-prefecture summary unchanged');
