...
// 投稿台帳 SSOT (完全DBレス)。旧 SQLite sns_posts テーブルから移行 (.claude/rules/data-storage.md)。
const postsStore = require("./sns-posts-store.cjs");
...
// ── L1-L3 + L5: 投稿台帳 (posts.json) チェック ──
// 旧 D1 sns_posts テーブルの SELECT を sns-posts-store 経由の filter に移植 (完全DBレス)。

function checkLedger(opts) {
  const cutoff = new Date(Date.now() - LOOKBACK_DAYS * 24 * 60 * 60 * 1000).toISOString();
  const rows = postsStore.query(
    (r) =>
      r.platform === "youtube" &&
      !r.deleted_at &&
      (r.status === "posted" || r.status === "scheduled") &&
      (r.posted_at || r.scheduled_at || "") >= cutoff,
  );
...
async function main() {
  ...
  const { titleHits, thumbHits, contentKeyHits, templateMetricsHits, totalChecked } = checkLedger(opts);
