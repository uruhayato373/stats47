 ...
 *
 * ★なぜ「小さいバッチを何度も」ではないか (実測)
 * `.claude/state/metrics/claude-usage/history.csv` の ai-content 成功 2 本:
 *   limit 1 → 1 件 / 24 分 / $15.87
 *   limit 5 → 5 件 / 58 分 / $30.80
 * 5 倍の件数で 1.94 倍しかかかっていない。差分から 1 件 ≈ 8.5 分 / $3.7、
 * **1 回あたりの固定費 ≈ 15 分 / $12** (起動時に CLAUDE.md と rules を読む分。
 * cache_read が毎回 1,000〜1,500 万トークンあることと符合する)。
 * したがって「1 件ずつ 4 回」は 1 件 $15.9、「10 件を 1 回」は $4.9 で **3 倍効率が違う**。
 * 正しい形は「5 時間枠ごとに 1 回、その枠に入るだけ大きなバッチ」。
 *
 * ★測れないこと
