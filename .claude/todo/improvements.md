...
| AFF-BRAND-FIT-01 | health 軸のブランド不適合広告を停止し、`精力`・`マカ`を blocklist に追加する。**2026-09-18 オーナー判断で停止 + blocklist 化を決定・実装済**: `AFFILIATE_DELIVERY_HOLDS` に af_s00000013307001_a8_001 / af_s00000013307001_a8_text_001 を追加（af_maca_emperor_001 は既存）、`a8-curated.json` の blocklistKeywords に `精力`・`マカ` を追加。3件とも `isActive=false` を確認済（配信中 255 件）。禁止事項として収益化戦略 §8 に恒久化。残: 本番反映後に配信 0 を実測 | in-progress | 2026-09-26 | claude | affiliate |
...
| AFF-MEASURE-RECOVER-01 | アフィリエイト観測の復旧を確認する。`ga4-affiliate-history.csv` の最終行は 2026-08-28、R2 `state/ads/ga4-affiliate/index.json` は 404。週次 cron は 08-30/09-06/09-13 と success で終わっていたが、当時 main が実行していた版に commit-back が無く観測がどこにも残らなかった。commit-back と R2 push は 2026-09-14 に main へ入り、初回の定期実行は **2026-09-20 (JST 22:00)**。再発防止として GA4 取得失敗と `measurementGate != ready` で run を落とす「🚦 計測ゲート」step を追加済。[検証: 09-21 に `curl -fsS https://storage.stats47.jp/state/ads/ga4-affiliate/index.json` が 200 を返し、`git log -1 --date=short -- .claude/state/ads/ga4-affiliate-history.csv` が 09-20 以降になること]。復旧しない場合は 09-13 run の GA4 step ログを読み、鍵・property ID・schema v2 のどれで止まっているかを特定する | pending | 2026-09-21 | claude | affiliate |
| ADSENSE-PAUSE-01 | 2026-08-16オーナー判断。AdSenseのscript・Auto ads・手動枠・fallback・空枠を全停止。**2026-08-29にPR #849のmainマージで本番反映済**（2026-09-18に本番HTMLで `adsbygoogle` / `pagead2` の出現 0 件を確認。旧記載の「未デプロイ」「Due 09-14」はデプロイ日を判断日と取り違えていた）。判定期日はデプロイ+28日の **2026-09-26**。実測: AdSense 確定7日 earnings は W32 ¥130 → W34 ¥8（`.claude/state/metrics/adsense/history.csv`、W35以降は未取得）。**代替収益の比較材料であるアフィリエイト実測が 2026-08-28 以降存在しない**（`AFF-MEASURE-RECOVER-01`）ため、09-26時点でCWV・engagementのみで判定するか期日を延ばすかをオーナーが決める | in-progress | 2026-09-26 | uruhayato373 | adsense |
