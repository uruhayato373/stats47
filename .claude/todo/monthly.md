...
> **2026-09-18 追記（事実の訂正）**。上の段落のうち、デプロイ状況は当時の記述であり現在は違う。
> AdSense 停止は 2026-08-29 に PR #849 の main マージで**本番反映済**（同日以降 `adsbygoogle` は 0 件）。
> `AFF-IMPRESSION-ROUTING-01` も 2026-09-13 に本番反映済。残る問題は計測経路だけで、
> アフィリエイト観測は 08-28 以降いまも存在しない（週次 cron は緑のまま observation を残していなかった）。
> 追跡は `AFF-MEASURE-RECOVER-01`、初回の復旧確認は 09-21。
