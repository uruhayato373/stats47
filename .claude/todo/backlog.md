...
- **現状**: CI run `35560007703` attempt 3でafb公式APIの成果取得・暗号化R2 read-back・正規化consumer復元が成功。2026-08-24〜09-20、stats47 site `959426`指定、発生日/確定日とも実応答`[]`（0件）。非ゼロ明細は仕様fixtureで帰属/重複/形式gateを検証し、実データ未発生。A8成果・もしも成果・ココナラ計測・GSC概要+5分類も同runで成功。GSCのconsumerは1000/1000/1000/431/21行を保持、旧`coverage-export`成功は無効。KDPは前回run `35555921976`で22 ASINの出版状態+昨日速報まで成功したが、最新runでは本棚確認後のReportsが`auth_required`に戻った。noteは全ページ/合計一致でも286記事中285記事で`report_incomplete`（欠落`n99561600d4fe`）。固定Issue #1000を継続。独立監視run `35556507463`は監査・記録・Issue #763更新まで実証（未解決のため赤、全cron正常ではない）。
- **未解決の証拠（2026-09-21）**: noteは同じ13:55 JST集計でも、2026-01-20〜09-20では286/286件・全合計一致・カバー結合PASS、08-24〜09-20では285/286件・全合計一致となる。欠落記事は公開中で、長期窓では表示される。期間依存の非表示まで確認できたが、非表示を0とする公式仕様は確認できず、短期窓の値はnullのまま保持する。再現入力は既存`note:metrics:fetch`の`--start`/`--end`、私有証跡は`.local/authenticated-measurement/note-{wide,current}-period/`。問い合わせ先: [note公式窓口](https://www.help-note.com/hc/ja/requests/new)。質問は「公開記事n99561600d4feが同一集計時刻の短期窓だけ一覧に無い。指標が全て0の期間に行を非表示にする仕様か、取得不具合か」。問い合わせ送信はオーナーの承認後。
- **次（実行順）**: ①KDP Reportsの本人再認証と次回CIでの維持を確認する（本棚ログインだけを成功としない）。②noteの上記非表示仕様を提供元へ確認し、回答を根拠に収集契約を更新する（無断の問い合わせ送信・0埋めはしない）。③mainの初回18:20 JST schedule起動と48時間鮮度監視を実測する（このタスクの19:00 JST一回確認を予約済み）。④KDPの[月別ロイヤリティ](https://kdp.amazon.co.jp/ja_JP/help/topic/G200641190)の実ファイルを取得し、現版/旧版ASIN・著者・通貨・月次期間を照合するparserを実装する。月次収益を週次へ按分せず、入金・手数料・税の帰属が未確認ならsales-ledgerへ入れない。afbは承認済み`AFB_API_KEY` Secretを使用し、Cookie移送の再ログインを反復しない。
