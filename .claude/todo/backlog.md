...
- **再開点（2026-09-07夜）**: `codex/finish-content-remaining` で73件の解消を実行中。固有背景は `apps/web/scripts/lib/assets/blog-article-backgrounds/<slug>.jpg` に逐次保存し、未生成分は対象slugとのファイル差分で導出する。全件の初回事前検査は71 PASS / 2 FAIL。空港比較2本は数値・定義を是正し、地熱・原子力2本は非商用P03由来のため商用利用可能な一次資料へ置換できるまで公開しない。画像の存在を公開完了と数えず、最後に本文・画像・索引と公開URLを照合する。
