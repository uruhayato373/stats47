...
- **現在地**: 承認済み旧資産の撤去・公開404確認、PR930の再発防止guard反映、共有一覧の再生成・公開照合まで完了（run34010311847）。旧SNS6投稿は2026-09-06ユーザー「削除して」で削除承認済み。削除API認証が無く、API操作のブラウザ代行禁止規約により実行経路の確認待ち。削除未実行。
  公開・削除・退避・検証の証跡と件数の正典は `.claude/state/metrics/geo-release-publication-2026-09-05.json` の `legacyLicense`。
- **次（実行順）**:
  1. 削除用API認証を整えるか、今回6件に限るブラウザ代行の例外承認を得て、旧投稿（X IDs592/632/636、Instagram IDs749/796/800）だけ撤去・再確認する。削除そのものの再承認は不要。X予約一覧72件に対象なし、theme draft2（841/855）は現行指標・画像へ同期済み。Instagramは再認証が必要になる場合がある。Geo15投稿の既存検証結果と混同しない。
