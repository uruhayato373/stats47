...
- **現状**: 成否の正典は`.claude/state/metrics/authenticated/latest.json`。本人Reports再認証後、ローカルbrowser/probeを追加せずrun `35561887453` attempts 6→7を直列実行した。attempt 6（`2026-09-21T06:48:27.457Z`）は22冊の出版状態・日次・8月の月次XLSX取得、private R2保存/read-back、原本SHA再照合・再解析、通常/過去月restoreまでPASS（evidence SHA `254d945a1523ed03102b3e94fbb3b5c655b2155ff5b5a50942bc9a1b253705b1`、git記録 `39d18d266`）。月次の現版/旧版ASIN・著者照合、24報告月の暗号化循環保管は実データ検証済み。更新sessionを使用したattempt 7（`06:58:45.076Z`）は本棚口座照合後のReports preflightで`auth_required`（evidence SHA `83bf60d5f3e5d349eab2995acd1be3a9a5a0119dbe3619c0b39a114962959062`、git記録 `5509381c3`）。同じbootstrap世代の更新stateでCookie24件の欠落・有効期限切れは無く、継続認証失敗の原因は未確定。並行ローカルブラウザ使用だけでは説明できず、初回取得成功を無人継続成功とはしない。33 Node/6 Python・全25workspace/8script型検査・develop品質CIはPASS、main反映は保留。noteは285/286件・全ページ/合計一致・`report_incomplete`。固定Issue #1000と独立監視Issue #763は継続し、全cron正常とはしない。
...
- **次（実行順）**: ①KDPは同一方式で本人ログイン→初回成功を反復せず、固定の専用ブラウザ環境で認証継続が成立するか検証する案の実行場所・費用・常時稼働条件をオーナーと決める（原因・解決を断定しない）。このrepoはPUBLICのため、個人Macをそのまま公開repoのself-hosted runnerとして登録しない（[GitHub公式の安全要件](https://docs.github.com/en/actions/reference/security/secure-use)）。環境新設・常駐化は承認後。認証保護を回避せず、無人の連続成功と定期起動を実測してからmain反映・運用完了を判断する。②noteの上記非表示仕様を提供元へ確認し、回答を根拠に収集契約を更新する（無断の問い合わせ送信・0埋めはしない）。③mainの初回18:20 JST schedule起動と48時間鮮度監視を実測する（このタスクの19:00 JST一回確認を予約済み）。KDP月次収益を週次へ按分せず、入金・手数料・税の帰属が未確認ならsales-ledgerへ入れない。afbは承認済み`AFB_API_KEY` Secretを使用し、Cookie移送の再ログインを反復しない。
