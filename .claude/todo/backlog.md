...
- **次（実行順）**: ①afb公式APIの既存キーをオーナー承認に基づき`AFB_API_KEY` Secretへ登録済み（2026-09-21）。サイト限定の28日成果・発生日/確定日の分離・帰属/重複/形式gateをCIで実測し、暗号化保存とconsumer復元まで確認する。Cookie移送の再ログインを反復しない。②noteは期間別ダッシュボードの欠落を提供元仕様と照合し、必要ならオーナーが問い合わせる。③初回18:20 JST schedule起動と48時間鮮度監視を実測する。④KDP確定ロイヤリティと商品別純収益の取得契約を決め、週次sales-ledgerの空欄を確定データで接続する（速報で代用しない）。
