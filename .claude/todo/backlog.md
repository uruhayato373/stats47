...
- **次**: 公開本文の `次に読む</h2>` を数える監査を `audit-note-circulation.mjs` に足し、2 個以上の記事について
  2 つ目の `<hr><h2>` だけを除く repair mode を `applyPublishedLinkRepairs` に追加して `--commit` する。
- **完了条件**: 全公開記事で `次に読む</h2>` の出現が 1 以下。
