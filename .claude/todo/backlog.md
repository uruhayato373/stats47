...
- **次（実行順）**: ①KDP Reportsの本人再認証と次回CIでの維持を確認する（本棚ログインだけを成功としない）。②noteは期間別ダッシュボードの欠落を提供元仕様と照合し、必要ならオーナーが問い合わせる。③mainの初回18:20 JST schedule起動と48時間鮮度監視を実測する。④KDP確定ロイヤリティと商品別純収益の取得契約を決め、週次sales-ledgerの空欄を確定データで接続する（速報で代用しない）。afbは承認済み`AFB_API_KEY` Secretを使用し、Cookie移送の再ログインを反復しない。
