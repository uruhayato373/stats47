...
- **カード画像（2026-09-09）**: 実GISから全国/都道府県/地域抜粋を選ぶ版付き設定と横長16:9・正方形1:1の生成器を実装。一覧・関連リンクで共用。進捗は `.claude/state/geo/source-thumbnails.json`、画像/表示検証は `.claude/state/geo/source-thumbnails-audit.json`。再開時は `apps/web/scripts/generate-geo-source-thumbnails.ts` → `apps/web/scripts/audit-geo-source-thumbnails.ts --browser` を実行し、stagingを本番公開済みと扱わない。本番反映時は生成器の `--plan` で最新remoteを検証して共通画像publisherに渡し、実URLの画像応答を確認する。
- **別PCでの再開**: 作業ブランチは `codex/workspace-updates-20260908`。Gitで引き継ぐのは実装・生成器・本台帳・機械検証記録であり、`.local/r2/` と `.local/image-staging/geo-thumbnails/` は含まれない。データが無いPCでは、下記の索引・GISを取得し、修正版がremote未反映ならGIS READMEの原典再生成手順で復元してから画像を生成する。ローカルの修復証跡やA31b分割manifestもGit対象外なので、以前の台帳の合格をそのPCの再現確認と扱わず、取得・版・SHAを確認して再検証する。本番配信へのpushをPC間同期の代わりに実行しない。
