...
- **現状**: 成否の正典は`.claude/state/metrics/authenticated/latest.json`。KDPは本人Reports認証後、run `35561887453` attempt 4で日次取得とprivate R2保存が成功したが、更新sessionを使うattempt 5は`auth_required`となった。元の本人state/CI更新stateともlocalでログインフォームへの遷移を確認し、継続認証は未解決。Cookie欠落は確認されておらず、同時ブラウザ使用・更新state移送・提供元の再認証条件のいずれが原因かは未確定。月次XLSXのASIN/著者/月/通貨照合、24月の暗号化循環保管と過去月復元は実装済み（commit `9d4489e59`/`1d82a7dcc`）。実ファイルで取得・解析と33 Node/6 Pythonテストは成功したが、月次版のCI保管・復元は未検証。noteは285/286件・全ページ/合計一致・`report_incomplete`。固定Issue #1000と独立監視Issue #763は継続し、全cron正常とはしない。
...
- **次（実行順）**: ①KDP Reportsの本人再認証後、同じ認証をローカルprobeとCIで並行使用せず、CI2回を直列実行して更新sessionの継続利用・日次/月次収集・private R2 read-back・過去月復元を実測する。その証拠をもってmainへ反映する（本棚ログインや初回成功だけで閉じない）。②noteの上記非表示仕様を提供元へ確認し、回答を根拠に収集契約を更新する（無断の問い合わせ送信・0埋めはしない）。③mainの初回18:20 JST schedule起動と48時間鮮度監視を実測する（このタスクの19:00 JST一回確認を予約済み）。KDP月次収益を週次へ按分せず、入金・手数料・税の帰属が未確認ならsales-ledgerへ入れない。afbは承認済み`AFB_API_KEY` Secretを使用し、Cookie移送の再ログインを反復しない。
