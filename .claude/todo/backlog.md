...
- **現在地**: 承認済み旧資産の撤去・公開404確認、PR930の再発防止guard反映、共有一覧の再生成・公開照合まで完了（run34010311847）。残件は外部実測で確認済みの旧SNS6投稿の削除承認・撤去。
  公開・削除・退避・検証の証跡と件数の正典は `.claude/state/metrics/geo-release-publication-2026-09-05.json` の `legacyLicense`。
- **次（実行順）**:
  1. 公開中の旧投稿6件（X IDs592/632/636、Instagram IDs749/796/800）の削除承認を得て、対象だけ撤去・再確認する。X予約一覧72件に対象なし、theme draft2（841/855）は現行指標・画像へ同期済み。Instagramは再認証が必要になる場合がある。Geo15投稿の既存検証結果と混同しない。
