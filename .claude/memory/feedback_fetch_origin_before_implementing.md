...
**How to apply**: 実装系タスク（特に改善バックログ/収益化マスタープランの施策）に着手する前に必ず:
1. `git fetch origin main develop`
2. `git rev-list --count HEAD..origin/main`（>0 ならローカルが古い → 先に ff 同期）
3. 着手予定の施策が `docs/02_実装計画/03_改善バックログ.md`（origin/develop 版）と `git log origin/develop -S "<施策ID/キーワード>"` に既存でないか検索
