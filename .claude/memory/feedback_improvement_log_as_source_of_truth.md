...
description: 改善施策 TODO を一元化した経緯。現行の真実源は .claude/todo/04_改善バックログ.md
metadata: 
  node_type: memory
  type: feedback
...
PR #308 時点では改善施策 TODO を旧改善ログへ集約した。その後の再整理により、現行の真実源は **`.claude/todo/04_改善バックログ.md`**、agent 用の詳細履歴は `.claude/skills/analytics/*/reference/improvement-log.md` となった。
...
**How to apply**:
- 新規施策は `.claude/todo/04_改善バックログ.md` に一意な ID で追加し、該当 metric の詳細ログへ参照を置く
...
- `.claude/todo/03_今週の計画.md` は改善ログから抽出した「当週ビュー」 (`/weekly-plan` が上書き)
...
**関連**:
- 現行バックログ: `.claude/todo/04_改善バックログ.md`（旧 SEO TODO 統合計画は Git 履歴）
