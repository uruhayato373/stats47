...
- この Mac は Google Drive をローカルマウントしている (`~/Library/CloudStorage/GoogleDrive-uruhayato373@gmail.com/マイドライブ/`)。
  2026-09-10 から vault は展開配置 (tar bundle 廃止) で、`source-vault restore/upload/verify --vault` がマウントを自動解決する
  → [[project_source_vault_expanded_layout]]。ファイルは dataless (初回アクセスで取得) なので大きい PDF は最初だけ遅い。
...
- revision 2 = PDF + `pages/pNNNN.jpg` 307 枚 (UI枠除去済み 220dpi) + `transcripts/` 307 本 + `page-dims.json`、revision 3 = + `md/` + `figures/` + `crop-manifest.json` (1,037 file)。いまは Drive の版 folder に展開されたまま置かれている。
...
- **note 56 本を実公開した (2026-09-06 完了)**: 県別47 + 論点8 + 有料1 (¥2,980)。全件 HTTP 200・
  ハッシュタグ 99 個・図が段落の切れ目にあることを実測。マガジン `s47-kakei-reading`。
  **投稿で踏んだ罠 4 つ**:
  ① `browser-use` の daemon を記事ごとに kill/再起動すると `about:blank` に落ちて連続失敗する。
     **1 セッション通しで実行**する (これが 8 本連続失敗の主因だった)。
  ② 画像挿入の `BU keys Home` は**視覚上の行頭** (折り返し行の先頭) へ飛ぶ。長い段落だと文の途中で
     改行が入り、図が文章を分断する (公開 56 本中 47 本で発生)。Range で段落ノード先頭へ置く。
     ただし**是正は公開 API の本文 HTML を組み直して PUT する方が安全** (エディタ経由だと同じ経路で再発)。
  ③ **Chrome Profile 5 は他セッションと共有**。前セッションが無関係な記事の公開画面を開いたまま
     残していたことがあり、確認せず確定すると誤記事を公開する。開始前に必ず開いている画面を確認する。
  ④ 分断の判定を「図の直前が句点で終わるか」にすると**閉じ括弧で終わる文中の切れ目を見逃す**
     (「清掃代（2.24倍）」+図+「やベッド…」)。原稿の図の直前行と突き合わせるのが正しい。
  **並行 Codex との競合**: 実行中に共有スクリプトを書き換えられ、構文エラー版を踏んだ。
  投稿は 1 者だけが行う。カタログ・マガジン名も同時編集されるので commit は明示パスで分ける。
- **NFC/NFD 罠**: macOS は日本語ファイル名を NFD で返すことがあるため、manifest との照合は `normalizeRelative` で NFC に揃える
  (2026-09-05 に bsdtar 展開で「missing / unexpected」が同名で出た。tar は廃止したが照合の正規化は残している)。
