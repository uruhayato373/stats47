...
| 角丸 | `rounded-xl` 等 | **フラット（`--radius:0`）** | 2026-06 採用。円形のみ `rounded-full`（doc 13） |
| フォント | — | **system スタック**（游ゴシック/Hiragino） | Web フォント非依存・Zenn 準拠（doc 13） |
| レイアウト/ナビ | サイドバー+メイン | **`PageShell` + ヘッダーメガメニュー**（PC サイドバー廃止） | 横幅統一・回遊は右レール（doc 13） |

> レイアウト/横幅/レール/フォント/角丸の**正典は `docs/01_技術設計/13_統一レイアウト設計.md`**。本リファレンスと食い違う場合は doc 13 が優先。
