...
| ページ幅・レール幅をページ内で直書き | ページ間で幅がばらつく温床 | `PageShell`（`@/components/layout`）経由（1700px / 右レール 360px、doc 13） |
...
> **そのページの表示内容を切り替えるページ内ナビ**は左レールに置いてよい。現在の該当は
> **テーマページ（`/themes/*`・`/areas/*/[themeSlug]`）の `ThemeSideNav`**（テーマ 20 件の一覧 + 地域選択）。
> ホーム / `/ranking` が本文内の `aside` にカテゴリ列（`PortalCategoryGrid variant="sidebar"`）を置いているのと同じ役割で、
> テーマページはそれを `PageShell` の `leftRail` スロットで持つ。
>
> 条件: (1) 中身はそのページを操作するナビだけ（グローバルナビを複製しない）、(2) `PageShell leftRail` 経由で幅を直書きしない、
> (3) xl 未満は `leftRailNarrowBehavior="hide"` で隠し、代替のセレクタを**本文上部**に出す
