...
> - `improvement-triage`: `.claude/todo/04_改善バックログ.md` の effect/status ラベル (**書込は triage のみ**・本 agent は read)。
...
- 境界 (`.claude/todo/04_改善バックログ.md` に書かない = improvement-triage の専有。デプロイは devops-runner/ranking-publisher に委譲し確認を要する)。
...
1. **状態リコンサイル (read-only)**: seo-strategy.json を読み、各施策 ID の effect/status を
   `.claude/todo/04_改善バックログ.md` から、消化状況を topic-queue.json / expansion-queue.json から、
...
- `.claude/todo/04_改善バックログ.md` — **read only** (effect/status の突合。書込は improvement-triage のみ)
