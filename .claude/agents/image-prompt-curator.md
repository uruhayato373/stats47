...
43 種 catalog に加え、ブログ背景の単一モチーフ catalog とCodex imagegen生成経路を維持する。

## 担当範囲

- 画像プロンプト生成 (`generate-ai-content` の画像系派生)
- 画像プロンプト catalog の維持 (`.claude/skills/image-prompt/reference/catalog.md`、 43 種)
- ブログ背景 catalog とgit JPEGの維持 (`apps/web/scripts/data/blog-codex-background-catalog.ts`)
...
| `/generate-blog-images` | Codex MCPでブログ単一モチーフ背景を生成・ingest・検証 |
...
- `apps/web/scripts/data/blog-codex-background-catalog.ts` — ブログ背景の意味仕様・prompt SSOT
...
- 2つのcatalogとブログ背景JPEGへの write は本 agent が排他
