...
Cloudflare R2 ストレージへの push (upload) / pull (download) / du (容量計測) を専任する agent。 snapshot 生成 (git TS / R2 観測値 → R2 用 JSON) は snapshot-exporter が行い、本 agent は完成した snapshot を R2 にアップロードする。
