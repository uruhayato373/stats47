...
# 2. sources を焼いた snapshot を使って、本文の手書き節を移行する
#    (引用だけ → 削除 / 計算方法・定義を含む → 「データについて」へ改名。Kindle 書籍の章は据え置き)
npx tsx .claude/scripts/blog/migrate-data-source-sections.ts --snapshot <all.json> --apply
# 3. 本文と source.json を R2 へ (put 後に GET で照合)
