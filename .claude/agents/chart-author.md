...
| スキル | 用途 |
|---|---|
| `/generate-article-charts` | ブログ記事の SVG チャート生成（`packages/svg-builder` 経由）。ランキングは**カード型・上位5+下位5固定**でPC横長(`<name>.svg`)+mobile/note縦長(`<name>-mobile.svg`)+Instagram 4:5(`<name>-ig.svg`)を自動出力。正典 `blog-data-schema.md` §1.5 |
