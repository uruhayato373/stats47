...
| スキル | 用途 |
|---|---|
| `/post-x-batch` | **X 定型投稿の量産 (主力)**。候補選定→画像→執筆→lint→draft 登録の 5 フェーズ。1 日 3 本 (§1) |
...
```
① 生成 (クラウド可): /post-x-batch --count <21>
...
- 単発は `/publish-x <key> <date>`。日次・週次の上限は §1 (現行: 1 日 3 / 週 21) に従う
...
| シナリオ | フロー |
|---|---|
| **量産 (主力)** | `/post-x-batch --count 21`（候補選定→画像→執筆→lint→draft）→ x-strategist（draft 確認）→ ローカル `/publish-x --from-queue`（予約）→ `promote-scheduled-x`（posted 昇格） |
...
- **stats47 サイトの OGP** → 本エージェントは触らない（blog-editor / image-prompt-curator の領分）
