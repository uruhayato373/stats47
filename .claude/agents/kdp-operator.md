...
node .claude/scripts/kdp/kdp-unpublish.mjs --all-withdrawn --probe  # 取り下げ (withdrawal 付き live 本) の確認ダイアログまで採取・確定しない
node .claude/scripts/kdp/kdp-unpublish.mjs --all-withdrawn --commit # ★出版停止を実行 (本棚 read-back「下書き」で成功判定・要オーナー指示)
```
