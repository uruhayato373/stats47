...
単一 PC 運用前提。ローカル D1 = 編集元、リモート D1 = 本番配信先の一方向 push。作業後は `/sync-remote-d1` で push（内部でリモートを R2 にバックアップしてから DELETE + INSERT する）。`/pull-remote-d1` は新規 PC セットアップ・PC 復旧・リモート直接編集の取り込み時にのみ実行する（routine で実行するとローカルの作業中データを失うリスクがある）。

差分の事前確認は `/diff-d1` または `npm run pull:d1 -- --dry-run`。
