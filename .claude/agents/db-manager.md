...
`.local/d1/` が存在しない場合はバックアップからの復元か新規セットアップが必要（リモート D1 は 2026-04-29 に解約済み）。
...
| ローカル DB のみ欠落（新テーブル） | `drizzle-kit push` or 手動 CREATE TABLE |
...
ローカル D1（source of truth）→ R2 スナップショット → 本番配信 の一方向フロー。

```
ローカル D1 → /sync-snapshots → R2 snapshot → Web app が読み取り
```

データ変更後は `/sync-snapshots` でスナップショットを更新すること。リモート D1 は 2026-04-29 に解約済み。
