...
- **定義整合 (★2026-09-19・BLOCK・正典 `blog-quality-standards.md`「定義整合」)**: 審査の最初に
  `npx tsx .claude/scripts/blog/build-metric-definition-sheet.ts --slug <slug>` (書籍は `--keys`) で指標定義シートを
  作り、本文の解釈をその表に照らす。(a) 値・順位・年が図 JSON と一致するか (b) 対象 (二人以上世帯全体 / 勤労者世帯 /
  事業所…) と分母の取り違え (c) 名目と実質、暦年と年度の混在 (d) 構成比の差の実額換算・平均の個人への置き換え
  (e) 相関を因果・説明割合として書く記述。どれか 1 つでも BLOCK。finding には突合した JSON / config の値を書く。
  **なぜ**: K-S1-01 (2026-09-19) 全章レビューの BLOCK 8 / MAJOR 19 の大半がこの種類で、旧 rubric (読者価値) では
  項目が無かった
...
- **図あたりの解釈の厚み** (★2026-06-06): 各 SVG 図の直後に「なぜ上位/下位か」の解釈段落があるか。図を貼って 1-2 文で次の図へ流していないか (良記事は図あたり ~600字)。`quality-gate.mjs` が図あたり字数の床 (<350 blocker) を機械検出するが、critic は「その文章が真因/構造に踏み込んだ解釈か、単なる順位の読み上げか」を意味審査する
- **記事アーキタイプの必須分析視点** (★2026-06-06、正典 `.claude/rules/blog-quality-standards.md`「記事アーキタイプ」): frontmatter `archetype: A|B|C|D|E` (宣言が無ければ本文から推定) ごとの**必須分析視点が満たされているか**を審査する — A=なぜ上位/下位か(地理・産業構造) / B=他指標との相関を散布図で可視化+相関≠因果 / C=時系列推移+変化の地理的偏り / D=内訳分解+読者の生活への含意 / E=代表2-3指標のハブ誘導。視点が欠けていれば MAJOR/BLOCK で指摘
- **callout が「読み違い防止の知識」か** (★2026-06-06): callout が本文の言い換え・全記事共通の「年次が異なる」定型になっていないか。記事固有の・独立した価値のある注意 (定義の落とし穴/相関≠因果/正規化視点) になっているか。型の機械的反復は MINOR/MAJOR で指摘
- **文体の一貫性 (ですます調)** (★2026-06-08、正典 `.claude/rules/blog-quality-standards.md`「文体」): 本文の地の文が ですます調 に統一されているか。`quality-gate.mjs` は である調 copula 文末 (である。/だ。/だった。/ではない。/だろう。/のだ。) を機械検出するが、critic は **動詞終止形の常体 (〜する。〜なる。〜連なる。)** の混在も意味審査する (gate では捕まえられない)。callout・引用・データ出典の体言止めは対象外。常体混入は MINOR、本文全体が である調 なら MAJOR で指摘
