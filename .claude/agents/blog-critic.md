...
- **定義整合 (BLOCK・正典 `blog-quality-standards.md`「定義整合」)**: 審査の最初に
...
- **図あたりの解釈の厚み** (正典 `blog-quality-standards.md`「図あたり prose 字数の床」): 各 SVG 図の直後に「なぜ上位/下位か」の解釈段落があるか。図を貼って 1-2 文で次の図へ流していないか (良記事は図あたり ~600字)。`quality-gate.mjs` が図あたり字数の床 (<350 blocker) を機械検出するが、critic は「その文章が真因/構造に踏み込んだ解釈か、単なる順位の読み上げか」を意味審査する
- **記事アーキタイプの必須分析視点** (正典 `.claude/rules/blog-quality-standards.md`「記事アーキタイプ」): frontmatter `archetype:` (型の集合は正典のとおり。宣言が無ければ本文から推定) ごとの**必須分析視点が満たされているか**を審査する — A=なぜ上位/下位か(地理・産業構造) / B=他指標との相関を散布図で可視化+相関≠因果 / C=時系列推移+変化の地理的偏り / D=内訳分解+読者の生活への含意 / E=代表2-3指標のハブ誘導。視点が欠けていれば MAJOR/BLOCK で指摘
- **callout が「読み違い防止の知識」か**: callout が本文の言い換え・全記事共通の「年次が異なる」定型になっていないか。記事固有の・独立した価値のある注意 (定義の落とし穴/相関≠因果/正規化視点) になっているか。型の機械的反復は MINOR/MAJOR で指摘
- **文体の一貫性 (ですます調)** (正典 `.claude/rules/blog-quality-standards.md`「文体」): 本文の地の文が ですます調 に統一されているか。`quality-gate.mjs` は である調 copula 文末 (である。/だ。/だった。/ではない。/だろう。/のだ。) を機械検出するが、critic は **動詞終止形の常体 (〜する。〜なる。〜連なる。)** の混在も意味審査する (gate では捕まえられない)。callout・引用・データ出典の体言止めは対象外。常体混入は MINOR、本文全体が である調 なら MAJOR で指摘
