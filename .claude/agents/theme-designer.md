...
> **2026-09-15 追加フィールドも本 agent が編集する**: `selection.adoptionCriteria`(採用基準) /
> `selection.readerQuestion` / `selection.targetReaderOrDecision`、`charts[].comparisonBasis` /
> `charts[].visualizationRationale`、`ThemeCatalog.harmRelevance` (HARM該当・任意)。
> いずれも読者向け UI には描画しない内部メタデータで、`/quality/catalog-audit` が監査表示する。
> 定型文で埋めず、根拠不足なら未記入のまま採用を保留してよい (validator は warn に留める)。
> 規約: `.claude/rules/theme-catalog-standards.md` §4/§4.7/§4.8。調査・提案の前段は `theme-researcher` が担い、採択分を本 agent がカタログ化する。
> **selection の置き場**: `expanded.ts` の tuple で定義された指標 (31 テーマ + 拡張 67 章) は
> `selection-evidence.ts` に書く (§4「置き場」)。定型文の selection を一次資料で埋める作業は
> skill `/backfill-theme-selection` (gate 付き writer) を通す — 手で JSON を書いて validator の
> `[selection-*]` error を踏まない。
...
- **primary 指標**: テーマのヘッドライン。ダッシュボード先頭のチャート付きカード。1〜3件。
  - 基準: 地域差が大きい、時系列変化が劇的、社会的関心が高い
- **secondary 指標**: primary を補完する関連データ。チャート付き stats-card に表示。3〜8件。
  - 基準: primary と異なる切り口を提供、相関関係がある
- **context 指標**: 背景情報。`tabIndicators` から除外され、ダッシュボードのカードには出さない（ランキングページ側で閲覧）。
...
export const <THEME>_CATALOG: ThemeCatalog = {
  ...
  metrics: [
    { rankingKey: "...", shortLabel: "...", role: "primary",
      selection: { proposedBy: "<白書/調査>", sourceUrl: "https://...", surveyedAt: "2026-..", rationale: "...",
        adoptionCriteria: ["representativeness", "readerValue"], // 満たす基準だけ列挙 (任意)
        readerQuestion: "指標1件に絞った読者の問い (任意)",
        targetReaderOrDecision: "対象読者/支える意思決定 (任意)" } },
  ...
  charts: [
    { componentKey: "...", componentType: "line-chart", title: "...",
      ...
      comparisonBasis: "何と比べる可視化か (任意)", visualizationRationale: "componentType選定理由 (任意・内部監査用)" },
  ],
  // harmRelevance: [{ axis: "money", reason: "具体的な読者の悩み・意思決定に紐づける (任意・複数可)" }],
  evidenceTopics: [
