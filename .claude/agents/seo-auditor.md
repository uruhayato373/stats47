...
**命名規則**: `T{Tier}-{Category}-{連番}` 例: `T1-MW-01`, `T0-404-01`

**Tier 区分**:
| Tier | 意味 | いつ着手 |
|---|---|---|
| T0 | URL 空間整理（404/5xx/リダイレクト/クロール未登録の物理削減） | **最優先**。ここが終わるまで他 Tier は効果測定が曖昧 |
| T1 | technical SEO（middleware/robots/noindex/canonical） | T0 と並行可 |
| T2 | コンテンツ品質（独自エディトリアル、テンプレ量産の縮退） | T0-T1 の効果が出た後 |
| T3 | 外部シグナル（SNS/被リンク/E-E-A-T） | T2 と並行可 |

**Category 例**:
- `MW` (middleware), `SRC` (source removal), `CRAWL` (crawl budget), `CF` (Cloudflare)
- `404` / `5xx` / `RDR` (redirect) / `CANON` (canonical) / `CONTENT` / `SNS`

**施策追加ルール**:
1. **1 施策 1 ID**。複数目的が混在する場合は分割する
2. **デプロイ前に想定効果値を明文化**（後付けの後方バイアスを防ぐ）。`/gsc-improvement action` が必須フィールドとして要求する
3. **Tier 0 を Tier 1 より必ず先に消化**する
4. **Tier 2/3 は Tier 0-1 が落ち着くまで着手しない**
5. 各施策の判定（PENDING/PARTIAL/FULL/NO/ADVERSE）は `/gsc-improvement observe` が自動計算する
...
Remotion 製 OGP の実装一覧は `apps/remotion/src/features/ogp/README.md`、セーフゾーン規約 (note.com 正方形クロップ対策含む) は `apps/remotion/src/shared/components/layouts/OgpSafeZone.tsx` の JSDoc を参照。
