...
- **生成物の検証（`products:kindle:verify-epub`）を生成のたびに実行する**。EPUB は `.local` にしか
  ...
  3 層 = ① `epubcheck`（仕様適合）② レイアウト不変量（`scripts/verify-epub.mts`）
  ③ 本文不変量（`src/channels/kindle/content-invariants.ts`: Web 記事の名残・年/年度混在・
  率系の単位・同一指標の重複・全冊検査時の冊間重複。2026-09-19 の販売中 22 冊監査で①②を
  通り抜けていた欠陥）。
