...
8. **指標定義シートを作り、解釈文はその範囲で書く** (★2026-09-19・`blog-quality-standards.md`「定義整合」):
   `npx tsx .claude/scripts/blog/build-metric-definition-sheet.ts --keys <metric_key>,<related...>` を実行し、
   単位・対象と分母・期間の型 (暦年/年度)・調査・「読み違い注意」を読んでから書く。表に無い対象 (世帯範囲・
   名目/実質・年) を本文に持ち込まない。critic は同じシートで審査し、違反は BLOCK になる
...
> ★**文体は「ですます調」で書く** (正典 `.claude/rules/blog-quality-standards.md`「文体」)。本文の地の文に
> である調 (である。/だ。/だった。/ではない。/だろう。/のだ。/動詞終止形) を混在させない。callout・引用の
> 体言止めは常体可。`quality-gate.mjs` が である調 copula 文末を **blocker** で検出するため、
> である調で書くと公開できない。最初から ですます で書くこと。

> ★まず**記事アーキタイプを 1 つ選ぶ** (正典 `.claude/rules/blog-quality-standards.md`「記事アーキタイプ」)。
> data の性質で決める: 単一指標を掘る=**A** / 2 指標の相関・真因=**B** (差別化主軸・R2 相関 snapshot に素材あり) /
> 時系列の変化(V字・急増減)=**C** / 物価・住居など読者の生活含意=**D** / **食品・家計消費の「消費量日本一」=D2** /
> カテゴリ送客のハブ=**E** / **市区町村内格差(決算カード)=F** / **県間人口移動(migration-flow)=G**。迷ったら **A** (既定)。
> 選んだ型の**章構成テンプレ・目安字数・必須分析視点**に従う。frontmatter に `archetype:` を宣言する (Phase 4)。
> B/C/D2/F/G を選んだら散布図/折れ線/内訳分解など型固有の SVG・分析が必要 → `/generate-article-charts` で生成。
> B型の散布図データは `fetch-correlation-scatter.mjs` が相関 snapshot から自動生成する (手 join 不要)。
>
...
6. **出典は本文に書かない** (★2026-09-25〜)
   - ページ末尾の「データ出典」は図の `source.json` から自動表示される (`DataSourceList`)。
     本文に `## データ出典` を書くと `quality-gate.mjs` が blocker で止める。出典は図の `source.json` に記録する
   - 計算方法・定義・対象の限定など読者に要る説明があれば `## データについて` 節に書く
...
archetype: <A|B|C|D|D2|E|F|G (Phase 3 で選んだ型。blog-critic が必須分析視点の審査に使う)>
