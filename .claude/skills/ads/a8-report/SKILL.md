...
- account-wide 由来の数値を引用するときは必ず「口座横断 (doboku-note 込み)」と併記する。
  併記なしに stats47 の EPC として報告しない (`.claude/rules/evidence-based-judgment.md`)。
...
```bash
node .claude/scripts/ads/check-a8-report-due.mjs
```
...
# 実収集
node .claude/scripts/ads/fetch-a8-ui-csv.mjs --reports all
```
