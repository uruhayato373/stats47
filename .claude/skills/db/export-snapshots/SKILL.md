...
description: ローカル D1 から全 R2 snapshot を一括 export する。リモート D1 撤廃後、本番反映の唯一経路。データ変更後に必ず実行。後段は /push-r2。
...
リモート D1 撤廃 (Phase 10, 2026-04-29) 以降、本番 reader は R2 snapshot のみを参照する。
データ変更 (`/populate-all-rankings`, `/import-blog-articles`, AI コンテンツ生成 等) のたびに、対応する snapshot を更新しないと本番で古いデータが配信される。
...
```bash
SKIP_VALUES=1 bash .claude/skills/db/export-snapshots/run.sh
```
...
```bash
bash .claude/skills/db/export-snapshots/run.sh
```

### 単独カテゴリのみ

```bash
bash .claude/skills/db/export-snapshots/run.sh --only blog
bash .claude/skills/db/export-snapshots/run.sh --only port-statistics
```

### dry-run (実行しない、走るスクリプトをリスト表示)

```bash
bash .claude/skills/db/export-snapshots/run.sh --dry-run
```
