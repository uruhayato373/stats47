...
| **ranking-values** (ranking-items の**後**) | `packages/ranking/src/scripts/generate-ranking-values.ts` | `app/ranking/{key}/values.json` — 正典 `app/stats/{metric}/values.json` から配信用に決定的変換 (rank は正典値を引き継ぐ)。実描画値・OGP・blog がこれを読む。★2026-07-27 に復活 (Phase 6 で writer が 2 ヶ月間不在化していた事故の恒久対策) | ~2,116 files |
