...
8. 検証結果をユーザーに報告:
   ...
   次のステップ:
   - /export-snapshots --only ranking-items,ranking-values で R2 snapshot を更新
   - /push-r2 --prefix snapshots/ranking-items/ snapshots/ranking-values/ で本番反映
...
13. ユーザーに次のアクション候補を提示:
    - `/export-snapshots --only ranking-items,ranking-values` → `/push-r2` — R2 反映
