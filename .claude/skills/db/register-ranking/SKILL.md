...
10. 差分を確認（新規追加 rankingKey が含まれていること）:
    ```bash
    git diff apps/web/src/config/known-ranking-keys.ts
    ```
11. git add + commit:
    ```bash
...
**重要**: この手順を飛ばすと、新規追加した rankingKey が middleware で **410 Gone** になりサイトでアクセス不可になる（CI ビルド環境に D1 が無いため、ビルド時は known-ranking-keys.ts のみを参照するため）。
