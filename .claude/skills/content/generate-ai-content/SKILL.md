...
NODE_ENV=development NODE_OPTIONS='--conditions react-server' \
  npx tsx packages/ai-content/src/scripts/generate-parallel.ts \
  --model claude --concurrency 5 --limit 500
```

または `generate-all.sh` 経由（デフォルトで並列モード）:

```bash
bash packages/ai-content/src/scripts/generate-all.sh
bash packages/ai-content/src/scripts/generate-all.sh --model claude --concurrency 10
bash packages/ai-content/src/scripts/generate-all.sh --limit 500
```
