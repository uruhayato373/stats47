...
テーマ定義・検索キーワード・ローテ順は `reference/rotation-schedule.md` を参照。

**テーマ未指定時の選定フロー:**
1. `reference/rotation-schedule.md` を読み込む
2. ローカル D1 の `sns_posts` から直近3件の `quote_rt` の `category_key` を取得（`ranking_items` と JOIN）
3. ローテ順の先頭から、直近使用テーマに含まれない最初のテーマを選択
4. 同一 `content_key` は7日間空ける制約も適用

ローテ順・ロジックの詳細と SQL は `reference/rotation-schedule.md` の「次テーマの自動選定ロジック」に記載。
...
**X 検索クエリの構成（鮮度重視）:**
```
<キーワード> min_faves:500 lang:ja -filter:replies since:<7日前の日付>
```

- `min_faves:500` — 500いいね以上。直近7日のツイートは累積が少ないので閾値を下げる
- `lang:ja` — 日本語のみ
- `-filter:replies` — リプライを除外
- `since:YYYY-MM-DD` — **必須**。直近7日以内に制限。これを入れないと `f=top` は累積いいね順で並ぶため、数週間〜数ヶ月前のバズが上位に来てしまう
- テーマが広い場合はキーワードを組み合わせる（例: `少子化 OR 出生率 min_faves:500 lang:ja since:2026-04-07`）

**なぜ鮮度が重要か:**
- 元ツイートのスレッドがまだ活発な方が、引用RT経由の到達が伸びる
- 古いツイートに引用RTすると元の議論が終わっていて埋もれる
- 3週間以上前のツイートは会話が終わっているケースがほとんど

### Phase 2: X 検索実行

```bash
# since は動的に「7日前」を計算する（macOS/Linux 両対応）
SINCE=$(date -v-7d +%Y-%m-%d 2>/dev/null || date -d '7 days ago' +%Y-%m-%d)

# URL エンコードした検索クエリで X 検索を開く
QUERY="少子化 OR 出生率 min_faves:500 lang:ja -filter:replies since:$SINCE"
...
**結果が少ない場合のフォールバック:**
1. `min_faves` を 300 まで下げる
2. `since` を 14日前まで拡大
3. それでも不足なら `f=live`（最新順）に切り替えて直近高エンゲージを拾う

古いツイート（`since` を満たしているのに `time` 属性が7日以上前）が混入することは通常ないが、Phase 5 で候補提示する際は各ツイートの `time` を確認し、7日以上前のものは除外すること。
...
$BU eval "(() => {
  ...
  for (let i = 0; i < articles.length; i++) {
    ...
    const ageDays = postedAt ? Math.floor((now - new Date(postedAt).getTime()) / (24*60*60*1000)) : 999;
    // 7日超のツイートは除外（鮮度ルール）
    if (ageDays > 7) continue;
    results.push({i, user: user.substring(0,30), text: text.substring(0,220), eng: eng.substring(0,100), url, postedAt, ageDays});
  }
  return JSON.stringify(results);
})()"
```

これで各ツイートの本文・エンゲージメント・URL・投稿者・投稿日時・経過日数が JSON で取得できる。**7日を超えるツイートは自動的に除外される。**
...
|---|---|
| 頻度 | 1日2〜3件まで（スパム判定回避） |
| 鮮度 | **投稿から7日以内のツイートのみ対象**（元スレッドが活発なうちに乗る） |
| 対象 | 500いいね以上（直近7日制約とのバランス） |
| 比率 | 引用RT 3 : オリジナル投稿 7 |
...
- **古いツイート（7日超）への引用RT** → 元スレが沈静化しており到達が伸びない
