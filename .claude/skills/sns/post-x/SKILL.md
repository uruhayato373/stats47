...
> **型・画像・頻度・UTM の SSOT は `.claude/rules/sns-content-standards.md`** (§1/§2)。
...
2. **画像**: `npx tsx .claude/scripts/sns/quick-still.ts --key <key>` (ranking-card, §2-9 の正典パスへ)。
...
4. **lint**: `node .claude/skills/sns/post-x-batch/scripts/lint-x-captions.cjs --in .local/r2/sns/_queue/captions.json` (PASS まで修正)。
5. **登録**: `node .claude/skills/sns/post-x-batch/scripts/register-drafts.cjs --in .local/r2/sns/_queue/captions.json`。

## 投稿 (ローカル)

```bash
npx tsx .claude/skills/sns/publish-x/publish-x.ts --from-queue --dry-run --limit 1   # 初回必須
npx tsx .claude/skills/sns/publish-x/publish-x.ts --from-queue --limit 1
```
...
- 量産版: `.claude/skills/sns/post-x-batch/SKILL.md`
