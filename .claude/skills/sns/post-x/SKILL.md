...
6. 投稿管理テーブルを更新する（`docs/11_SNS投稿管理/posts/<contentType>.md`）
   - テーブル内で contentKey を含むリンク行を検索する（URL末尾の `/ranking/<key>` で特定）
   - 行が存在する場合: X 列を `generated` に更新
   - 行が存在しない場合: テーブル末尾に新しい行を追加（X 列のみ `generated`、他は `-`）
7. 生成された画像パスと投稿テキストをユーザーに報告する
