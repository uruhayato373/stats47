...
1. **測定対象 URL の決定**
   ...
   `--type all` のデフォルト対象（最大 34 URL）:
   - トップページ: 1
   - テーマダッシュボード: 最大 5（全テーマ）
   - ランキングページ: PV 上位 10
   - 都道府県ページ: PV 上位 5 + ランダム 3
   - ブログ記事: PV 上位 5
   - その他（相関分析・比較等）: 最大 5
...
2. **PSI API 呼び出し**
   ...
   URL="https://www.googleapis.com/pagespeedonline/v5/runPagespeedtest"
   curl -s "${URL}?url=https://stats47.jp${PATH}&strategy=mobile&key=${PSI_API_KEY}"
   ```

   - **2秒以上の間隔**を空けて呼び出すこと（API レート制限）
   - 最大 34 URL × 2 strategy = 68 呼び出し（`both` の場合）

3. **結果の DB 保存**

   `performance_metrics` テーブルに INSERT する:
   ```sql
   INSERT INTO performance_metrics (
     url, strategy, performance_score, accessibility_score, best_practices_score, seo_score,
     fcp_ms, lcp_ms, cls, tbt_ms, si_ms, tti_ms,
     total_byte_weight, main_thread_time_ms,
     crux_lcp_ms, crux_cls, crux_inp_ms, crux_ttfb_ms,
     measured_at
   ) VALUES (...);
   ```
...
- **API レート制限**: PSI API は無料枠で 25,000 クエリ/日。2秒間隔を守ること
