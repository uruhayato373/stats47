...
#### 何をしたか
- `dynamicParams = false` を追加 + `generateStaticParams` の try/catch を throw に変更

#### なぜ失敗したか
- GitHub Actions の Deploy to Cloudflare Workers で D1 binding が無く `listActiveRankingKeys` が失敗
- 追加した throw が発火してビルドが落ちた（run 24566738457）
- 本番は古いコードのまま、T0-RKG 問題は未解決

#### 学び
- CI ビルド環境では D1 依存コードは使えない
- `generateStaticParams` は static data（git commit されたファイル）から生成すべき
- → v2（ファイルベース）へ方針転換

### [T0-RKG-200-01-v2] `/ranking/{任意キー}` 200 → 410 化（ファイルベース設計）

**施策 ID**: `T0-RKG-200-01-v2`
**Tier**: T0 (URL 空間整理)
**デプロイ日**: 2026-04-17（予定）
**コミット**: — （デプロイ時に追記）
...
- 登録済み: **+300 ~ +800**（1,860 → 2,100-2,700、クロール予算の集中化）
...
- **新規**: `apps/web/scripts/generate-known-ranking-keys.ts` — ローカル D1 から全 active rankingKey を抽出して `apps/web/src/config/known-ranking-keys.ts` に書き出す tsx スクリプト
- **新規**: `apps/web/src/config/known-ranking-keys.ts` — 1,899 件の有効 rankingKey を格納した `ReadonlySet<string>`（git commit 対象、自動生成）
- **新規**: `.claude/skills/db/generate-known-ranking-keys/SKILL.md` — スキル化
- **編集**: `apps/web/src/app/ranking/[rankingKey]/page.tsx`:
  - `export const dynamicParams = false` 追加
  - `generateStaticParams` を `KNOWN_RANKING_KEYS` 参照に変更（D1 不依存）
- **編集**: `apps/web/src/middleware.ts`:
  - 新ブロック「Fix 6」: `/ranking/{key}` で `!KNOWN_RANKING_KEYS.has(key) && !GONE_RANKING_KEYS.has(key)` なら 410 Gone
- **編集**: `.claude/skills/db/register-ranking/SKILL.md`:
  - Phase 5 に known-ranking-keys.ts 再生成の必須ステップを追加

#### 根拠

2026-04-17 本番 curl 検証で `/ranking/this-does-not-exist-12345` 等ランダムキー全てが 200 を返すことを確認（クロール済み未登録 2,415 の主因）。

v1 の失敗から学び、CI ビルド環境で使える設計に変更:
- D1 非依存の静的ファイル（`known-ranking-keys.ts`）を git commit
- ビルド時はこのファイルを読む → CI で D1 binding 不要
- 未知キーは middleware で 410 Gone に（Next.js の 404 より強い「確定削除」シグナル）

`GONE_RANKING_KEYS`（489 件）と同じパターン。CLAUDE.md の「記録先の統一原則」と整合（build 時参照の静的リストは `src/config/`）。
