...
### [T0-RKG-200-01] `/ranking/{任意キー}` 200 → 404 化
...
- `apps/web/src/app/ranking/[rankingKey]/page.tsx`:
  - `export const dynamicParams = false` を追加 — generateStaticParams で生成されない rankingKey は Next.js が自動で 404 を返す
  - `generateStaticParams` の try/catch を削除 — D1 アクセス失敗時に空配列返しを廃止（dynamicParams=false と組み合わせた時の「静かな全 404」を防ぐ）。失敗時は明示的に throw してビルドを落とす

#### 根拠

2026-04-17 本番 curl 検証で `/ranking/this-does-not-exist-12345` 等ランダムキー全てが 200 を返すことを確認。page.tsx には `notFound()` が書かれているが、ISR キャッシュや dynamicParams=true の挙動で無効化されていた。Google は「無限に薄いページが湧く」と判定し、クロール済み未登録 2,415 件の主因になっていた。
