...
description: Google Search Console の検索パフォーマンスとインデックス問題を docs/02_実装計画/improvement-backlog.md で追跡し、週次 snapshot と施策の効果判定を記録する。Use when user says "GSC改善", "GSC記録", "インデックス改善", "SEO課題記録", or when analyzing gscエラー/ CSV files.
primary_agent: gsc-analyst
---

GSC の週次メトリクス（Clicks / Impressions / CTR / Position / Index Coverage）を **docs/02_実装計画/improvement-backlog.md (施策一覧) + reference/improvement-log.md (agent 用詳細)** の 1 層構造で時系列追跡し、打った施策と効果を記録するスキル。
...
| 要約 (人間向け) | git: `docs/02_実装計画/improvement-backlog.md` | 施策一覧・status・想定/実測/判定を 1 ファイルで俯瞰 |
...
$ARGUMENTS — [mode]
             mode:
               ...
               - action  : 新しい施策 section を docs/02_実装計画/improvement-backlog.md に追加
               - next    : 次に着手すべき改善候補を提示
```
...
- **1 層構造を維持** — `docs/02_実装計画/improvement-backlog.md` (施策一覧・簡易表)、reference/improvement-log.md は agent が深掘りする詳細
...
- `docs/02_実装計画/improvement-backlog.md` が存在（front-matter `type: improvement-log` / `metric: gsc`）
