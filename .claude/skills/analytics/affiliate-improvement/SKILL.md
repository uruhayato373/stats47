...
description: アフィリエイト広告の impression / click / CTR を GA4 (affiliate_impression / affiliate_click) と在庫棚卸しで分析し、弱い枠を特定して改善施策を .claude/todo/improvements.md に記録するループ。在庫の管理画面 (単体 HTML) を開く機能も持つ。Use when user says "アフィリエイト改善", "アフィリエイト分析", "imp/click 増やす", "広告クリック改善", "アフィリエイト管理画面", "アフィリエイト在庫見せて". 汎用の「管理画面を開いて」は admin-console.
...
相対日数でも GA4 へは絶対日付へ固定して送り、当日途中の値を混ぜない。snapshot の `date` と
ファイル名は `periodEnd`、実行時刻は `generatedAt` に記録する。固定期間は両端を含み、昨日以前・366日以内だけを許可する。
