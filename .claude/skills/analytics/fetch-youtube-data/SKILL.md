...
```bash
# チャンネル日別サマリー（過去28日）
node .claude/scripts/youtube/analytics.js overview 28

# 動画別パフォーマンス（過去28日）— 視聴回数・平均視聴秒・維持率
node .claude/scripts/youtube/analytics.js videos 28

# 再生数上位動画（過去90日）— 維持率・視聴時間付き
node .claude/scripts/youtube/analytics.js top 90

# 特定動画の視聴維持率（離脱曲線）
node .claude/scripts/youtube/analytics.js retention <videoId>

# トラフィックソース別の視聴回数
node .claude/scripts/youtube/analytics.js traffic 28

# 視聴者の年齢・性別分布
node .claude/scripts/youtube/analytics.js demographics 28
```
...
- 再認証が必要な場合: `node .claude/scripts/youtube/oauth-setup.js`
