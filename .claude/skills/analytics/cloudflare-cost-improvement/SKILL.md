...
description: Cloudflare Workers / D1 / R2 の月次コストと主要メトリクス（D1 rows read、Workers CPU ms、storage、月額課金）を docs/02_実装計画/03_改善バックログ.md で追跡し、budget 超過検知・施策と効果を記録する。Use when user says "Cloudflare コスト確認", "D1 使用量", "Workers CPU 使用量", "請求書チェック", or when analyzing Cloudflare invoices / cost anomalies.
primary_agent: performance-auditor
---

Cloudflare の月次コスト・リソース使用量を **`docs/02_実装計画/03_改善バックログ.md` で時系列追跡**し、打った施策と効果を記録するスキル。
...
| 月次スナップショット（人間向け要約） | `docs/04_レビュー/YYYY-MM-cloudflare-cost.md` | `.claude/scripts/cloudflare/monthly-snapshot.mjs` が自動書き出し |
| 日次アラート（自動起票） | GitHub Issues ラベル `cloudflare-alert,auto-generated` タイトル `[Cloudflare Alert] ...` | 閾値違反時のみ起票・解決後 close で運用 |

→ **1 層構造**: `docs/02_実装計画/03_改善バックログ.md` (簡易表) + agent 用詳細は `.claude/skills/analytics/cloudflare-cost-improvement/reference/improvement-log.md`。日次アラートのみ Issues に残す（PSI/Cloudflare daily アラート方針）。
...
$ARGUMENTS — [mode]
             mode:
               ...
               - next    : 次に着手すべき改善候補を提示
               - invoice : 請求書 PDF から snapshot + docs/04_レビュー/YYYY-MM-cloudflare-cost.md を作成
```
...
4. budgets.json 判定
5. docs/04_レビュー/YYYY-MM-cloudflare-cost.md を Write（frontmatter `type: cloudflare-cost-snapshot` / `month: YYYY-MM` / `invoice_id: IN-XXXXXXXX`）:
   - metrics 表
   - budget 判定
   - 前月比（前月の docs/04_レビュー/YYYY-MM-cloudflare-cost.md を Read して比較）
   - 進行中施策の status 一覧（docs/02_実装計画/03_改善バックログ.md から抽出）
...
- `docs/02_実装計画/03_改善バックログ.md` が存在すること（施策 ID は `CF-*` 等）
- `docs/04_レビュー/` ディレクトリ存在
