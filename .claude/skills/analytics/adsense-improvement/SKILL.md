...
| 施策（1 施策 1 section、人間向け要約） | `docs/05_改善ログ/adsense.md` | Obsidian で時系列・status 別に絞り込み可能 |
...
→ **2 層構造**: `docs/05_改善ログ/adsense.md` は人間が眺める要約、`reference/improvement-log.md` は agent が深掘りする詳細。
...
2. docs/05_改善ログ/adsense.md を Read し status: pending / in-progress の section を抽出
...
6. 進行中施策の効果判定（最重要）:
   docs/05_改善ログ/adsense.md を Read し status: pending の section を抽出。
   各施策に対して:
   ...
   - 判定結果を docs/05_改善ログ/adsense.md の該当 section の「実測」「判定」欄に Edit insert:
     ```
     ### 実測
     - 経過日数: N 日
...
2. docs/05_改善ログ/adsense.md を Read し、見出し直下（最新を上）に以下 section を Edit insert:
...
```
1. docs/05_改善ログ/adsense.md を Read し status: pending を除いた施策 + 過去 effect/full の派生候補を抽出
...
- **docs/05_改善ログ/adsense.md は append-only** — section の追加・status の更新のみ。過去判定の改竄は禁止
...
# 進行中（pending）施策
cat docs/05_改善ログ/adsense.md | grep -B1 -A4 'status.*pending'

# 効果測定済み施策
cat docs/05_改善ログ/adsense.md | grep -B1 'status.*effect/'
...
- `docs/05_改善ログ/adsense.md` が存在（front-matter `type: improvement-log` / `metric: adsense`）。存在しなければ初回 action 時に新規作成
