...
> 概念正典 (天井ループ): `.claude/rules/blog-quality-standards.md` §継続品質ループ
...
```bash
node .claude/scripts/blog/analyze-winning-patterns.mjs            # 既定 imp>=15
# node .claude/scripts/blog/analyze-winning-patterns.mjs --min-imp 30   # 母数を厳しく
```
...
confidence hi/mid + 定性裏取り済の勝ちパターンのみ `.claude/rules/blog-quality-standards.md` の
...
```bash
node .claude/scripts/blog/build-remediation-queue.mjs   # conformance を取り込む
