...
> **⚠ 記事内テーブル禁止ルール**
> - **マークダウンテーブルは原則使わない。** データの提示には上記の SVG チャートを使い、少量の数値（4行以下）は本文の太字で表現する。例外: 採点方法・多次元スコア（8列以上）・相関係数テーブルなど、本文やチャートで表現困難な構造化データのみ許可。
> - **`<ranking-table>` コンポーネントは使用禁止。** 47都道府県の全データは専用ランキングページにあるため、記事内に埋め込まず `<source-link>` で導線を確保する。
> - **理由:** モバイルでの可読性低下、本文との情報重複、ランキングページとのカニバリゼーション。
