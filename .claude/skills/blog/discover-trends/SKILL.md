...
2. **ループ本体**（1 候補あたり **最大 2 周**）:
   ...
   **(b) データ取得**: 見つかった `statsDataId` を `/fetch-estat-data` で取得し、必要なら TS-config 追加 → `/sync-metrics-cache --apply` → `/page-data-batch --metric <key>`（`packages/database/README.md` の取り込みフロー準拠）。
