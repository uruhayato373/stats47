...
不一致なら未完（例: ranking を `isActive:true` にしても `KNOWN_RANKING_KEYS` / R2 `all.json` 未反映だと middleware の `isGone || !isKnown` で 410 のまま）。ranking 公開の多段依存は memory `project_ranking_publish_pipeline_gap` / `docs/02_実装計画/feature-backlog.md`「122 metric (完全データ) の本番公開」参照。
