...
GitHub Issue に出力する（`critical-review` ラベル、タイトル `[Critical Review] Monetization Strategy`）。

既存の Issue がある場合はコメント追記で更新する。初回のみ新規作成:

```bash
# 本文を /tmp/monetization-strategy-body.md に書き出し後:
# 初回:
gh issue create \
  --title "[Critical Review] Monetization Strategy" \
  --label "critical-review" \
  --body-file /tmp/monetization-strategy-body.md

# 既存 Issue（例: #NN）に追記する場合:
gh issue comment NN --body-file /tmp/monetization-strategy-body.md
```

過去の戦略変遷は `gh issue view <number> --comments` で参照できる。
