...
>
> 本 skill の中核 `ingest-indicator.mjs` が削除済 `stats_prefecture` テーブルへ INSERT する設計のため、現状実行できない。Phase 8 で R2 直行投入版 (`packages/data-configs/scripts/page-data-batch.ts` 流用) に refactor 予定 ([feature-backlog.md](../../../../docs/02_実装計画/feature-backlog.md) の Phase 8 issue を参照)。
>
> **暫定運用** (1 metric ずつ手動):
...
> 7. `docs/02_実装計画/indicator-backlog.md` を手動編集 (status: pending → done)
> 8. `docs/02_実装計画/improvement-backlog.md` に batch entry append

stats47 の指標を継続的に拡充する。`docs/02_実装計画/indicator-backlog.md` の pending 候補から優先度上位 N 件を順次取得・登録し、backlog の status を更新、`docs/02_実装計画/improvement-backlog.md` に施策バッチとして append する 1 コマンド型スキル (refactor 完了後)。
...
- `docs/02_実装計画/indicator-backlog.md` が存在し、pending 候補が 1 件以上ある
...
9. `docs/02_実装計画/improvement-backlog.md` を Read し、`## 実行履歴` セクション末尾に下記テンプレで新 batch entry を Edit append:
...
11. 完了サマリ報告 (成功 / 失敗 / backlog 残):
    ...
    [expand-indicators] 完了
      ...
      改善ログ: docs/02_実装計画/improvement-backlog.md#BATCH-2026-05-19-01
...
| 更新対象 | 内容 |
|---|---|
| `docs/02_実装計画/indicator-backlog.md` | 該当行 `status` 更新 + frontmatter `updated:` |
| `docs/02_実装計画/improvement-backlog.md` | 新 batch entry append + frontmatter `updated:` |
...
- backlog: [`docs/02_実装計画/indicator-backlog.md`](../../../../docs/02_実装計画/indicator-backlog.md)
- 改善ログ: [`docs/02_実装計画/improvement-backlog.md`](../../../../docs/02_実装計画/improvement-backlog.md)
