...
`/nsm-experiment` および関連スキル（`/weekly-plan`, `/weekly-review`）が参照する **NSM の計測側の実装サマリ**。戦略的な定義の真実源は `docs/04_レビュー/{YYYY-MM-DD}-nsm.md`（最新版は `ls -t docs/04_レビュー/*-nsm.md | head -1` で取得）。本ファイルはそれを計測実装と繋ぐリファレンス。
...
| **判断根拠** | `docs/04_レビュー/*-nsm.md` §North Star Metric |
...
- `docs/04_レビュー/*-nsm.md` — NSM の完全な定義（戦略側の真実源）
- `docs/04_レビュー/*-growth-loops.md` — 成長ループ設計
