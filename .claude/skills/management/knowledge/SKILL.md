...
## pull-remote-d1 はスキーマ差分で大量失敗する（→ 2026-05 `/pull-remote-d1` 廃止により本問題は消滅）

> **注記 (2026-05-03)**: Phase 10 でリモート D1 撤廃に伴い `/pull-remote-d1` スキル自体が削除済み。本エントリは過去事例として保持。新しいデータ取り込みは `docs/01_技術設計/11_データ基盤設計.md` 参照。
...
2. **`.github/workflows/oauth-token-health-check.yml`** が週次（月曜 09:00 JST）で `youtube.channels.list` を叩いて健康確認、失効検知時に `[OAuth Alert]` Issue 自動起票
