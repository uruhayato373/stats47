...
- **真実源は既存のまま。** 改善施策は `01_改善バックログ.md`、機能は `02_機能バックログ.md`、指標は `03_指標バックログ.md` が SSOT。月次計画はそこから**今月分を抜き出した優先順位ビュー**であり、TODO の実体を二重管理しない。
...
3. **3 つのバックログから今月着手すべき pending を抽出**（真実源・実体はここ）
   ...
   grep -E "pending|in-progress|effect/pending" docs/todo/01_改善バックログ.md | head -30
   # 機能・自動化: tier の高い未着手
   grep -E "tier: ?[12]|status: ?(pending|active)" docs/todo/02_機能バックログ.md
   # 指標拡充
   head -40 docs/todo/03_指標バックログ.md
   ```
...
7. **TODO インボックスの triage**（セッション中に捕捉した未整理 TODO を振り分ける）
   ```bash
   cat docs/todo/inbox.md
   ```
   → 「未整理」表の各行を適切なバックログへ振り分ける（改善→`01_改善バックログ.md` / 機能→`02_機能バックログ.md` / 指標→`03_指標バックログ.md` / コンテンツ→各 backlog / バグ→Issues）。
...
- `docs/todo/01_改善バックログ.md` / `02_機能バックログ.md` / `03_指標バックログ.md` — TODO 真実源
