...
- **真実源は既存のまま。** 改善施策は `03_改善バックログ.md`、機能は `04_機能バックログ.md`、指標は `05_指標バックログ.md` が SSOT。月次計画はそこから**今月分を抜き出した優先順位ビュー**であり、TODO の実体を二重管理しない。
...
3. **3 つのバックログから今月着手すべき pending を抽出**（真実源・実体はここ）
   ...
   grep -E "pending|in-progress|effect/pending" docs/02_実装計画/03_改善バックログ.md | head -30
   # 機能・自動化: tier の高い未着手
   grep -E "tier: ?[12]|status: ?(pending|active)" docs/02_実装計画/04_機能バックログ.md
   # 指標拡充
   head -40 docs/02_実装計画/05_指標バックログ.md
   ```

4. **ロードマップ上の現在地**
   ```bash
   head -60 docs/02_実装計画/02_実装ロードマップ.md   # 現在のスプリント・月次目標
   ```
...
7. **TODO インボックスの triage**（セッション中に捕捉した未整理 TODO を振り分ける）
   ```bash
   cat docs/03_週次運用/TODOインボックス.md
   ```
   → 「未整理」表の各行を適切なバックログへ振り分ける（改善→`03_改善バックログ.md` / 機能→`04_機能バックログ.md` / 指標→`05_指標バックログ.md` / コンテンツ→各 backlog / バグ→Issues）。
   → 振り分けた行は `整理先` を記入し「整理済み」表へ移動。**今月の重点テーマ候補**にもインボックス由来の項目を含めて検討する（ユーザーが「やりたい」と明示した意思の表れなので優先度が高い）。

### Phase 2: 重点テーマの選定（1-2 テーマに絞る・予算制約）

集約結果から、**今月確実に前進させるテーマを 1-2 個だけ**選ぶ。選定基準（優先度順）:

1. **収益・PV に直結するか**（収益化マスタープラン `01_収益化マスタープラン.md` の現フェーズと整合）
...
- **構成タスク**:
  ...
  - [ ] ...
- **依存・ブロッカー**: <ユーザー操作待ち等>
- **真実源リンク**: `../../02_実装計画/03_改善バックログ.md#<id>` 等
...
- 機能バックログ: `../../02_実装計画/04_機能バックログ.md`
- 指標バックログ: `../../02_実装計画/05_指標バックログ.md`
- ロードマップ: `../../02_実装計画/02_実装ロードマップ.md`
```
...
- `docs/02_実装計画/01_収益化マスタープラン.md` — 収益化フェーズ（重点選定の最上位基準）
- `docs/02_実装計画/02_実装ロードマップ.md` — スプリント・月次目標
- `docs/02_実装計画/03_改善バックログ.md` / `04_機能バックログ.md` / `05_指標バックログ.md` — TODO 真実源
