...
`docs/02_実装計画/03_改善バックログ.md` の pending / in-progress 施策を、**Tier × 期日カテゴリ** のマトリクスで一望し、人間が「次にどれを潰すか」を判定するための UX レイヤースキル。
...
- **EXP-005** (gsc, tier 1, deployed 2026-04-20, 28d): 期限切れ警告: 検証コマンド実行 or due 延長 → docs/02_実装計画/03_改善バックログ.md#exp-005
- **T2-CLEAN-03** (ga4, tier 2, due 2026-05-10): effect 判定実施を本週内に → docs/02_実装計画/03_改善バックログ.md#t2-clean-03
```
...
- `docs/02_実装計画/03_改善バックログ.md` — 改善バックログ全体 (TODO 真実源)
- `.claude/rules/docs-vs-issues.md` — 改善施策の置き場所
- `.claude/rules/evidence-based-judgment.md` — effect 判定の実証ベース原則

## 制約・注意

- 本スキルは `docs/02_実装計画/03_改善バックログ.md` を直接書き換えない (read-only)
