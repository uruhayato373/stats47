...
- ブログ記事の未実行企画（`gh issue list --label content/blog --state open --json number,title` 件数）
...
- 未着手の Issue 一覧と優先度（`gh issue list --state open --label enhancement --label "content/*"` を tier ラベルで優先度判定）
- 直近の批判的レビュー・Pre-Mortem Issue
  gh issue list --label critical-review --state all --limit 5 --json number,title,updatedAt
  gh issue list --label pre-mortem --state all --limit 3 --json number,title,updatedAt
  → 繰り返し指摘されているパターンを抽出（`gh issue view <番号>` で本文確認）
- 前週の計画・レビュー Issue
  gh issue list --label weekly-plan --state all --limit 2 --json number,title,body
  gh issue list --label weekly-review --state all --limit 1 --json number,title,body
  → 前週 [Weekly Plan] YYYY-W(n-1) と [Weekly Review] YYYY-W(n-1) を `gh issue view` で取得
...
2. **「先週と同じ失敗を繰り返してないか？」**
   - 前週の計画 Issue（`gh issue list --label weekly-plan --state all --limit 2` で取得した `[Weekly Plan] YYYY-W(n-1)`）と照合
...
GitHub Issue として作成する。テンプレは `.github/ISSUE_TEMPLATE/weekly-plan.md` に準拠。

```bash
# 本文を /tmp/plan-body.md に書き出し後:
gh issue create \
  --title "[Weekly Plan] YYYY-Www" \
  --label "weekly-plan" \
  --body-file /tmp/plan-body.md
```

作成後、Issue 番号を標準出力で報告する。前週 `[Weekly Review] YYYY-W(n-1)` Issue の「来週への申し送り」から引用した内容がある場合は、本文の「前週の申し送り」に `#番号リンク` 形式で参照を含める。
...
## 前週の申し送り
<!-- `[Weekly Review] YYYY-W(n-1)` の「来週への申し送り」から引用（#番号リンク付き） -->
...
<!-- Phase 4 の結果を引用形式で記載 -->
<!-- 関連する [Critical Review] Issue があれば #番号で参照 -->

## 関連 Issue

<!-- 施策 Issue, Pre-Mortem, NSM 実験、snapshot Issue 等を #番号で列挙 -->
- `[Weekly Review] YYYY-W(n-1)` Issue: #NNN
- 前月 `[Pre-Mortem] YYYY-MM` Issue: #NNN（該当あれば）
...
- 前週の計画 Issue が存在する場合、**自動で振り返りを生成**する
- 前週の計画で未達のタスクは、自動的に今週の Must 候補に昇格させて検討する
- 計画 Issue のタスク状態（完了/未達）は、**週中にユーザーが checkbox をチェック**して更新する（GitHub Web UI or `gh issue edit`）
- 計画 Issue は蓄積する（クローズは `/weekly-review` が翌週実行時に担当）。過去の計画は傾向分析に使用

## Issue ラベル

- `weekly-plan` — 本スキルが作成する Issue のラベル
- `weekly-review` — ペアの週次レビュー Issue
- `critical-review` — Phase 4 で参照する批判的レビュー Issue
- `pre-mortem` — Phase 4 で参照する事前検死 Issue

## 参照

- `docs/02_実装計画/01_実装ロードマップ.md` — KPI・スプリント目標
- `gh issue list --state open` — 未解決タスク（ラベルで絞る: `enhancement`, `content/*`, `tier-*`）
- `gh issue list --label critical-review --state all` — 過去の批判的指摘
- `gh issue list --label weekly-plan --state all` / `gh issue list --label weekly-review --state all` — 過去の週次計画・レビュー
