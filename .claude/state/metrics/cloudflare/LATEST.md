# Cloudflare Usage — 2026-08-11

> 計測時刻: 2026-08-12T18:26:18.027Z
> 前日比: 2026-08-10

## D1

| 指標 | 当日 | 前日比 |
|---|---|---|
| Databases (active) | 0 | → |
| Read queries | 0 | → |
| Rows read | 0 | → |
| Write queries | 0 | → |
| Rows written | 0 | → |

## Workers

| 指標 | 当日 | 前日比 |
|---|---|---|
| Requests | 33.61K | ▼ -12.5%  |
| Errors | 0 | → |
| Subrequests | 5.01K | ▲ +9.8%  |

**Error rate**: 0.00% (0/33608)

## R2

| 指標 | 当日 | 前日比 |
|---|---|---|
| Class A ops (writes) | 2.76K | ▼ -14.4% ✅ |
| Class B ops (reads) | 150.53K | ▼ -9.1% ✅ |
| Egress | 79648MB | ▼ -10.6% ✅ |
| Storage | 15.63GB | ▲ +0.9%  |
| Objects | 60,270 | ▲ +1.3% ✅ |

### Bucket breakdown

| Bucket | Storage (GB) | Objects | Class A | Class B | Egress (MB) |
|---|---|---|---|---|---|
| stats47 | 11.63 | 50,975 | 2.76K | 148.62K | 79433 |
| doboku-note | 2.34 | 9,000 | 0 | 1.91K | 215 |
| stats47-cache | 0.00 | 0 | 0 | 0 | 0 |
| doboku-note-archive | 1.67 | 295 | 0 | 0 | 0 |

## History

Last 7 days (`.claude/state/metrics/cloudflare/history.csv`):

```
date,d1_databases,d1_read_queries,d1_rows_read,d1_write_queries,d1_rows_written,workers_requests,workers_errors,workers_subrequests,r2_class_a_ops,r2_class_b_ops,r2_egress_mb,r2_storage_gb,r2_objects
2026-08-04,0,0,0,0,0,63781,0,13479,9127,236759,67420,15.697,58691
2026-08-06,0,0,0,0,0,48988,0,6225,4025,203453,65169,15.527,57889
2026-08-07,0,0,0,0,0,48061,0,6440,3704,225720,89630,15.584,58168
2026-08-08,0,0,0,0,0,41786,0,4553,8887,273973,96011,15.600,59502
2026-08-09,0,0,0,0,0,36646,0,3164,3347,154107,75711,15.470,59430
2026-08-10,0,0,0,0,0,38399,0,4566,3221,165565,89142,15.485,59486
2026-08-11,0,0,0,0,0,33608,0,5013,2756,150528,79648,15.630,60270
```
