...
<sources>.local/seo-rank-watch/{selection,gsc,target-page}.json、target-page.txt（直前にHTTP 200で取得した公開本文・リンク）、data/seo/improvement-log.json、selected.editableFile、WebSearchと公式資料。</sources>
...
- 対象の公開本文は必ずReadで `.local/seo-rank-watch/target-page.txt` を読む。これはCIが直前に取得し、HTTP成功と本文hashを検証した実際の公開内容。WebFetchの403・タイムアウトは閲覧成功として扱わず、ソースコードを公開本文の代わりにしない。競合の取得が失敗したら読める上位ページを再調査する。
