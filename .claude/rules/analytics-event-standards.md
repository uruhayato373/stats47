...
| `cta_click` | `trackCtaClick` | `cta_id` / `target_type`（content_id・target_key はどの呼び出し元も送っていないので登録しない。2026-09-26 確認） | ✅登録済 (2026-09-26 Admin API で作成・audit-api で確認) | buzz-map §7.3 / ファネル |
...
| `home_featured_impression` / `home_featured_click` | `trackHomeFeatured*` | `card_variant` / `slot` / `experiment_variant` | ✅登録済 (2026-09-26 Admin API で作成・audit-api で確認) | `apps/web/src/features/ranking/components/FeaturedRankings/README.md` |
...
| `csv_download_purpose` | `trackCsvDownloadPurpose` | `download_purpose`（値は work・study・media・personal・other の固定語彙。`ranking_key` は登録済み） | ✅登録済 (2026-09-26 Admin API で作成・audit-api で確認) | ranking / 行政資料 (`CSV-DL-INTENT-SURVEY-01`) |
| `page_view` | `pageview` (`pageview.ts`) | `pv_trigger` / `theme_slug` / `area_code`（`content_group` は GA4 標準、`ranking_key`・`category_key` は登録済み） | ✅登録済 (2026-09-26 Admin API で作成・audit-api で確認) | 全ページ (`GA4-FULL-MEASUREMENT-01`) |
| `ui_interaction` | `trackUiInteraction` | `ui_action` / `ui_target`（値は `UI_ACTIONS`・`UI_TARGETS` の固定語彙） | ✅登録済 (2026-09-26 Admin API で作成・audit-api で確認) | 全ページ |
| `read_progress` | `trackReadProgress` | `progress`（25・50・75・100） | ✅登録済 (2026-09-26 Admin API で作成・audit-api で確認) | 全ページ |
| `search_result_click` | `trackSearchResultClick` | `result_type` / `result_position` | ✅登録済 (2026-09-26 Admin API で作成・audit-api で確認) | 検索 |
| `contact_click` | `trackContactClick` | `link_position` は登録済み（key event 候補） | 要否× | 行政資料 |
| ユーザー単位 `declared_purpose` | `setDeclaredPurpose` | `declared_purpose`（USER scope。値は CSV 後アンケートの固定語彙） | ✅登録済 (2026-09-26 Admin API で作成・audit-api で確認) | 行政資料 |
| `geo_analysis_view` | `trackGeoAnalysisView` | `analysis_slug` / `geography`（`analysis_id` は slug と重複、`data_version` は分析価値が低いので登録しない） | ✅登録済 (2026-09-26 Admin API で作成・audit-api で確認) | geo M1 |
| `geo_map_interaction` | `trackGeoMapInteraction` | `analysis_slug` / `geography` / `interaction_type` / `area_code` | ✅登録済 (2026-09-26 Admin API で作成・audit-api で確認) | geo M1 |
| `geo_region_select` | `trackGeoRegionSelect` | `analysis_slug` / `geography` / `area_code` | ✅登録済 (2026-09-26 Admin API で作成・audit-api で確認) | geo M1 |
