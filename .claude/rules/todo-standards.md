...
- 作業中に見つけた環境・ツールの不具合 (MCP の接続失敗・認証切れ、CLI の故障など) は、作業を止めたか
  次のセッションでも再現するものだけ起票する。先に backlog を ID・サービス名で検索し、同じ原因のカードが
  あれば観測日と症状を追記して新規カードを作らない。ユーザーしか直せないもの (OAuth の再認証など) は
  `[実行:ユーザー]` を付ける。利用上限や一度きりの接続断など期限付きで解消するものは起票せず、報告に留める。
