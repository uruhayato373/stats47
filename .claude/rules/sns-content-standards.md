...
| **Threads** | 新規 (2026-09-23〜) | サイト送客・IG 以外の Meta 面の反応検証 | 予約 2 / 日 (X の朝枠・昼枠の題材を 10 時台・17 時台に転用。Playwright で Threads Web の予約機能を使う `/publish-threads`。予約は同時 25 件までなので枠が空いたら補充) | テキスト + 画像 1 枚 | `x-strategist` |
...
| **Threads は 1 日 2 本** | 2 / 日 | 2026-09-23 新設。`plan-from-x.cjs` が 1 日 2 件だけ下書きにする。本文 500 字以内。Threads 側の予約は同時 25 件まで (実測) |
...
> 期間終了後は既定に戻った。2026-09-23 以降の既定は §2-3c の曜日ローテーション。
...
### 2-3c. Instagram 曜日ローテーション (2026-09-28 週〜)

カルーセルは毎日 1 本、切り口を曜日で固定して回す。ランキング単体の画像 1 枚は出さない
(2026-07-12〜08-10 の 1 日 3 本実験でリーチ中央値 58・いいね 0 が 208 件中 163 件。本数では伸びなかった)。
...
- リールは週 1 本 (§2-4)。クイズのリールと年次推移 (バーチャートレース) を隔週で交互に出す
- すべての項目に出典と年を載せる。家計調査は「県庁所在市の値」と明記する
- **判定 (2026-10-25)**: 4 週間の切り口別の保存数・リーチを `/update-sns-metrics` で比べる。
  カルーセル全体のリーチ中央値が単枚画像期の 58 を超えなければ、本数を週 3 本へ戻す

### 2-4. Instagram リール

- **15 字以内のフック**を冒頭に (予測 → 答え合わせ型)。例:「1 位は意外なあの県」
- 数字対比 × 意外性で 3-5 秒以内に引く。完視聴・保存を狙う
- **予想クイズ型リールは YouTube pilot から独立して週 1 本出してよい (2026-09-23 オーナー判断)**。
  pilot が動画 0 本のまま止まり、IG のリールも 0 本になっていたため。§0 の「リールは通常動画から切り出す」は
  pilot の切り出し分にだけ適用する。生成は Remotion `RankingQuizInstagram-Reel` (1080x1920・無音) で、
  入力はカルーセルと同じ quiz props (§2-3b)。正解・順位・値はデータから導出し、カルーセルで失敗する spec はリールでも失敗する
- **配信**: domain は `ranking-quiz-reel` (台帳は domain + content_key で重複判定するため、同じ指標のカルーセル
  `ranking-quiz` と分ける)。R2 は `sns/ranking-quiz-reel/<rankingKey>/instagram/` に `reel.mp4` と `caption.txt` を置き、
  schedule JSON に `{"type":"reels","domain":"ranking-quiz-reel","content_key":"<rankingKey>",...}` を足すと IG cron が投稿する
...
- **題材は悩み起点と「県の暮らしを読む」型を優先する (2026-09-23)**。例: 悩み起点ブログの衛星記事、
  「○○県の家計は何にお金が偏っているか」シリーズ。単一ランキングの A シリーズは 1 位が東京都の指標を
  生成段階で止める (`post-note-ranking` の `PREDICTABLE_LEADER`)。2026-09-06 公開分の多くが「1 位は東京都」で、
  人口・経済規模がそのまま出る予想どおりの記事になっていたため
...
- **IG cron の schedule ファイルは自動選択** (2026-07-07〜): `post-from-schedule.cjs` は
  `instagram-w*-schedule.json` を全走査して前日・当日のエントリを集める。
  旧実装は特定週固定で更新忘れ→ cron 空振り事故が実発生 (w20 期間に w19 を読み続け 1 ヶ月未投稿)
- **前日の未投稿も拾う** (2026-09-24〜): GitHub Actions の夜枠 cron (19:03 予定) は実測で 22:30〜翌 01:24 に
  発火し、日付をまたぐと当日分しか見ない実装では前日 19:00 枠が消えていた (2026-09 の 3 週で 3 回)。
...
別の字に見えていた。原因は見出し書体 Dela Gothic One の字形そのもの (400 の単一ウェイトでも字面が極端に詰まる)。
疑似太字を止めた `IG_HEADLINE_STYLE` の修正後も同じ形で描かれることを 2026-09-24 に実測し (Google Fonts 単体の描画と一致)、
見出しを **M PLUS 1p 900** (実ウェイト) に替えて予約中の全カルーセルを再レンダー・R2 差し替えした。
**見出し書体を選ぶときは、画数の多い漢字 (何・稿・徴・係) を実寸で描いて内側が抜けるかを確かめる。
テンプレートを直したら予約中の素材も再レンダーする。**
