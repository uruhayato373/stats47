...
- **書き込み**はローカル / CI 両方から remote R2 へ直接可能。常駐するローカル R2 ミラーは廃止済み
  (`.local/r2/app` は snapshot/article の push staging に限り年齢では消さない。`.local/r2/gis` は公開可 KSJ の
  R2 ミラーで再取得できるため `local:cleanup` が 60 日未使用で回収する。2026-09-14 実測: 2.1GB のうち
  `app/` 14MB・`gis/` 2,130MB)。
