...
### 可視化は SVG 図に統一・表は原則禁止 (markdown 表 全面禁止)
...
### 記事 markdown の正典テンプレート (コンポーネント二重・未描画を排除)
...
### 図あたり prose 字数の床 (「図はあるが薄い」を決定的に弾く)
...
- 全型共通: 個数の最低条件は設けない (目安は記事全体で 0-3 個・上の「callout」節と同じ)。置くときは該当データを論じる H2 内 (冒頭でなく文脈の中) に置く。
