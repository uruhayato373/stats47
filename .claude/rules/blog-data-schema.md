...
- **左上の余白**にタイトル（長ければ折返し）・サブタイトル・**上位 3 県**（色チップ + 県名（「宮崎県」まで） + 値と単位）。地図だけでは読めない実数値をここで出す。値の大小を機械的に並べるだけで、指標の良し悪しは判断しない。**下位 3 県は出さない**（地図を最大化した分スペースが減り、上位だけの方が読みやすい）。見出し（「多い順」等）も置かず「1. 宮崎県」の形で並べる。
