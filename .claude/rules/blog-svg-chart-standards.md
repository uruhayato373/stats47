...
| ------------------------- | ------------------ | ------------------------------------------------------------------------------------ | --------------------------------------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `generateBarChartSvg`     | `bar-chart.ts`     | `BarItem[]` + `BarChartOptions`                                                      | `*-ranking.json` / `*-top5-bottom5.json`      | ~239   | ランキングはカード型のみ・**上位5+下位5固定**（10件廃止）。`layout:"columns"`=横長2列カード（`960×404`、PC本文用 `<name>.svg`）/ `layout:"mobile"`=縦長本文カード（`640×960`、ブログ mobile・note 本文用 `<name>-mobile.svg`）/ `layout:"portrait"`=縦長SNSカード（`1080×1350` 4:5、Instagram 用 `<name>-ig.svg`）/ `layout:"single"`=縦1列+「…中略…」(680幅)。`generate-article-charts.ts` は3媒体版を同じデータから自動出力する                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| ランキング本文縦長（mobile・ブログ/note） | `640`  | `960`（2:3固定）         | スマホで文字を縮小しない縦長スタック               |
...
**svg-builder の各チャートは §5 標準幅に収斂済**: bar single=680（width/height も 680 に一致、旧 DISPLAY_W=780 スケーリングは廃止）/ **ランキングカード PC=960×404・mobile/note=640×960・Instagram=1080×1350** / scatter=720×720（2026-08-02）/ map=720×720 / line=680×420 / stacked=680 / findings=960。新規生成は自動的にこの規格になる。
...
| `bar-mobile`           | 640                           | **error** |
| `bar-social`（`-ig`）  | 1080                          | **error** |
...
- ブログ本文が参照する横長ランキング SVG には、同名の `-mobile.svg` を必須とする。`quality-gate.mjs` と
  `audit-chart-quality.mjs` が欠落を blocker として検出する。
- `-mobile.svg` は `640×960`、最小フォント `20px`。横長画像を CSS だけで縮小して代用しない。
- Web は `<picture>` で `639px` 以下だけ `-mobile.svg` へ切り替える。未移行の公開記事は画像切れを避けるため
  desktop へフォールバックするが、新規・再生成時の欠落は gate で許可しない。
- note 本文は mobile レイアウトを PNG 化して使う。Instagram は `-ig.svg` の4:5を使い、本文画像と共用しない。
- X は現状 PC横長版を利用する。媒体ごとのクロップや文字密度を別々に最適化し、単一画像の万能化を禁止する。

### 6-4. 散布図品質・データ完全性 gate（2026-08-02 追加）
