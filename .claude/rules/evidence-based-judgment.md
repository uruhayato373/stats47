...
実証なしの仕様主張・効果判定が過去の改善ログに多数混入し、その判定を前提に次の施策が組まれる連鎖が起きた
(例: Google の再クロールを確認しないまま「410 では検証に合格しない」と仕様を断定した)。本ルールはその再発防止である。
...
GSC（URL Inspection API）/ GA4 / PSI・CrUX / Googlebot UA での HTTP 確認 / サードパーティ公式ドキュメントの
取得コマンドは `.claude/skills/analytics/seo-audit/reference/verification-commands.md` にある。
「Google にどう見えているか」を主張するときは URL Inspection API の生レスポンスを引用する。
