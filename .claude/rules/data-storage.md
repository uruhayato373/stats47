...
> **⚠️ 2026-05-29 更新: データ層は DB レス設計が正典 → [`docs/01_技術設計/18_DBレスデータ設計.md`](../../docs/01_技術設計/18_DBレスデータ設計.md)。**
> 以下「ローカルビルド DB (SQLite) に置くもの」は、現在は **R2 JSON を SSOT とする Authored データ**
> （定義は git TS、反映は冪等スクリプト）と読み替える。`.claude/` と `docs/` の使い分け（本ファイルの主眼）は不変。
> アプリが CRUD するエンティティの判定フローは「→ DB」ではなく「→ R2 JSON (Authored) / 再生成 (Reference) / エフェメラル計算 (Derived)」。
...
```
スキルが生成するデータの本質は？
  ├─ アプリが読む Authored エンティティ        → R2 JSON (app/...) を SSOT、定義は git TS + 冪等反映スクリプト
  │    （旧: D1。詳細は 18_DBレスデータ設計.md）
  ├─ 人間が読み返す計画・レビュー・改善ログ    → docs/
  ├─ エージェントが参照する詳細ログ・state    → .claude/
  └─ PR/Issue 連携が本質                     → GitHub Issues (enhancement/bug)
```

迷う場合は **docs/ または .claude/ を優先**。アプリが読むデータは **R2 JSON**（永続 DB は持たない）。
