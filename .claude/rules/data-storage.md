...
| 計測→記録→改善サイクルの週次 state (無人 triage の入力と記録結果) | `.claude/state/metrics/measurement-cycle/{latest.json,LATEST.md,history.csv,triage-latest.json}`（計測 state は `fetch-metrics-weekly.yml` の `build-measurement-cycle.mjs`、`triage-latest.json` は `improvement-cycle-weekly.yml` の `verify-improvement-cycle-run.mjs` が書く。週次メトリクス Issue と `/weekly-review` が読む） |
