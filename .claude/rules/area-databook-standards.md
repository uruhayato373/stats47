...
| 特産品イラスト | R2 `app/areas/<code>/specialty/<slug>.webp` | Gemini 生成 (task `area-specialty`)。欠損時はテキストカードに degrade |
...
| 特産品イラスト生成 | Gemini (`gemini-image-run.yml` task `area-specialty`) |
