...
| 旧 AreaOgp | `apps/web/src/features/ogp/AreaOgp.tsx` は **generator から deprecated** (generate-ogp-images.ts は参照しない)。ただし route `app/areas/[areaCode]/opengraph-image.tsx` は **dead ではない** — 子孫 `/areas/*/cities/*` の og:image がこの ancestor ファイル規約を継承する (`openGraph.images` metadata では深い子孫でファイル規約を抑止できない。2026-07-20 検証で確定。→ city 系の og:image 500 の実体)。**★2026-07-20 深掘りで判明した blocker**: このルートは `loadOgpFonts()` が **`fetch('https://fonts.googleapis.com/...')` でランタイム日本語フォントを取得する**ため、Worker (外部 fetch 不可) でも Node dev サンドボックスでも **render 自体が 500** になる (themes/[themeSlug]/opengraph-image は `fontFamily:"sans-serif"` で fetch せず prerender 200)。**したがって `generateStaticParams` を単純追加して prerender 化してはならない** — ビルド時に Google Fonts fetch が失敗すると prerender エラーで**ビルド全体が落ち全デプロイがブロックされる**。**正しい是正 (未実施 follow-up)**: areas opengraph-image (と font-loader) を `@expo-google-fonts/noto-sans-jp` 等の**ローカル同梱フォント**に変更 (generate-ogp-images.ts / blog-thumbnail-render.ts §5 と同方式) してランタイム fetch を排し、その上で `generateStaticParams` で prerender 200 化する。これで city 系が 200 を継承する。area 詳細自体は `generate-area-metadata.ts` の静的 R2 (県シルエット) で既に 200。**この route を削除してはならない** (削除すると city が root opengraph-image=500 に落ちる)。§3 課題0 の本筋 (root/area OGP 完全静的化) の一部として扱う |
