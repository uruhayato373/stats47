...
> **⚠️ 2026-05-29 更新: 正典は [`docs/01_技術設計/18_DBレスデータ設計.md`](../../docs/01_技術設計/18_DBレスデータ設計.md) に移行した。**
> stats47 は **DB レス設計**（永続 DB を持たない）に決定。Authored データ (page_components 等) は
> **R2 JSON が SSOT**（git TS から冪等スクリプトで反映）、Derived (area_profiles/相関) は
> **エフェメラル計算 → R2**。`db:pull` の SQLite 持ち回りは段階廃止中。
> 本ファイルの「ローカルビルド DB を SSOT 派生 cache とする」記述は移行期の参考。
> **新規実装は 18 の DB レス経路で書くこと。** 食い違う場合は 18 が優先。
