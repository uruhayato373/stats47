...
- **ルートグループ**: 機能別に分離されたページ構造
  - `(public)/` - 公開ページ（トップ、About、Blog）
  - `(stats)/` - 統計データページ（カテゴリー、サブカテゴリー）
  - `admin/` - 管理画面（認証必須）
- **動的ルーティング**: `/{category}/{subcategory}/area/{areaCode}` と `/{category}/{subcategory}/ranking/{rankingKey}` による階層的なページ構造
...
- **管理画面**: 開発・管理ツールの統合（認証必須）
  - 地図データキャッシュ管理
  - 可視化設定管理
  - e-Stat API 関連ツール（開発環境のみ）
