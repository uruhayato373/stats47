...
const CAT01_PAVED_ROAD_LENGTH = "H7120"; // 舗装道路実延長

interface PavedRoadLengthCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 舗装道路実延長統計カード
 */
export async function PavedRoadLengthCard({
  areaCode,
  timeCode,
  title = "舗装道路実延長",
}: PavedRoadLengthCardProps) {
  try {
    // e-Stat APIから舗装道路実延長データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_PAVED_ROAD_LENGTH,
      areaFilter: areaCode,
    ...
    console.error("[PavedRoadLengthCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
