...
interface MarriageCountCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 婚姻件数統計カード
 */
export async function MarriageCountCard({
  areaCode,
  timeCode,
  title = "婚姻件数",
}: MarriageCountCardProps) {
  try {
    // e-Stat APIから婚姻件数データを取得
    ...
    console.error("[MarriageCountCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
