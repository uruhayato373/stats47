...
interface DivorceCountCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 離婚件数統計カード
 */
export async function DivorceCountCard({
  areaCode,
  timeCode,
  title = "離婚件数",
}: DivorceCountCardProps) {
  try {
    // e-Stat APIから離婚件数データを取得
    ...
    console.error("[DivorceCountCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
