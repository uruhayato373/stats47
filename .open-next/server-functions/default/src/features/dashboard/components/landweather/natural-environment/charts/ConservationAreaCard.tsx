...
const CAT01_CONSERVATION_AREA = "B1108"; // 自然環境保全地域面積
 ...
 * 自然環境保全地域面積統計カード
 */
export async function ConservationAreaCard({
  areaCode,
  timeCode,
  title = "自然環境保全地域面積",
}: ConservationAreaCardProps) {
  try {
    // e-Stat APIから自然環境保全地域面積データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_CONSERVATION_AREA,
      areaFilter: areaCode,
    ...
    console.error("[ConservationAreaCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
