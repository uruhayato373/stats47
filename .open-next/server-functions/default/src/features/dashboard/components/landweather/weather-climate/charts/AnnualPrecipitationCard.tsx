...
interface AnnualPrecipitationCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 年間降水量統計カード
 */
export async function AnnualPrecipitationCard({
  areaCode,
  timeCode,
  title = "年間降水量",
}: AnnualPrecipitationCardProps) {
  try {
    // e-Stat APIから年間降水量データを取得
    ...
    console.error("[AnnualPrecipitationCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
