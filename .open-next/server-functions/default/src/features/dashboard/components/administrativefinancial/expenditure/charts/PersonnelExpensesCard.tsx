...
const CAT01_PERSONNEL_EXPENSES = "D310401"; // 人件費（都道府県財政）

interface PersonnelExpensesCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 人件費（都道府県財政）統計カード
 */
export async function PersonnelExpensesCard({
  areaCode,
  timeCode,
  title = "人件費（都道府県財政）",
}: PersonnelExpensesCardProps) {
  try {
    // e-Stat APIから人件費データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_PERSONNEL_EXPENSES,
      areaFilter: areaCode,
    ...
    console.error("[PersonnelExpensesCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
