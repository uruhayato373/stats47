...
const CAT01_TOTAL_GENERAL_PROJECT_INVESTMENT = "D5200"; // 一般事業総投資額

interface TotalGeneralProjectInvestmentCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 一般事業総投資額統計カード
 */
export async function TotalGeneralProjectInvestmentCard({
  areaCode,
  timeCode,
  title = "一般事業総投資額",
}: TotalGeneralProjectInvestmentCardProps) {
  try {
    // e-Stat APIから一般事業総投資額データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_TOTAL_GENERAL_PROJECT_INVESTMENT,
      areaFilter: areaCode,
    ...
    console.error("[TotalGeneralProjectInvestmentCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
