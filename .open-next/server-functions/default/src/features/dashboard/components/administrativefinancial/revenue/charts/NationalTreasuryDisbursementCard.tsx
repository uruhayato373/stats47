...
const CAT01_NATIONAL_TREASURY_DISBURSEMENT = "D310108"; // 国庫支出金（都道府県財政）

interface NationalTreasuryDisbursementCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 国庫支出金（都道府県財政）統計カード
 */
export async function NationalTreasuryDisbursementCard({
  areaCode,
  timeCode,
  title = "国庫支出金（都道府県財政）",
}: NationalTreasuryDisbursementCardProps) {
  try {
    // e-Stat APIから国庫支出金データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_NATIONAL_TREASURY_DISBURSEMENT,
      areaFilter: areaCode,
    ...
    console.error("[NationalTreasuryDisbursementCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
