...
interface InvestmentExpensesCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 投資的経費（都道府県財政）統計カード
 */
export async function InvestmentExpensesCard({
  areaCode,
  timeCode,
  title = "投資的経費（都道府県財政）",
}: InvestmentExpensesCardProps) {
  try {
    // e-Stat APIから投資的経費データを取得
    ...
    console.error("[InvestmentExpensesCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
