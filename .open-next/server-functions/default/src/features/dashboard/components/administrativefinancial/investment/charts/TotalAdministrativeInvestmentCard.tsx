...
const CAT01_TOTAL_ADMINISTRATIVE_INVESTMENT = "D5100"; // 行政総投資額

interface TotalAdministrativeInvestmentCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 行政総投資額統計カード
 */
export async function TotalAdministrativeInvestmentCard({
  areaCode,
  timeCode,
  title = "行政総投資額",
}: TotalAdministrativeInvestmentCardProps) {
  try {
    // e-Stat APIから行政総投資額データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_TOTAL_ADMINISTRATIVE_INVESTMENT,
      areaFilter: areaCode,
    ...
    console.error("[TotalAdministrativeInvestmentCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
