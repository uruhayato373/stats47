...
const CAT01_JUNIOR_HIGH_SCHOOL_TEACHER_COUNT = "E3401"; // 中学校教員数

interface JuniorHighSchoolTeacherCountCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 中学校教員数統計カード
 */
export async function JuniorHighSchoolTeacherCountCard({
  areaCode,
  timeCode,
  title = "中学校教員数",
}: JuniorHighSchoolTeacherCountCardProps) {
  try {
    // e-Stat APIから中学校教員数データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_JUNIOR_HIGH_SCHOOL_TEACHER_COUNT,
      areaFilter: areaCode,
    ...
    console.error("[JuniorHighSchoolTeacherCountCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
