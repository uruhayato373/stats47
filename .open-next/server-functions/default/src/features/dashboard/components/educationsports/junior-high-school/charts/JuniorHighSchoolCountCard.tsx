...
const CAT01_JUNIOR_HIGH_SCHOOL_COUNT = "E3101"; // 中学校数

interface JuniorHighSchoolCountCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 中学校数統計カード
 */
export async function JuniorHighSchoolCountCard({
  areaCode,
  timeCode,
  title = "中学校数",
}: JuniorHighSchoolCountCardProps) {
  try {
    // e-Stat APIから中学校数データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_JUNIOR_HIGH_SCHOOL_COUNT,
      areaFilter: areaCode,
    ...
    console.error("[JuniorHighSchoolCountCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
