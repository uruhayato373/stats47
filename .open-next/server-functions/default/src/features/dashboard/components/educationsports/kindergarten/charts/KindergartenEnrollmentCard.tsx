...
interface KindergartenEnrollmentCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 幼稚園在園者数統計カード
 */
export async function KindergartenEnrollmentCard({
  areaCode,
  timeCode,
  title = "幼稚園在園者数",
}: KindergartenEnrollmentCardProps) {
  try {
    // e-Stat APIから幼稚園在園者数データを取得
    ...
    console.error("[KindergartenEnrollmentCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
