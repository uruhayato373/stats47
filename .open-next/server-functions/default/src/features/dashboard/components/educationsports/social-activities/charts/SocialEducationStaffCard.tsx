...
interface SocialEducationStaffCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 社会教育関係職員数統計カード
 */
export async function SocialEducationStaffCard({
  areaCode,
  timeCode,
  title = "社会教育関係職員数",
}: SocialEducationStaffCardProps) {
  try {
    // e-Stat APIから社会教育関係職員数データを取得
    ...
    console.error("[SocialEducationStaffCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
