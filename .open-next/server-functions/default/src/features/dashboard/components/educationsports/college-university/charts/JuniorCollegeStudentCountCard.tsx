...
const CAT01_JUNIOR_COLLEGE_STUDENT_COUNT = "E6301"; // 短期大学学生数

interface JuniorCollegeStudentCountCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 短期大学学生数統計カード
 */
export async function JuniorCollegeStudentCountCard({
  areaCode,
  timeCode,
  title = "短期大学学生数",
}: JuniorCollegeStudentCountCardProps) {
  try {
    // e-Stat APIから短期大学学生数データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_JUNIOR_COLLEGE_STUDENT_COUNT,
      areaFilter: areaCode,
    ...
    console.error("[JuniorCollegeStudentCountCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
