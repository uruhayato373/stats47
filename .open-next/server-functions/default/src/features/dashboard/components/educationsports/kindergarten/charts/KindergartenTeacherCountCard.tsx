...
const CAT01_KINDERGARTEN_TEACHER_COUNT = "E1301"; // 幼稚園教員数

interface KindergartenTeacherCountCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 幼稚園教員数統計カード
 */
export async function KindergartenTeacherCountCard({
  areaCode,
  timeCode,
  title = "幼稚園教員数",
}: KindergartenTeacherCountCardProps) {
  try {
    // e-Stat APIから幼稚園教員数データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_KINDERGARTEN_TEACHER_COUNT,
      areaFilter: areaCode,
    ...
    console.error("[KindergartenTeacherCountCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
