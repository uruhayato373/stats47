...
const CAT01_PRIVATE_GYMNASIUM_COUNT = "G320106"; // 体育館数（民間）

interface PrivateGymnasiumCountCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 体育館数（民間）統計カード
 */
export async function PrivateGymnasiumCountCard({
  areaCode,
  timeCode,
  title = "体育館数（民間）",
}: PrivateGymnasiumCountCardProps) {
  try {
    // e-Stat APIから体育館数（民間）データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_PRIVATE_GYMNASIUM_COUNT,
      areaFilter: areaCode,
    ...
    console.error("[PrivateGymnasiumCountCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
