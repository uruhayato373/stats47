...
const CAT01_UNIVERSITY_COUNT = "E6102"; // 大学数

interface UniversityCountCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 大学数統計カード
 */
export async function UniversityCountCard({
  areaCode,
  timeCode,
  title = "大学数",
}: UniversityCountCardProps) {
  try {
    // e-Stat APIから大学数データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_UNIVERSITY_COUNT,
      areaFilter: areaCode,
    ...
    console.error("[UniversityCountCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
