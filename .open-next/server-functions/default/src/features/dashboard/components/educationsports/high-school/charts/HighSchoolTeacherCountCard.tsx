...
const CAT01_HIGH_SCHOOL_TEACHER_COUNT = "E4401"; // 高等学校教員数

interface HighSchoolTeacherCountCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 高等学校教員数統計カード
 */
export async function HighSchoolTeacherCountCard({
  areaCode,
  timeCode,
  title = "高等学校教員数",
}: HighSchoolTeacherCountCardProps) {
  try {
    // e-Stat APIから高等学校教員数データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_HIGH_SCHOOL_TEACHER_COUNT,
      areaFilter: areaCode,
    ...
    console.error("[HighSchoolTeacherCountCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
