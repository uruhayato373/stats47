...
const CAT01_PUBLIC_SWIMMING_POOL_COUNT = "G310204"; // 水泳プール数（屋内、屋外）（公共）

interface PublicSwimmingPoolCountCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 水泳プール数（公共）統計カード
 */
export async function PublicSwimmingPoolCountCard({
  areaCode,
  timeCode,
  title = "水泳プール数（公共）",
}: PublicSwimmingPoolCountCardProps) {
  try {
    // e-Stat APIから水泳プール数（公共）データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_PUBLIC_SWIMMING_POOL_COUNT,
      areaFilter: areaCode,
    ...
    console.error("[PublicSwimmingPoolCountCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
