...
const CAT01_CONSUMER_PRICE_INDEX = "C5115"; // 消費者物価指数変化率・帰属家賃を除く総合

interface ConsumerPriceIndexCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 消費者物価指数変化率統計カード
 */
export async function ConsumerPriceIndexCard({
  areaCode,
  timeCode,
  title = "消費者物価指数変化率",
}: ConsumerPriceIndexCardProps) {
  try {
    // e-Stat APIから消費者物価指数変化率データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_CONSUMER_PRICE_INDEX,
      areaFilter: areaCode,
    ...
    console.error("[ConsumerPriceIndexCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
