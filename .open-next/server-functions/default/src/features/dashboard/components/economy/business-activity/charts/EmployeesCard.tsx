...
const CAT01_EMPLOYEES = "C2201"; // 従業者数（事業所・企業統計調査結果）
 ...
 * 従業者数統計カード
 */
export async function EmployeesCard({
  areaCode,
  timeCode,
  title = "従業者数",
}: EmployeesCardProps) {
  try {
    // e-Stat APIから従業者数データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_EMPLOYEES,
      areaFilter: areaCode,
    ...
    console.error("[EmployeesCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
