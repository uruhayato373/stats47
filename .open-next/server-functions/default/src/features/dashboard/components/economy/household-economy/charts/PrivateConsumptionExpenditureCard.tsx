...
interface PrivateConsumptionExpenditureCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 民間最終消費支出統計カード
 */
export async function PrivateConsumptionExpenditureCard({
  areaCode,
  timeCode,
  title = "民間最終消費支出",
}: PrivateConsumptionExpenditureCardProps) {
  try {
    // e-Stat APIから民間最終消費支出データを取得
    ...
    console.error(
      "[PrivateConsumptionExpenditureCard] データ取得エラー:",
      error
    );
    return (
      <Card>
