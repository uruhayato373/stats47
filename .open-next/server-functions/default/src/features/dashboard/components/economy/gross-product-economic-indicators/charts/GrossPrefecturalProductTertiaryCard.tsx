...
const CAT01_GROSS_PREFECTURAL_PRODUCT_TERTIARY = "C1127"; // 県内総生産額（第3次産業）（平成27年基準）

interface GrossPrefecturalProductTertiaryCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 県内総生産額（第3次産業）統計カード
 */
export async function GrossPrefecturalProductTertiaryCard({
  areaCode,
  timeCode,
  title = "県内総生産額（第3次産業）",
}: GrossPrefecturalProductTertiaryCardProps) {
  try {
    // e-Stat APIから県内総生産額（第3次産業）データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_GROSS_PREFECTURAL_PRODUCT_TERTIARY,
      areaFilter: areaCode,
    ...
    console.error(
      "[GrossPrefecturalProductTertiaryCard] データ取得エラー:",
      error
    );
    return (
      <Card>
