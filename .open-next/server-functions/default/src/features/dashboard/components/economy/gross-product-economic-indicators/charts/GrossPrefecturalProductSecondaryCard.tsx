...
const CAT01_GROSS_PREFECTURAL_PRODUCT_SECONDARY = "C1126"; // 県内総生産額（第2次産業）（平成27年基準）

interface GrossPrefecturalProductSecondaryCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 県内総生産額（第2次産業）統計カード
 */
export async function GrossPrefecturalProductSecondaryCard({
  areaCode,
  timeCode,
  title = "県内総生産額（第2次産業）",
}: GrossPrefecturalProductSecondaryCardProps) {
  try {
    // e-Stat APIから県内総生産額（第2次産業）データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_GROSS_PREFECTURAL_PRODUCT_SECONDARY,
      areaFilter: areaCode,
    ...
    console.error(
      "[GrossPrefecturalProductSecondaryCard] データ取得エラー:",
      error
    );
    return (
      <Card>
