...
const CAT01_MANUFACTURING_EMPLOYEES = "C3404"; // 製造業従業者数

interface ManufacturingEmployeesCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 製造業従業者数統計カード
 */
export async function ManufacturingEmployeesCard({
  areaCode,
  timeCode,
  title = "製造業従業者数",
}: ManufacturingEmployeesCardProps) {
  try {
    // e-Stat APIから製造業従業者数データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_MANUFACTURING_EMPLOYEES,
      areaFilter: areaCode,
    ...
    console.error("[ManufacturingEmployeesCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
