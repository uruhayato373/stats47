...
interface TotalOvernightGuestsCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 延べ宿泊者数統計カード
 */
export async function TotalOvernightGuestsCard({
  areaCode,
  timeCode,
  title = "延べ宿泊者数",
}: TotalOvernightGuestsCardProps) {
  try {
    // e-Stat APIから延べ宿泊者数データを取得
    ...
    console.error("[TotalOvernightGuestsCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
