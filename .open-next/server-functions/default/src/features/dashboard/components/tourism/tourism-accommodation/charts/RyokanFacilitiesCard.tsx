...
interface RyokanFacilitiesCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 旅館営業施設数統計カード
 */
export async function RyokanFacilitiesCard({
  areaCode,
  timeCode,
  title = "旅館営業施設数（ホテルを含む）",
}: RyokanFacilitiesCardProps) {
  try {
    // e-Stat APIから旅館営業施設数データを取得
    ...
    console.error("[RyokanFacilitiesCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
