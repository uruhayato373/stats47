...
const CAT01_FOREIGN_OVERNIGHT_GUESTS = "G7102"; // 外国人延べ宿泊者数

interface ForeignOvernightGuestsCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 外国人延べ宿泊者数統計カード
 */
export async function ForeignOvernightGuestsCard({
  areaCode,
  timeCode,
  title = "外国人延べ宿泊者数",
}: ForeignOvernightGuestsCardProps) {
  try {
    // e-Stat APIから外国人延べ宿泊者数データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_FOREIGN_OVERNIGHT_GUESTS,
      areaFilter: areaCode,
    ...
    console.error("[ForeignOvernightGuestsCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
