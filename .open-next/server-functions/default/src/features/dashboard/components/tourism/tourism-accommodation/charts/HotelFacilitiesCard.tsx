...
const CAT01_HOTEL_FACILITIES = "C3803"; // ホテル営業施設数

interface HotelFacilitiesCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * ホテル営業施設数統計カード
 */
export async function HotelFacilitiesCard({
  areaCode,
  timeCode,
  title = "ホテル営業施設数",
}: HotelFacilitiesCardProps) {
  try {
    // e-Stat APIからホテル営業施設数データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_HOTEL_FACILITIES,
      areaFilter: areaCode,
    ...
    console.error("[HotelFacilitiesCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
