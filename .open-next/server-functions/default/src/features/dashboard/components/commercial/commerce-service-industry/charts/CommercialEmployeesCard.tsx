...
const CAT01_COMMERCIAL_EMPLOYEES = "C3503"; // 商業従業者数（卸売業＋小売業）

interface CommercialEmployeesCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 商業従業者数統計カード
 */
export async function CommercialEmployeesCard({
  areaCode,
  timeCode,
  title = "商業従業者数",
}: CommercialEmployeesCardProps) {
  try {
    // e-Stat APIから商業従業者数データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_COMMERCIAL_EMPLOYEES,
      areaFilter: areaCode,
    ...
    console.error("[CommercialEmployeesCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
