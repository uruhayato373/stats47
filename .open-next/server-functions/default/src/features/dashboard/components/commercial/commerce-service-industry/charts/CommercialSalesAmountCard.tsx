...
interface CommercialSalesAmountCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 商業年間商品販売額統計カード
 */
export async function CommercialSalesAmountCard({
  areaCode,
  timeCode,
  title = "商業年間商品販売額",
}: CommercialSalesAmountCardProps) {
  try {
    // e-Stat APIから商業年間商品販売額データを取得
    ...
    console.error("[CommercialSalesAmountCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
