...
const CAT01_PUBLIC_ASSISTANCE_EXPENSES = "D3103034"; // 生活保護費（都道府県財政）

interface PublicAssistanceExpensesCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 生活保護費（都道府県財政）統計カード
 */
export async function PublicAssistanceExpensesCard({
  areaCode,
  timeCode,
  title = "生活保護費（都道府県財政）",
}: PublicAssistanceExpensesCardProps) {
  try {
    // e-Stat APIから生活保護費データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_PUBLIC_ASSISTANCE_EXPENSES,
      areaFilter: areaCode,
    ...
    console.error(
      "[PublicAssistanceExpensesCard] データ取得エラー:",
      error
    );
    return (
      <Card>
