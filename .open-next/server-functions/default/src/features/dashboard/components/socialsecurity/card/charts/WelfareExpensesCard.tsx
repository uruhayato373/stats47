...
const CAT01_WELFARE_EXPENSES = "D310303"; // 民生費（都道府県財政）

interface WelfareExpensesCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 民生費（都道府県財政）統計カード
 */
export async function WelfareExpensesCard({
  areaCode,
  timeCode,
  title = "民生費（都道府県財政）",
}: WelfareExpensesCardProps) {
  try {
    // e-Stat APIから民生費データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_WELFARE_EXPENSES,
      areaFilter: areaCode,
    ...
    console.error("[WelfareExpensesCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
