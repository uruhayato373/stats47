...
interface EmployedCountCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 就業者数統計カード
 */
export async function EmployedCountCard({
  areaCode,
  timeCode,
  title = "就業者数",
}: EmployedCountCardProps) {
  try {
    // e-Stat APIから就業者数データを取得
    ...
    console.error("[EmployedCountCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
