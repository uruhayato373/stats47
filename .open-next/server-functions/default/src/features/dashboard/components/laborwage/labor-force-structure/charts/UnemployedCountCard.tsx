...
const CAT01_UNEMPLOYED_COUNT = "F1107"; // 完全失業者数

interface UnemployedCountCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 完全失業者数統計カード
 */
export async function UnemployedCountCard({
  areaCode,
  timeCode,
  title = "完全失業者数",
}: UnemployedCountCardProps) {
  try {
    // e-Stat APIから完全失業者数データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_UNEMPLOYED_COUNT,
      areaFilter: areaCode,
    ...
    console.error("[UnemployedCountCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
