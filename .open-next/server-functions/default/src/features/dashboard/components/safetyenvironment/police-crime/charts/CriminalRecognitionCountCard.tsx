...
interface CriminalRecognitionCountCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 刑法犯認知件数統計カード
 */
export async function CriminalRecognitionCountCard({
  areaCode,
  timeCode,
  title = "刑法犯認知件数",
}: CriminalRecognitionCountCardProps) {
  try {
    // e-Stat APIから刑法犯認知件数データを取得
    ...
    console.error("[CriminalRecognitionCountCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
