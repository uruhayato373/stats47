...
const CAT01_FIRE_INSURANCE_PAYMENT_COUNT = "K2213"; // 火災保険保険金支払件数

interface FireInsurancePaymentCountCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 火災保険保険金支払件数統計カード
 */
export async function FireInsurancePaymentCountCard({
  areaCode,
  timeCode,
  title = "火災保険保険金支払件数",
}: FireInsurancePaymentCountCardProps) {
  try {
    // e-Stat APIから火災保険保険金支払件数データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_FIRE_INSURANCE_PAYMENT_COUNT,
      areaFilter: areaCode,
    ...
    console.error("[FireInsurancePaymentCountCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
