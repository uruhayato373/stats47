...
interface EmergencyDispatchCountCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 救急出動件数統計カード
 */
export async function EmergencyDispatchCountCard({
  areaCode,
  timeCode,
  title = "救急出動件数",
}: EmergencyDispatchCountCardProps) {
  try {
    // e-Stat APIから救急出動件数データを取得
    ...
    console.error("[EmergencyDispatchCountCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
