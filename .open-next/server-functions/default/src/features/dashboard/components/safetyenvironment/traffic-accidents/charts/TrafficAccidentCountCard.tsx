...
interface TrafficAccidentCountCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 交通事故発生件数統計カード
 */
export async function TrafficAccidentCountCard({
  areaCode,
  timeCode,
  title = "交通事故発生件数",
}: TrafficAccidentCountCardProps) {
  try {
    // e-Stat APIから交通事故発生件数データを取得
    ...
    console.error("[TrafficAccidentCountCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
