...
const CAT01_FIRE_DEPARTMENT_COUNT = "K1101"; // 消防本部・署数

interface FireDepartmentCountCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 消防本部・署数統計カード
 */
export async function FireDepartmentCountCard({
  areaCode,
  timeCode,
  title = "消防本部・署数",
}: FireDepartmentCountCardProps) {
  try {
    // e-Stat APIから消防本部・署数データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_FIRE_DEPARTMENT_COUNT,
      areaFilter: areaCode,
    ...
    console.error("[FireDepartmentCountCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
