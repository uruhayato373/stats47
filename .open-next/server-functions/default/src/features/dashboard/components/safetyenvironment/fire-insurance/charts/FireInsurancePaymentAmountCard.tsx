...
const CAT01_FIRE_INSURANCE_PAYMENT_AMOUNT = "K2215"; // 火災保険保険金支払金額

interface FireInsurancePaymentAmountCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 火災保険保険金支払金額統計カード
 */
export async function FireInsurancePaymentAmountCard({
  areaCode,
  timeCode,
  title = "火災保険保険金支払金額",
}: FireInsurancePaymentAmountCardProps) {
  try {
    // e-Stat APIから火災保険保険金支払金額データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_FIRE_INSURANCE_PAYMENT_AMOUNT,
      areaFilter: areaCode,
    ...
    console.error("[FireInsurancePaymentAmountCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
