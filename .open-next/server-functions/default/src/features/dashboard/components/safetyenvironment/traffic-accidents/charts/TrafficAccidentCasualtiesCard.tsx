...
const CAT01_TRAFFIC_ACCIDENT_CASUALTIES = "K3102"; // 交通事故死傷者数

interface TrafficAccidentCasualtiesCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 交通事故死傷者数統計カード
 */
export async function TrafficAccidentCasualtiesCard({
  areaCode,
  timeCode,
  title = "交通事故死傷者数",
}: TrafficAccidentCasualtiesCardProps) {
  try {
    // e-Stat APIから交通事故死傷者数データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_TRAFFIC_ACCIDENT_CASUALTIES,
      areaFilter: areaCode,
    ...
    console.error("[TrafficAccidentCasualtiesCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
