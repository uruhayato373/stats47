...
interface TotalHousingUnitsCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 総住宅数統計カード
 */
export async function TotalHousingUnitsCard({
  areaCode,
  timeCode,
  title = "総住宅数",
}: TotalHousingUnitsCardProps) {
  try {
    // e-Stat APIから総住宅数データを取得
    ...
    console.error("[TotalHousingUnitsCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
