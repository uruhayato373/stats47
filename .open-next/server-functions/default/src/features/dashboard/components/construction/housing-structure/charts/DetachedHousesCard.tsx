...
interface DetachedHousesCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 一戸建住宅数統計カード
 */
export async function DetachedHousesCard({
  areaCode,
  timeCode,
  title = "一戸建住宅数",
}: DetachedHousesCardProps) {
  try {
    // e-Stat APIから一戸建住宅数データを取得
    ...
    console.error("[DetachedHousesCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
