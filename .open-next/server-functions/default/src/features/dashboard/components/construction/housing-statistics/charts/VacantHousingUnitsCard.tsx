...
const CAT01_VACANT_HOUSING_UNITS = "H1102"; // 居住世帯なし住宅数（空き家数）

interface VacantHousingUnitsCardProps {
  /** 地域コード */
  areaCode: string;
 ...
 * 空き家数統計カード
 */
export async function VacantHousingUnitsCard({
  areaCode,
  timeCode,
  title = "空き家数",
}: VacantHousingUnitsCardProps) {
  try {
    // e-Stat APIから空き家数データを取得
    const response = await fetchStatsData(STATS_DATA_ID, {
      categoryFilter: CAT01_VACANT_HOUSING_UNITS,
      areaFilter: areaCode,
    ...
    console.error("[VacantHousingUnitsCard] データ取得エラー:", error);
    return (
      <Card>
        <CardHeader>
          <CardTitle>{title}</CardTitle>
