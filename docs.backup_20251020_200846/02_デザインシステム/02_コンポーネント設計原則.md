...
Atoms は、アプリケーション全体で使用される最小単位の UI コンポーネントです。これらは単一の責任を持ち、他のコンポーネントの基盤となります。

#### Alert

メッセージ表示用の共通コンポーネント。成功・エラー・情報・警告の 4 種類のメッセージを統一されたスタイルで表示します。

**特徴**:

- 4 種類のタイプ（success, error, info, warning）
- アイコンの表示/非表示制御
- カスタムアイコンの使用
- 閉じるボタン（オプション）
- ダークモード対応
...
<Alert type="error" message="エラーが発生しました" showIcon={true} />
<Alert type="warning" message="警告メッセージ" onDismiss={() => setShow(false)} />
```

**詳細**: [Alert コンポーネントドキュメント](./components/atoms/alert.md)

#### Message

シンプルなメッセージ表示コンポーネント。Alert よりも軽量で、基本的なメッセージ表示に使用します。
...
入力フィールドコンポーネント。フォームで使用される基本的な入力要素です。
...
### 3.2 e-Stat コンポーネント構造

e-Stat 関連のコンポーネントは、機能別に以下のディレクトリに整理されています：

```
src/components/estat-api/
├── metainfo/                    # メタ情報関連のコンポーネント
├── ranking-settings/          # ランキング設定関連
├── statsdata/                   # 統計データ表示関連
...
e-Stat メタ情報管理ページのコンポーネント群です。これらは `src/components/estat-api/metainfo/` ディレクトリに配置されており、関心事の分離と再利用性を高める設計になっています。
...
1. **SavedEstatMetainfoDisplay** (クライアントコンポーネント)
   ...
   - `useEffect` 内で `/api/estat-api/meta-info/saved` エンドポイントからデータを非同期に取得します。
...
```
src/components/estat-api/statsdata/EstatDataDisplay/
...
│       └── EstatStatsDataService.ts  # データ変換
└── types/
    └── choropleth.ts               # 型定義
```
...
1. **ユーザー入力** → 統計表 ID を入力
2. **API 呼び出し** → `/api/estat-api/meta-info/save`に POST
...
- **API ルート**: `src/app/api/estat-api/meta-info/save/route.ts`
