...
description: note ランキング記事（Aシリーズ）をR2観測値から生成する詳細runbook。テキストと画像を一括出力する。
disable-model-invocation: true
primary_agent: note-manager
---

> Detailed article template and render commands. Current skill entrypoint: `../SKILL.md`.

noteランキング記事（Aシリーズ）をR2観測値から生成する詳細runbook。
...
ここまで上位・下位の10県を見てきましたが、残る37県の順位が気になった方も多いはずです。全47都道府県の順位は、色分け地図と並べ替え可能なグラフ付きで stats47 で確認できます。
...
- 偏差値は小数第1位まで（例: 70.7）
- 倍率は小数第1位まで（例: 1.4倍）
- 「あなたの県は何位？」で記事中盤に本ランキングへの CTA を 1 つ置く（下位5 の直後）。記事末尾まで読まない読者にも stats47 への動線を確保するため。本ランキング URL は末尾「もっと詳しく」とあえて重複させてよい（主要 CTA の反復）
...
- note からの流入は GA4 の `sessionSource=note.com` で判別可能なため UTM は不要。記事単位の送客は GA4 `landingPage`（note 記事ごとに着地 URL が異なる）で識別する
...
- [ ] 記事中盤「あなたの県は何位？」に本ランキングへの CTA（素 URL カード）が1つある
