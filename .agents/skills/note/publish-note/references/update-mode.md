...
- **無料記事は全自動**。有料記事の更新は本文差し替え後に有料エリア境界（`ここから先は有料部分:`）の
  再設定が絡むため、公開更新の依頼がある場合に限り、Phase 7-Boundaryで境界を設定してから
  **エージェントがscreenshotを目視確認し、同じセッションで確定**する。詳細は
  `scheduling.md`の運用ルール。境界が確認できない場合は公開せずユーザーへ引き継ぐ。
...
有料記事の場合はPhase 7-Boundaryで境界を設定し、screenshot目視後に同じセッションで「更新する」を押す。
下書き保存・画面を閉じる・再読込で境界が保持されるとは限らない。中断した場合は境界設定と目視からやり直す。
