...
- **必ず `.Codex/rules/coconala-product-standards.md §8`（Kindle 出版チャネル）に従う**。
- 書籍定義は git TS (`src/channels/kindle/book-catalog.ts` = `KINDLE_BOOKS`) が SSOT。本文素材は
  **R2 `app/blog/<slug>/article.md` + `data/*.svg`**。生成物 `.local/kindle-books/<id>/v1/book.epub` は
  **git 管理外・公開 R2 へ置かない**。長文の書き下ろしは `src/channels/kindle/manuscripts/<id>/*.md` (freshFile)。
...
npm run products:kindle:report    --workspace=@stats47/product-factory              # 台帳 .Codex/state/products/kindle-status.json
```
...
5. **オーナーへ受け渡し**: `READINESS.md` に沿って人間が Kindle Previewer で表示確認 → KDP アップロード。
...
- 台帳: `.Codex/state/products/kindle-status.json`
- 企画・市場判断: `.Codex/rules/coconala-product-standards.md §8` / `packages/product-factory/src/channels/kindle/book-catalog.ts`
- agent: `.Codex/agents/kindle-publisher.md`
- 品質ゲート（書き下ろし）: `.Codex/rules/blog-quality-standards.md`（ですます調・critic PASS）
