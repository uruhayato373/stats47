...
**先に戦略レーンを読む。** 重点は `docs/00_プロジェクト管理/02_収益化戦略.md` §5「戦略レーンと優先順位」で
構えが「攻める」のレーンからだけ選び、frontmatter の `focus_lanes` に 1〜2 個書く (「維持」「凍結」は置けない。
`npm run docs:check` の DG076 が error にする)。構えを変えたいときは月次計画ではなく収益化戦略の表を改訂する。
管理画面 `/strategy/lanes` でレーンごとのカード数と今週の整合を確認できる。
...
- 月内の進捗は **週次計画 `/weekly-plan` が分割消化**する。週次は `.claude/todo/monthly.md` の `focus_themes` と `focus_lanes` を読む。
