...
`.Codex/rules/docs-vs-issues.md`、決定的な許可パス・必須項目は
`.Codex/config/docs-governance.json` とする。本文へ規則を複製しない。

## 実行

1. `.Codex/rules/docs-vs-issues.md` を最後まで読む。
2. 変更前に `npm run docs:report` を実行し、現状のerror・warningを把握する。
3. 新規作成要求でも、既存SSOT、`.Codex/todo/`、コード近傍README、skill/rule、
...
Codexでは`.Codex/hooks/check-docs-on-stop.js`が文書差分のあるturnを自動検査する。
