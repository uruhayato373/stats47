...
## 規約 (正典: `.Codex/rules/sns-content-standards.md` §5.5 / `.Codex/rules/ogp-image-standards.md`)

- 投稿台帳 SSOT は `.Codex/state/sns/posts.json`。**書込は sns-posts-store.cjs 経由のみ** (server も同経路)
...
- X 投稿: `.Codex/skills/sns/publish-x/` / IG cron: `.Codex/scripts/instagram/post-from-schedule.cjs`
