...
description: .Codex/todo のバックログを分類して処理し、機械ゲートを通したものだけ行削除する。ledger に証拠を残し verify で突合する。Use when user says "バックログを処理", "バックログを消化", "process-backlog".
...
`.Codex/todo/backlog.md` / `backlog.md` のエントリを分類して処理し、
**機械ゲートを通ったものだけ**を行削除する。CI 日次ループ (Phase 1) と同じ手順を
セッション内で回す入口。

規約の正典は `.Codex/rules/backlog-loop.md`。class 定義・gate 表・安全境界はそちらを読む。
...
```bash
node .Codex/scripts/backlog-loop/build-backlog-queue.mjs --limit 3 --json
```
...
(`.Codex/rules/agent-output-contract.md`)。委譲の設計とモデル選定は
`.Codex/rules/model-prompting.md` に従う。
...
```bash
node .Codex/scripts/backlog-loop/record-backlog-outcome.mjs \
...
新規追加を既定で落とすが、名指しした ID だけは通す (正典 `.Codex/rules/backlog-loop.md`)。
...
```bash
node .Codex/scripts/backlog-loop/verify-backlog-run.mjs --base "$BASE" --queued "<ID1>,<ID2>"
npm run docs:check
```
...
- `.Codex/state/backlog-loop/ledger.json` を直接編集する (証拠の捏造)
- `.Codex/todo/improvements.md` を触る (improvement-triage の排他 write)
...
- 正典: `.Codex/rules/backlog-loop.md`
- agent: `.Codex/agents/backlog-processor.md` / `.Codex/agents/backlog-solver-hard.md`
- policy: `.Codex/config/backlog-routing-policy.json`
- TODO 作成契約: `.Codex/rules/docs-vs-issues.md` / `.Codex/rules/todo-standards.md`
