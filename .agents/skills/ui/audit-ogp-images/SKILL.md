...
`.Codex/rules/ogp-image-standards.md` を基準とする。**read-only** (画像生成・R2 書き込みをしない)。

## 実行

```bash
# 全タブ (ranking はサンプル 30) → /tmp/ogp-image-gallery.html
node .Codex/scripts/ogp/build-image-gallery.mjs

# 特定タブだけ・件数を絞る
node .Codex/scripts/ogp/build-image-gallery.mjs --tabs blog-ogp,blog-card --limit 20

# 欠落を GET で確定 (stdout に tab: entries/images/ok/missing 集計 + missing 一覧)
node .Codex/scripts/ogp/build-image-gallery.mjs --tabs ranking-card --check

# 全種別を棚卸し → .Codex/state/ogp/inventory.json (既定サンプリング、全量は --all 併用)
node .Codex/scripts/ogp/build-image-gallery.mjs --audit
```
...
- **クラウド (Codex on the web)**: 生成 HTML を `SendUserFile` (display:render) でユーザーに渡し、
...
- 欠落が見つかったら該当種別と件数を Missing 列に記す。改善候補は
  `.Codex/todo/improvements.md` への追記を **提案** する (書き込みは `improvement-triage` に委譲)。
...
- 書き込みは自分の成果物のみ: 生成 HTML (`/tmp/`) と `.Codex/state/ogp/inventory.json` (`--audit` 時)。
...
- 基準 SSOT: `.Codex/rules/ogp-image-standards.md`
- スクリプト: `.Codex/scripts/ogp/build-image-gallery.mjs`
- 棚卸し state: `.Codex/state/ogp/inventory.json`
