...
description: Google Search Console の検索パフォーマンスとインデックス問題を .Codex/todo/improvements.md で追跡し、週次 snapshot と施策の効果判定を記録する。Use when user says "GSC改善", "GSC記録", "インデックス改善", "SEO課題記録", or when analyzing gscエラー/ CSV files.
...
| データ | 保管先 | 理由 |
|---|---|---|
| 生メトリクス CSV | git: `.Codex/skills/analytics/gsc-improvement/reference/snapshots/YYYY-Www/` | immutable、diff 比較、オフライン可 |
| 目標しきい値設定 | git: `.Codex/skills/analytics/gsc-improvement/reference/budgets.json` | プロジェクト設定 |
| 詳細ログ (agent 用) | git: `.Codex/skills/analytics/gsc-improvement/reference/improvement-log.md` | 過去判定の根拠・検証コマンド・仮説を含む詳細 |
| 要約 (人間向け) | git: `.Codex/todo/improvements.md` | active 施策の ID・要約・status・期日を俯瞰 |
| 週次スナップショット | `.Codex/state/metrics/gsc/{history.csv,LATEST.md}` | GitHub Actions が日曜 JST 20:00 に自動更新 |
| 運用サイクル監査 | `.Codex/state/metrics/gsc/operations-cycle-LATEST.{json,md}` | 計測→review→候補判断→plan→月次集約の接続状態 |
...
$ARGUMENTS — [mode]
             mode:
               ...
               - action  : 新しい施策行を .Codex/todo/improvements.md に追加
               - next    : 次に着手すべき改善候補を提示
```
...
2. .Codex/todo/improvements.md の6列表から status: pending / in-progress / effect-pending の行を抽出
3. reference/improvement-log.md を Read し未判定の検証コマンド一覧を抽出
4. .Codex/state/metrics/gsc/LATEST.md を Read し週次推移を取得
5. `node .Codex/scripts/gsc/audit-operations-cycle.mjs --stage monitor` を実行し、未接続工程を取得
...
4. 前週 snapshot との前週比を計算:
   ...
   - .Codex/state/metrics/gsc/history.csv から取得しても可

5. 進行中施策の効果判定（最重要）:
   ```bash
   node .Codex/scripts/lib/effect-verdict/cli.mjs --week <YYYY-Www>
   node .Codex/scripts/gsc/audit-operations-cycle.mjs --stage review-input --week <YYYY-Www>
   ```
   - effect ラベルの自動判定は `effect-verdict` の閾値・4ガードを正典とし、同じ比較を手作業で再実装しない。
   - `insufficient-target` は推測で補わない。`.Codex/config/gsc-operations-cycle.json` の既知負債以外が1件でも増えたら運用監査をFAILにする。
   - 確定 verdict の `.Codex/todo/improvements.md` 反映は `improvement-triage` に委譲する。確定済みIDがactive一覧に残れば運用監査がFAILにする。
...
2. .Codex/todo/improvements.md を Read し、該当 Tier の6列表に1行だけ追加する:
...
```
1. .Codex/todo/improvements.md のactive行と、reference/improvement-log.md の過去判定から派生候補を抽出
...
- **.Codex/todo/improvements.md は active-only** — 未完了施策の追加・status 更新に限定する。effect 確定後は詳細を improvement-log に残し、行を削除する
...
- **責務を分離する** — `.Codex/todo/improvements.md` はactive一覧、reference/improvement-log.md は判定履歴

## 参照パターン

```bash
# 直近スナップショット
ls -t .Codex/skills/analytics/gsc-improvement/reference/snapshots/ | head -3
cat .Codex/state/metrics/gsc/LATEST.md

# 進行中施策
node .Codex/scripts/lib/scan-pending-improvements.mjs --format markdown

# 効果測定済み施策・詳細ログ
cat .Codex/skills/analytics/gsc-improvement/reference/improvement-log.md

# 計測→review→候補判断→plan→月次集約の接続
node .Codex/scripts/gsc/audit-operations-cycle.mjs --stage monitor
node --test .Codex/scripts/gsc/__tests__/audit-operations-cycle.test.mjs
```
...
- [ ] 検証コマンドを実行したか:
  - URL 単位の Google 認識: `node .Codex/scripts/gsc/url-inspection-daily.cjs --limit 10`（pageFetchState / coverageState / lastCrawlTime を取得）
...
- `.Codex/todo/improvements.md` が存在（front-matter `type: improvement-log` / `metric: gsc`）
