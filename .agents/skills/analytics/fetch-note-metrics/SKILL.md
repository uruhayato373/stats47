...
description: note.com sitesettings/stats から記事別メトリクス (view / comment / like) を取得する。Use when user says "noteメトリクス", "note統計", "note ビュー数取得", "fetch-note-metrics". browser-use CLI で Chrome Profile 1 (note ログイン済) 経由で取得、.Codex/state/metrics/note/ に JSON snapshot 保存。
...
```bash
bash .Codex/scripts/note/fetch-note-metrics.sh
```
...
6. `.Codex/state/metrics/note/note-YYYY-MM-DD.json` に保存
...
- L2 集約: `.Codex/state/metrics/note/history.csv` に週次で append、前週比表示
...
- `.Codex/skills/note/publish-note/SKILL.md`（note 書き込み側、browser-use 共通パターン）
- `.Codex/skills/management/knowledge/SKILL.md`（cookie persistence の学び）
