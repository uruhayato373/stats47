...
description: Google AdSense の広告収益・RPM・CTR・ビューアビリティを .Codex/todo/improvements.md で追跡し、週次 snapshot と施策の効果判定を記録する。Use when user says "AdSense改善", "広告収益改善", "RPM改善", "AdSense記録".
primary_agent: adsense-analyst
---

AdSense の週次メトリクス（Earnings / Page RPM / CTR / Impressions / Active View）を snapshot と詳細ログで時系列追跡し、active 施策だけを `.Codex/todo/improvements.md` で管理するスキル。
...
| 週次推移サマリ | `.Codex/state/metrics/adsense/LATEST.md` / `history.csv` | GitHub Actions が自動更新 |
| **広告ユニット別推移** | `.Codex/state/metrics/adsense/history-units.csv` | ユニット単位の最適化を効果測定するための時系列。`match_status` (matched / legacy-name-matched / unmanaged / orphan) で突き合わせ可否を明示する。legacy-name-matched は後方互換で、削除条件は `adsense-report-contract.mjs` の match_status 定義を参照 |
...
→ **責務分離**: `.Codex/todo/improvements.md` はactive一覧、agent 用詳細は `.Codex/skills/analytics/adsense-improvement/reference/improvement-log.md`。
...
4. .Codex/state/metrics/adsense/LATEST.md を Read し週次推移を取得
...
4. 前週 snapshot との前週比を計算:
   ...
   - .Codex/state/metrics/adsense/history.csv から取得しても可
...
6. 進行中施策の効果判定（最重要）:
   .Codex/todo/improvements.md のAdSense対象行を抽出。
   各施策に対して:
...
```bash
# 直近スナップショット
ls -t .Codex/skills/analytics/adsense-improvement/reference/snapshots/ | head -3
cat .Codex/state/metrics/adsense/LATEST.md

# 進行中施策
node .Codex/scripts/lib/scan-pending-improvements.mjs --format markdown

# 効果測定済み施策・詳細ログ
cat .Codex/skills/analytics/adsense-improvement/reference/improvement-log.md
```
...
- `.Codex/todo/improvements.md` が存在すること（施策 ID は `ADSENSE-*` または `AFF-*`）
