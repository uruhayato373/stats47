...
- `event-volume.csv` — Japan-only のイベント別発火量 (未登録 custom dimension を登録する価値の判断に使う)

週次 snapshot に無い施策固有の内訳は `node .claude/scripts/metrics/ga4-query.mjs` で照会する (read-only・コピペ再現できる 1 行コマンド。記法はスクリプト冒頭)。改善ログの再現コマンドにはこの形を書く。
...
クリーン値併記の背景と恒久対策は memory `feedback_ga4_history_unreliable_wow` を参照。
