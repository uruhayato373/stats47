...
9. `bash .Codex/skills/db/sync-snapshots/run.sh --only blog` を実行
...
- `Agent(article-writer)` — 本スキルの入力 (article.md) を生成する。起動時は
  `.Codex/rules/model-prompting.md` と `.Codex/rules/agent-output-contract.md` に従い、
  同一slugを複数agentへ割り当てず、同時起動は最大3
- `/sync-snapshots --only blog` — Phase 3 で内部呼び出し
- `/draft-from-trend` — 量産フローの上流 (1 metric = 1 記事)
- `.Codex/rules/branch-workflow.md` — sync 後にデプロイが必要な場合 (今回は不要、R2 反映のみで本番に出る)
- `.Codex/rules/data-storage.md` — D1 vs R2 の使い分け
