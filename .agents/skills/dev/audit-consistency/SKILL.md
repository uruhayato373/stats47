...
description: エージェント/スキル/スクリプト/フックの整合性ドリフトを点検する。Use when 会話で .Codex/{agents,skills,scripts,hooks} や SKILL.md を変更した後、または Stop hook の「整合性監査が未実施」差し戻しを受けたとき、または "整合性チェック" "修正漏れ" "consistency audit" と言われたとき。機械チェック(床)＋意味レビュー(統合バグ)の二層で確認しマーカーを記録する。
...
>
> このスキルは Stop hook (`.Codex/hooks/check-consistency-on-stop.js`) からの差し戻しで起動することを想定する。
> 関連: `.Codex/scripts/lib/check-agent-skill-consistency.cjs` / `.Codex/todo/improvements.md`

## 実行手順

### Step 1 — 機械チェック (床)
```bash
node .Codex/scripts/lib/check-agent-skill-consistency.cjs
```
検出:
- **[E1]** SKILL.md の `primary_agent`/`co_agents` が指す `.Codex/agents/<name>.md` が無い
- **[E2]** SKILL.md が参照する `.Codex/scripts|hooks/...` が存在しない
...
3. **主張の裏取り**: doc/memory/SKILL に書いた「○○は××しない/する」を実装で確認したか? (AGENTS.md 行動原則8「書く前に読む」)
4. **波及更新**: 新規 skill/agent を参照すべき場所を更新したか?
   - 担当 agent doc の「担当スキル」表 (`.Codex/agents/*.md`)
   - cadence (`/weekly-review` `/weekly-plan` が拾うか)
   - index (`docs/*/00_INDEX.md`)、記録先ルール (`.Codex/rules/data-storage.md`)
   - 真実源 (改善バックログ `.Codex/todo/improvements.md`)
...
床と天井の確認が済んだら:
```bash
node .Codex/scripts/lib/check-agent-skill-consistency.cjs --mark-audited
```
現在の関連変更ファイル集合のハッシュを `.Codex/state/consistency/audited.json` に記録する。
...
## 関連
- 機械チェッカー: `.Codex/scripts/lib/check-agent-skill-consistency.cjs`
- prompt設計SSOT: `.Codex/rules/model-prompting.md`
- Stop hook: `.Codex/hooks/check-consistency-on-stop.js` / 登録: `.Codex/settings.json`
- 既存ガード群: memory `project_recurrence_guard_scripts` (`.Codex/scripts/lib/check-*.cjs`)
- 親方針: AGENTS.md 行動原則8「書く前に読む」/ 12「失敗を隠さない」
