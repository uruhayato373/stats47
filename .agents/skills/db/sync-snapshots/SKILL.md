...
全 Web アプリデータは `app/` 名前空間に統一されている。詳細は `.Codex/rules/r2-storage-design.md` を参照。
...
URL → R2 パス対応は `.Codex/rules/r2-storage-design.md` を参照。
...
方針: `.Codex/rules/local-environment.md` / `.Codex/rules/r2-storage-design.md`。
...
契約は `.Codex/scripts/lib/__tests__/sync-snapshots-run-contract.test.mjs` が機械で固定する
...
```bash
bash .Codex/skills/db/sync-snapshots/run.sh
```
...
bash .Codex/skills/db/sync-snapshots/run.sh --only ranking-values
bash .Codex/skills/db/sync-snapshots/run.sh --only station-passengers
```

### dry-run (実行しない、走るスクリプトをリスト表示)

```bash
bash .Codex/skills/db/sync-snapshots/run.sh --dry-run
```
...
- **R2 キーパスは `app/` 名前空間に統一すること**: 新規 snapshot 追加時は `.Codex/rules/r2-storage-design.md` の URL → R2 対応表に従う。`app/` プレフィックスなし・`all.json` モノリスは禁止。
