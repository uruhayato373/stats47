...
describe("resolveOgpVisual — 優先順位", () => {
  ...
  it("explicit ogpMotif が有効なら採用、visualType 由来でも上書きできる", () => {
    const r = resolveOgpVisual({ category: "population", ogpMotif: "generations" });
    expect(r.visualType).toBe("people");
    expect(r.motif).toBe("generations");
  });

  it("category → visualType 対応表を使う", () => {
    expect(resolveOgpVisual({ category: "economy" }).visualType).toBe("economy");
    expect(resolveOgpVisual({ category: "agriculture" }).visualType).toBe("industry");
    expect(resolveOgpVisual({ category: "landweather" }).source).toBe("category");
  });
...
describe("resolveOgpVisual — 既定・不正値の安全な落とし込み", () => {
  ...
  it("不正な ogpVisualType は無視して次 tier / 既定へ落ちる", () => {
    expect(resolveOgpVisual({ ogpVisualType: "banana" }).source).toBe("default");
    expect(
      resolveOgpVisual({ ogpVisualType: "banana", category: "population" }).visualType,
    ).toBe("people");
  });

  it("不正な ogpMotif は visualType の既定 motif に落ちる", () => {
    const r = resolveOgpVisual({ ogpVisualType: "economy", ogpMotif: "not-a-motif" });
    expect(r.visualType).toBe("economy");
...
describe("computePromptHash — 決定性と変化", () => {
  const base = { visualType: "people" as const, motif: "households", title: "秋田の人口" };
  ...
  it("title / motif / model / promptVersion / visualType の変化で hash が変わる", () => {
    ...
    expect(computePromptHash({ ...base, promptVersion: "blog-ogp-v2" })).not.toBe(h0);
    expect(computePromptHash({ ...base, visualType: "map" })).not.toBe(h0);
  });

  it("hash は sha256- プレフィックス付き", () => {
    expect(computePromptHash(base)).toMatch(/^sha256-[0-9a-f]{16}$/);
  });
});

describe("buildOgpPrompt", () => {
  it("固定スタイルの no-text ガードと motif 主題を含み、タイトルはテーマ文脈で入る", () => {
    const p = buildOgpPrompt({ visualType: "map", motif: "prefecture-comparison", title: "日本の人口" });
