 ...
 *
 * 真実源: `@stats47/data-configs` の isActive=true metric + R2 公開 URL での item.json 存在確認。
 * build-ranking-item-from-metric.ts で areaType は常に "prefecture" のため、
 * isActive かつ R2 item.json が 200 を返すキーを KNOWN として登録する。
 *
...
const activeMetrics = listAllMetrics().filter((m) => m.isActive).map((m) => m.key);
console.log(`[generate-known-ranking-keys] checking ${activeMetrics.length} active metrics...`);
 ...
 *
 * 真実源: data-configs isActive=true かつ R2 \`app/ranking/<key>/item.json\` が存在するキー。
