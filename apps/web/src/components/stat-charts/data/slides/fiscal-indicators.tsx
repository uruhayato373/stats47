...
import {
  FISCAL_INDICATOR_ICON_COLOR,
  FISCAL_PUBLIC_ENTERPRISE_CARD,
  FISCAL_SLIDE_ACCENT,
} from "./fiscal-indicators.palette";
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-3 gap-4 text-center">
          <SurfaceCard className="text-center">
            <div className="text-info font-bold text-xl mb-1">可視化</div>
            <p className="text-xs text-muted-foreground">連結ベースの指標導入</p>
          </SurfaceCard>
          <SurfaceCard className="text-center">
            <div className="text-info font-bold text-xl mb-1">早期発見</div>
            <p className="text-xs text-muted-foreground">健全化基準の設定</p>
          </SurfaceCard>
          <SurfaceCard className="text-center">
            <div className="text-info font-bold text-xl mb-1">徹底公開</div>
            <p className="text-xs text-muted-foreground">住民への情報開示義務</p>
          </SurfaceCard>
        </div>
      </div>
    ),
    accent: FISCAL_SLIDE_ACCENT.introduction,
  },
  {
    id: 2,
    category: "INDICATORS",
    ...
    content: (
      <div className="grid grid-cols-2 gap-4">
        {[
          { label: "実質赤字比率", icon: <TrendingDown className="h-5 w-5" />, desc: "一般会計の赤字", color: FISCAL_INDICATOR_ICON_COLOR.realDeficit },
          { label: "連結実質赤字比率", icon: <PieChart className="h-5 w-5" />, desc: "全会計合算の赤字", color: FISCAL_INDICATOR_ICON_COLOR.consolidatedDeficit },
          { label: "実質公債費比率", icon: <Landmark className="h-5 w-5" />, desc: "借金の返済負担", color: FISCAL_INDICATOR_ICON_COLOR.debtService },
    ...
    accent: FISCAL_SLIDE_ACCENT.indicators,
  },
  {
    id: 3,
    category: "THRESHOLDS",
    ...
    content: (
      <div className="space-y-4">
        <div className="relative p-5 bg-warning-soft border-2 border-warning/30 rounded-card flex gap-4">
          ...
          <div>
            <div className="font-black text-warning text-lg">
              早期健全化基準（イエローカード）
            </div>
        ...
        <div className="relative p-5 bg-negative-soft border-2 border-negative/30 rounded-card flex gap-4">
          ...
          <div>
            <div className="font-black text-negative text-lg">
              財政再生基準（レッドカード）
            </div>
            <p className="text-xs text-negative leading-relaxed mt-1">
    ...
    accent: FISCAL_SLIDE_ACCENT.thresholds,
  },
  {
    id: 4,
    ...
    content: (
      <div className="flex flex-col items-center justify-center h-full space-y-4">
        <div className={`w-full ${FISCAL_PUBLIC_ENTERPRISE_CARD.surface} p-6 rounded-card shadow-sm text-center relative overflow-hidden`}>
          ...
          <div className={`mt-4 pt-4 border-t ${FISCAL_PUBLIC_ENTERPRISE_CARD.divider} flex justify-between items-center px-4`}>
    ...
    accent: FISCAL_SLIDE_ACCENT.publicEnterprises,
  },
  {
    id: 5,
    category: "PROCESS",
    ...
    accent: FISCAL_SLIDE_ACCENT.process,
  },
];
