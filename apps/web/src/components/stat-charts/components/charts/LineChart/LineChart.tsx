...
export const LineChart = async ({
  ...
  const { estatParams, labels, unit, description, yAxisConfig, sharedYDomain, seriesColors, showLatestValues } = config as DashboardItemProps<"line-chart">["config"] & { sharedYDomain?: [number, number]; seriesColors?: string[] };
  ...
  try {
    ...
    if (firstError && "error" in firstError) {
      ...
      if (!hasAnyData) {
        ...
        chartData = toLineChartData(rawDataList, labels, seriesColors, unit);
      }
    }
  } catch (err) {
