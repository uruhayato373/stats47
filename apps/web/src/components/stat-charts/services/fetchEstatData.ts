...
const UNSUPPORTED_FILTER_RE =
  /^(?:lv|cdCat(?:0[6-9]|1[0-5])|cdCat\d+(?:From|To)|cdTab(?:From|To)|cdArea(?:From|To))$/;

function recipeKey(params: LegacyStatParams): string {
  return RECIPE_KEYS.filter((key) => params[key] !== undefined)
...
const metricKeysByRecipe = (() => {
  ...
  for (const keys of byRecipe.values()) {
    keys.sort((left, right) => {
      const activeDiff =
        Number(METRICS_REGISTRY[right]?.isActive) -
        Number(METRICS_REGISTRY[left]?.isActive);
...
const cachedReadMetric = cache(
  async (metricKey: string, includeJapan: boolean) => {
    const [prefecture, japan] = await Promise.all([
      readStatsValues(metricKey, 'prefecture'),
      includeJapan ? readJapanSeries(metricKey) : Promise.resolve(null),
    ]);
...
function candidateMetricKeys(params: LegacyStatParams): string[] {
  if (Object.keys(params).some((key) => UNSUPPORTED_FILTER_RE.test(key)))
    return [];
...
function filterRows(
  ...
  const targetArea =
    typeof params.cdArea === 'string' ? params.cdArea : areaCode;
  const targetYear = typeof params.cdTime === 'string' ? params.cdTime : null;
  const yearFrom =
    typeof params.cdTimeFrom === 'string' ? params.cdTimeFrom : null;
...
async function readCompatibleRows(
  ...
  for (const metricKey of candidates) {
    ...
    const { prefecture, japan } = await cachedReadMetric(
      metricKey,
      areaCode === null
    );
...
export async function fetchEstatDataWithCategories(
  ...
  const results = await Promise.all(
    categoryCodes.map((cdCat01) =>
      fetchEstatData(areaCode, { statsDataId, cdCat01 })
    )
  );
