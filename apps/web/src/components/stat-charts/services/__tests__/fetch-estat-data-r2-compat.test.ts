...
describe('legacy stat params — MetricConfigから正典R2へ解決', () => {
  ...
  it('生parameterをtotal-populationへ一意に解決し、指定県だけを返す', async () => {
    ...
    expect(readStatsValues).toHaveBeenCalledWith(
      'total-population',
      'prefecture'
    );
    expect(readJapanSeries).not.toHaveBeenCalled();
    expect(result).toMatchObject({
      data: [{ areaCode: '28000', value: 5, metricKey: 'total-population' }],
    });
  });

  it('全地域取得はR2の47県系列と利用可能な日本系列を結合する', async () => {
    const result = await fetchEstatDataAllAreas(params);
    expect(readJapanSeries).toHaveBeenCalledWith('total-population');
    expect(
      'data' in result ? result.data.map((row) => row.areaCode) : []
    ).toEqual(['13000', '28000', '00000']);
  });

  it('MetricConfigへ解決できないparameterは外部APIへfallbackしない', async () => {
    const result = await fetchEstatData('28000', {
      statsDataId: 'unknown',
      cdCat01: 'X',
    });
