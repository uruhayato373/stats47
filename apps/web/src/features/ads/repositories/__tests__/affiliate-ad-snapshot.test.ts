...
describe('配信安全ゲート・案件重複', () => {
  ...
  it('古いsnapshotのactive=trueでも、未提携3案件をad IDとprogramRefの双方で止める', async () => {
    ...
    expect(
      (await readActiveBannersByVerticalsFromR2(['labor'], 20)).map(
        (ad) => ad.id
      )
    ).toEqual([genericAd.id]);
  });
  ...
  it('同一案件の別リンク・別IDの同じリンクをlimit適用前に除き、優先順位の高い1件だけ返す', async () => {
    ...
    expect(
      (await readActiveBannersByVerticalsFromR2(['labor'], 2)).map(
        (ad) => ad.id
      )
    ).toEqual(['a', 'c']);
  });

  it.each(['health', 'population'] as const)(
    '%sの分類だけで行動広告を出さない',
    async (vertical) => {
      setAds([
        banner({ id: 'generic', vertical }),
        banner({
          id: 'targeted',
          vertical,
          targetRankingKeys: ['explicit-context'],
        }),
      ]);
      ...
      expect(
        await readActiveBannersByVerticalsFromR2(
          [vertical],
          10,
  ...
  it('単一text・area固定枠・実験もtargetRankingKeysを迂回しない', async () => {
    ...
    expect(
      (await readActiveExperimentVariantsByVerticalFromR2('labor', 'it')).map(
        (ad) => ad.id
      )
    ).toEqual(['variant']);
  });
  ...
  it('paused/blockedと期間外は配信しない。未分類を承認済みへ書き換えない', async () => {
    ...
    expect(
      (await readActiveBannersByVerticalsFromR2(['labor'], 10)).map(
        (ad) => ad.id
      )
    ).toEqual([genericAd.id]);
  });
