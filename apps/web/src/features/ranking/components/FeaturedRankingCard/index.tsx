...
export function FeaturedRankingCard({
  ...
  if (topAreaName) {
    return (
      <Link
        ...
        title={title}
        className="group block rounded-lg border border-border hover:border-primary/50 hover:shadow-md transition-all overflow-hidden"
      >
        {tileMapSvg && (
          <div
  ...
  return (
    <Link
      href={`/ranking/${rankingKey}`}
      title={title}
      className="group block rounded-lg border border-border hover:border-primary/50 hover:bg-accent/50 transition-colors overflow-hidden"
    >
      <div className="flex gap-3 p-2.5">
        {baseThumbnailUrl && (
          <div className="flex-shrink-0 w-20 rounded-md overflow-hidden bg-muted aspect-square">
            <RankingThumbnail
