...
export function useThemedLeafletTile(theme: string | undefined, defaultIndex = 0): {
  ...
  const defaultTile = tileOptions[defaultIndex] ?? tileOptions[0];
  const [currentTile, setCurrentTile] = useState<TileProvider>(defaultTile);
