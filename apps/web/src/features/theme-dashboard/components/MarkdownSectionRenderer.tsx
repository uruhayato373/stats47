...
import { ContentDisclosure, FaqSection } from '@/components/content';
...
interface Props {
  ...
  /** 概況ダッシュボードでは本文を必要時に開く。FAQ は既存の質問別開閉を使う。 */
  collapsible?: boolean;
}
 ...
 */
export function MarkdownSectionRenderer({
  title,
  props,
  fallbackSourceName,
  collapsible = false,
  ...
  if (collapsible) {
    return (
      <ContentDisclosure title={title} className="px-3 py-0">
        {content}
      </ContentDisclosure>
    );
  }
  ...
  return (
    <SurfaceSection aria-label={title} className="p-6">
