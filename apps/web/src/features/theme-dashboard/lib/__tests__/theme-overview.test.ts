...
it('固定比較年を尊重し、最新年のメタデータと混同しない', () => {
  const fixed = data([
    row('01000', 10, '2021'),
    row('02000', 20, '2021'),
    row('03000', 999),
  ]);
  expect(getOverviewValues(fixed, '2021').map((value) => value.value)).toEqual([
    20, 10,
  ]);
  expect(getComparisonYearName(fixed, '2021')).toBe('2021年');
});

it('月次の比較へ同年の別月を混ぜない', () => {
  const monthly = data([
    row('01000', 10, '202301'),
    row('02000', 999, '202302'),
  ]);
  expect(
    getOverviewValues(monthly, '202301').map((value) => value.value)
  ).toEqual([10]);
});

it('比較指標は既存グループとタブから重複なく導出する', () => {
  expect(
    getComparisonMetricKeys(
      {
        tabIndicators: [
          { rankingKey: 'main', tabLabel: '主要' },
          { rankingKey: 'ungrouped', tabLabel: '補助' },
        ],
      },
      [
        {
          key: 'group',
          title: '章',
          rankingKeys: ['main', 'fixed-context'],
          defaultCheckedKeys: ['main'],
          comparisonYear: '2021',
        },
      ]
    )
  ).toEqual(['main', 'fixed-context', 'ungrouped']);
});
