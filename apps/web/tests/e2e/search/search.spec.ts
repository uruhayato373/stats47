...
const getSearchInput = (page: import("@playwright/test").Page) =>
  page.getByPlaceholder("ブログ記事、ダッシュボード、ランキングを検索...");

const submitSearch = async (page: import("@playwright/test").Page) => {
...
test.describe("検索ページ", () => {
  ...
  test("ページが正常に表示される", async ({ page }) => {
    ...
    await expect(heading).toHaveText("サイト内検索");
  ...
  test("該当なしの場合にメッセージが表示される", async ({ page }) => {
    ...
    await expect(noResultText.first()).toBeVisible({ timeout: 10000 });
  });
});
