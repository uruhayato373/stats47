...
import { findBuzzMapIdea, listCuratedIdeaIds, readBuzzMapCatalog } from "./buzz-map";
...
function assertIdeaId(ideaId: unknown): { ok: true; id: string } | { ok: false; error: string } {
  if (typeof ideaId !== "string" || !IDEA_ID_RE.test(ideaId)) {
    return { ok: false, error: "ideaId は英数字とハイフンのみ" };
  }
  if (!listCuratedIdeaIds().has(ideaId)) {
    return { ok: false, error: `ideaId が catalog (curated レーン) に存在しません: ${ideaId}` };
  }
...
export function routeLanding(): ActionResult {
  const script = path.join(projectRoot(), ".claude/scripts/sns/build-buzz-map-catalog.ts");
...
const SPEC_HELPER_SCRIPT: Record<NonNullable<GenerateSpecInput["helper"]>, string> = {
...
function remotionArgs(ideaId: string, kind: RenderKind, outAbs: string): string[] {
...
export function renderBuzzMap(body: { ideaId?: string; kind?: RenderKind }): ActionResult {
  const idea = assertIdeaId(body.ideaId);
  if (!idea.ok) return { status: 400, body: { error: idea.error } };
  const kind = body.kind;
  if (kind !== "still" && kind !== "preview" && kind !== "full") {
    return { status: 400, body: { error: "kind は still | preview | full のいずれか" } };
  }
...
export function pushBuzzMapR2(body: { ideaId?: string }): ActionResult {
  const idea = assertIdeaId(body.ideaId);
  if (!idea.ok) return { status: 400, body: { error: idea.error } };
  const script = path.join(projectRoot(), "packages/r2-storage/src/scripts/diff-push-r2.ts");
  const args = ["tsx", script, "--prefix", `sns/buzz-map/${idea.id}`];
...
export async function registerBuzzMapDraft(body: RegisterDraftInput): Promise<ActionResult> {
  const idea = assertIdeaId(body.ideaId);
  if (!idea.ok) return { status: 400, body: { error: idea.error } };
  const channel = body.channel;
  if (channel !== "x" && channel !== "instagram") {
    return { status: 400, body: { error: "channel は x | instagram のいずれか" } };
  }

  const entry = findBuzzMapIdea(idea.id);
  if (!entry) return { status: 400, body: { error: "catalog entry が見つかりません" } };
  ...
  const [captionOk, stillOk] = await Promise.all([headOk(captionUrl), headOk(stillsUrl)]);
  ...
  const facts = {
    ...
    landingContractPass: entry.landingReadiness === "live" || entry.landingReadiness === "ready",
    landingLive,
  ...
  if (!gate.postable) {
    return {
      status: 422,
      body: { error: `draft gate 未通過: ${gate.reasons.join(" / ")}`, reasons: gate.reasons },
    };
  }
