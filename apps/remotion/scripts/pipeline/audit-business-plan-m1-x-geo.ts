...
interface SourceRecord {
  ...
  analysisIds: string[];
  claimMetricKey: string;
  observationSources: Array<{ analysisId: string; path: string; sha256: string }>;
  sourceLayers: Array<{ geometry: string }>;
  composition: string;
  output: string;
  outputSha256: string;
...
async function auditItem(item: QueueItem): Promise<void> {
  assert(item.imageKind === 'geo-insight-card', `${item.key}: Geo専用imageKindではありません`);
  assert(item.mediaPath.includes(`/sns/geo/${item.key}/`), `${item.key}: Geo専用パスではありません`);
  assert(!item.mediaPath.includes('ranking-card'), `${item.key}: ranking画像参照は禁止です`);
  const output = path.resolve(repoRoot, item.mediaPath);
  assert(output.startsWith(`${allowedMediaRoot}${path.sep}`), `${item.key}: 出力先がsns/geo外です`);
  const dimensions = await pngDimensions(output);
  assert(
    dimensions.width === GEO_X_CANVAS.width && dimensions.height === GEO_X_CANVAS.height,
    `${item.key}: 1080x1350ではありません (${dimensions.width}x${dimensions.height})`,
  );

  const sourcePath = path.join(path.dirname(output), 'source.json');
  const source = JSON.parse(await fs.readFile(sourcePath, 'utf8')) as SourceRecord;
  assert(source.kind === 'geo-x-insight-card', `${item.key}: source.kindが不正です`);
  assert(source.composition === 'GeoX-InsightCard', `${item.key}: Geo専用compositionではありません`);
  assert(source.contentKey === item.key, `${item.key}: source contentKey不一致`);
  assert(source.geoRole === item.geoRole, `${item.key}: geoRole不一致`);
  assert(sameStrings(source.analysisIds, item.analysisIds), `${item.key}: analysisIds不一致`);
  assert(source.claimMetricKey === item.claimMetricKey, `${item.key}: claimMetricKey不一致`);
  assert(source.output === item.mediaPath, `${item.key}: output不一致`);
  assert((await sha256(output)) === source.outputSha256, `${item.key}: 画像SHA不一致`);
  ...
  for (const [index, observation] of source.observationSources.entries()) {
    assert(observation.analysisId === item.analysisIds[index], `${item.key}: observation analysisId不一致`);
    const observationSource = path.resolve(repoRoot, observation.path);
    assert(observationSource.startsWith(`${r2Root}${path.sep}`), `${item.key}: 観測値sourceが.local/r2外です`);
    assert(
  ...
  if (item.geoRole === 'cross-analysis') {
    assert(item.analyses.length === 1, `${item.key}: cross-analysisは分析1件に固定します`);
    assert(item.analyses[0].analysisKind === 'spatial-cross', `${item.key}: spatial-cross契約がありません`);
    assert(source.sourceLayers.length >= 2, `${item.key}: 空間クロスは2層以上が必要です`);
    assert(
  ...
  if (item.geoRole === 'baseline') {
    assert(item.canonicalUrl === '/geo/2050-population', `${item.key}: baseline landing不一致`);
  }
}
...
async function main(): Promise<void> {
  ...
  assert(Array.isArray(queue) && queue.length === 15, `Geo X queueは15件必要です: ${queue.length}`);
  assert(new Set(queue.map((item) => item.key)).size === 15, 'contentKeyが重複しています');
  ...
  for (const [role, count] of Object.entries(expected) as Array<[GeoRole, number]>) {
  ...
  console.log('✅ Geo M1 asset audit: 15/15 PASS (role 3/9/2/1, PNG, source, SHA, layer contract)');
}
