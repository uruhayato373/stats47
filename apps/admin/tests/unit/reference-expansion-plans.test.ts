...
`,
  ...
  it("参考文献のtheme未統合指標を企画表が全件カバーする", () => {
    ...
    const inventory = JSON.parse(
      fs.readFileSync(
        path.join(root, ".claude/state/source-inventory/japan-zue/2025-26/inventory.json"),
        "utf8",
      ),
    ) as {
      items: Array<{
    ...
    const expectedMissing = referenceThemeKeys.filter(
      (key) => !integratedText.includes(`"${key}"`) && !integratedText.includes(`'${key}'`),
    );
    ...
    expect(themePlans).toHaveLength(14);
    expect(themePlans.filter((plan) => plan.status === "blocked")).toHaveLength(2);
    expect(blogPlans).toHaveLength(4);
    for (const plan of blogPlans) {
      expect(plan.metricKeys.every((key) => referenceThemeKeys.includes(key))).toBe(true);
    }
  });
});
