...
地域統計ダッシュボードでは、e-Stat API を使用して日本の地域統計データを取得します。API キーがない場合のフォールバックとして、サンプルデータも提供しています。
