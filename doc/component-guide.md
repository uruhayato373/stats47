...
本プロジェクトでは、`categories.json` の設定に基づいて、サブカテゴリーごとに表示するReactコンポーネントを動的に選択するシステムを採用しています。
...
3.  **動的なコンポーネント取得**
    `getSubcategoryComponent` 関数が、サブカテゴリーIDを基に `categories.json` を参照し、指定されたコンポーネント名のコンポーネントを `componentMap` から取得して返します。都道府県別ページも同様に `getAreaPageComponent` が対応します。
...
まず、サブカテゴリーに対応する新しいReactコンポーネントを作成します。コンポーネントは、関連するカテゴリーのディレクトリ（例: `src/components/subcategories/population/`）内に配置します。
...
以上の手順で、新しいサブカテゴリーページがシステムに統合され、`/population/population-composition` のようなURLでアクセスできるようになります。

## 3. 共通コンポーネント

### e-Statコンポーネント構造

e-Stat関連のコンポーネントは、機能別に以下のディレクトリに整理されています：
...
### EstatMetainfoコンポーネント群

e-Statメタ情報管理ページのコンポーネント群です。これらは `src/components/estat/metainfo/` ディレクトリに配置されており、関心事の分離と再利用性を高める設計になっています。
...
**責任**:
- ページタイトルの表示
- アクションボタン（更新、e-STATサイトへのリンク）の表示
- ローディング状態の管理
...
ユーザーが統計表IDを入力して、e-Stat APIからメタ情報を取得するためのUIを提供します。
...
取得したメタ情報をD1データベースに保存するためのUIを提供します。
...
D1データベースに保存済みのメタデータを一覧表示または個別表示するコンポーネントです。
...
保存されたe-STATメタデータを表示するクライアントコンポーネントです。
...
1. **SavedEstatMetainfoDisplay** (クライアントコンポーネント)
   ...
   - ローディング中、エラー発生時、データが空の場合のUIをそれぞれ表示します。
...
- **リッチなUI**: ローディングスピナーやエラーメッセージなど、状態に応じたインタラクティブなUIを容易に実装できます。
...
開発環境でも本番環境と同じリモートのD1データベースを使用します。これにより、環境間の差異をなくし、一貫したデータ管理を実現しています。
