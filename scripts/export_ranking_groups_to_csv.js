...
/**
 * ranking_groupsテーブルの内容をCSV形式で出力するスクリプト
 */
...
// CSVファイルの出力先
const outputPath = path.join(process.cwd(), "ranking_groups_export.csv");

try {
  // D1データベースからデータを取得（JSON形式で出力）
  const command = `npx wrangler d1 execute stats47 --local --command "SELECT group_key, subcategory_id, group_name, label, icon, display_order, created_at, updated_at FROM ranking_groups ORDER BY subcategory_id, display_order;" 2>&1`;
