#!/usr/bin/env python3
"""
articleテーブルのシードファイル生成スクリプト

現在のローカルD1データベースからarticlesテーブルのデータを取得し、
INSERT OR REPLACE形式のSQLファイルを生成します。

使用方法:
    python scripts/generate_articles_seed.py

出力:
    database/seeds/articles_seed.sql
"""
...
SEED_OUTPUT_PATH = PROJECT_ROOT / "database" / "seeds" / "articles_seed.sql"
...
def fetch_data_from_database() -> list[dict]:
    """ローカルD1データベースからarticlesデータを取得"""
    print("📊 データベースからarticlesデータを取得中...")
    ...
    try:
        ...
        result = subprocess.run(
            [
                ...
                "--local",
                "--command",
                "SELECT category, slug, time, title, description, tags, file_path, file_hash, created_at, updated_at FROM articles ORDER BY category, slug, time;",
            ],
            capture_output=True,
            text=True,
            check=True,
...
def generate_seed_sql(rows: list[dict]) -> str:
    """SQLシードファイルの内容を生成"""
    if not rows:
        return "-- articles テーブルにデータがありません\n"
    ...
    for i, row in enumerate(rows):
        category = format_value(row.get("category"))
        slug = format_value(row.get("slug"))
        time = format_value(row.get("time"))
        title = format_value(row.get("title"))
        description = format_value(row.get("description"))
        tags = format_value(row.get("tags"))
        file_path = format_value(row.get("file_path"))
        file_hash = format_value(row.get("file_hash"))
        ...
        value_line = (
            f"  ({category}, {slug}, {time}, {title}, {description}, {tags}, "
            f"{file_path}, {file_hash}, {created_at}, {updated_at}){comma}"
        )
...
def main():
    """メイン処理"""
    print("🔧 articlesテーブルシードファイル生成スクリプト")
    print("=" * 50)
