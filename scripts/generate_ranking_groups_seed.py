...
def generate_groups_and_items_sql(items):
    ...
    # ヘッダー
    sql_parts.append("""-- ランキンググループとアイテムの関連付けseed
-- 生成日: 2025-01-28
...
""".format(group_count=len(items), item_count=len(items)))
    ...
    for i, item in enumerate(items):
        ...
        # グループを作成（name は item_name）
        group_inserts.append(
            f"  ('{group_key}', '{subcategory_id}', '{item_name}', "
...
""")
    ...
    # グループIDの取得と更新
    # 注意: SQLiteでは個別にUPDATEを実行する必要があるため、
    # 複数のUPDATE文を生成
    update_statements = []
    for item in items:
        item_code = item['item_code']
