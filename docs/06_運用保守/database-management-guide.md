...
#### ローカルD1からステージング環境への同期

ローカルD1を正として、ステージング環境を常にローカルと同じ状態に保つ場合：
...
このコマンドは以下を実行します：
1. ステージング環境のバックアップを取得
2. ローカルD1のエクスポート
3. ステージング環境へのマイグレーション適用（念のため）
4. ステージング環境へのデータインポート
5. 同期結果の確認

**注意事項**:
- 同期前にステージング環境のバックアップが自動的に取得されます
- ローカルD1が初期化されていない場合はエラーになります（`npm run db:init:local`を先に実行）
- マイグレーションは自動的に適用されますが、手動で確認することも推奨
