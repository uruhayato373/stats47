...
database/backups/
  2025-11-01/
    local_backup_20251101_042030.sql
    staging_backup_20251101_043030.sql
    production_backup_20251101_044030.sql
  2025-11-02/
    local_backup_20251102_042030.sql
    ...
```
...
# または
bash database/scripts/backup.sh local

# ステージング環境の完全バックアップ
npm run db:dump:staging
# または
bash database/scripts/backup.sh staging

# 本番環境の完全バックアップ
npm run db:dump:production
# または
bash database/scripts/backup.sh production
```
...
バックアップスクリプト（`database/scripts/backup.sh`）は、年月日ごとのディレクトリでバックアップを管理します。
...
**スクリプトの動作：**
1. 当日の日付ディレクトリ（例: `2025-11-01`）を作成
2. 指定された環境のバックアップファイルを保存
3. 30日以上前の日付ディレクトリを自動削除
...
0 2 * * * cd /path/to/project && bash database/scripts/backup.sh local

# 毎日午前3時にステージング環境のバックアップ
0 3 * * * cd /path/to/project && bash database/scripts/backup.sh staging

# 毎日午前4時に本番環境のバックアップ
0 4 * * * cd /path/to/project && bash database/scripts/backup.sh production
```
...
年月日ごとのディレクトリ構造に基づいてバックアップファイルを指定します：

```bash
# ローカル環境へのリストア
npx wrangler d1 execute stats47 --local --file=database/backups/2025-11-01/local_backup_20251101_020000.sql

# ステージング環境へのリストア
npx wrangler d1 execute stats47 --env staging --file=database/backups/2025-11-01/staging_backup_20251101_030000.sql

# 本番環境へのリストア（注意が必要）
npx wrangler d1 execute stats47 --env production --file=database/backups/2025-11-01/production_backup_20251101_040000.sql
```
...
npx wrangler d1 export stats47 --env production --output=temp_prod_backup.sql
...
# 1. 最新のバックアップファイルを特定（年月日ごとのディレクトリから）
LATEST_BACKUP=$(find database/backups -name "production_backup_*.sql" -type f | sort -r | head -n1)
...
# 最新のバックアップファイルをチェック（年月日ごとのディレクトリから）
LATEST_BACKUP=$(find "$BACKUP_DIR" -name "production_backup_*.sql" -type f 2>/dev/null | sort -r | head -n1)
...
# バックアップファイルの作成日時をチェック（macOS/Linux対応）
