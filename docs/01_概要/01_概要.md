...
統計局のデータを活用した47都道府県比較ブログサイト。Next.js 15とTailwind CSSを使用して構築し、SEO最適化とユーザー体験の向上を目指す。

## 参考リソース

### データソース
- [統計局ホームページ/令和３年社会生活基本調査 47都道府県ランキング](https://www.stat.go.jp/data/shakai/2021/rank/index.html)
- 海岸統計データ
...
#### メタタグ最適化
- Next.js 15のMetadata APIを使用
- 動的メタデータ生成（`generateMetadata`関数）
- OGP設定（Open Graph、Twitter Cards）
- 構造化データ（JSON-LD）の実装

#### サイトマップ・robots.txt
- `app/sitemap.ts`で自動生成
- `app/robots.ts`でクローラー制御
- 動的ルート対応
...
- ISR（Incremental Static Regeneration）で定期的更新
- オンデマンド再生成（`revalidatePath`）
...
# メタタグ最適化（title, description, OGP設定）
