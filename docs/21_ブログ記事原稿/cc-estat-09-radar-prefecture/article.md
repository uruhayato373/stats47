...
これで 1 枚の SVG に東京と京都の多角形がオーバーレイされます。重なる部分は色が混ざって紫っぽくなり、ずれている部分は青と赤が独立して見えます。**人間の脳は「形の差分」を数値の差分よりずっと速く検知する** ので、レーダーの overlay は強力なコミュニケーション手段です。実際に stats47 の確定値（全 47 都道府県で min-max 正規化、医療費と犯罪は反転）で東京都と京都府を重ねると、こんなシルエットになります。

![東京都と京都府を6指標で重ねたレーダーチャート。全47都道府県でmin-max正規化し、医療費・犯罪は反転している](data/tokyo-kyoto-radar.svg)

この図は架空例ではなく、**本文がリンクしている実在ランキングの確定値**を上の `radar.mjs` と同じ手順で正規化して描いたものです（軸ラベルの 2 行目に全国の min〜max を併記する **方法 A** を適用しています）。形を読むと、東京都は **人口規模（1 位）と世帯年収（2 位）** の方向へ大きく張り出す一方、刑法犯認知件数が全国最多のため **治安（犯罪率の逆）の軸が中心まで深くえぐれて** います。逆に京都府は人口・所得が控えめでも **治安の軸が外側まで伸びて** いて、東京とは対照的な「角の出方」になります。教育費（1 世帯あたり）は両県とも中位どまりで、レーダーにすると「大都市＝全項目で強い」という思い込みが崩れるのが見どころです。数字の羅列では埋もれるこの非対称が、1 枚の形として立ち上がります。

<source-link href="/ranking/japanese-population">人口ランキングをもっと見る</source-link>
<source-link href="/ranking/criminal-recognition-count">刑法犯認知件数ランキングをもっと見る</source-link>

> [!WARNING]
> 上のレーダーは確定値ですが、軸ごとに**最新年がそろっていません**（人口 2024 / 世帯年収 2019 / 教育費 2022 / 医療費 2022 / 犯罪 2023 / 観光資源 2014）。レーダーは複数指標を 1 枚に重ねるぶん、年次のズレが形に紛れ込みやすいチャートです。公開する際は各軸の出典年を必ず添え、年がバラつくこと自体を読者に開示してください。
