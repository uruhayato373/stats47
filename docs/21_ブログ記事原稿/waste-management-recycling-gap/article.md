...
上位と下位を並べてみると、1位の岡山県（29.0%）と最下位の和歌山県（11.9%）の差は17.1ポイント、倍率にして2.4倍にのぼります。マップでは色のグラデーションとして見えていた差が、ランキングにすると「分別を徹底している県」と「焼却に大きく依存している県」の隔たりとしてくっきり浮かび上がります。中位の県は20%前後に密集しており、上位・下位の数県だけが平均から大きく離れている構図です。

<source-link href="/ranking/waste-recycling-rate">47都道府県のごみリサイクル率ランキングをもっと見る</source-link>

## 埋立 vs 焼却 vs リサイクル

リサイクルされなかったごみは、どこへ行くのでしょうか。ごみの行き先には「資源化（リサイクル）」「焼却」「埋立」の3つがあり、リサイクル率が同じくらい低い県でも、その先を埋立に回すのか焼却に回すのかは県ごとに大きく異なります。だからこそ、リサイクル率だけを見て循環型社会の進み具合を語ることはできません。

![リサイクル率 vs ごみ埋立率](data/recycle-vs-landfill-scatter.svg)

散布図を見ると、リサイクル率と埋立率には弱〜中程度の逆相関（相関係数 -0.57）はあるものの、きれいな一直線にはならず、例外県が散らばっています。注目したいのは、リサイクル率が低い県の多くが、埋立率まで低いわけではないという点です。たとえばリサイクル率が下位の和歌山県（11.9%）・青森県（12.6%）・大阪府（12.6%）はいずれも埋立率が10%超で、大阪府の埋立率11.3%は全国8位タイの高い水準です。これらの県は「リサイクルが進まず、その分を埋立に回している」タイプであり、最終処分場を消費し続けている点で別の課題を抱えています。

一方、リサイクル率も埋立率もともに低い「焼却主体」型の代表例が、左下に位置する沖縄県（リサイクル率14.9%・埋立率6.0%）です。リサイクルにも埋立にも多くを回していないということは、残りの大部分を焼却で処理していることを意味します。島嶼部で最終処分場の用地が限られる沖縄県では、ごみを燃やして減容する焼却処理への依存度が自然と高くなります。同じように、埋立率が4〜5%台と低い東京都（リサイクル率24.9%・埋立率4.6%）や千葉県（22.6%・5.5%）も、高性能の焼却施設（ごみ焼却発電）でごみを燃やし、その熱で発電したり地域に熱を供給したりしています。統計上はリサイクルにカウントされませんが、限られた土地で大量のごみを処理しながらエネルギーを回収する方式には一定の合理性があります。したがって、リサイクル率の低さだけを取り出して「環境意識が低い」と断じるのは早計です。処理方式の違いまで踏み込んで評価する必要があります。

対照的なのが「埋立主体」型で、その典型が[北海道](/areas/01000)（埋立率16.1%）です。全国で突出して高い埋立率で、広大な土地に最終処分場を確保しやすい反面、人口が分散しているため大規模な焼却施設を集約しにくいことが背景にあると考えられます。土地の事情が処理方式を方向づける典型例といえます。
