...
静岡県の人口10万人当たり交通事故発生件数は**494.5件**。島根県は**107.8件**。同じ日本なのに、交通事故リスクにはおよそ**4.6倍もの開き**があります（2024年）。なお2位は群馬県の479.3件で、静岡・群馬の2県が突出しています。
...
![高齢化率×交通事故致死率の散布図（47都道府県・2024年）](data/aging-fatality-scatter.svg)

実際に47都道府県を「高齢化率（横軸）×致死率（縦軸）」でプロットすると、点は右肩上がりに分布し、相関係数は r=0.63 と中程度の正の相関を示します（いずれも2024年）。高齢化率が35%前後の県では致死率が2〜3%台に達する一方、高齢化率が25%前後の都市圏は1%前後にとどまります。事故の「件数」では下位に並ぶ地方の県が、致死率ではむしろ上位に来るという冒頭の逆説が、この1枚で裏づけられます。
