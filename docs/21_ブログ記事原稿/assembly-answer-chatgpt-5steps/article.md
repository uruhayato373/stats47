...
> [!WARNING]
> 個人情報・非公開情報を ChatGPT に貼り付けてはいけません。本記事は **公開済の質問書 + 過去答弁** だけを扱う想定です。LGWAN 環境制約と組み合わせる注意点は本文末尾と関連記事を参照してください。
...
本記事は ChatGPT (Web UI) ベースの 5 ステップです。これを **コマンドラインで自動化** したい方、**過去答弁集を一括ベクトル検索** したい方には Claude Code が向いています。具体的な実装例は [公務員 1 人で毎日 2,150 ランキング更新｜Claude Code × e-Stat 自動化](/blog/koumuin-claude-code-estat-automation) で公開しています。
...
> [!TIP]
> 答弁に使う統計データは e-Stat から取得するのが基本ですが、探しにくさで詰まりがち。効率的な取得手順は [e-Stat の「使えない」を「使える」に変える 7 つのテクニック](/blog/estat-7-techniques-from-unusable-to-usable) にまとめています。
