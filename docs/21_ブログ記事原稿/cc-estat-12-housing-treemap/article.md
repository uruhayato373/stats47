---
title: "住宅着工を1枚に押し込む｜なぜ棒グラフでは足りないのか"
seoTitle: "住宅着工ツリーマップの作り方｜Claude Code×D3で階層データ可視化"
subtitle: "Claude Code 未経験エンジニアのための実例集 Part 12"
slug: cc-estat-12-housing-treemap
description: "「同じ住宅着工でも、東京と高知では桁が2つ違う」──地域→都道府県→種別の3軸データを棒グラフで見せようとすると軸が足りません。本記事は d3.treemap で量と内訳を1枚に押し込む手順を、Claude Code 越しに整形から描画まで通しで解説します。"
archetype: A
...
「47 都道府県のデータを 1 枚の絵に押し込む」というお題は、ランキング系の棒グラフが定番です。ですが、これを **地域ブロック（北海道・東北・関東…）でグルーピングして、さらに持家／貸家／分譲の内訳まで一気に見せたい** と思うと、棒では足りなくなります。並べる軸が 3 つになるからです。

そんなときに刺さるのが **ツリーマップ（treemap）** です。長方形の面積が値を表し、その長方形を入れ子状に並べることで、**「全体の量」と「内訳構成」を 1 枚で同時に表現できる** 数少ない手法です。Excel の積み上げ棒だと泣きたくなるレイヤー数を、ツリーマップは涼しい顔で処理します。

本記事はシリーズ「Claude Code × e-Stat API 実例集」の Part 12 です。前回 Part 11 でバブルチャートを扱いましたが、今回はテーマを変えて **住宅着工統計** を題材に、`d3.hierarchy` + `d3.treemap` の使い方を Claude Code 越しに学びます。前提知識は「ターミナルで `node` が動く」「`npm install` が通る」程度で十分です。
...
## なぜ住宅着工が「3軸データ」の練習に向くのか

最初に、今回 1 枚に押し込みたいデータの正体を共有します。

着工新設住宅戸数（2024年度・国土交通省 住宅着工統計）を都道府県でランキングすると、上位と下位の落差が極端です。下の図は上位5県と下位5県だけを抜き出したものです。

![着工新設住宅戸数 上位5・下位5（2024年度）](data/new-housing-starts-ranking.svg)

最も多い東京都は 129,571 戸で、これは 2 位の大阪府 71,929 戸の約 1.8 倍にあたります。3 位以下は神奈川県 66,361 戸、愛知県 55,347 戸、埼玉県 51,669 戸と続き、上位はすべて三大都市圏とその近接県で占められます。住宅着工は人口流入・世帯形成・地価のすべてが効く指標なので、人が集まる場所ほど着工が増えるという素直な構造が出ます。

一方で下位は、高知県 2,110 戸を最下位に、鳥取県 2,526 戸、徳島県 2,733 戸、島根県 2,777 戸、秋田県 3,065 戸と並びます。1 位の東京都と最下位の高知県では戸数が約 61 倍も開いており、桁が 2 つ違います。下位に並ぶのは人口減少が先行する地方県で、新規世帯がそもそも生まれにくいことが着工の少なさに直結しています。

> [!NOTE]
> 着工新設住宅戸数は「住宅 1 戸ごとに、いつ・どこで・どんな種別の建物が着工されたか」を集計した戸数です。完成戸数ではなく **着工（工事開始）時点** でカウントするため、景気の先行指標として扱われます。建て替えも新規も区別なく「新設」に含まれる点に注意してください。

<source-link href="/ranking/new-housing-starts">着工新設住宅戸数ランキングをもっと見る</source-link>

ここで重要なのは、この棒グラフが見せられるのは「県 × 戸数」の 2 軸だけだという点です。本記事で本当にやりたいのは、ここに **地域ブロック** と **着工種別（持家／貸家／分譲）** をもう 2 つ重ねて、**「ブロック × 県 × 種別」の 3 階層** を 1 枚で見せることです。だからこそツリーマップが要ります。

着工統計を選んだ理由も単純で、**「ブロック × 県 × 種別」の 3 階層が自然に揃う数少ない指標** だからです。人口や所得は階層化しにくい（県の中の更なる細分が地域差で揃わない）のに対し、着工は全国で利用関係区分が同じなので、ツリーマップに乗せやすいのです。教材として優秀な被写体だと言えます。
...
ターミナルから `node generate.mjs` を 1 回叩くと SVG が落ちてくる、というシンプルな構成にします。Next.js などフレームワークは使いません。シリーズ Part 3 以降と同じ最小依存構成です。処理の流れをまとめると次のようになります。
...
利用関係の 4 区分のうち、本記事では **持家 / 貸家 / 分譲住宅** の 3 区分のみを使います。給与住宅（社宅）は全国で年間数千戸しか着工されず、ツリーマップ上の矩形がほぼ見えなくなるので除外します。「全部入れる」より「読める粒度で残す」のが、ツリーマップを成功させる最大のコツです。
...
ツリーマップ最上位の階層には、以下の **8 地域ブロック** を使います。総務省統計局や経産省の地域区分とは流儀がいくつか違いますが、本記事では一般的な 8 区分を採用します。
...
> [!WARNING]
> 上の 8 区分は「描画のための便宜的なグルーピング」であって、統計の公式分類ではありません。e-Stat が返す地方区分（地方ブロック合計の行）とはコード体系が違うため、**都道府県の 5 桁コードから自前でブロックに割り当てる** のが安全です。地方合計の行（例: 「東北」という集計行）をそのまま葉に混ぜると、県単位の戸数と地方合計が二重に積まれて総和が合わなくなります。
...
`cdArea` / `cdTime` を付けない方針はシリーズ通しての約束です。理由は Part 3 でも書きましたが、簡単に言えば **「キャッシュキーをずらさない」「47 県を毎回 1 リクエストで取る方が結局速い」** の 2 点です。本サイトの `.claude/rules/estat-api.md` にも同じ規約を書いています。

Claude Code が吐くスクリプトは、おおむね以下のような形になります。実際に動くサンプルとして掲載します。
...
47 県 × 3 種別 = 141 件の flat な配列が `housing_raw.json` に書き出されます。ここまでが Step 1 です。**実 API を叩く前に、まず flat な JSON を 1 つ作る** のがコツです。後段の階層整形・描画はこの flat JSON だけ見ればよくなり、API のレスポンス構造を毎回考えなくて済みます。
...
実行後の `housing_tree.json` はこんな構造になります（抜粋・値はサンプル）。
...
// ルート要素のタグ名は定数化しておく（後で属性を増やしても 1 箇所で済む）
const SVG = "svg";
...
ブラウザで開くと、地域ブロックごとに区分けされ、その中に都道府県の矩形が、さらに県の中に持家／貸家／分譲の小矩形が入った 3 階層が見えます。先ほどの棒グラフでは「東京がずば抜けて多い」しか分かりませんでしたが、ツリーマップにすると **東京の中でも貸家の矩形が突出して大きい**、つまり首都圏の着工は賃貸住宅が牽引しているといった内訳まで一目で読めるようになります。これが軸を 3 つに増やした見返りです。

> [!TIP]
> ツリーマップは「面積で量を比べる」図なので、棒グラフのように厳密な順位比較には向きません。読者に正確な順位を伝えたいときは、本記事冒頭のような **上位・下位の棒グラフを併設** し、ツリーマップは「全体の構成感をつかむ俯瞰図」として役割分担させるのが定石です。1 枚で全部やろうとしないのがコツです。
...
- 明度（lightness）= 着工種別（持家 / 貸家 / 分譲）。同じブロック内の 3 種別をグラデで連続表現します
- 面積 = 着工戸数。ツリーマップ本来の主軸です

色相で「県」を分けようとすると 47 色必要になって破綻するので、**県は色を割り振らず、ブロック共通の色で塗る** のが現実解です。「東京の貸家」と「神奈川の貸家」は色は同じですが、面積で大きさを比較する、という設計になります。
...
色は最低限、以下の制約を満たすようにしてください。

- ブロック間: 隣接ブロックが補色 / 類似色にならないよう調整します（北海道=青 / 東北=緑 / 関東=赤 がデフォルト推奨）
- 種別間: 同一色相のまま明度のみ 3 段階にします。彩度は触りません
- コントラスト: 文字色 `#0f172a` 上で読めるよう、最暗で `lightness >= 0.45` を確保します
...
## Step 5: ラベル配置｜小領域は諦める

ツリーマップ最大の運用上の悩みが **「ちっちゃい矩形にラベルが入らない」** 問題です。鳥取の分譲（数百戸）と東京の貸家（数万戸）が同じ画面に並ぶので、両方読めるようにラベルを置くのは原理的に不可能です。冒頭の棒グラフで見たとおり、上位と下位は 61 倍も差があるからです。
...
これ以下の領域は **無記載のまま放置** にします。ユーザーはマウスホバーで詳細を見るか、別のサマリ表で確認する、という割り切りです。本サイトでは別途、地域ブロック単位のサマリをツリーマップの近くに添えるパターンが定番化しています。
...
これを `svg` 文字列の `leaves.map(...)` 出力の **後ろ** に挿入すれば、葉の上にブロック名が浮かびます。SVG は後勝ちなので、順序は必ず葉 → ブロック名の順にします。

フォントサイズは、ブロック名を 12px の太字、県名を 11px の通常、種別と数値を 10px の 70% 不透明あたりを目安にしてください。これ以上小さくすると、Retina ディスプレイ前提でも厳しくなります。読めない情報をしまい込むより、最初から「見えない領域は割愛」と決めてしまった方が、情報設計として誠実です。
...
`d3.hierarchy(...).sum((d) => d.value)` は **すべてのノードに対して** コールバックを呼び、自分の `value` + 子の合計を加算します。中間ノード（ブロック / 県）に `value: 0` ですら入っていれば正しく集計されますが、`value: 123` のように整形ミスで残っていると **二重計上** になります。
...
対策は **葉ノード生成時に value が 0 以下のものを除外** することです。本記事では給与住宅を最初から除外しているのでこの問題に当たりませんが、別の指標で試すときは要注意です。

### 3. フォントサイズ vs 矩形サイズの閾値ミス

`showLabel = w > 60 && h > 24` の値は経験則です。本サイトでは 1200×800px のツリーマップでこの値に落ち着いていますが、出力サイズを変えると当然調整が必要になります。

雑な目安として、**「県名 + 種別を 2 行で書ける最小矩形」** を意識します。フォント 11px × 県名の最長文字数（「神奈川県」= 4 文字 ≈ 44px）+ 余白 16px ≈ 60px がだいたいの下限です。これより小さい矩形は無記名にして、ホバーや別表で補う設計にしてください。


## まとめ：3 軸データを 1 枚で見せたいときの第一選択

ツリーマップは「全体の量 × グルーピング × 内訳」の 3 軸を 1 枚で押し込める、希少な可視化手法です。本記事で見たとおり、住宅着工は東京の 129,571 戸から高知の 2,110 戸まで約 61 倍の開きがあり、しかも県の中で持家・貸家・分譲の比率が違います。この「量も内訳も一度に見たい」というニーズに、棒グラフは答えられません。データさえ階層構造に整形できれば、`d3-hierarchy` + `d3-treemap` の組み合わせで 60 行以内で描けます。Claude Code に頼むときのコツは以下のとおりです。

1. flat な JSON を先に作る（API レスポンスを直接いじらない）
2. 階層化は「中間ノードに value を入れない」だけ厳守する
3. 色は「色相 = カテゴリ」「明度 = サブカテゴリ」の 2 段運用にする
4. ラベルは「読めない矩形は諦める」のが正解
5. 合計値の突合スクリプトを最後に必ず走らせる

シリーズ次回 Part 13 は **サンキー図（Sankey Diagram）** を扱う予定です。県間の人口移動や、産業間の取引フローのような「フロー量」を可視化する手法で、ツリーマップが「量と内訳」なのに対し、サンキーは「流れと滞留」を描けます。データ整形のクセが似ているので、本記事の `d3.hierarchy` がしっくり来た人は、そのまま読み進めやすいはずです。

シリーズの他の回も、e-Stat からの取得と D3 描画を 1 セッションで完結させる構成で書いています。基礎から追いたい方は [Part 3: 都道府県別人口ランキングを Claude Code で1分で作る](/blog/cc-estat-03-population-bar) から、前回のバブルチャートは [Part 11: 観光客数の積み上げ棒](/blog/cc-estat-11-tourism-stacked) からどうぞ。


## データ出典

- 国土交通省「住宅着工統計」（着工新設住宅戸数・2024年度）
- e-Stat（政府統計の総合窓口）経由で取得・整備
