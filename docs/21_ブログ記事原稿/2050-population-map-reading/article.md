...
最後に、地図は都道府県を一つの値で塗っていると確認します。県内の市区町村差や1kmメッシュ差は、この図には出ません。県全体が-20%でも、中心市街地と周辺部が同じ割合で減るとは限りません。この単一指標の比較はランキングに集約し、複数レイヤーを空間演算するGeo分析とは区別しています。[東京都の地域データ](/areas/13000)と[秋田県の地域データ](/areas/05000)も開くと、将来人口増減率とは別の人口・暮らしの指標を確認できます。

## 上位でも減少、下位は3割超減少

![2050年 将来人口増減率の上位5県と下位5県](data/2050-population-map-reading-prefecture-rankings.svg)

上位5県では東京都だけが+2.5%で、沖縄県-5.21%、神奈川県-7.72%、千葉県-9.46%、埼玉県-9.68%と続きます。正の値と負の値が同じ「上位」に並ぶため、順位だけでなく0%との位置関係を読む必要があります。この図は増減率の上位5県・下位5県を数値カードで比較し、棒の長さではなく符号と値を読む設計です。最大値を最小値で割る倍率も、符号が異なるこの指標では意味を持ちません。地域差は東京都と秋田県の44.09ポイント差として表す方が適切です。
...
県別増減率は、三つの空間横断分析を読むための背景情報です。計算の共通の分母ではありません。[人口減少×住宅地価](/geo/population-land-price)では住宅地点を1km人口メッシュに接続し、地価上昇と所在メッシュの将来人口減少が重なる地点の割合を、比較可能地点数を分母に求めます。[2050年人口×洪水浸水想定](/geo/population-flood-risk)と[2050年人口×駅アクセス](/geo/population-station-access)は、区域や直線800m圏に中心点が入るメッシュの人口を合計した概算です。こちらの割合は県の人口メッシュ合計が分母になります。
...
再現用データを利用するときは、収録されている推計版、基準年、単位、対象47県を先に確認してください。将来人口の値だけで投資、移住、出店を決めるのではなく、この記事の無料データ導線から自分の問いに必要な指標を照合できます。同じ版・同じ定義で再検証できることが、販売物でも無料記事でも共通の品質条件です。
