...
write('land-price-spatial-evidence.source.json', { ...source, surveyScope: 'not-applicable', surveyScopeReason: '地価公示地点と将来人口メッシュの包含結合を独自に計算した県別値、全国保存則、地点追跡例の補助成果物であり、統計調査の集計表ではない。入力49件のSHAと計算手順は本manifestに保持する。' });
const map = { title: '地価上昇 × メッシュ人口減少', subtitle: '比較可能な住宅地点に占める割合', unit: '%', scheme: 'Blues', data };
write('land-price-spatial-map.json', map);
write('land-price-spatial-map.source.json', { ...source, surveyScope: 'not-applicable', surveyScopeReason: '地価公示地点を将来人口メッシュへ包含結合し、比較可能な住宅地点に占める地価上昇かつ人口減少地点の割合を独自計算したGIS派生値であり、統計調査の集計表ではない。' });
...
write('land-price-spatial-lineage.source.json', { ...source, surveyScope: 'not-applicable', surveyScopeReason: '地価地点と人口メッシュの包含結合、比較対象の選別、東京都の保存則と1地点の数値例を説明するGIS計算工程図であり、統計調査の集計表を再掲する図ではない。' });
