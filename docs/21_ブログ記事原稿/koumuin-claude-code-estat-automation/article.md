...
subtitle: "1 人で 47 都道府県・2,150 ランキングを毎日更新する開発記録"
...
stats47 は、政府統計（e-Stat）の都道府県別データを **2,150 個のランキング**・**22,487 行の地域指標**として整理し、毎日更新している個人開発サイトです。たとえば [財政力指数のランキング](/ranking/fiscal-strength-index) や [人口のランキング](/ranking/japanese-population) のように、1 指標ごとに 47 都道府県を並べて比較できる形に落とし込んでいます。

> [!NOTE]
> ここで言う「2,150 個のランキング」は、e-Stat の統計表を都道府県軸で切り出した**指標の数**です。1 つの統計表（例：賃金構造基本統計調査）から、職種別・年度別に複数のランキングが派生します。「2,150 件の記事を書いた」という意味ではなく、**機械的に量産できる単位がそれだけある**、と読んでください。手作業で全部書いていたら 1 人では不可能な規模です。
...
e-Stat には公式 API があります。ただ、素直に叩くだけだと運用は破綻します。実際に踏んだ落とし穴は次の 3 つでした。

- **年度範囲（`cdTimeFrom` / `cdTimeTo`）でリクエストを分割する** → キャッシュが分断され、同じデータを何度も取得することになります。
- **地域コード（`cdArea`）を都道府県ごとに指定する** → 47 回 API を呼ぶ羽目になり、待ち時間も API 負荷も跳ね上がります。
- **API レート制限を無視する** → 一定回数で 403 が返ってきて、バッチが途中で止まります。

> [!WARNING]
> e-Stat API は「年度範囲」「地域コード」でクエリを絞れますが、絞るほどキャッシュキーが細切れになり、結果として API 呼び出しが増えます。**全年度・全地域を一括取得してからメモリ上でフィルタする**のが、レート制限を避けつつキャッシュヒット率を上げる定石です。直感に反するので、最初に必ず踏む落とし穴です。
...
キャッシュの効果も具体的に出ています。代表的な 29 URL のデータを再計算するときは **約 2 分**、フルキャッシュ更新（2,865 URL）でも **約 25 分** で完了します。1 つ 1 つを手作業でダウンロードしていたら、1 日では到底終わらない量です。R2 ストレージを使ったキャッシュ設計の詳細は、[e-Stat データを R2 にキャッシュする実装](/blog/cc-estat-18-cache-r2) で具体的なコードとともに解説しています。

> [!TIP]
> キャッシュ層を入れるかどうかは「同じデータを 2 回以上取得するか」で判断します。月次レポートのように毎月同じ統計表を引くなら、初回だけ API を叩いて以降はキャッシュを読む設計が効きます。逆に、1 回しか使わない使い捨て集計にキャッシュを入れても効果は薄く、むしろ実装コストの方が大きくなります。
...
スキルをどう組み合わせて 1 本のパイプラインにするかは、[Claude Code スキルでデータ更新を自動化する](/blog/cc-estat-19-skill-pipeline) で実際の連結例を紹介しています。スキルの中身を簡単に紹介すると、典型的な SKILL.md はこんな構造です。
...
- `gsc-url-inspection-daily`（毎日）— Google Search Console の URL Inspection API でクロール状態を確認
...
ここまでの 3 レイヤーを、公務員の典型的な業務に当てはめてみます。公務員が AI スキルを身につけることのキャリア面の価値については、[公務員から AI 人材へのキャリアパス](/blog/koumuin-ai-tenshoku-1500man) でも別の角度から触れています。
...
stats47 は今も進化中で、施策・実装は順次公開しています。実装をゼロから追いたい方は [Claude Code で e-Stat を扱う最初のセットアップ](/blog/cc-estat-01-setup) から、e-Stat データを「使えない状態から使える状態」へ整える具体テクニックは [e-Stat データ整形の 7 つのテクニック](/blog/estat-7-techniques-from-unusable-to-usable) が参考になります。


## データ出典

- 本記事で言及した統計データ（都道府県別ランキング・地域指標・相関データ）は、すべて **総務省統計局「政府統計の総合窓口（e-Stat）API 機能」** を経由して取得・整備したものです。
- 取得したデータは e-Stat 公開の統計表（人口動態調査・賃金構造基本統計調査・地方財政状況調査など）を都道府県軸で再集計しています。
- 自動化レイヤー・スキル数・ワークフロー数・処理時間（2 分 / 25 分など）は、stats47.jp の運用実績値（2026 年 5 月時点）です。
