---
title: "公務員 1 人で毎日 2,150 ランキング更新｜Claude Code × e-Stat 自動化の全公開"
seoTitle: "公務員 1 人で 47 都道府県 2,150 ランキング毎日更新｜Claude Code × e-Stat 自動化の裏側"
subtitle: "「人手不足の自治体でも回る」AI 業務効率化の実装パターン"
description: 「47 都道府県の統計データを毎日更新する」──通常なら 5-10 人体制が必要なこの仕事を、現役公務員が 1 人 + Claude Code で運用している実例。e-Stat 自動集計から可視化までの実装パターンと、AI 導入時のセキュリティ境界設計を公開します。
category: technology
tags:
  - automation
  - claude-code
...
通常なら 5〜10 人のチームを思い浮かべるかもしれません。答えは **1 人 + AI（Claude Code）** です。

本記事では、現役公務員が運用している [stats47.jp](https://stats47.jp) の裏側として、e-Stat 自動集計 × Claude Code の実装パターンを公開します。自治体・公務員業務への応用可能性についても触れます。

> [!NOTE]
> 本記事はオープンソース化されている [stats47 リポジトリ](https://github.com/uruhayato373/stats47) の実装に基づきます。具体的なコード例は [/blog/estat-7-techniques-from-unusable-to-usable](/blog/estat-7-techniques-from-unusable-to-usable) もあわせてご覧ください。
...
過去の答弁データ・統計トレンドから、議題に応じた資料の下書きを生成する用途。「過去 3 年で同じテーマの答弁を抽出 → 最新の統計を当てはめ → 想定問答を生成」のような流れは、Claude Code の最も得意な領域です。具体的な ChatGPT 活用パターンは [/blog/assembly-answer-chatgpt-5steps](/blog/assembly-answer-chatgpt-5steps) で詳述しています。

> [!WARNING]
> **個人情報・機密データは絶対にクラウド LLM に渡さない。** Claude Code は API キーを持っていますが、データは Anthropic のサーバに送信されます。住民台帳・税情報・人事評価など機密データを扱う場合は **オンプレ LLM・庁内 VPC 内に閉じた LLM** (例: Llama 3, Mistral 等) に切り替える運用が必須です。stats47 は公開データのみを扱っているため Claude Code で運用していますが、公務員業務では「公開データを扱うスキル」と「機密データを扱うスキル」を完全に分け、後者は閉域網に閉じる構成が現実的です。
...
> [!TIP]
> 「自分の業務をどう分解するか」が見えない場合は、まず 1 週間の作業を 30 分単位で書き出してみるのがおすすめ。「データ集める」「Excel に貼る」「グラフ作る」「上司に確認」のような粒度まで分解できれば、自動化候補が見えます。
...
- [e-Stat の「使えない」を「使える」に変える 7 つのテクニック](/blog/estat-7-techniques-from-unusable-to-usable)
- [この記事の前提となる stats47 のサイト全体について](/about)
