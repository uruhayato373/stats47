...
<!-- data-source: average-height-middle-school-second-grade-male-prefecture-rankings.json -->
![中学2年男子 平均身長 上位5県と下位5県](data/inline-chart-1.svg)
...
**[暫定結論] 3仮説のうち、現時点で証拠と最も整合するのは「遺伝・集団的背景（仮説2）」と「気候・体格適応（仮説3）」の組み合わせだ。** 秋田・青森が複数年にわたり安定して上位を保つ長期的な傾向は、短期の食習慣変化だけでは説明しにくい。ベルクマン則に類する気候要因も南北勾配と整合する。一方、栄養（仮説1）は東京が中位にとどまる点で説明力が弱い。もっとも、三仮説を実証的に検証するには「都道府県別の食事調査データと身長の相関」「同一家系の転居前後の追跡データ」が必要であり、現データでは断定不能だ。次に見るべきデータとして、[子どもの身長の地域差](/blog/child-height-regional-gap)での年齢横断データが参考になる。
...
- 東京161.4cm・大阪161.0cmなど**大都市は中位**で、「栄養が良い=高い」では説明しきれない。
- 3仮説のうち現時点で最も整合するのは**遺伝・集団的背景と気候適応の組み合わせ**。栄養単独では東京が中位にとどまる点が説明しにくい。平均差を個人の優劣に結びつけない読み方が重要。
