...
プロジェクトは機能別にルートグループを使用して整理されています：
...
- SSR/ISR: 初期体験が重要・共有可能なデータに適用（例: ランディング、ランキングの初期リスト）
- CSR+SWR: ユーザー依存・頻更新・対話性重視のデータに適用（例: 絞り込み、個別ユーザー設定）
- 判断目安: 「CPU/IO コスト高 or 初速重要 → SSR/ISR」「ユーザー固有/頻更新 → CSR+SWR」
