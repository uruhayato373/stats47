...
実装状況（2026-07-13）: 第2段階を実装済み。`check-asset-policy.cjs --baseline` を PR Static Gates に接続。**git 管理対象の画像を repo 全体（787枚: raster 440 / svg 347、CI checkout と同一 scope）で棚卸し**し、デコード・pixel寸法・容量・**形式↔拡張子の一致（FORMAT_MISMATCH）**・SVG 安全性（viewBox / script・foreignObject / 外部URL / malformed）・**SHA-256 完全同一の重複（DUPLICATE_IMAGE）**・**MD / HTML / CSS / TS(X) のローカル画像参照解決（存在・case-sensitive・/public 絶対＋相対）**について新規悪化を block する。**未参照画像（424件）は非ブロックの UNREFERENCED_IMAGE warning** として report する。OGP は 1200×630 preset のみ確定適用し、favicon / PWA は `check-static-assets.cjs` を正典として重複させない。text overlap / font fallback / alpha・color profile の用途別制約は誤検知回避のため既存の専用 lint（svg-lint.mjs 等）を維持する。runtime 788ms / 10000ms（suite 合計 4017 / 30000ms）。単体テスト 8件。

**baseline burn-down（2026-07-13、50→27）**: 「既存を baseline 化するだけでは品質向上にならない」という指摘を受け、初期 baseline 50件のうち実際に減らせる分を修正した。**(1) MISSING_REFERENCE 23件を解消**: ①公開中2記事（`library-museum-cultural-capital` / `overseas-travel-gap`、共に published:true・R2 live は健全）の docs/21 outbox に欠けていた SVG 8件を **R2 正典から復元**（live 画像は非接触。findings は「壊れた公開ページ」ではなく未 prune の outbox 残骸だった）、②note screenshot-guide の**コードスパン/コードブロック内の例示 `![](…)` 15件**を、markdown ref 抽出がコード内記法を拾う精度バグとして**チェッカー側で除外**（`stripMarkdownCode`、回帰テスト追加）。**(2) 残 27件（FORMAT_MISMATCH 3 / DUPLICATE 3 / OVERSIZED 21）は全て「既存画像を削除/圧縮/再生成しない」制約でブロック**され、実修正には制約緩和のオーナー判断が要る。この 27件は隠さず `docs/todo/01_改善バックログ.md#ASSET-POLICY-BURNDOWN-01` に unblock 条件（圧縮承認 / dedup 承認 / banner 中間物削除承認・logo 再エンコード）付きで台帳化した。既存 finding は `.claude/config/asset-policy-baseline.json`（27件、baseline 識別子は `CODE:relpath:message` で絶対パス / mtime 非依存）に固定。
