...
## モバイル 375px 回帰（コードレベル検証済・実機は任意）

17px 化は **rem を 6% 比例拡大**するが、新規の横はみ出しは**導入しない**と確認:

- **ランキング/市区町村/カテゴリのデータ表**: 共有 `DataTable`（`packages/.../data-table.tsx:91`）が `<table>` を `overflow-auto` + `min-w-max table-fixed` でラップ → **横スクロールが既定挙動**。固定列幅（60/150/120/80px）は元々 375px を超えスクロール前提。6% 増はスクロール幅に吸収され**回帰なし**。
- **テーマダッシュボード等の `grid-cols-[1fr_380px]`**（ThemeDashboardTabbed:262 / PortMapClient:226）: **デスクトップ専用 branch**。モバイルは別の単一カラム layout（ThemeDashboardTabbed:256 で return）。→ モバイル無影響（Explore の "critical" は branch 見落としの誤検出）。
- **セレクタ `w-[100px]`/`w-[140px]`・ヘッダー**: flex 内・短いロゴ文字で 375px に収まる。低リスク。

**結論: 17px 化は 375px で新規はみ出しを生まない（コード解析）。** 実機/エミュレータでの 375px パスは確認の上積みとして任意。

## 残課題（未対応）

1. [polish] home 中段グラデプレビューの彩度抑制 / 空プレースホルダの CLS 対策

> 色トークン統一・motion 鈍化解消は 81923ccb で対応済。モバイル回帰はコードレベルで verified（上記）。
