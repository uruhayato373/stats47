...
| **Instagram Graph API** | 削除 API、Page 経由 | ❓ **未追加の可能性** (Graph API Explorer に `instagram_basic` 等が出ないため) |
...
3. 既存 Product 一覧を確認
4. 「**Instagram Graph API**」が無ければ「Add Product」で追加
5. Set Up ダイアログで **Stats47 Page** + **Doboku-note Page** 両方を選択

### 必要な Permissions (Graph API Explorer 用)
...
Facebook Graph API で IG メディアを操作するには、Page と IG Business が **連携済** である必要。
...
# Doboku-note Page → IG 連携確認
curl -s "https://graph.facebook.com/v21.0/1094537940409154?fields=instagram_business_account&access_token=$META_PAGE_TOKEN_DOBOKUNOTE" | python3 -m json.tool
```
...
```json
{
  ...
  "instagram_business_account": {
    "id": "26834754356143704"
  }
}
```

`instagram_business_account` フィールドが空なら、**Facebook Page 設定で IG account を連携する必要**:

1. https://business.facebook.com → 「ビジネス設定」→「アカウント」→「ページ」
2. Stats47 Page (or Doboku-note Page) を選択
3. 「Instagram」タブ → 「アカウントを接続」→ 対応する IG (stats47jp / doboku-note) でログイン

> **注意:** 1 つの IG account は 1 つの FB Page にしか連携できない。
...
### Phase 2: Token 取得 (Graph API Explorer)

各アカウント分、以下を取得:

- [ ] User Access Token (`instagram_basic` + `instagram_manage_contents` + `instagram_content_publish` 含む)
...
`.github/workflows/meta-tokens-monthly.yml` で毎月 1 日に refresh + 失敗時 Slack/Issue 通知。

## トラブルシューティング

### 「Add a Permission」に `instagram_basic` が出ない
→ アプリの Products に **Instagram Graph API** が追加されていない。Meta Dashboard で追加。

### `instagram_business_account` フィールドが空
→ Page と IG が連携されていない。Meta Business Suite で連携。

### scopes に追加した権限が含まれない
→ Generate Access Token 時の Facebook 承認ダイアログで明示的に許可していない。「詳細を編集」で各権限の状態を確認。

### IG Business Account ID が分からない
→ `/{page-id}?fields=instagram_business_account` で取得 (Page Token 必要)。
