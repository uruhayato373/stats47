...
9. [GeoShape自動キャッシング](#geoshape自動キャッシング)
...
1. **都道府県データの管理**: 47都道府県の基本情報の提供
2. **市区町村データの管理**: 全国約1,900の市区町村情報の提供
...
- **estat-api**: e-Stat APIから取得したデータの地域コード解決
...
- **R2ストレージ採用**: 静的データの効率的な配信
- **環境別データソース**: mock環境ではローカルファイル、本番環境ではR2

### 設計原則

- **環境判断**: `NEXT_PUBLIC_ENV=mock`で判断（追加の環境変数不要）
- **フォルダ構成**: `data/mock/r2/areas/`にmockデータを配置
...
| フィールド | 型     | 必須 | 制約                   | 例                |
| ---------- | ------ | ---- | ---------------------- | ----------------- |
| prefCode   | string | ✅   | 5桁の数字、末尾000    | "01000"           |
| prefName   | string | ✅   | 2-4文字 + 都/道/府/県 | "北海道"          |
...
```json
{
  ...
  "kanto-chubu": {
    "name": "関東・中部地方",
    "prefectures": ["茨城県", "栃木県", "群馬県", "埼玉県", "千葉県", "東京都", "神奈川県", "新潟県", "富山県", "石川県", "福井県", "山梨県", "長野県", "岐阜県", "静岡県", "愛知県"]
  },
  "kinki": {
    "name": "近畿地方",
...
1. **prefCodeの一意性**: 47都道府県で一意
2. **prefNameの一意性**: 全国で一意
3. **コード範囲**: 01000-47000
...
1. **codeの一意性**: 全国で一意
2. **prefCodeの整合性**: 存在する都道府県コード
3. **parentCodeの整合性**: 存在する市区町村コード
...
3. **拡張性**: 将来的なAPI直接取得への布石
4. **階層情報**: level, parentCodeが完全保持
...
**パラメータ**:
- `areaCode`: 地域コード（2桁、5桁、または00000）

**戻り値**:
- 成功時: AreaHierarchyオブジェクト
- 失敗時: null
...
export class MockAreaRepository implements AreaRepository {
  ...
  async getMunicipalitiesByPrefecture(prefCode: string): Promise<Municipality[]> {
...
GeoShapeの行政区域データ（市区町村境界データ）を外部URLから取得した際に、自動的にCloudflare R2ストレージに保存し、次回以降のアクセスを高速化する自動キャッシング機能です。
...
- [x] `getAllPrefectures()` - 全47都道府県の取得
- [x] `getPrefectureByCode()` - コードによる取得（2桁・5桁）
...
describe('都道府県→市区町村連携', () => {
  it('都道府県の市区町村を正しく取得できる', () => {
    ...
    const municipalities = MunicipalityService.getMunicipalitiesByPrefecture("13");
