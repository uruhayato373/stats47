...
`src/features/estat-api/meta-info/services/batch-processor.ts`

### 使用方法

#### `EstatMetaInfoBatchProcessor.processBulk()`
...
- `statsDataIds` (string[]): 統計表 ID の配列
- `options.batchSize` (number, optional): バッチサイズ（デフォルト: 10）
- `options.delayMs` (number, optional): バッチ間の待機時間（デフォルト: 1000）
- `options.onProgress` (function, optional): 進捗コールバック

**戻り値**: `BatchProcessResult`

- `totalProcessed`: 処理した総数
- `successCount`: 成功した数
- `failureCount`: 失敗した数
- `results`: 各 ID の処理結果

#### `EstatMetaInfoBatchProcessor.processRange()`

統計表 ID の範囲を指定して一括処理します。
...
- `startId` (string): 開始 ID
- `endId` (string): 終了 ID
- `options`: `processBulk()`と同じ
