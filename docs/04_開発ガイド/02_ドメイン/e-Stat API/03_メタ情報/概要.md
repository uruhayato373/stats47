...
```
src/infrastructure/estat-api/meta-info/
├── index.ts                    # エントリーポイント
├── fetcher.ts                  # API取得処理
...
```typescript
import { EstatMetaInfoFetcher } from "@/infrastructure/estat-api/meta-info";
...
```typescript
import { EstatMetaInfoBatchProcessor } from "@/infrastructure/estat-api/meta-info";
...
```typescript
import { EstatMetaInfoFormatter } from "@/infrastructure/estat-api/meta-info";
...
```bash
npm test -- src/infrastructure/estat-api/meta-info/__tests__
```

## 関連ドキュメント

- [API 仕様](04_ドメイン設計/地域管理/specifications/api.md) - get-meta-info API の詳細
