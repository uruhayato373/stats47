...
- 親: `docs/02_実装計画/01_収益化マスタープラン.md` (収益化 SSOT)。本計画は SSOT の **P2「トラフィック成長」** のランキングページ側を具体化する従属計画。ゲート判定・WIP 制限・Agent 分業原則は SSOT に従う (本計画で独自に緩めない)。
...
> 現状: AI 解説文の再生成パイプラインは DB レス移行で削除済み・破損 (docs/todo/01_改善バックログ.md AICONTENT-001 blocked、`enhance-ranking-ai-content` SKILL.md が D1 参照で stale)。再設計が前提インフラとなる。
...
Wave 命名は `blog-data-schema.md` の wave 規約準拠 (`RANK-WAVE-YYYY-MM-DD`)。各 wave は docs/todo/01_改善バックログ.md に section 登録。

| Wave | 期間 | 対象 | 手法 | 監査 |
|---|---|---|---|---|
| R1 (パイロット) | W25-W26 | Tier A 15 件 | §2 仕様フル適用。Sonnet 執筆 → critic PASS。**2026-06-12 進捗: seoTitle 10 件 deploy 済、構造解釈文生成 W26 着手予定** | Fable 抜き打ち 5/15 |
...
下記 4 件は 2026-06-12 時点で `docs/todo/01_改善バックログ.md` に登録済。TODO 真実源は docs/todo/01_改善バックログ.md (status / Tier / 期日はそちらを参照)。
...
| 日付 | 変更 |
|---|---|
| 2026-06-12 | 初版。01_収益化マスタープラン.md P2 の従属計画として作成 |
| 2026-06-13 | ファイル名リンクを現行ファイル名に修正 (improvement-backlog→01_改善バックログ)。§8 に登録済ステータス注記追加。Wave R1 進捗 (seoTitle 10件 deploy 済) を反映 |
