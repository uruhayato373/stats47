...
> 現状: AI 解説文の再生成パイプラインは DB レス移行で削除済み・破損 (03_改善バックログ.md AICONTENT-001 blocked、`enhance-ranking-ai-content` SKILL.md が D1 参照で stale)。再設計が前提インフラとなる。
...
Wave 命名は `blog-data-schema.md` の wave 規約準拠 (`RANK-WAVE-YYYY-MM-DD`)。各 wave は 03_改善バックログ.md に section 登録。
...
下記 4 件は 2026-06-12 時点で `docs/02_実装計画/03_改善バックログ.md` に登録済。TODO 真実源は 03_改善バックログ.md (status / Tier / 期日はそちらを参照)。
...
| 2026-06-13 | ファイル名リンクを現行ファイル名に修正 (improvement-backlog→03_改善バックログ)。§8 に登録済ステータス注記追加。Wave R1 進捗 (seoTitle 10件 deploy 済) を反映 |
