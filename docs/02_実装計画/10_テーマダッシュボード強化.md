...
- **ルーティング**: 1 個の動的 `app/themes/[themeSlug]/page.tsx`（旧 17 静的ページを集約）。`local-finance` は **bespoke 専用ページ**（`LocalFinanceDashboard`＝財政 stats-card + 財政フロー Sankey。`loadFinanceCards`(bundled) + R2 公開URL fetch で動き、汎用 `loadThemeData`/`shouldSkipRemoteR2Read` 経路は経由しない）。`local-finance/cities` は汎用 `ThemePageLayout`（市区町村）。両ページとも左ナビ (`ThemeSidebar`) 付き。
  - 注: 2026-06-20 に一度 local-finance を汎用レイアウトへ統合したが、ユーザー要望で **bespoke の財政 stats-card に戻した**（汎用の dashboard-1 カードより財政専用カードが好ましいため）。
