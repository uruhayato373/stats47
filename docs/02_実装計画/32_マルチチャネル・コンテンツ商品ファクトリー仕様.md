...
本仕様は `docs/02_実装計画/30_ココナラ商品ファクトリー実装仕様.md` を置換しない。doc 30はOffice・データ商品の
生成基盤、本仕様は1つの商品原料をnote、KDP、Brain、ココナラ等へ変換し、ポートフォリオとして選別する上位仕様である。
...
- YouTubeは月1慎重再開、Instagram/Xは既存頻度ガードを維持する。商品販売のために投稿上限を緩めない。
- TikTok撤退は維持する。
- 有料note・KDP・Brain・外部販売は自動公開せず、人間承認を必須にする。
- ココナラB-01のOffice実機検証を飛ばして商品数を増やさない。
...
```text
無料認知
  X / Instagram / YouTube / 無料note / stats47
    ↓ 反応あり
入口商品
...
| 月次判断 | `docs/04_レビュー/YYYY-MM-content-products.md` |
| TODO | `docs/todo/02_機能バックログ.md` |
...
- doc 30、note catalog、product catalog、experiments、sales ledgerを棚卸し。
...
- [ ] doc 30、note catalog、SNS、実験、sales ledgerを再利用し、同義基盤を増やしていない。
...
BEHAVIOR CONTRACT:
- docs/02_実装計画/32_マルチチャネル・コンテンツ商品ファクトリー仕様.md のPhase 0だけを実施する。
...
- docs/02_実装計画/30_ココナラ商品ファクトリー実装仕様.md
...
6. docs/04_レビュー/YYYY-MM-DD-content-product-phase0-audit.md に監査結果を保存する。
