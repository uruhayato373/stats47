...
├── index.ts                    # エントリーポイント
├── EstatStatsDataService.ts    # メインサービス
...
EstatStatsDataService.getAndFormatStatsData()
    │
    ├─► getStatsDataRaw() → 生APIレスポンス
    └─► formatStatsData() → 整形済みデータ
            │
            ├─► formatAreas() → 地域情報
...
const prefectureData = await EstatStatsDataService.getAndFormatStatsData(
  "0000010101",
  {
    categoryFilter: "A1101",
    yearFilter: "2020",
    areaFilter: "prefecture", // 都道府県のみ
  }
);

// 特定年度のデータを取得
const yearData = await EstatStatsDataService.getAndFormatStatsData(
  "0000010101",
  {
    categoryFilter: "A1101",
    yearFilter: "2020",
