...
e-Stat APIドメインのアーキテクチャ設計について説明します。レイヤー構造、データフロー、サービスクラス設計、依存関係、設計パターンについて詳述します。
...
class EstatStatsDataService {
  ...
  private static formatCategories(categories: RawCategory[]): FormattedCategory[]
...
class EstatMetaInfoService {
  ...
  async getSavedMetadataByStatsId(statsDataId: string): Promise<SavedMetadata[]>
...
class ConfigManager {
  static getConfig(): ServiceConfig {
    return {
      apiBaseUrl: process.env.ESTAT_API_BASE_URL || 'https://api.e-stat.go.jp/rest/3.0/app/json',
