...
**Auth.jsのauthorize関数が期待** (`src/lib/auth/auth.ts:37`)
```typescript
...
**問題のコード** (`src/lib/auth/jwt.ts:34-38`)
```typescript
...
**NextAuth設定** (`src/lib/auth/auth.ts:98-101`)
```typescript
session: {
  strategy: "jwt",
...
**独自JWT実装** (`src/lib/auth/jwt.ts:63-64`)
```typescript
// 7日間の有効期限
...
```typescript
import { auth } from "@/lib/auth/auth";  // NextAuthのauth関数
...
- Remove src/lib/auth/jwt.ts
- Remove src/lib/auth/api-auth.ts
...
`src/lib/auth/auth.ts`にセキュリティ設定を追加：
