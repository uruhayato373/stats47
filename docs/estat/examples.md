...
このドキュメントでは、e-Statライブラリの実践的な使用例を紹介します。
...
async function fetchFilteredData() {
  ...
  data.values.forEach(value => {
    console.log(`${value.areaName} (${value.timeName}): ${value.value?.toLocaleString()}${value.unit}`);
  });
...
async function fetchPrefectureData() {
  ...
  sorted.slice(0, 10).forEach((data, index) => {
    console.log(
      `${index + 1}. ${data.areaName}: ${data.value?.toLocaleString()}${data.unit}`
    );
  });
...
async function saveMetaInfo(db: D1Database, statsDataId: string) {
  ...
  if (result.success) {
    console.log(`✅ 成功: ${result.entriesProcessed}件のメタ情報を保存しました`);
  } else {
...
async function createRanking() {
  ...
  ranked.forEach(item => {
    console.log(
      `${item.rank}位: ${item.areaName} - ${item.value?.toLocaleString()}${item.unit}`
    );
  });
...
### 例20: API Routeでの使用例（Next.js App Router）
...
このドキュメントでは、e-Statライブラリの様々な使用例を紹介しました。実際のアプリケーション開発では、これらの例を組み合わせて使用することで、効率的にe-Statデータを活用できます。
