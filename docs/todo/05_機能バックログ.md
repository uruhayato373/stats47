...
## KINDLE-AUDIT-S1-01 Kindle S1 の数値監査で truth 集合を広げる

- **status**: pending
- **次**: `audit-book-facts.mts` の `collectMetricKeys` が S1 で集められる指標を増やす
- **完了条件**: S1 12 冊の truth 指標数が S2/S3 並み (28 件以上) になり、残る不一致が
  派生値だけになること

S1 (論点読み物) は章がブログ記事なので、指標を記事本文の `/ranking/` リンクからしか
集められない。実測で **5〜24 件**しか読み込めず (S2/S3 は 28〜30 件)、本文が語る数値の
多くが truth に無い。県だけは他の指標で見つかるため `unknown-pref` にならず「不一致」として
出るので、**S1 の監査結果は現状そのままでは使えない** (2026-08-12 に 8 冊 39 件を実測)。

案: ブログ記事の `data/*.source.json` が持つ rankingKey を読む / 本文に現れる指標名で
METRICS_REGISTRY を引く。どちらも実装前に誤検出が増えないか実測すること。
