...
- **残り**:
  1. **note-operator 自動化を新設** (coconala-operator 相当・Playwright)。マガジン作成 + 記事割当を note.com へ反映する。**note ログインは人手** (初回・所有者アカウント)、実反映は draft-first + 承認境界。まず 1 マガジン (件数最多 = `s47-sports-culture`) で実証してから横展開。
     ...
     - **残 (Track B・記事割当 = add-article)**: 作成したマガジンは**まだ空** (0本)。74件等の記事をマガジンへ束ねる自動化が未完。**判明した実態**: (1) creator の追加フローは記事の **📖 アイコン** or マガジンページ「自分の記事から探す」→ `/stats47/all` 記事一覧の per-article 追加。記事ページの「…」メニューは創作者管理 (編集/複製/削除) で追加項目なし。(2) API は `POST https://note.com/api/v1/our/magazines/{key}/notes` が**存在** (422 = ペイロード形式違い)。試した body 全て 422、CSRF は meta/cookie に無く機構が特殊。**次手**: 実 UI の 📖 追加を network 監視して正確な API request (URL+body+headers) を捕捉 → bulk add CLI を実装 (対象は catalog の magazine 割当 = intent SSOT。koumuin-estat には 13件追加も含む)。当面は note.com UI で人手追加も可 (📖 アイコンから)。
