...
- **★未検証 A (この施策の中核が未実証)**: 「1 task 失敗時でも成功分が push される」ことは
  **次に失敗する run が出るまで実測できない**。さらに `sync-snapshots.yml` は
  **main を checkout する** (`ref: main`) ので、`run.sh` の修正が実際に効くのは
  **develop → main のデプロイ後**。それまでは失敗すると従来どおり全部捨てられる。
  → デプロイ後、意図的に失敗させずに済む機会 (次の実失敗 run) で `❌ 失敗した task:` の直前に
  `════ R2 push ════` が出ていることをログで確認する。
- **★未検証 B (別件として切り出す)**: 同じ run で **9 件中 2 件だけ反映されなかった**
  (`port-entry-vessel-count` / `recreational-fishing-count`)。`app/stats` は 07:35 に正しく
  更新済み・`config.years` も一致していたのに、`ranking-values` がその 2 件だけ「観測値なし」と
  判定して skip した。**CI の reader が公開 URL (`R2_PUBLIC_FETCH_URL`) 経由で古い object を
  読んだ疑いが濃い** — `storage.stats47.jp` は Cloudflare 側の Cache Rule で最大 4 時間保持され
  (`ogp-image-standards.md` §5.0)、edge キャッシュは PoP 単位なので「7 件は新しく 2 件は古い」は
  この仮説と整合する。**確証は無い** (当時のログを取れていない)。
  2 件は手動で再生成・push して是正済み (08:28)。
  → **同型の危険**: 「R2 に書いた直後に、同じ object を公開 URL 経由で読む」CI 手順は
  すべてこの罠を踏みうる。`run.sh` は `calculated-stats` の直後に中間 push を挟む理由を
  書いているが、**キャッシュの観点は書かれていない**。次にやることは (a) 書込→読取が
  連続する箇所の洗い出し (b) reader を S3 API 経由にするか、書込側で該当 key を purge するか
  の判断。**推測で対策を打たず、まず再現条件を確定させる**。
- **次**: 上記 A (デプロイ後の実失敗 run で確認) と B (再現条件の確定) を片付ける。あわせて
  「生成は成功したが後段の検証で落ちた」を「生成が途中で死んだ」と区別できるようにするか検討する
  (現状は task 単位でしか区別できない)。
- **禁止**: 検証ゲート自体を緩めない。捨てられていたのは push であって、9 件の欠測を
  見逃してよいという話ではない。B について、原因未確定のまま reader を書き換えない。
- **完了条件**: ~~`app/ranking/<key>/values.json` の `generatedAt` が当日になり~~ (達成)、
  1 task 失敗時でも成功分が push されることを次回の失敗 run で確認できている (A)、
  B の再現条件が確定して対策の要否が決まっている。
