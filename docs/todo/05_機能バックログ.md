...
- **問題**: 旧 `register-ksj-rankings.ts` の `findNearestPref` が施設座標から**最寄りの県庁所在地**で
  ...
  配信値と完全一致することを確認済み。
- **済 (原発・地熱の 2 指標は本番反映まで完了)**: 帰属の共有モジュール
  (`packages/gis/src/mlit-ksj/prefecture-assign.ts`、属性 → 空間結合、決められなければ null で
  推測しない) + `generate-ksj-stats-values.ts`。`findNearestPref` を含む旧 script は削除。
  あわせて `unit: "か所"` に対して号機を数えていた問題も是正した (施設名 + 住所で重複排除。
  原発 68 レコード → 21 施設 / 地熱 19 → 17)。R2 は app/stats → app/ranking (values / item /
  正規化 / national-trend) まで反映済みで、誤りを引き継いでいた ai-content と相関 snapshot は
  削除して再生成待ちにした (退避 = `.local/{ai-content,correlation}-removed/`)。
- **次**: 残り 11 指標 (火力・水力・太陽光・風力・バイオマス・ダム・湖沼・空港・鉄道駅・道の駅・
  観光資源・漁港・高速道路IC) を同じ経路で再生成する。属性を持つデータセットは
  `registry.ts` の `prefectureSource` に宣言する (P35=prefName / P12=prefCode / C09=muniCode を確認済み)。
  属性を持たない C28・N02・N06 は空間結合で解決する。
...
- **禁止**: 全プロパティを走査して「それらしい値」を県コードに使わない。P12 観光資源の `P12_001` は
  資源 ID だが 5 桁で、市区町村コードとして読むと別の県に化ける。
