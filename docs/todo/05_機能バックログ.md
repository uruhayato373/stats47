...
- **次**: R2 反映 (`data-refresh.yml` → `sync-snapshots.yml`) を先に済ませ、allowlist の
  該当エントリと `MAX_KNOWN_BROKEN` を下げる。そのうえで下記の残り 7 件に着手する。
- **残り (7 件)**:
  - 港湾 5 件 (`port-inbound-ships` / `port-ships-tonnage` / `port-cargo-{total,import,export}`):
    航路・貨物種別に総数コードが無くメンバーの合算が要る。cat 軸の結合機構が未実装
    (`tabCombination` を `axisCombination: { axis, parts }` へ一般化する)
  - `convenience-store-sales-yoy`: title「販売額」/ subtitle「対前年比」/ unit「百万円」が
    矛盾。どれが正かは推測になるため人の判断待ち
  - `maternal-child-health-guidance`: 提案 pin に e-Stat が STATUS=1「該当データなし」を返した。
    座標の再調査が要る
- **別枠**: 率系 13 件は分子/分母の `calculation` 設計が要る (`needs-calculation`)。
  `calculated` source は page-data-batch が実行しない設計なので、実行機構から要検討
