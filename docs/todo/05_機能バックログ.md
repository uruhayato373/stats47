...
- **残り**:
  1. **note-operator 自動化を新設** (coconala-operator 相当・Playwright)。マガジン作成 + 記事割当を note.com へ反映する。**note ログインは人手** (初回・所有者アカウント)、実反映は draft-first + 承認境界。まず 1 マガジン (件数最多 = `s47-sports-culture`) で実証してから横展開。
     ...
     - 次 (Track B): note-operator の session/form lib + `note-magazine.mjs` CLI を実装。**dry-run/plan を既定**にし (catalog から「作成すべきマガジン18・追加すべき記事」を算出)、`--commit` + オーナー承認で初回 pilot (`s47-sports-culture` 74件) を作成。実書き込み前に必ず停止して確認。
