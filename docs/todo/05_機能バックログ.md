...
- **実行順**:
  ...
  9. `.claude/rules/metric-config-standards.md`のunit規約に「金額族はe-Stat `@unit`と10^k整合、
     換算は`valueScale`で宣言」を追記。`data-ingester` agentと`/verify-value-distribution` skillも小改訂。
...
### [INCOME-PERIOD-BASIS-01] 月額と年額を引き算している計算型 metric の是正

- **tier**: 1
- **status**: pending
- **created**: 2026-08-05
- **owner**: Claude Code
- **根拠**: `disposable-income-after-rent`(家賃控除後可処分所得)は**月額から年額を引いている**。
  金額の単位はどちらも「円」なので `MONEY-UNIT-SCALE-01` の 10^k スケール検査では原理的に拾えない
  (ずれの軸が**期間**であって倍率ではない)。e-Stat のメタも期間を明示しないため、**現状どの機械検査でも
  検出できない**。控除額が約 12 倍過大になる。
- **期間基準の実測 (2026-08-05・どちらも桁で確定した)**:
  - 被減数 `disposable-income-worker-households` = **月額**。SSDS `0000010112`×`L3130` の 2024 全国は
    **522,569 円**で、家計調査の勤労者世帯・月額と一致する(年額なら約 627 万円になる)。
  - 減数 `private-rent-consumption-expenditure` = **年額**。同一表(家計調査 家計収支編 二人以上の世帯
    `0003348239`)の消費支出 `001100000` の 2024 全国が **3,602,915 円**で、この表が年間額であることが
    確定する(月額なら約 30 万円)。config の subtitle「年間民営家賃消費支出額」とも一致。
  - e-Stat 側の手掛かりは無い: `@unit` は両方「円」、`TABLE_INF` にも月/年の記載なし。
- **影響 (2024 年・47 県全件で実測)**: 控除額が 12 倍過大(東京 175,694 → 正しくは 14,641)。
  **順位は 46/47 県が動き、最大 26 位**。1 位が山形(545,206)から**東京(623,317)**へ入れ替わる。
  公開済みの影響先は `/ranking/disposable-income-after-rent` の全 47 県の値・順位・seoTitle と、
  ブログ `/blog/real-disposable-income-reversal`(タイトルが「実質手取り1位は山形54.5万円」= この誤値)。
- **今日の変更との関係**: `61aa3a773` で years を 2024 単年 → **2007-2024** に広げた(空チャート是正の一環)。
  欠陥は以前から公開済みだが、**data-refresh 完走後は誤った値が 18 年分になる**。
  年拡充自体は正しい(分子は e-Stat に 1975-2025 が実在する)ので、戻すのではなく期間基準を直す。
- **実行順**:
  1. 表現方法を決める。案 A = 減数を月額の家賃指標に差し替える、案 B = `calculation` に期間換算
     (÷12)を宣言できるようにする。現行の `calculation.type: "subtraction"` は換算を持てないので、
     **どちらを採るかを決めるまで config を書き換えない**。
  2. 是正 → `data-refresh` の push トリガーで再取り込み(`MONEY-UNIT-SCALE-01` 実行順 7 と同じ経路)。
  3. `seoTitle`/`seoDescription` を再取得後データから機械計算して書き直す(現行の「1位山形県 545,206円」は誤り)。
  4. ブログ `/blog/real-disposable-income-reversal` を是正する。タイトル・本文の数値・SVG・`source.json` が
     すべて誤値に依存しているので、`/brushup-blog` で記事単位に直す。**prerender 済みなので再デプロイが要る**。
  5. 再発防止: 計算型の operand の期間基準が一致するかを検査するゲート。`money-unit.ts` は 10^k 専用なので
     別モジュールにする。期間を config に宣言させる(例: `periodBasis: "monthly" | "annual"`)のが前提になる。
  6. 他の計算型(`subtraction`/`ratio`)に同じ期間混在が無いか全数棚卸しする。
- **他の計算型の状況**: `real-disposable-income`(可処分所得 ÷ 消費者物価地域差指数 × 100)は**問題なし**。
  指数は無次元なので期間が混ざらない(637,958 ÷ 104.0 × 100 = 613,421 と R2 実測が一致)。
- **停止条件**: 手順 1 の決定前に config を触らない。触ると再取り込みで誤った値が 18 年分に広がる。
- **完了条件**: R2 実測で東京 2024 が約 623,300 円(= 月額 − 年額/12)、item.json の seoTitle がデータと整合、
  ブログ記事の数値と SVG が是正済みで再デプロイ済み、期間基準ゲートが両方向テスト付きで配線済み、
  棚卸しで期間混在 0 件。
- **スコープ外**: `MONEY-UNIT-SCALE-01`(金額スケールの 10^k 軸。本件とは別の軸で、対象 metric も重ならない)。
  `CALCULATED-METRIC-PIPELINE-01`(計算結果を書く工程が無い件。**是正は同時に設計する必要がある** —
  期間換算を入れる場所がその計算工程そのものだから)。
- **正典**: `.claude/rules/metric-config-standards.md` / `.claude/rules/evidence-based-judgment.md`

### [CALCULATED-METRIC-PIPELINE-01] 計算型 metric の app/stats を再生成する工程が存在しない

- **tier**: 1
- **status**: pending
- **created**: 2026-08-05
- **owner**: Claude Code
- **根拠**: `fetcherKey:"calculated"` の 3 metric について、**分子・分母から計算結果を作って
  `app/stats/<key>/values.json` に書く工程をどのパイプラインも持っていない**。現在配信されている値は
  単発実行の産物で、分子・分母を更新しても追従しない。
- **コードで確認した経路 (2026-08-05)**:
  - `packages/data-configs/scripts/page-data-batch.ts:932` — `source.kind === "external"` は
    `skip`("fetcher not implemented yet")。該当 3 件は `kind:"external"` + `fetcherKey:"calculated"` なので
    **e-Stat 取得の対象外**。`app/stats` は書かれない。
  - `packages/ranking/src/scripts/generate-ranking-values.ts:240` — `app/stats` を読んで
    `app/ranking/<key>/values.json` へ**射影するだけ**で計算しない。除外条件が
    `source.kind === "calculated"` なので **`external` の 3 件は素通りし、stale な入力をそのまま配信側へコピー**する。
  - `calculateRankingValues`(per_capita/ratio/subtraction)は実装済みだが、呼び出し元は
    ランタイムの on-demand 取得と read-only 検証スクリプト(`verify-calculated-rankings.ts`)だけで、
    **R2 へ書く経路が無い**。
- **実測 (R2・2026-08-05)**: 更新が単発である証拠として、同じ扱いの 3 件で状態が揃っていない。
  - `disposable-income-after-rent` 1 年 (last-modified 5/28)
  - `real-disposable-income` 1 年 (5/28)
  - `engel-coefficient` 18 年 (7/09)
  - 一方で分子・分母は揃っている: 可処分所得 50 年(1975-2024)・民営家賃 18 年(2007-2024)・
    消費者物価地域差指数 12 年(2013-2024)。**計算すれば作れるのに作られていない**。
- **影響**: (a) テーマページの推移チャートが 1 点のまま(切替チャートは計算型に対し `readStatsValues` =
  `app/stats` を読む)。(b) `/ranking/<key>` の年セレクタが 1 年しか出ない。(c) config の `years` を
  広げても配信に反映されないため、**設定と配信が恒久的に乖離する**。
- **PR #733 の説明の訂正**: 同 PR は「計算型 2 件のチャートが空になる根因は分子が 1 年だったこと」と
  書いたが不正確。分子が 1 年だったのは事実で年拡充も正しい前提整備だが、**それだけでは直らない**。
  真の根因は本項目。
- **実行順**:
  1. 書く場所を決める。案 A = `page-data-batch` に `fetcherKey:"calculated"` の分岐を足し、依存 metric の
     `app/stats` を読んで計算 → `app/stats` へ書く(依存順の解決が要る)。案 B = `sync-snapshots` の
     TASKS に計算型専用 task を足す(依存の `app/stats` が確定した後に走らせられる)。
     **案 B のほうが依存順が自然**(page-data-batch 完了後に走る)。
  2. `INCOME-PERIOD-BASIS-01` の期間換算をこの計算工程に載せる。**別々に実装しない**。
  3. 3 件を再生成して R2 反映。年数が分子・分母の交差と一致することを実測する。
  4. ゲート: `verify-calculated-rankings.ts` を read-only 検証のまま週次に配線するか、
     「計算型の `app/stats` の年数が依存の交差と一致するか」を `audit-ranking-data-integrity.ts` の
     検査として足す。**現状はどちらの監査でも stale を検出できない**。
- **停止条件**: 手順 1 の決定前に config の `years` をこれ以上広げない(配信に反映されないまま
  設定だけが乖離する)。
- **完了条件**: 3 件の `app/stats` が分子・分母の交差年数と一致、テーマページの推移チャートが線を引く、
  stale を検出する監査が両方向テスト付きで配線済み。
- **正典**: `docs/01_技術設計/02_データアーキテクチャ.md`(Derived はエフェメラル計算 → R2) /
