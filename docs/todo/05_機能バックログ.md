...
- **残り**:
  1. **note-operator 自動化を新設** (coconala-operator 相当・Playwright)。マガジン作成 + 記事割当を note.com へ反映する。**note ログインは人手** (初回・所有者アカウント)、実反映は draft-first + 承認境界。まず 1 マガジン (件数最多 = `s47-sports-culture`) で実証してから横展開。
     ...
     - 次: (a) 既存3マガジンを catalog へ**照合取り込み** (noteUrl + 有料状態を magazines.ts へ。D3配色は誤vertical判定した第2-6章の正体 = 実在有料マガジン)。(b) s47-* カテゴリマガジンを note.com へ**新規作成** (session/form lib + publish CLI・draft-first + `--commit` gate)。
  2. **誤 vertical 16 件の再評価**: D3配色の章5件は**実在有料マガジン (¥500) の中身**で junk ではない → D3配色マガジンへ帰属 + vertical 訂正。Claude Code 記事は koumuin 有料マガジンへ帰属。note.com 上の別URL重複投稿3件 (災害SNS/苦情/FAQ) は削除判断 = オーナー領域。
