...
- **次**: `diagnose-unpinned-axes.ts --fetch` の wave 分類に沿って 20 件程度ずつ config を是正し、
  各 wave の締めで `data-refresh.yml` を `--metric` カンマ列挙で dispatch して R2 反映、
  同じ PR で該当 allowlist エントリと `MAX_KNOWN_BROKEN` を下げる。消化順は流入の多い順
  (ユーザー決定: 194 件すべて・公開したまま是正する。noindex にしない)。
