...
- **影響範囲の確認が先**: ranking ページの表示値・順位、正規化値 (values-per-*)、national-trend、
  ...
  合わせて書き換えると劣化する)。
- **原因** (2026-07-30 に e-Stat getMetaInfo と getStatsData で確定): 配信変換ではなく**取り込み側**。
  metric config が統計表の分類軸を絞りきれておらず、複数系列が同じ metric に投入されていた。
  例: `sports-participation-rate-swimming` は `cat03`(スポーツの種類 24件) 未指定で 24 種目すべてを取得し、
  rank が 1〜1128 の通し番号になっていた。`page-data-batch.ts` は cdCat01/02/03 しか API に渡さず、
  4 軸目・表章項目(tab)・時間粒度を絞る手段がなかった。
- **修正済み (8件)**: `cdCat04` / `cdTab` / `timeScope:"annual"` を型と `page-data-batch.ts` に追加し、
  8 件の config に軸を指定した。e-Stat 実データで全件 47県×1行・重複0 を確認済み
  (swimming=東京8.6% / retail-establishments=東京87,895事業所 など seoTitle の数値と一致)。
  対象: swimming(cat03=15) / hiking(cat03=17) / cinema(cat03=04) /
  ...
  retail-establishments(cdTab=701-2021)。
- **次**: 残り 7 件はいずれも**単一セルでは取れず計算が必要**。`calculation` の設計から行う。
  - 年収系 2 件 (school-teacher / university-professor): tab が 16 系列あり、年収 =
    きまって支給する現金給与額(tab=08)×12 + 年間賞与(tab=12)。現在は単一 tab の生値が入っている
  - 率系 4 件 (telework-rate / non-regular-employment-rate / wooden-housing-ratio / vacant-housing-rate):
    分子と分母の 2 セルが要る。`vacant-housing-rate` は unit が「％」なのに値が空き家戸数 (東京 896,500)
  - `dairy-cattle-count`: 2018年の県数が 39 しかなく北海道が欠落する。e-Stat 側の表構造を要調査
- **反映**: config 修正だけでは R2 に届かない。`page-data-batch --metric <key>` の再実行 (R2 書込のため CI)
  → `generate-ranking-values` → `generate-ranking-items` の順で再生成が要る。
