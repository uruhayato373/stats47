...
- **created**: 2026-07-30
- **owner**: claude
- **次**: refresh tokenをscope `https://www.googleapis.com/auth/adsense`（read-onlyでない方）で再発行し、GitHub Secretsの`GOOGLE_ADSENSE_REFRESH_TOKEN`を更新する。これが揃うまで作成・改名の実行はできない（inventory取得と突き合わせ検査は現行scopeで動く）。
