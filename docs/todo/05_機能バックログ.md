...
### [CODEQL-JS-BACKLOG-01] CodeQL JS/TS の既存 alert 10 件

- **tier**: 3
- **status**: pending
- **created**: 2026-07-30
- **owner**: Claude Code
- **背景**: 2026-07-30 に `security-scan.yml` へ SARIF ダンプを配線して初めて中身が見えた (それまでは件数だけが見え、Security タブと code-scanning API はどちらも一部セッションから 403 で読めなかった)。ブランチ全体で 12 件あり、うち 2 件 (`js/incomplete-html-attribute-sanitization` × 2) は同日に是正済み。残り 10 件が本項目。**CodeQL は required check ではないので merge はブロックしない** (`mergeable_state: unstable`)。
- **一覧** (severity は実測。`node .github/scripts/dump-codeql-sarif.mjs sarif-results` が Security Scan job の summary に出す)

| severity | rule | file:line | 実際の危険度の評価 |
|---|---|---|---|
| 9.8 | `js/shell-command-injection-from-environment` | `apps/gallery/lib/server/jobs.ts:68` | **低い**。gallery は localhost 専用 (127.0.0.1 固定)。taint 源は `process.env.STATS47_PROJECT_ROOT` で、これを設定できる者はすでにローカルでコードを実行できる。`regenerate()` は `kind` をホワイトリスト・`keys` を `/^[a-z0-9,_-]+$/` で検証済み |
| high | `js/path-injection` × 4 | `packages/gis/src/geoshape/services/geoshape-service.ts:54,55,81,82` | 未評価 |
| medium | `js/request-forgery` × 4 | `packages/gis/src/geoshape/adapters/{fetch-topology-from-r2.ts:59,geoshape-api-client.ts:22}` / `packages/gis/src/mlit/adapters/fetch-mlit-from-r2.ts:56` / `packages/r2-storage/src/lib/operations/fetch.ts:61` | 未評価。R2 key を URL に組む経路なので key の allowlist 化で塞げる見込み |
| warning | `js/identity-replacement` | `apps/gallery/lib/server/dashboard.ts:53` | セキュリティではなく単なる無意味な `replace`。`'-'` を自分自身に置換している |

- **次**: `jobs.ts:68` から着手する。`sh -c "A && test -f plan && B"` を**argv の逐次実行**に置き換える (`startJobSteps(kind, steps: {cmd,args}[])` を jobs.ts に足し、`test -f plan` は step 間の `fs.existsSync` に移す)。シェルを完全に排除するのでこの rule 類型ごと消える
- **完了条件**: SARIF ダンプの表で該当行が消える。gallery の regenerate が実際に 1 回成功する (画像生成が走るので `--max-generate` を小さくして確認)
- **制約**: gallery は動いている開発ツール。E2E 検証には実際の画像生成が走るため、step runner は単体テストで先に検証してから配線する。`js/identity-replacement` は挙動を変えない (無意味な replace を削るだけ)
