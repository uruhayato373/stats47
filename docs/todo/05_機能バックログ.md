...
- **問題**: 旧 `register-ksj-rankings.ts` の `findNearestPref` が施設座標から**最寄りの県庁所在地**で
  ...
  配信値と完全一致することを確認済み。
- **済**: 帰属の共有モジュール (`packages/gis/src/mlit-ksj/prefecture-assign.ts`、属性 → 空間結合、
  決められなければ null で推測しない) + テスト 29 件。`findNearestPref` を含む旧 script は削除。
  配信の正典を作る `generate-ksj-stats-values.ts` を新設し、**原発・地熱の 2 指標**を再生成済み
  (属性で 68/68・19/19 解決、未解決 0)。
...
- **別件として残る問題**: KSJ P03 のレコードは**号機単位**で、`unit: "か所"` と一致しない
  (原発 68 レコード = 20 施設)。件数の基準を号機とするか施設とするかはオーナー判断。
