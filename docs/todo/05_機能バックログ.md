...
- **次**: (1) Cloudflare 側の Cache Rule の実 TTL を確認する (ダッシュボード = オーナー領域)。
  (2) snapshot 同期後にゾーン側も purge するか判断する。`purge-cdn.yml` は prefix 空で
  `purge_everything` になりゾーン全体を落とすので、影響範囲の判断が要る
  (`purge-cache.ts --files` は `storage.stats47.jp/<key>` しか組み立てず `stats47.jp` の HTML を狙えない)。
  (3) 恒久策を決めたら `sync-snapshots` に配線する。
- **完了条件**: snapshot 同期の直後に本番 `<title>` を Googlebot UA で実測して新値になっている
...
- **残り 2 件**: `packages/r2-storage/src/lib/operations/fetch.ts:61` は同ファイル L21 の
  ...
  `apps/gallery/lib/server/jobs.ts:68` は `sh -c "A && test -f plan && B"` の argv 化
