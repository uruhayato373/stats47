...
### [SYNC-SNAPSHOTS-ALLORNOTHING-01] 1 task の失敗で全 snapshot の push が捨てられていた

- **tier**: 1
- **status**: in-progress
- **created**: 2026-08-17
- **owner**: claude
- **問題**: `sync-snapshots/run.sh` は末尾の `diff-push-r2` の**前**で `exit 1` していたため、
  **task が 1 つでも失敗すると成功した task の成果物ごと捨てられていた**。生成物は runner の
  `.local/r2` にあり runner は破棄されるので復旧手段も無い。
- **実害 (2026-08-17 実測)**: `app/ranking/<key>/values.json` が **2026-08-11 から 6 日間
  site-wide で凍結**していた。`ranking-values` は 2,244 件を書き切った**後**の検証
  (「観測値が無い未登録キー 9 件」) で exit 1 しており、生成は全件成功しているのに 1 バイトも
  push されていなかった。`total-population` など無関係な metric まで巻き添え。
- **原因 (9 metric)**: R2 の観測値の年が `config.years` と食い違い、`yearInSpec` で全行が
  落ちて「観測値なし」になっていた。①漁業・港湾系 8 件は e-Stat 表 (2003年漁業センサス) に
  **time 軸が無く**、`fillMissingTimeFromSurveyDate` が入る前の取り込みが年を 2018 と誤って
  刻んでいた ②`convenience-store-count-commercial` は 2026-07-30 の軸是正後に再取り込みが
  走っておらず、94 行 (47県×2系列)・2025年のまま残っていた。**再取り込みで 9 件すべて解消**
  (2003年39県 ×8 / 2022-2024年141行 ×1)。再取り込み後の dry-run は
  `written=2253 / 未登録の空 0` で通る。
- **完了済**: ①9 metric を再取り込みして `app/stats` へ push ②`run.sh` の push を失敗判定より
  前に移した (通過分は反映し run は赤のまま。方針は `ranking-content-standards.md` §2026-08-07
  「オールオアナッシングにしない」と同じ) ③全 task を再実行し `app/ranking` が
  **2026-08-17T07:50 生成**に更新されたことを実測。9 metric も 2003年39件 / 2022-2024年 へ是正。
- **★同型の第 2 の穴 = timeout による push の途中打ち切り (2026-08-17 に run 32006827498 で確定)**:
  `sync-snapshots.yml` の sync job は `timeout-minutes: 45` だったが、フル run の実測は
  **生成 30 分 + 末尾 push 21 分 = 約 52 分**で、**構造的に完走できなかった**。
  実ログでは push が `Progress: 9,416 / 14,033 (errors: 0)` の時点で `The operation was canceled.`
  となり、**書けた snapshot の 1/3 が届かないまま runner ごと破棄**された。
  検証・purge の後続 step と `sync-ranking-keys` job も丸ごと skip される。
  → **run.sh の「失敗しても push する」修正ではこの打ち切られ方は救えない** (push 自体が殺される)。
  `timeout-minutes: 120` へ引き上げた。**「差分 push」という名前だが CI ではフル push である**点に注意
  — manifest (`.local/r2-manifest/`) は runner ローカルなので毎回空 (`マニフェスト記録済み: 0`)、
  アップロード対象は常に全件になる。
  → **未着手の改善候補**: manifest を `actions/cache` で持ち越すか remote と突合すれば
  push は数十件で済み、所要時間もリスクも大きく下がる (今回は最小修正の timeout 引き上げに留めた)。
- **★未検証 A → 検証済み (2026-08-17)**: 「1 task 失敗時でも成功分が push される」ことを
  **実失敗 run を待たずに機械検証できるようにした**。`.claude/scripts/lib/__tests__/sync-snapshots-run-contract.test.mjs`
  が PATH 先頭に fake `npx` を置いて `run.sh` の制御フローだけを走らせ、
  ①1 task 失敗時に `diff-push-r2` が**失敗 task より後に**呼ばれ exit 1 すること
  ②全成功時に push まで到達すること ③`--dry-run` では push を呼ばないこと
  ④sync job の timeout が実測所要時間を上回ること、を固定する。
  **検査自体の感度も実測済**: 旧ロジック (push より前で `exit 1`) を注入した `run.sh` では
  push が呼ばれないことを assert する mutation テストを同ファイルに入れてあり、
  timeout も 45 に戻すと落ちることを手元で確認した (5 件中 1 件 fail)。
  CI 配線は `npm run test:workflow-commit-back` (PR gate 🔁 Workflow Commit-back Contract Gate)。
- **★未検証 B → 原因確定・CDN キャッシュ仮説は棄却 (2026-08-17)**: 同じ run のログを読んで解決した。
  `ranking-values` は 07:50 に `[values] written=2253 skipped(no values)=2 failed=0` を出しており、
  **skip されたのは `prefectural-income-per-capita` / `station-passengers-annual-total` の既知 2 件だけ**で、
  `port-entry-vessel-count` / `recreational-fishing-count` は**正常に書かれていた**。
  届かなかった理由は上記の timeout で、**末尾 push が 9,416/14,033 で殺され、この 2 件が
  未 upload の残り 4,617 件に含まれていた**だけ。当初立てた「reader が公開 URL 経由で古い
  `app/stats` を読んで skip した」という Cloudflare キャッシュ仮説は**この run については棄却**する
  (skip 件数と skip されたキー名が仮説と合わない)。2 件は手動で再生成・push して是正済み (08:28)。
  → **ただし一般論としての危険は残す**: 「R2 に書いた直後に、同じ object を公開 URL
  (`R2_PUBLIC_FETCH_URL`) 経由で読む」CI 手順は、Cloudflare の Cache Rule (最大 4 時間・PoP 単位、
  `ogp-image-standards.md` §5.0) を踏みうる。`run.sh` が `calculated-stats` の直後に中間 push を
  挟む理由は書かれているが**キャッシュの観点は書かれていない**。実害を観測したらここに追記する。
- **次**: (1) develop → main をデプロイして CI の `run.sh` を新ロジックにする
  (sync job は `ref: main` を checkout するため) (2) フル sync を 1 回走らせ
  `Progress: 14,033 / 14,033` まで到達して conclusion=success になることを実測する
  (3) manifest 持ち越しによる push 削減を検討する。
- **禁止**: 検証ゲート自体を緩めない。捨てられていたのは push であって、9 件の欠測を
  見逃してよいという話ではない。timeout を再び 52 分未満へ下げない (契約テストが止める)。
  上記の一般論のキャッシュ懸念について、**再現を観測しないまま reader を書き換えない**。
- **完了条件**: ~~`app/ranking/<key>/values.json` の `generatedAt` が当日になり~~ (達成)、
  ~~1 task 失敗時でも成功分が push される (A)~~ (契約テストで固定・達成)、
  ~~B の再現条件が確定して対策の要否が決まっている~~ (原因確定・対策は timeout 引き上げ・達成)、
  デプロイ後のフル run が push 完走して緑になる。
