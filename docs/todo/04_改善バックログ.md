...
| PERF-RANKING-PAYLOAD-01 | sidebar側はデプロイ済 (HTML 1,287,062→1,232,628 bytes = 4%減)。**残: TopoJSON 1.9MBを品質gate付き150KB以下のR2候補へ削減**して50%目標へ届かせる | in-progress | 2026-08-19 | claude | performance |
| PERF-AREA-DOM-01 | デプロイ済 2026-08-05。rail上限12/navを本番で確認。**残: DOM総数の裏取り** — PSI batchは`dom-size`を保存せず、この環境からPSI API・Chrome DevTools・ブラウザに到達できないため9,101比の70%削減が未検証 | effect/pending | 2026-08-12 | claude | performance |
