...
|---|---|---|---|---|---|
| PERF-RANKING-LCP-02 | rankingモバイルの先頭地図tileだけを全viewportでhigh priority preloadし、5.8秒のload delayを解消する。ベース地図とTileSwitcherは維持する | pending | 2026-08-12 | claude | performance |
| PERF-RANKING-PAYLOAD-01 | sidebar itemをserver側で最大20件へ選別し、TopoJSONを品質gate付き150KB以下のR2候補へ削減してranking HTMLを50%以上軽量化する | pending | 2026-08-19 | claude | performance |
| PERF-AREA-DOM-01 | area railの全国上位・下位を各12件へ制限し、`/areas/13000` のDOM 9,101を70%以上削減する | pending | 2026-08-12 | claude | performance |
| PERF-STATIC-CACHE-01 | hashed `/_next/static/*` の`max-age=0`をCloudflare Traceで確認し、1年immutableのCache Response Rule案を作る。外部変更は承認後 | pending | 2026-08-19 | claude | performance |
...
| ID | タイトル | Status | Due | Owner | Metric |
|---|---|---|---|---|---|
| A11Y-AREA-CONTRAST-01 | area男女KPIの固定青・桃（contrast 3.67 / 3.52）をlight/darkとも4.5:1以上へ是正し、Lighthouse contrast違反を0にする | pending | 2026-08-19 | claude | accessibility |
