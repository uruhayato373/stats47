...
| PERF-WORKERS-CACHE-01 | cache無効gatewayでRSC/認証を先に判定し、共有可能な内部entrypointだけproduction cacheを有効化。path/data Cache-TagとR2公開Workflowの自動purgeも実装済。**残: 修正版デプロイ後にHTML HIT・RSC no-store・warm TTFB・公開直後のpurgeを本番実測する** | in-progress | 2026-08-19 | claude | performance |
