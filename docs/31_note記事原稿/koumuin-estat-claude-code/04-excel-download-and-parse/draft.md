...
note_url: https://note.com/stats47/n/n23262330a5e5
published: true
...
財政課が補助金申請の根拠資料を求めてきます。「過去 10 年分の県内総生産推移を、令和換算と西暦両方で並べてくれ」。

e-Stat から該当の Excel を落とすと、シートが 8 個あります。1 枚目は注釈、2 枚目は単位の説明、3 枚目から本表で、コード列と名称列が結合セルになっています。年度欄は「平成 28 年度」「令和 5 年度」が混在しています。

統計担当が Power Query でこれを開いて、結合解除、コード列分離、元号変換マスタとの突き合わせをやると、3〜4 時間が消えます。

資料が完成して数値検算したら、合計行を本文と一緒に集計してしまっていた、というミスも珍しくありません。

**こんな方に向けた記事です**

- e-Stat の多シート・結合セル・元号年の Excel を Power Query で毎回手作業整形している統計担当の職員
- 補助金申請の根拠資料づくりで、Excel の整形に 3-4 時間かけている財政課の方
- 合計行の混入や元号変換のミスに悩まされている統計担当の方

**この記事でわかること**

- e-Stat の厄介な Excel を Claude Code で構造把握し、整形済み CSV/JSON にする手順
- pandas と DuckDB を場面で使い分ける判断基準
- 令和・平成・昭和を西暦に変換し、合計行・小計行を安全に除外する具体的な方法
...
掲載している製造品出荷額や県内総生産などのランキングは、e-Stat の Excel データを本記事と同じ手順で整形して作っています。
...
民間のデータ基盤は「機械可読」が前提で設計されています。e-Stat の Excel は逆で、「人間が印刷して綴じる」前提のレイアウトを残したまま、API ではなくダウンロードファイルとして配布されています。

これは官民の文化的な差というより、e-Stat が長年「白書・報告書の配布チャネル」だった歴史的経緯によるものです。
...
2. **数億行でもインメモリで集計できます** (pandas より速い場面が多いです)
3. **CSV / Parquet / JSON / Excel をクロス JOIN できます** ので、人口データと県内総生産を結合して人口当たり GDP を作るなどが SQL 1 本で書けます
...
### Step 7: pandas vs DuckDB 使い分け

場面ごとの推奨ツールは次の通りです。

- **多シート + 結合セル + 元号変換の場合**: pandas が向いています。柔軟性が高く、openpyxl の細かい制御が効くためです
- **複数ファイル横断の集計（人口 + GDP + 出生数）の場合**: DuckDB が向いています。SQL の JOIN が圧倒的に書きやすいです
- **47 都道府県 × 数十年の中規模の場合**: どちらでも問題ありません。10 万行以下なら速度差はほぼありません
- **数億行・全国市区町村 1,700 自治体の場合**: DuckDB が必須です。pandas はメモリに乗りません
- **`.xls`（旧形式）の場合**: pandas（xlrd）を使います。DuckDB は非対応です
- **1 シート 1 表のシンプル Excel の場合**: DuckDB が便利です。1 行で読めます
...
ここまでのスクリプトを `scripts/parse-estat-excel.py` のように汎用化して残しておけば、次回以降は引数を変えるだけで動かせます。

さらに `.claude/skills/` 配下にスキルとして登録すれば、Claude Code から「parse-estat-excel で `data/raw/foo.xlsx` を整形して」と呼び出せます (詳細は本マガジン #10 で解説)。
