...
description: e-Stat の Excel は多シート・セル結合・元号年が混在する厄介な形式。pandas と DuckDB の使い分け、構造把握を Claude Code に任せる手順、令和・平成・昭和の西暦変換まで実装例付きで解説する。
created: 2026-05-26
...
財政課が補助金申請の根拠資料を求めてくる。「過去 10 年分の県内総生産推移を、令和換算と西暦両方で並べてくれ」。e-Stat から該当の Excel を落とすと、シートが 8 個ある。1 枚目は注釈、2 枚目は単位の説明、3 枚目から本表で、コード列と名称列が結合セルになっている。年度欄は「平成 28 年度」「令和 5 年度」が混在する。

統計担当が Power Query でこれを開いて、結合解除、コード列分離、元号変換マスタとの突き合わせをやると、3〜4 時間が消える。資料が完成して数値検算したら、合計行を本文と一緒に集計してしまっていた、というミスも珍しくない。
...
本記事の執筆者は元自治体職員。Claude Code で 47 都道府県統計サイト [stats47.jp](https://stats47.jp) (約 2,000 のランキングを毎日自動更新) を個人で開発・運用している。掲載している製造品出荷額や県内総生産などのランキングは、e-Stat の Excel データを本記事と同じ手順で整形して作っている。

## TL;DR

- e-Stat の Excel は多シート + 結合セル + 元号年が標準仕様。手作業前提では作られていない
- pandas (openpyxl エンジン) と DuckDB (`read_xlsx`) を場面で使い分ける
- 構造把握 (シート一覧 → 中身サマリ → 集計対象シート特定) を Claude Code に任せる
- 元号 → 西暦変換は「令和 5 年 = 2023」を 4 行のマッピング関数で処理する
- 初回手作業 3〜4 時間 → 月次運用 5〜10 分。テンプレを残せば次の担当者も同じ品質で再現できる
...
民間のデータ基盤は「機械可読」が前提で設計されている。e-Stat の Excel は逆で、「人間が印刷して綴じる」前提のレイアウトを残したまま、API ではなくダウンロードファイルとして配布されている。これは官民の文化的な差というより、e-Stat が長年「白書・報告書の配布チャネル」だった歴史的経緯による。

結果として:

- 1 ブックに **複数の調査結果** が同居する (1 シート 1 調査ではない)
- 表の上に **タイトル・調査概要・凡例** が 5〜10 行入る
- コード列と名称列が **セル結合** される
- 年度欄に **「平成」「令和」** が混在する
- 末尾に **合計行・小計行** が本文と同じテーブル内に置かれる

自治体の統計担当は、これを Excel の手作業で処理することを前提に研修を受けている。が、Claude Code を使えば、構造把握から整形まで自然言語で指示できる。e-Stat の利用規約上、ダウンロードした Excel の機械加工も商用利用も可 (出典「政府統計の総合窓口 (e-Stat)」を明記すれば OK)。

## 手順 / 解説

### Step 1: Excel をダウンロードする

e-Stat の統計表ページ右上に「DB」「EXCEL」「CSV」「PDF」のアイコンがある。「EXCEL」をクリックすると `<statsDataId>.xlsx` 形式のファイルが落ちてくる。
...
CSV ダウンロードもあるが、e-Stat の CSV は **コード列と名称列が別の行** に出力される独特な形式で、Excel よりむしろ取り回しが難しい。本記事では Excel を扱う方針で進める。

ダウンロードしたファイルを `data/raw/` に置く。
...
Excel を開かずに、Claude Code に「`data/raw/prefecture-gdp.xlsx` のシート構成と各シートの先頭 10 行を見せて」と頼む。以下のスクリプトが生成される。
...
これだけで「表 1 が本表」「L1-L2 がヘッダー」「データは L3 以降」「年度欄に元号」というのが一目で分かる。Claude Code に「表 1 の本表だけ抽出して、年度を西暦に変換した CSV を作って」と次の指示を出す。

### Step 3: pandas (openpyxl) で読み込む

pandas は柔軟性が高く、e-Stat のような不整形 Excel に強い。
...
1. `header=[0, 1]` で **多段ヘッダーを正しく解釈** する
2. `astype(str).str.zfill(2) + "000"` で **2 桁コードを 5 桁に統一** する
3. 全国 (`00000`) を除外して 47 行に絞る

### Step 4: 元号年を西暦に変換する

「平成 23 年度」を `2011`、「令和 5 年度」を `2023` に変換する関数を入れる。
...
これを年度カラム名に適用して、出力時にはきれいな西暦カラムにする。
...
長形式 (long format) にしておくと、後段の集計・グラフ生成がどちらも楽になる。
...
pandas に慣れていない統計担当でも、SQL は触ったことがある人が多い。DuckDB は単一バイナリで動く高速な分析 DB で、Excel を直接 SQL から読める。
...
1. **SQL ベースなので Power Query / Access 経験者の学習コストが低い**
2. **数億行でもインメモリで集計可能** (pandas より速い場面が多い)
3. **CSV / Parquet / JSON / Excel をクロス JOIN できる** (人口データと県内総生産を結合して人口当たり GDP を作るなどが SQL 1 本で書ける)

注意点: DuckDB の `read_xlsx` は **`.xls` (旧形式) には対応していない**。e-Stat の古い統計表は `.xls` で配布されている場合があり、その場合は openpyxl (Python 経由) で `.xlsx` に変換するか、pandas を使う必要がある。
...
e-Stat の Excel で最大の地雷は、結合セルと合計行が見た目に区別できないことだ。表の最後に「合計」「小計」「再掲」「再掲計」のような行が紛れていて、これを集計に含めると数値が 2 倍になる。

セル結合は openpyxl が自動でばらしてくれる (結合セルの値は左上に入り、他は `None` になる)。
...
`"計"` だけだと「会計」や「設計」を含む行も落ちる。そのため `都道府県名` カラムが「47 都道府県名のいずれか」と一致するもののみ残す方が安全な場合もある。Claude Code に「47 都道府県名のホワイトリストでフィルタする関数を書いて」と頼めば、5 行で実装してくれる。
...
実運用では「最初に pandas で構造把握 + 整形 → 結果を Parquet に保存 → 以降の集計は DuckDB」というハイブリッドが効率的になる。
...
1. **`pd.read_excel` で数値が文字列になる** → セルに `"-"` `"…"` `"X"` (秘匿マスク) が混在しているケース。`na_values=["-", "…", "X", "..."]` を指定して数値変換を強制する
2. **多段ヘッダーが壊れる** → ヘッダー行が 3 段以上ある場合、`header=[0, 1, 2]` で明示的に段数を指定する
3. **元号変換で「平成 31 年度 = 2019 年度」を「令和 1 年度」に統一したい** → 4 月始まりの会計年度ベースか、暦年ベースかで取り扱いが変わる。改元日 (令和は 2019-05-01) を超えるかで判定する関数を用意する
4. **Excel の数値が `1.9234e+07` のような指数表記** → `pd.options.display.float_format = "{:,.0f}".format` で出力フォーマットを整える

### Step 9: stats47.jp での実運用例

stats47.jp が公開している製造品出荷額や県内総生産は、本記事で扱った pandas + 元号変換のパイプラインで日次更新されている。具体例:
...
これらすべてが「元号 → 西暦」「コード正規化」「合計行除外」のセットを通って JSON 化されている。

### Step 10: 初回 3 時間 → 月次運用 5 分のためのテンプレ化

ここまでのスクリプトを `scripts/parse-estat-excel.py` のように汎用化して残しておけば、次回以降は引数を変えるだけで動かせる。さらに `.claude/skills/` 配下にスキルとして登録すれば、Claude Code から「parse-estat-excel で `data/raw/foo.xlsx` を整形して」と呼び出せる (詳細は本マガジン #10 で解説)。

初回 3〜4 時間の作業が、テンプレ + スキル化で月次運用 5〜10 分まで圧縮できる。人事異動で担当が変わってもスキル定義は引き継がれるため、属人化も解消する。

## よくあるつまずきと回避策

- ⚠️ **`pd.read_excel` でシート名が日本語だと文字化け** → `engine="openpyxl"` を明示し、ファイル保存時は UTF-8 (`encoding="utf-8-sig"`) で BOM 付き出力にする
- ⚠️ **DuckDB の `read_xlsx` が「sheet not found」** → 大小文字とスペースに敏感。`SHOW TABLES FROM read_xlsx(...)` で正確なシート名を取得してから指定する
- ⚠️ **「令和」が `R5` `R5年度` `令和5年度` で表記揺れ** → 正規表現に `[Rr]` パターンも追加するか、Claude Code に「表記揺れを吸収する `to_seireki` 関数を作って」と頼む
- ⚠️ **コード列が空白セルで終わる** → e-Stat の Excel は表の末尾に空行 + 注釈が入る。`df.dropna(subset=["都道府県コード"])` で先に空行を落とす
- ⚠️ **「秘匿マスク」が数値カラムに混入** → 集計値が極端に小さい場合に `X` `…` `-` で秘匿される。`na_values` 指定で `NaN` に倒し、用途に応じて 0 埋めか除外かを選択
