...
note_url: https://note.com/stats47/n/n0477d544669b
published: true
...
議会で「医師不足が他県と比べてどの程度深刻か、人口 10 万人当たりで示してくれ」と質問が来ます。生の医師数を並べるだけでは答えになりません。

人口の多い都道府県は医師数も多くて当たり前で、住民から見た医療アクセスを比較するには「人口 10 万人当たり」「順位」「全国平均との差」「偏差値」のような **派生指標** が必要になります。

統計担当が Excel で人口データと医師数データを VLOOKUP で結合し、割り算カラムを作り、順位を RANK 関数で出して、平均と標準偏差を計算して偏差値カラムを作ります。1 指標あたり 30 分〜1 時間。10 指標分の資料となると、半日が消えます。

**こんな方に向けた記事です**

- 議会答弁用に「人口 10 万人当たり」「順位」「偏差値」を Excel で毎回組み立てている統計担当の職員
- VLOOKUP や RANK 関数の入れ間違い・行ずれで数値ミスが起きやすいと感じている方
- 生データを人口当たりや前年比などの派生指標に加工する作業を効率化したい企画課・財政課の方

**この記事でわかること**

- 人口当たり指標・順位・偏差値・5 年平均を pandas / DuckDB で 5 行ずつ出す書き方
- `.rank()` `.std()` `.rolling()` と `RANK() OVER` `STDDEV` `LAG` の対応
- 総数・男女別・年齢階級別を cdCat01 の切替で扱う e-Stat に沿った方針
...
本記事の執筆者は元自治体職員です。Claude Code で 47 都道府県統計サイト [stats47.jp](https://stats47.jp) (約 2,000 のランキングを毎日自動更新) を個人で開発・運用しています。

サイトに掲載している「人口 10 万人当たり医師数」「年齢調整死亡率」「世帯当たり年収」などはすべて本記事の派生指標計算ロジックを通って生成されています。
...
生データ (人口 / 医師数 / 婚姻数 / 製造業出荷額) はそのまま並べると、人口規模で結果が決まってしまいます。「東京は何でも 1 位、鳥取は何でも 47 位」というランキングしか作れず、政策の議論になりません。

そこで「人口 10 万人当たり」「年齢調整」「順位」「偏差値」のような **派生指標** が登場します。ですが、Excel ではこれらを 1 つずつ手作業で組み立てる必要があり、計算式の入れ間違い・行ずれ・参照元シートの切れ・四捨五入の桁数違いなどミスが入り込みやすいです。

Claude Code を使うと、派生指標のパターンが pandas / DuckDB のスニペット 1 つで決まります。同じロジックを 10 指標、20 指標に展開しても結果がブレません。e-Stat (政府統計の総合窓口) の生データに対して、再現性のある加工を施せます。

## 手順 / 解説

### Step 1: 派生指標の 4 大パターンを整理する

自治体業務で頻出する派生指標は、ほぼ以下の 4 パターンに収まります。

- **単位当たり**: 値 ÷ 母集団 (人口・面積・世帯数)。用途例は「人口 10 万人当たり医師数」「面積当たり林道 km」など
- **順位**: 47 県で並べた rank。用途例は「議会答弁の『全国 N 位』」
- **偏差 (z-score)**: (値 − 平均) ÷ 標準偏差。用途例は「全国比でどれだけ突出しているか」
- **経年平均・差分**: 5 年平均、前年比、年率変化。用途例は「政策効果のトレンド検証」

この 4 つを組み合わせれば、ほとんどの議会答弁・補助金申請資料が組み立てられます。

### Step 2: pandas で書く (4 パターン × 5 行)

医師数 (生データ) と人口 (母集団) を結合した DataFrame `df` が手元にある前提です。
...
これだけで 4 つの派生指標カラムが出来ます。1 指標 5 行程度です。Excel で 30 分かかっていた作業が、テンプレを置けば数秒で終わります。

### Step 3: DuckDB で同じことを SQL で書く

DuckDB なら CSV を直接 SQL で読んで派生指標を計算できます。
...
1. **`PARTITION BY year`** で「年度ごとに集計する」を表現します (年度横断にしません)
2. **`RANK()`** は同値タイランクです。`DENSE_RANK()` だと飛ばさず連番、`ROW_NUMBER()` だと同値でも別順位になります
3. **`ROWS BETWEEN 4 PRECEDING AND CURRENT ROW`** で 5 年平均 (現在含む) を計算します
...
本マガジン #03 で取得した JSON を pandas に読み込みます。
...
複数年データを扱いたい場合、年度ごとの JSON を連結します。
...
これで `(areaCode, year)` をキーにした long format の DataFrame ができ、派生指標計算の入力になります。

### Step 5: 人口データを結合する (1 つの母集団に依存しない設計)

派生指標の核は「何で割るか」です。人口で割るのか、世帯数で割るのか、就業者数で割るのか、面積で割るのかで意味が変わります。
...
母集団を切り替えやすい設計にしておくと、「人口当たり」「就業者当たり」「世帯当たり」を選択肢として並列に出せます。stats47.jp の世帯当たり年収はこの仕組みで計算されています。

ここから先は有料部分:

### Step 6: e-Stat の「総数」「男女別」「年齢階級別」を切り替える

e-Stat の同じ統計表に「総数」「男」「女」「0〜14 歳」「15〜64 歳」「65 歳以上」などのカテゴリが入っていることが多いです。これらは **同じ `statsDataId` の中で `cdCat01` を切り替える** ことで取り分けます。
...
`cdCat01` を切り替えるだけで「年齢階級別人口」「男女別給与」「業種別事業所数」がそのまま取れる構造になっています。

これを Claude Code に頼むときは「`0003448243` から `#A03101` と `#A03303` を別々に fetch して、`(areaCode, year)` で結合してくれ」と頼めば、上記スクリプトが返ってきます。

### Step 7: 年齢調整死亡率を作る (応用編)

「老年人口比率が高い県は死亡率が高く見える」というバイアスを除くために、**年齢調整死亡率** を計算します。基準人口 (e.g. 平成 27 年モデル人口) の年齢構成に合わせて重み付けします。
...
これで「老年人口比率の影響を除いた、純粋な医療水準・健康水準の比較指標」ができます。stats47.jp の健康関連ランキングはこの方式で計算されています。

### Step 8: 偏差値・順位を組み合わせて「総合スコア」を作る

議会・補助金申請の資料では、複数指標を組み合わせた「総合スコア」を要求されることがあります。偏差値ベースで合成すると、桁数の違う指標も対等に並べられます。
...
注意: 偏差値合成はあくまで「ベンチマーク用の参考値」です。各指標の重み付けや因果関係を主張する用途には向きません。資料化するときは「単純合成のため傾向把握に限る」など注記を入れてください。

### Step 9: 順位 vs 偏差値 — どう使い分けるか

両者は似て非なるものです。場面で使い分けます。

- **議会答弁「全国何位か」の場面**: 順位 (rank) が向いています。議員・住民に直感的に伝わるためです
- **上位 / 下位の差を比較する場面**: 偏差値 (z-score) が向いています。1 位と 2 位の差、46 位と 47 位の差を絶対量で見られます
- **経年変化を比較する場面**: 偏差値が向いています。全国平均が動いても自県の相対位置が分かります
- **補助金申請のエビデンスとして示す場面**: 順位 + 偏差値の併記が向いています。「順位は 35 位、偏差値は 38」のように両方提示します
...
stats47.jp は本記事のロジックを使って、約 2,000 のランキングを毎日自動計算しています。代表例:
...
これらすべて、本記事の派生指標計算スニペットの組み合わせで実装されています。Claude Code に「県別の医師数と人口を結合して、人口 10 万人当たりのランキングと偏差値を出して」と頼めば、ここまでのコードがそのまま返ってきます。

### Step 11: 初回 30 分 → 月次運用 30 秒のための関数化

10 指標、20 指標と数を増やすなら、上記スニペットを汎用関数にまとめておきます。
...
新しい指標を追加するときも 1 行です。テンプレが残るので、人事異動があっても次の担当者が同じロジックで再現できます。

## よくあるつまずきと回避策

- ⚠️ **`std()` が `nan` を返す** → 47 県のうち欠損が多い指標 (入港船舶総トン数のように 40 県しかデータがない等) で起きます。`std(ddof=0)` で母分散ベースに切り替えるか、欠損県を除外してください
- ⚠️ **順位が `1.0` `2.0` の float になる** → pandas の `.rank()` はデフォルト float です。`.astype(int)` で整数化してください。同値タイで小数点が出る場合は `method="min"` で揃えます
- ⚠️ **DuckDB の `STDDEV` が標本標準偏差** → `STDDEV_SAMP` と `STDDEV_POP` があります。Excel の `STDEV` は標本、`STDEVP` は母分散です。資料の表記に合わせてください
- ⚠️ **5 年平均が初年度に偏る** → `min_periods=1` を付けると初年度から平均が計算されますが、本来 5 年に満たない値です。資料には「3 年以上のデータがある県のみ」など限定を入れてください
- ⚠️ **偏差値の桁数で結果がブレて見える** → 小数 1 桁に丸めます (`.round(1)`)。Excel と DuckDB と pandas で四捨五入挙動が微妙に違うため、丸め後の値を CSV に書き出して固定してください
