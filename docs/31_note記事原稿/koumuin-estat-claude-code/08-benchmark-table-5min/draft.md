...
description: 議会答弁・政策資料・補助金申請で必要になる「他自治体との比較表」を 5 分で作る手順です。47 県取得 → 同規模自治体抽出 → 3 軸比較 → Markdown / Excel 出力までを Claude Code で自動化します。
created: 2026-05-26
...
note_url: https://note.com/stats47/n/nd4afc48385a3
published: true
...
自治体職員にとって **「他自治体と比べてどうか」** は最頻出の問いです。議会答弁では「県内 X 市と比べて高いのか低いのか」、政策資料では「同規模自治体の平均水準は」、補助金申請書では「全国平均との乖離は」。同じデータを異なる軸 (順位 / 全国平均比 / 同規模比) で何度も出し直します。

統計担当が Excel で「県内市町村を縦に並べ、当該指標を横に並べ、全国平均を計算し、人口規模の似た自治体を抽出してハイライト」を作る作業は 1 案件 1〜2 時間かかります。これが議会前の繁忙期に 5-10 件並行で来ると、夜は溶けてしまいます。

**こんな方に向けた記事です**

- 議会答弁や政策資料で「他自治体との比較表」を繰り返し求められる統計担当・企画課の職員
- 同規模自治体の抽出や全国平均比の計算を毎回 Excel で組み直している方
- 順位・全国平均比・同規模比の 3 軸をまとめて短時間で出したい方

**この記事でわかること**

- 47 県データの取得から同規模自治体の抽出、3 軸比較表の生成までの具体的な手順
- 「人口 ±30%・面積 ±50%」で同規模自治体を安定して絞り込む考え方
- Markdown / Excel で議会答弁・決裁ルートに乗せられる形に整える流れ

執筆者は元自治体職員です。Claude Code で 47 都道府県の統計サイト stats47.jp（約 2,000 のランキングを毎日自動更新）を個人で開発・運用しています。stats47 では 47 県データの取得・整形・配信を毎日バッチで回しており、本記事の「ベンチマーク表 5 分生成」も同じ仕組みを応用したものです。

人口 10-30 万人規模の市役所で、企画課・財政課・議会事務局が「ベンチマーク資料を翌朝までに」と頼む頻度は月 5-10 件が典型例です。**1 件 1.5 時間 → 5 分** に短縮できれば、月 7-14 時間の節約になります。
...
自治体の「他自治体比較」は、3 つの典型シーンで発生します。

1. **議会答弁準備**: 「X 市と比べて多いのか少ないのか」「同規模自治体の中で何位か」 — 議員が質問しそうな比較軸を先回りで全部出す作業です
2. **政策エビデンス**: 「人口 N 万人規模の自治体で先行事例は」 — 補助金申請書・総合計画策定で必須です
3. **議事・委員会資料**: 「ご近所の自治体ではどうしているか」 — 県内市町村の取り組み比較です

これらは「同じ指標を違う軸で切り出す」作業の繰り返しです。Excel で県内市町村のデータを縦に並べ、全国データを別シートに置き、自治体規模で並べ替えて該当行をハイライト——慣れていても 1 件 1〜2 時間かかります。

e-Stat の API は 47 県データを 1 リクエストで返します。市区町村レベルも同じ構造で取得できます。Claude Code に「人口 10 万人前後の自治体で当該指標が高い順 10 件、自分の自治体に赤いマーカー」と頼めば、表が 5 分で完成します。

e-Stat 利用規約では商用利用可、ただし **「出典を明記すること」が条件** です (https://www.e-stat.go.jp/api/ アクセス日 2026-05-26)。議会資料の脚注に「政府統計の総合窓口 (e-Stat) より」を 1 行入れれば完了します。

## 手順 / 解説

### Step 1: 47 県データを 1 リクエストで取得

`/fetch-estat-data` スキルで対象指標を取得します。たとえば「都道府県別の人口 10 万人あたり医師数」なら、`statsDataId` を指定して 1 コマンドで取れます。
...
`--lv-area 2` は都道府県レベル、`3` は市区町村レベルです。`cdArea` で地域指定すると **キャッシュが分断され API 呼び出しが増える** ため、全件取得してメモリでフィルタします (CLAUDE.md 規約)。

レスポンスから当該指標を CSV で受け取り、`areaCode` を 5 桁で正規化しておきます。
...
同規模の判定基準は組織により揺れますが、実務では下記のいずれかで十分です。

- **人口**: 対象自治体 ±30%（出典: 住民基本台帳人口 (e-Stat)）
- **面積**: 対象自治体 ±50%（出典: 国土地理院「全国都道府県市区町村別面積調」）
- **人口密度**: 対象自治体 ±50%（上記 2 つから派生）
- **一般会計予算**: 対象自治体 ±30%（出典: 総務省「市町村別決算状況調」）

人口だけで絞ると都市/地方の構造差が混じります。**人口 ±30% かつ面積 ±50%** で絞ると、生活実態が近い自治体が残ります。
...
「順位」「全国平均比」「同規模比」の 3 軸で当該指標を切り出します。
...
ファイル末尾の **出典表記を忘れないようにします**。e-Stat 利用規約で必須項目になっています。

### Step 5: Excel / PDF 出力に変換

Markdown を渡せば、Claude Code がそのまま Excel に変換するスクリプトも書けます。`openpyxl` で書き出すか、`pandoc` で PDF にします。
...
PDF にしたい場合は `pandoc benchmark.md -o benchmark.pdf --pdf-engine=lualatex -V documentclass=ltjarticle` です (日本語 LaTeX 環境が必要)。簡易には Word から PDF 保存でも構いません。
...
毎回コマンドを打つ代わりに `.claude/skills/benchmark-table/SKILL.md` としてスキル化しておくと、`/benchmark-table` で呼び出せます。
...
詳しいスキル化の手順は [#10 Claude Code スキル化で月次定型業務を自動化する](../10-claude-skills-routinize/draft.md) に書きました。

## stats47 で動いている実例

本記事の手順は stats47.jp で毎日動いています。
...
47 県分のデータ取得 → 整形 → 配信を毎日バッチで自動化しています。同じ仕組みを「自分の自治体周辺だけ」「県内市町村だけ」に縮小して回すと、本記事のベンチマーク表になります。

## よくあるつまずきと回避策

- ⚠️ `cdArea` で地域指定 → キャッシュ分断。全国取得 + メモリフィルタが正解です (CLAUDE.md 規約)
- ⚠️ Excel が `01000` を `1000` に丸める → CSV 読み込み時 `dtype={"areaCode": str}` を必ず指定します
- ⚠️ 同規模の閾値が緩すぎて 30 件返る / 厳しすぎて 1 件しか返らない → 人口 ±30% / 面積 ±50% から開始し調整します
- ⚠️ 「全国平均」が `NaN` を含むデータで歪む → `.mean(skipna=True)` (pandas デフォルト)、`-` や `X` は事前に `NaN` 化します
- ⚠️ 出典表記を忘れる → 出力テンプレートに「政府統計の総合窓口 (e-Stat) より」を必ず含めます
- ⚠️ 議会答弁で「同規模自治体」を聞かれて定義を答えられない → 「人口 ±30%、面積 ±50%」を資料末尾に明記します
...
- [#10 Claude Code スキル化で月次定型業務を自動化する](../10-claude-skills-routinize/draft.md) — スキル化で月次運用に乗せる手順
