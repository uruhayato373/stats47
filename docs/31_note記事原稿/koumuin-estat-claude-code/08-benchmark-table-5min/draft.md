...
- ベンチマーク資料は「順位」「全国平均比」「同規模比」の **3 軸** で出します
- 47 県データの一括取得は `/fetch-estat-data` です (e-Stat API 経由、`cdArea` は使いません)
- 同規模自治体の抽出は「人口 ±30%」「面積 ±50%」を組み合わせるとブレが少なくなります
- Markdown 表は議会答弁原稿に貼れる形式、Excel は決裁ルートに乗せる形式です
- 「政府統計の総合窓口 (e-Stat) より」の出典表記を必ず付けます (商用利用可、出典明記必須)

## 背景: なぜ自治体職員にこの課題があるか

自治体の「他自治体比較」は、3 つの典型シーンで発生します。

1. **議会答弁準備**: 「X 市と比べて多いのか少ないのか」「同規模自治体の中で何位か」 — 議員が質問しそうな比較軸を先回りで全部出す作業
