...
note_url: https://note.com/stats47/n/nc53fa60114f8
published: true
...
- 「AI に住民情報を判断させたのでは」と問われたとき、否定する証拠を出せず不安な自治体職員
- 情シスから「条件付き OK」を勝ち取るために、ログ整備を求められている部署の担当者
- 内部監査・住民監査請求・議会答弁に耐える利用記録を残しておきたい方

**この記事でわかること**

- `.claude/settings.json` の Hook で送信・ファイル変更・セッション終了を自動ログ記録する手順
- 実行者・日時・対象ファイル・プロンプトハッシュ・トークン数を、本文なしで残す設計
- チェーンハッシュ (前エントリの SHA-256) で改ざん検知を仕込み、月次レポート化する方法
...
**民間**: 経営陣の判断で開示範囲を絞れます。守秘契約・営業秘密の盾があります。
**公務員**: 住民監査請求 / 情報公開請求 / 議会の資料要求が来れば、原則としてすべて開示します。
...
1. **ログが残ること** — 何を送ったかが後から検証可能であること
2. **マスキングがあること** — 個人情報の流出を機械的に止めること
3. **管理者がアクセスできること** — 当該職員以外も検証可能であること

しかも公務員のログ保管期間は文書管理規程で 5 年〜永年とされがちで、「ログを残す」よりも「肥大化せず、改ざんできず、必要なときに取り出せる」設計が肝心です。

多くの自治体の文書管理規程では電子データを「行政文書ファイル」として扱い、業務記録は内容に応じて 1 年 / 3 年 / 5 年 / 10 年 / 永年の 5 区分で保管期間を設定します。

AI 利用ログは「事務の経過に係る記録」または「電子計算機処理に係る経過情報」として **5 年保管に位置付ける運用が現実的**で、特に住民監査請求の出訴期間 (1 年) + 控訴期間を踏まえると 3 年未満は短すぎる扱いとなります。
...
監査で問われるのは概ね以下の 5 点で、設計はこれらを必ず満たします。

![表: 観点 / 何を記録 / 用途](./images/table-1.png)
<!-- SVG: table | 観点 / 何を記録 / 用途 -->

ポイントは 「プロンプト本文そのものは残さない」 ことです。
...
<!-- SVG: terminal | `.claude/settings.json` を VS Code で開いた状態で -->
...
自治体の情報セキュリティポリシーでは「重要ログの完全性確保」が一般的に要求されており、総務省の「地方公共団体における情報セキュリティポリシーに関するガイドライン」(令和 5 年改定版) でも「ログの改ざん検知措置」が推奨事項として明記されています。

具体的措置として WORM (Write Once Read Many) 媒体への保存、デジタル署名、ハッシュチェーンの 3 方式が例示されており、本記事のチェーンハッシュ実装はこのうち第三の方式に該当します。

庁内 NAS への日次バックアップを併用すれば、改ざん検知 + 物理的喪失対策の両立が低コストで実現できます。
...
1. **ログにプロンプト本文をそのまま残す** — 個人情報含有時にログ自体が漏洩源になります。ハッシュのみ保存が原則です
2. **ログ保管がローカルディスクのみ** — PC 故障で消失します。庁内ファイルサーバまたは外付け SSD に日次バックアップします
3. **改ざん検知の仕組みがない** — 単純な追記ログでは「不都合な行を削除」が物理的に可能です。チェーンハッシュが必須です
4. **監査担当者が読めない** — JSONL は人間可読でありません。Markdown 月次レポートを併存させます
5. **Hook の `timeout` 漏れで Claude Code が固まる** — 必ず `"timeout": 3000` 等を指定します
6. **`.claude/settings.json` のスキーマ変更追従漏れ** — Claude Code のバージョンアップで Hook 仕様が変わります。月 1 で公式ドキュメントを確認します
...
1. **90 日経過分は月単位 zip にアーカイブ**します
2. **アーカイブ時に GPG 公開鍵で暗号化**します — 秘密鍵は監査担当者が保管 (二重承認)
3. **オリジナル jsonl は安全削除**します — `shred` コマンド使用
4. **アーカイブインデックス維持** — `.claude/logs/audit/archive/index.json` で月別・件数・暗号化ハッシュを記録します
