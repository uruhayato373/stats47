...
note_url: https://note.com/stats47/n/n59c27bbc973e
published: true
...
起案文を提出してから決裁が下りるまで、係長 → 課長補佐 → 課長 → 部長 → 関係課合議 → 財政課協議 → 政策法務協議 → 副市長 → 市長と回ります。

各経路で「ここが抜けてる」「この根拠は何」「予算科目は」「合議経路漏れてない?」と毎回違う観点で差し戻されて、**提出 1 回・修正 5 回、決裁完了まで 2 週間** といった経験はありませんか。

新人時代は「何を見られているか」が分からないまま起案するので、上席で OK だったのに財政で差し戻され、財政を通したら法務で別の指摘が入り、結果として手戻りが連鎖します。

本記事では Claude Code に「決裁経路で必ず指摘される 20 項目」を一括査読させるチェックリストを、`/proposal-review` スキル化込みで全公開します。

中規模自治体での運用想定として、起案差し戻し率を Claude Code 導入後に **月 8 件 → 月 2 件 (75% 削減)** 程度に圧縮できる試算があります (架空整理 / 実測は環境依存)。
...
- 起案文を提出するたびに財政課・法務担当から差し戻され、修正を繰り返している自治体職員
- 決裁経路で「何を見られるか」が分からず、手戻りが連鎖して困っている配属 1-2 年目の方
- 上席チェックの前に、自己査読の精度を上げて提出回数を減らしたい方

**この記事でわかること**

- Claude Code に決裁経路の必須 20 項目を一括査読させるチェックリストの作り方
- 「項目・判定・該当箇所・想定指摘者・想定指摘文」の 5 列で欠落を提出前に潰す手順
- `/proposal-review` としてスキル化し、毎回のプロンプトコピペをなくす方法
...
経験豊富な職員でも全経路の最新ルールを把握するのは困難です (財政課の予算科目改編、法務の条例改正、総務の電子決裁ルール変更などが頻繁にあります)。結局「差し戻されてから直す」のが常態化しています。
...
上席の指摘が最多なのは形式・趣旨確認が広く拾われるためです。一方で件数は少なくても差し戻し 1 回あたりの修正工数が大きいのは **法務・財政の指摘** で、こちらは事前査読の費用対効果が特に高い領域とされています。
...
<!-- SVG: terminal | 査読結果の markdown 表出力例 (5 列 × 20 行) -->

「欠落」項目から優先的に修正します。経験上、20 項目のうち **平均 5-7 個が「欠落」、3-5 個が「要確認」** で出てきます。「欠落」を全部潰し、「要確認」は時間との兼ね合いで判断する運用が現実的です。

修正後に再度 `/proposal-review` を回して「全項目 OK」になってから提出すると、差し戻し回数が大きく減ります。

中規模市の総務系課 (係員 5-8 名) で AI 査読を 6 か月導入した事例 (架空整理) では、導入前 3 か月の差し戻し件数が月平均 8-10 件だったものが、導入後 3 か月で **月平均 2-3 件に減少** したという報告があります。

決裁完了までの平均日数も 12-14 日から 5-7 日に短縮されました。

係員の体感としては「提出前に自分で品質確認できる安心感」「上席チェックが形式指摘ではなく内容議論に集中するようになった」が主なフィードバックです。上席側からも「初稿の質が上がって本来見るべき政策判断に時間を割けるようになった」という反応が想定されます。
...
1. **20 項目を全部潰そうとして時間を食う**: 「欠落」のみ対応し、「要確認」はリスクと天秤にかけましょう。経験上、要確認は半分が「経験上問題ない」で済むので、判断軸を自分で持っておくことが大切です
2. **AI が項目を勝手に解釈する**: チェック観点を具体的に書きましょう (「予算科目が記載され、節の残額根拠があるか」のように)。「整合性チェック」のような抽象表現は AI が表面的に解釈して見落とします
3. **機微情報入り起案文をそのまま投げる**: 個人情報保護の設定 (`.claude/hooks/pre-tool-use-pii-check.sh`) を先に行いましょう (別記事参照)
4. **チェック結果を起案文に添付してしまう**: 査読結果は手元メモとして扱います。決裁文書には含めません (AI 利用の事実は別途「政策法務協議結果」欄に注記程度に留めましょう)
5. **AI 査読を理由に上席チェックを省略する**: AI は補助です。最終責任は人間にあります。AI 査読 OK でも上席チェックは必ず通しましょう (差し戻しが減るだけで、ゼロにはならない前提です)
6. **自治体規程の改正に追随できない**: `reference/jichitai-rules.md` を年 1 回見直しましょう。財政課の予算科目改編 (3 月)、法務の条例改正 (4 月)、総務の電子決裁ルール変更を反映することが重要です
...
スキル化して git 共有すれば、チーム全体で同じ品質ラインを引けます (実測値は自治体規模・業務分野で大きく変動します)。
...
起案文が決裁経路のたびに違う観点で差し戻され、提出 1 回・修正 5 回を繰り返している新任職員の人。

20 項目を細分化した 47 項目フル版プロンプトと、差し戻し実例での精度検証まで踏まえて手戻りを減らしたい自治体職員に向いた内容です。
...
無料部分の 20 項目は汎用版です。実運用では各項目をさらに細分化した 47 項目で回しています。
...
実際に過去 1 年で受けた差し戻し 10 件を、AI 査読が事前に予測できたか検証した結果を公開します。予測ヒット率・見落としパターン・改善点まで含めて分析します。

![表: 案件種別 / 差し戻し指摘者 / 指摘内容 / AI 予測](./images/table-2.png)
<!-- SVG: table | 案件種別 / 差し戻し指摘者 / 指摘内容 / AI 予測 -->

中規模市の総務系課で過去 1 年間の差し戻し実例 10 件を AI 査読で再検証した事例 (架空整理) では、予算・法務・公用文に関する指摘 7 件はすべて事前予測可能 (**ヒット率 100%**) でした。

一方で、議案番号採番ルール違反・組織改編に伴う合議経路変更・首長判断による政治的修正の 3 件は AI 単独では予測困難 (見落とし率 30%) という結果が出ています。
