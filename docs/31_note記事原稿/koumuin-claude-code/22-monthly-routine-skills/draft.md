...
Claude Code への移植検討は、これら既存資産を「廃止する」「現状維持する」「Claude Code に巻き取る」の 3 択で判定し、メンテ困難度 × 使用頻度の 2 軸でスコアリングする手順が現実的になる。
...
Skills 化の入出力実例として現場で語られる典型例は、「月次予算執行報告」のケースだ。入力は各課提出の Excel ファイル 12-18 通 (1 ファイルあたりシート 3-5 枚、行数 50-200 行)、ファイル名規則は「<課コード 4 桁>_<業務名>_<YYYYMM>.xlsx」が標準形。
