...
「住民税の賦課決定通知書の文面を AI で整えたい」「生活保護のケース記録要約を AI に手伝ってほしい」——最も AI が効くのは基幹系 (マイナンバー利用事務系) の文書業務ですが、その端末は LGWAN-IF すら遮断され、USB すら挿せません。クラウド AI を使う余地はゼロです。
...
本記事ではこの壁を Ollama (ローカル LLM ランタイム) で突破する手順を解説します。Wi-Fi を物理的に切った状態でも動く、データが PC の外に絶対に出ない、ベンダーロックインゼロ、コストは PC 1 台のみ——公務員のリスク許容と相性のよい選択肢です。Claude Code の UI を使いながら推論だけローカル LLM に切り替える構成も含めて、コピペで動く実装と情シス説明用テンプレ付きで進めます。

人口 10-30 万人規模の自治体では、LGWAN 端末は USB ポート物理封鎖 + ソフト導入は情シス申請 (承認 2-4 週間) + 専用 ID 必須という三重制約が典型例です。基幹系端末はさらに厳しく、執務室外への持ち出し禁止 + 操作ログ常時記録 + 印刷物のシュレッダー必須が標準。一方でインターネット系端末は職員 1 人 1 台配布されるケースが多く、AI 検証は実質ここでしかできない構造になっています。
...
| 層 | 接続 | データ性質 | AI 利用可能性 |
|---|---|---|---|
| **基幹系** (マイナンバー利用事務系) | 完全独立、USB 禁止 | 住民票・税・福祉等 | クラウド AI 完全不可 |
| **LGWAN 系** | 自治体間専用網、外部遮断 | 自治体間連携文書 | クラウド AI 原則不可 |
| **インターネット系** | 通常のインターネット | Web 閲覧・職員間メール | クラウド AI 利用可 |
...
先行自治体の事例では検証用 PC の調達ルートに 3 パターンあります。第一に情シスが「AI 検証用スタンドアロン PC」として 1-3 台共用機を整備する方式 (横須賀市・つくば市等で確認)、第二に各部署が消耗品予算 (10-20 万円) で単年度購入する方式、第三に職員研修・自己研鑽枠で個人 PC を業務利用申請する方式です。第三は規程整備が追いついていない自治体が多く、業務利用承認 + データ取扱誓約書のセットが必要になります。Ollama の検証用途であれば 16GB メモリの中古 PC (5-8 万円) で十分動作するため、第二パターンが最も着手しやすい現実解です。
...
業務 PC が Windows の場合はインストーラ exe を情シスに事前申請して許可をもらう経路が多いです。Homebrew や winget が使えない閉鎖環境では、別 PC で `.zip` を取得 → USB 転送 → 展開、というフローになります。
...
| モデル | サイズ | RAM 要件 | 日本語 | 推奨用途 | 取得コマンド |
|---|---|---|---|---|---|
| Qwen 2.5 7B | 4.4GB | 8GB+ | 高 | 議事録要約・翻訳・文体調整 | `ollama pull qwen2.5:7b` |
| Llama 3.1 Swallow 8B | 4.7GB | 8GB+ | 高 | 日本語生成全般 | `ollama pull okamototk/llama-swallow:8b` |
| Gemma 3 12B | 7.5GB | 16GB+ | 中 | 複雑な要約 | `ollama pull gemma3:12b` |
| Phi 4 14B | 9GB | 16GB+ | 中 | 推論・コード | `ollama pull phi4` |
| Qwen 2.5 14B | 8.7GB | 16GB+ | 高 | 高精度日本語タスク | `ollama pull qwen2.5:14b` |

日本語業務なら **Qwen 2.5** または **Llama 3.1 Swallow** が現状ベスト。Swallow は東京工業大学の研究グループが日本語データで継続学習したモデルで、行政文書との相性が良いです。
...
> 📸 [スクリーンショット] Wi-Fi をオフにした状態の Mac で `ollama run` が応答している様子 (機内モード表示 + ターミナル応答を 1 画面に収めた状態)
...
M2 MacBook Air 16GB + Qwen 2.5 7B + litellm proxy の構成で動作検証した事例では、応答速度は 12-18 token/sec、初回応答開始まで 1-3 秒という結果が得られています。実用判定の感覚としては、(a) 議事録の 5 項目要約・苦情対応文の語調チェック・短文翻訳は十分実用、(b) 1500 字超の文書リライトは集中力が切れて文脈が崩れる、(c) 議会答弁の論点分解や政策立案補助は精度不足で実用に届かない、という線引きが現実的です。「個人情報含有 × 単純作業」のみに用途を絞れば、クラウド AI が使えない領域で確実な戦力になります。Windows 11 + WSL2 + 同モデルでも同等の動作速度で、Mac との差はほぼ体感されません。

### Step 4: オフライン環境での運用設計

「Wi-Fi を物理的に切った状態でも動く」を定期検証することが、情シスへの説明根拠になります。
...
| タスク | ローカル LLM (Qwen 7B 等) | クラウド (Claude) |
|---|---|---|
| 個人情報を含む要約 | ◎ (唯一の選択肢) | ✗ (情報漏洩) |
| 議事録の簡易要約 (匿名化済) | ○ | ◎ |
| 公文書のリライト (文体調整) | △ | ◎ |
| 複雑な政策立案補助 | ✗ | ◎ |
| 日英・英日翻訳 | ○ | ◎ |
...
ローカル LLM は「クラウド AI が使えない場面でも諦めない」ための最重要オプションです。精度はクラウドに劣りますが、「データが絶対に外に出ない」という性質は公務員業務において代えがたい価値があります。

基幹系・LGWAN 系・インターネット系の 3 層構造に合わせた AI 利用ポートフォリオを設計し、ローカル LLM を「禁止区域での唯一の選択肢」として位置づけましょう。最初の構築に 2-3 時間、月次運用は無人。「住民情報を扱う業務でも AI が使える」という新しい働き方が拓けます。
...
| モデル | 網羅性 (5点) | 固有名詞保持率 | 平均応答時間 | 誤情報混入率 |
|---|---|---|---|---|
| Qwen 2.5 7B | (実測値) | (実測値) | (実測値) | (実測値) |
| Qwen 2.5 14B | (実測値) | (実測値) | (実測値) | (実測値) |
| Llama 3.1 Swallow 8B | (実測値) | (実測値) | (実測値) | (実測値) |
| Gemma 3 12B | (実測値) | (実測値) | (実測値) | (実測値) |
| Phi 4 14B | (実測値) | (実測値) | (実測値) | (実測値) |
...
評価データには公開議事録の利用が必須です。代表的な選定理由として、(1) 中核市以上の自治体 (人口 20-50 万人規模) の本会議会議録は議題が多様で評価ノイズが減る、(2) 直近 1-2 年 (2024-2025 年) のデータは政策論点が時事的で AI の知識カットオフ差の影響を切り分けやすい、(3) 会派構成・議席数が公開されているため固有名詞検証が容易、の 3 点が挙げられます。事例として東京都 23 区・政令市・中核市の公開会議録から無作為抽出 50 本を用いる手法が再現性の観点で推奨されます。
