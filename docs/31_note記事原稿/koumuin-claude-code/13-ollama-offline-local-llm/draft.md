...
note_url: https://note.com/stats47/n/n1e72ba0b4a43
published: true
...
- LGWAN・基幹系端末で外部 API が遮断され、AI を業務に使えない自治体職員
- 個人情報・基幹系データを扱うため、クラウド AI を選べない部署の担当者
- データを PC の外に出さず、完全オフラインで AI を動かしたい方

**この記事でわかること**

- Ollama で OSS の LLM をローカル実行し、Claude Code の UI のまま推論を切り替える手順
- 16GB メモリの一般 PC で 7B モデルを実用速度で動かす構成
- 「個人情報含有の単純作業」専用にローカル LLM を振り分ける使い分けの考え方
...
<!-- SVG: terminal | Wi-Fi をオフにした状態の Mac で `ollama run` が応答している様子 -->
