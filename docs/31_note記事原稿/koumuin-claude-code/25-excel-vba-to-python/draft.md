...
予算係の棚の奥に「予算執行月次集計.xlsm」というファイルがあります。**2005 年に当時の係長が書いた VBA マクロ、400 行**。書いた本人はとうに退職し、コメントは無く、`Range("AZ65536")` と `Cells(1,1)` が混在し、シート名は「Sheet1」「データ」「集計」「集計2」「集計2_old」が並びます。

それでも毎月誰かが「マクロを有効にしますか? → はい → F9」という呪文を頼りに使い続けています。Office 365 への更新で動かなくなった日、係内が止まります。

本稿は、この VBA レガシー資産を Claude Code を使って Python に移植する実務手順を、5 段階のステップで示します。完全自動移植ではなく、**AI が 7 割(コード移植・テスト生成)、人間が 3 割(業務理解の検証・廃止判定)**で再生する現実的な方法です。最初の 1 本を 3-4 時間で動かします。

「動いているがメンテ担当が不在の VBA マクロ」は、**中規模市の事業部単位で 10-30 個、全庁では 100-500 個規模**で存在するケースが報告されています。用途別の典型例は次の 5 系統に集中します。

- 予算執行月次集計 (各課または予算係に 1 個ずつ)
- 勤怠・超過勤務集計と給与システム連携データ生成 (人事課に 3-5 個)
- 固定資産税の課税明細整形 (税務課に 5-10 個)
- 公共施設利用統計の集計 (各施設の所管課ごと)
- 住民意見の集計と回答取りまとめ (広報・市民協働系)

作成時期は 2000 年代前半-2010 年代前半が中心で、**作者の異動・退職率が 60-80%** に達しているという内部調査結果も自治体 IT 担当者の発表で公表されています。

**こんな方に向けた記事です**

- 作者不在でブラックボックス化した VBA マクロを抱え、Office 更新で止まる前に移植したい予算・人事・税務系の職員
- 「マクロを有効にしますか」の呪文で古い .xlsm を使い続けていることに不安がある方
- 20 年もののレガシー資産を、今後 20 年メンテできる Python の形に再生したい方

**この記事でわかること**

- VBA を Claude Code で Python (openpyxl + pandas) へ移植する 5 段階の実務手順
- 字面の翻訳で終わらせないための「業務理解を先にさせる」プロンプトの使い方
- 移植と同時に廃止判断を行い、VBA の隠れバグを並列テストで潰す進め方
...
VBA マクロは、1990 年代後半〜2010 年代前半の自治体 IT 化期に大量生産されました。当時の典型パターンは次のとおりです。
...
これが 2026 年現在、3 つの理由で限界に来ています。

![表: 限界 / 内容 / 影響](./images/table-1.png)
<!-- SVG: table | 限界 / 内容 / 影響 -->

総務省「自治体 DX 推進計画」でも、レガシーシステム(Excel マクロ含む)の刷新が明示的に掲げられています。Claude Code を使えば、Python 移植のハードルは大きく下がります。**Python なら今後 20 年は安心**、と判断する材料が揃いました。

Excel マクロが動かなくなる事例は、Office のバージョン更新タイミングで集中して発生します。2020 年代に多く報告されたのは次の 3 系統です。

- Office 2016 → Office 365 移行で `Range("A1").Activate` の挙動が変わり、画面遷移を前提とした VBA が無限ループに陥るケース
- Excel のセキュリティ強化で「インターネットから取得したファイルのマクロ実行ブロック」が既定動作となり、係内共有フォルダ経由のファイル配布で「マクロを有効にできない」事例
- Windows 11 への OS 移行で 32bit ActiveX 参照が失敗するケース

マクロ停止が業務に直撃すると、**復旧までの半日-3 日間は手作業で集計表を作る対応**となり、係長級が休日出勤で対応する例も自治体 IT 障害報告で散見されます。

## 手順 / 解説

### ステップ 1: 移植対象の選定(3 軸判定 + 廃止検討)

全 VBA マクロを移植しようとすると挫折します。優先順位を 3 軸で判定します。

![表: 軸 / 優先度高 / 優先度低](./images/table-2.png)
<!-- SVG: table | 軸 / 優先度高 / 優先度低 -->

**3 軸すべて「高」のマクロが最優先**です。逆に「年 1 回 + 代替あり + 単純」なら、廃止か手作業で十分です。

判定の前に必ず「そもそもこの業務は要るのか」を問います。例えば「決裁簿の月次集計マクロ」が紙決裁前提で書かれているものの、現在は電子決裁に移行済み、というケースがあります。これはマクロごと廃止が正解です。

### ステップ 2: VBA コードを Claude に読ませる(文字コード変換必須)

Excel ファイルから VBA を抽出します。
...
抽出した .bas ファイルは Shift_JIS で出力されることが多いです。UTF-8 に変換してから Claude に渡します。
...
いきなり「Python に変換して」と頼むと、コードの字面だけ翻訳された使えないものが出ます。先に業務理解させます。
...
ここで Claude が出した業務理解を、**人間(できれば元業務担当者・後任者)が読んで「合っている / 違っている」を判定します**。違っていれば修正して、Claude に「では正しい業務理解を踏まえて再度説明して」と返します。
...
不明点を業務担当者に確認 → 回答を踏まえて再度業務理解させます。これを 1-2 回繰り返します。

レガシー VBA の難読パターンとして自治体現場で語られる典型例は、次の 4 系統です。

- 1 つの Sub 関数が 300-500 行で、ネスト 5-6 段、グローバル変数 30-50 個が散在するケース
- `Application.OnTime` を使った遅延実行と `DoEvents` の組み合わせで、シート再計算の順序に依存する非同期処理
- `Worksheet_Change` イベントで別シートのセルを書き換え、それが連鎖的に複数イベントを発火させる「イベント連鎖型」
- 印刷用シートを毎回複製して `_old1` `_old2` `_old3` と残し続ける「履歴保持型」(シート数が 50-100 枚に膨張する例も)

いずれも作者の意図がコメントから読み取れず、移植時は**コードを動かして挙動を観察する作業が業務理解の起点**となります。

### ステップ 4: Python 移植プロンプト

業務理解が合致したら、Python 移植に進みます。要件を明示します。
...
移植後は必ず「同じ入力で VBA と Python の出力が一致するか」を検証します。Claude にテストパターンを生成させます。
...
VBA マクロの「怪しい挙動」として現場で報告される典型ポイントは、次の 4 系統です。

- 月末日の集計で 30 日と 31 日で結果が変わるケース (`Day(Date)` ベースの分岐ロジックが月末対応していない)
- 年度切替期の 3 月 31 日 → 4 月 1 日跨ぎで前年度データが消えるケース (`Year(Date)` ベースの会計年度判定にずれ)
- 課コードに枝番 (例: 1101-1 と 1101-2) が混在すると片方が脱落するケース (文字列比較の半角全角混在)
- 1,000 行を超えると `For Each cell In Range` が極端に遅くなる (`ScreenUpdating = False` を入れ忘れた典型)

移植時はまずこれら 4 点をテストケースに含め、**VBA 側の隠れバグを Python 側で潰す方針**が現実的になります。
...
5. 「動いている」を信用しない: VBA で動いている = 正しい、ではありません。移植時に隠れバグが見つかることが多いです。並列テストで両者の差分が出たら、VBA 側を疑う癖をつけましょう
6. **配列とコレクションの罠**: VBA の 1-indexed と Python の 0-indexed のズレで、月初の 1 件が欠落するパターンが頻発。境界値テスト必須

## まとめ

VBA → Python 移植は、Claude Code を使えば**1 マクロあたり 3-4 時間で完了します**(従来は 1-2 週間)。鍵は次の 3 つです。

- 業務理解を先にやらせる(人間が検証)
- 並列テストで VBA vs Python の差分を見る
- 廃止判断を同時にする

20 年もののレガシー資産を、向こう 20 年メンテできる形に再生する作業に、AI は最適なパートナーです。最初の 1 本を選び、**月次定例日の翌日に着手するのがおすすめ**です(直近の実行ログが温かいため)。
...
作者不在でブラックボックス化した VBA マクロを抱え、Office 更新で止まる前に Python へ移植したい予算・人事・税務系の職員の方。5 段階の進め方は無料部でわかっても、実際の移植には移植前後のコード全文・3 種のプロンプト完全版・廃止判定シートが要ります。最初の 1 本を確実に動かしたい方に向いています。
...
「予算執行月次集計マクロ」を題材に、移植前 VBA(400 行)→ 移植後 Python(280 行)を全文掲載します。
...
移植難度が高いと報告される VBA の典型コードパターンは、「全課・全科目をループでクロス集計するロジック」です。具体的には外側ループで `For sectionIndex = 1 To 50`、内側ループで `For accountIndex = 1 To 200` を回し、各セルに対して `If Worksheets("マスタ").Cells(sectionIndex, 2).Value = wsData.Cells(i, "B").Value Then` のような線形検索を入れる構造になっています。

計算量が O(n×m×k) となり、**データが 10,000 行を超えると処理時間が 5-30 分に膨張します**。Python では `pandas.merge` と `pivot_table` で同等処理を O(n+m+k) に圧縮でき、処理時間が 0.5-3 秒に短縮されます。

難読箇所の核は「ループ内に検索ロジックが埋め込まれている」点で、移植時は検索とループを分離する設計判断が必要となります。

### 有料セクション 2: 3 種のプロンプト完全版

業務理解 / 移植 / テスト の 3 プロンプトをそれぞれ 30-50 行で詳細化しています。コピペで使えます。出力フォーマット指定 + 不明点リスト要求 + 段階確認の 3 構造を採用しています。
...
合計スコアで「即移植」「移植検討」「廃止検討」「現状維持可」の 4 段階で判定します。
...
![表: VBA / Python (openpyxl/pandas)](./images/table-3.png)
<!-- SVG: table | VBA / Python (openpyxl/pandas) -->

20 パターン全て、入出力サンプル + 性能比較(VBA 30 秒 → Python 0.5 秒の例多数)を収録しています。

庁内勉強会で生成 AI を扱った事例では、参加者の典型反応として次の 3 系統が集中します。

- 「個人情報を入れたら漏れるのでは」というセキュリティ懸念 (参加者の 6-8 割が最初に質問する)
- 「決裁・稟議をどう通せばよいか」という導入手続きの質問
- 「マクロが動かなくなった時の業務継続をどう保証するか」という運用懸念

つまずきポイントは次の 3 点。

- `pip install` でのライブラリ導入が情報セキュリティポリシー上できない自治体での代替手段 (社内パッケージミラーや Anaconda 包括契約)
- Python のインデント構文に慣れない VBA 経験者が複数行貼り付け時にエラーを起こす
- ファイルパスの絶対参照と相対参照の使い分け

研修時はこの 3 点を**最初の 30 分で重点解説する構成**が定着しています。
