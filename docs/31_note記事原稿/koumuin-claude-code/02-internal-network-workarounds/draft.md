...
note_url: https://note.com/stats47/n/n974a0b7cde75
published: true
...
「Claude Code を試したいのに、庁内ネットワークでは外部 API がブロックされて動かない」——多くの自治体職員が直面する問題です。

本記事では、技術的に可能な 3 つのルートと、それぞれのリスク・コスト・現実的な実行難易度を正直に整理します。

Claude Code を業務で本格的に使うなら、最終的なゴールは **会社（庁内）PC で動かせる状態にすること** です。

そのための本筋は **抜け道 2: 庁内 PC + プロキシ正攻法**——情シスにドメイン許可を取り、正規の手順で会社 PC を利用環境にする道です。

情シス交渉に時間はかかりますが、**業務データを扱える形で本格利用できるのはこのルートだけ**です。

**抜け道 1: 私物 PC + モバイル WiFi** は、その正式ルートが通るまでの「早期着手手段」——先に手を動かして実例を作り、情シス交渉を有利に進めるための繋ぎとして位置づけます。

**こんな方に向けた記事です**

- 庁内ネットワークで外部 API がブロックされ、Claude Code を試せずにいる自治体職員
- 会社 PC で本格利用したいが、情シスへのドメイン許可申請の進め方が分からない方
- 私物 PC で先に触るべきか、正式ルートが通るのを待つべきか判断に迷っている方

**この記事でわかること**

- 私物 PC・庁内プロキシ・VDI という 3 つのルートそれぞれのリスク・コスト・実行難易度
- 業務データを扱える本筋ルート (抜け道 2) を情シスに通すための準備と交渉の順序
- 懲戒対象になりかねない絶対 NG パターン (3 層分離違反など) の線引き

執筆者は元自治体職員で、複数の庁内ネットワーク制約を実体験しています。現在は Claude Code を使い、47 都道府県の統計サイト stats47.jp（約 2,000 のランキングを毎日自動更新）を個人で開発・運用しています。

人口 10-30 万人規模の中核市・一般市では、インターネット系端末のホワイトリストに `api.anthropic.com` `registry.npmjs.org` `github.com` のいずれも入っていないケースがほとんどです。

`curl https://api.anthropic.com` がプロキシ手前で 403 を返し、原因切り分けに半日かかる、というつまずきが典型的に発生します。

新規ドメイン許可申請を出すと「業務利用の必要性」「セキュリティ評価書」「他自治体導入実績」の 3 点セットを求められ、**申請から承認まで 2-3 ヶ月**かかるのが実情です。

時間はかかりますが、ここを通さないと業務での本格利用には到達しません。だからこそ、私物 PC で先に成果を作り、その実例を持って交渉に臨むのが現実的な進め方になります。
...
外部 API への接続は **インターネット系端末** でのみ可能ですが、そこもプロキシ経由で **許可ドメインのホワイトリスト運用** が一般的です。

Anthropic API (`api.anthropic.com`) はデフォルトで許可されていません。npm registry (`registry.npmjs.org`) や GitHub (`github.com`) も同様にブロックされているケースが多いです。

![自治体ネットワーク 3 層分離と Claude Code が必要とする通信経路](./images/structure-1-3layer-network.png)
<!-- SVG: structure | 3 層分離 + Claude Code 通信経路 -->

申請して許可を取る道もありますが、「業務利用の必要性」「セキュリティ評価」「監査ログ取得」などの説明資料が必要で、**現場の一職員が数週間で通せるレベルではありません**。

政令市・中核市では特に厳しく、申請から承認まで 2-3 ヶ月かかることも珍しくありません。

許可申請で必要になる典型的な書類:

![表: 書類 / 内容](./images/table-1.png)
<!-- SVG: table | 書類 / 内容 -->

外部ドメイン許可申請の現場感としては、自治体規模で傾向が分かれます。

- **町村・小規模自治体**: 情シスが兼務で柔軟な判断が下しやすく、申請から承認まで 2-4 週間で通ることも
- **一般市 (人口 5-20 万)**: 「上司決裁 + 情シス課長決裁」の二段階で 1-2 ヶ月
- **中核市 (20-70 万)**: 「セキュリティ評価書 + 撤退手順 + 他自治体事例」を要求され 3-6 ヶ月
- **政令市**: 「先行政令市の導入事例がないと検討も始まらない」という前例主義の壁が高くなる

落とされた場合の代表的理由は「他自治体導入事例の不足」と「監査ログ仕様の曖昧さ」で、この 2 点を予め埋めると承認率が大きく上がります。

## 抜け道 1: 私物 PC + モバイル WiFi（早期着手手段 / 正式ルートが通るまでの繋ぎ）

庁内ネットワークを一切触らないため、即日始められて組織のセキュリティポリシーとも衝突しません。

会社 PC の正式利用（抜け道 2）が情シス交渉で時間を要する間、**先に手を動かして「議事録作成が 30 分 → 5 分」のような実例を作っておく**ための手段として有効です。この実例が、抜け道 2 の許可申請を通す際の最も強い材料になります。

なお、これはあくまで繋ぎであり、業務データを扱う本格利用には会社 PC（抜け道 2）への移行が前提です。
...
![表: 項目 / 内容](./images/table-2.png)
<!-- SVG: table | 項目 / 内容 -->

![私物 PC ルートのデイリーフロー (朝の接続から知見蓄積まで)](./images/flow-1-personal-pc-daily.png)
<!-- SVG: flow | 私物 PC ルートのデイリーフロー -->

## 抜け道 2: 庁内 PC + プロキシ正攻法（会社 PC で本格利用する本筋ルート）

組織的に承認を取り、情シスにドメイン許可を依頼する正規ルートです。**業務データを扱える形で Claude Code を本格利用できるのは、このルートだけ**です。

情シス交渉に時間はかかりますが、Claude Code を職場の実務に組み込むなら、ここを通すことが最終的なゴールになります。以下では、その通し方を具体的に整理します。
...
庁内導入の承認可否を分けた事例として現場で語られる典型パターンは 2 つあります。

承認された事例の共通点は、次の 3 点です。

- 申請前に半年〜1 年の個人利用実績 (時短時間の実数値) を作っていた
- 撤退手順を「5 分以内に API キー無効化 + 監査ログ即時提出」の形で具体的に明文化していた
- 「6 ヶ月試行 → 効果検証 → 継続可否判断」のスコープを区切っていた

逆に蹴られた事例は、次のいずれかに該当することが多いと想定されます。

- 「とりあえず使わせてください」と業務利用計画が曖昧
- Anthropic 社の Trust Center URL や Data Usage Policy を引用していない
- 他自治体事例ゼロ
...
VDI 環境が AI ツール経由で利用可能か否かは、自治体ごとの差が極めて大きいのが現状です。

Amazon WorkSpaces や Microsoft AVD を導入済みの自治体では、VDI 内部のネットワークがインターネットフリーに近い設計のケースもあります。

このような環境では、Claude Code を含む外部 API 利用が事実上可能な事例があります。

一方、Citrix ベースで「業務専用 VDI」を構築している自治体では、外部 API は VDI 側で同様にブロックされていることが多く、抜け道として機能しません。

導入時期 (2020 年以降のテレワーク前提構成か否か) と委託事業者のセキュリティ要件で結論が分かれるため、該当 VDI がある場合は情シスに「VDI 内から外部 API を叩いて良いか」を最初に確認するのが現実的です。
...
庁内ネットワークで Claude Code を動かす方法は技術的には 3 つありますが、**業務で本格的に使うゴールは「会社 PC を正規の利用環境にする」こと**——つまり抜け道 2（庁内 PC + プロキシ正攻法）です。

抜け道 1（私物 PC）はそこへ至るまでの早期着手手段、抜け道 3（VDI）は環境が合えば使える別ルートという位置づけになります。

進め方の順序はこうです。まず私物 PC で手を動かし、業務改善の実例（議事録作成時間が 30 分 → 5 分等） を作る。次にその実例とセキュリティ説明資料をそろえて情シスに申請し、会社 PC でのプロキシ正攻法（抜け道 2）を通す。

実例ゼロで「Claude Code を導入したい」と言っても、情シスから返ってくるのは「他自治体の事例は?」「セキュリティ評価書は?」の連打で終わります。

私物 PC での実例づくりは、この交渉を通すための準備工程です。

![会社 PC 正規利用までの 3 フェーズロードマップ (私物 PC で実例づくり → 情シス交渉 → 会社 PC 本格利用)](./images/flow-2-phase-roadmap.png)
<!-- SVG: flow | 会社 PC 正規利用までの 3 フェーズ ロードマップ -->
