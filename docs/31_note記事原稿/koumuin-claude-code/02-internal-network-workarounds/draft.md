...
- **抜け道 2 が本筋**: 庁内 PC + プロキシ正攻法 (ドメイン許可申請) — 会社 PC で業務利用するための正規ルート。情シス交渉に 2-3 ヶ月かかるが、本格利用はこの道
- **抜け道 1 は早期着手手段**: 私物 PC + モバイル WiFi / テザリング — 正式ルートが通るまでの繋ぎ。先に実例を作り交渉を有利にする
- **抜け道 3**: VDI / リモートデスクトップ経由 — 庁内 VDI 環境次第、監査責任が曖昧でグレー
- どのルートでも「庁内データを送らない」ルールは抜け道 2 が通るまでは絶対。技術より運用ルールの整備が先
