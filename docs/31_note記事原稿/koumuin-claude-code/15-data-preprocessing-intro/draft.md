...
note_url: https://note.com/stats47/n/naf04e81afb7b
published: true
...
データ自体は e-Stat や自治体オープンデータポータルから無料で取れます。が、**ダウンロードした CSV / Excel を答弁資料・レク資料に使える形に整えるまで**が、地味で膨大な時間を奪います。
...
- 同じ統計でも年度ごとに項目構成や CSV ヘッダが変わります
- 市町村合併で過去データの地域コードが変動し時系列接続が壊れます
- 「-」「***」「秘匿」など欠損表現が複数混在し統一処理が必要です

**こんな方に向けた記事です**

- 答弁資料やレク資料のために e-Stat の CSV / Excel を整形している統計・企画・政策担当
- ヘッダ複数行・セル結合・注釈混入で「そのまま読めない」統計ファイルに手を焼いている方
- Python の知識がなくても、データ前処理を自然言語で任せる方法を知りたい方

**この記事でわかること**

- e-Stat 実データを例に、クリーニング・整形・集計・検証を Claude Code に任せる手順
- 「セル結合 / ヘッダ複数行 / 注釈混入」の 3 大障害を tidy データに直す流れ
- 単位・年度・地域コードの罠を人間が検証すべき勘所

執筆者は元自治体職員です。現在は Claude Code を使い、47 都道府県の統計サイト stats47.jp（約 2,000 のランキングを毎日自動更新）を個人で開発・運用しています。stats47 が毎日行っている処理は、まさに本記事のテーマである「公開統計の生データを、集計・可視化に使える形へ整える前処理」そのものです。
...
統計データは公務員にとって最も信頼できる定量根拠ですが、**扱いの面倒さは民間ビジネスデータの比ではありません**。原因は配布フォーマットが 「人間が見やすい Excel」を前提に設計されているためです。
...
`pd.read_csv` でそのまま読むと、列名は `Unnamed: 0`、データ部に注釈テキストが混入します。

1 ファイルなら頑張れますが、**複数年度 × 複数地域 × 複数指標を横断する作業**になると現実的ではありません。

さらに公務員特有の事情として:

- **配布形式が選べない** — e-Stat の一部統計は CSV / Excel / DB API の混在で、年度ごとに形式が変わります
- **市町村合併で過去データの地域コードが変動** — 時系列分析の重大バグ原因になります
- **元号 / 西暦混在** — 「令和 5 年度」「平成 30 年度」「2020 年度」が同じ表に併存します
- **「秘匿」「-」「*」表示** — 数値の欠損が複数の表現で混在します
...
**必ず原典との突合検証コードを最後に置きます**。Claude Code への指示にも「検証コードを付けてください」と明記します。
...
これらはいずれも AI が出力する数値をそのままコピペした際に発生しやすく、単位確認は人間チェックの最重要項目です。「数値を出した直後に必ず単位を声に出して読む」運用ルールが現場で有効とされています。

## よくあるつまずきポイント

1. **CSV の文字コード (Shift_JIS vs UTF-8)** — e-Stat は SJIS 配布が多いです。`encoding='cp932'` を指定するか、`chardet` で自動判定します
2. **数値列に記号混入 (「-」「***」「X」「秘匿」)** — `pd.to_numeric(errors='coerce')` で欠損化し、欠損のまま集計か 0 埋めかは指標で判断します
3. 「合計」「総数」行をデータと一緒に集計 — 二重計上で値が倍になります。`isin(都道府県リスト)` で必ずフィルタします
4. **市町村合併で過去データの地域コード変動** — 平成大合併以降の時系列分析は対応表が必須です。e-Stat の「市町村コード対応表」を活用します
5. **生成 AI が「もっともらしい数字」を出す** — Hallucination の典型です。**原典との突合検証コードを必ず最後に置きます**
6. **Excel で開くと文字化け** — UTF-8 BOM 付き (`encoding='utf-8-sig'`) で出力するか、.xlsx で出力します
...
重要なのは AI に **丸投げしない** ことです。単位確認・合計突合・原典との照合は人間の責任です。この役割分担さえできれば、議会答弁資料・首長レク資料の作成時間が桁違いに短縮されます。
...
`.claude/skills/stats/preprocess-estat/SKILL.md` のテンプレを提供します。**原典突合検証を内蔵** しているのが他のテンプレと違うポイントです。
...
実装ファイル一式 (約 350 行、5 統計種別対応) を完全提供します。CI に組み込めば、毎月の e-Stat 更新時に自動で検証が走ります。
...
検証コードを内蔵することで、過去には e-Stat 配布 CSV の一部年度で「全国計が原典公表値と数十人ずれる」「特定地域コードのレコードが脱落する」といったデータ側のバグも自動検知できた事例があり、AI スキル化の副次効果として「原典データの品質チェック」も同時に行える点が現場で評価されています。
