...
note_url: https://note.com/stats47/n/nc5126820c00d
published: true
...
「Claude Code を使ってみたいけれど、庁内 PC では何から始めればいいかわからない」「IT に詳しくないし、インストール手順を見ても専門用語ばかりで詰まる」——自治体の現場で最もよく聞かれる相談です。

本記事は、自治体特有の制約 (管理者権限なし / プロキシ閉域 / 私物 PC 持ち込み禁止 / API キー取得の稟議が通らない) を踏まえた完全マニュアルです。

Windows / Mac / WSL2 の 3 環境で Claude Code を「30 分以内に起動し、自分のプロジェクトディレクトリで対話できる」状態まで持っていくことを目指します。

**こんな方に向けた記事です**

- 庁内 PC で Claude Code を使いたいが、管理者権限やプロキシ制約で何から手をつければよいか分からない自治体職員
- npm の permission denied・E407・SSL 証明書エラーで詰まり、民間向けの解説では解決できずにいる方
- ターミナル操作に不安があり、まずは VS Code の中で Claude Code を試したい方

**この記事でわかること**

- Windows / Mac / WSL2 の 3 環境で Claude Code を 30 分以内に起動するまでの具体的な手順
- 庁内ネットワークを通すためのプロキシ設定 3 点セット (環境変数・npm・CA 証明書) のやり方
- 会社 PC を本筋・私物 PC をお試しと整理した、現実的な導入ルートの選び方
...
`choco install`・`brew install`・`apt install` で一発インストールという発想が通用しません。情シスが許可した範囲のソフトだけがインストール済みで、新規追加には申請書が必要です。Node.js も自前で入れる手段がない場合があります。
...
自治体ネットワークは多くの場合 3 層構成 (LGWAN / 庁内系 / インターネット系) で、外部 API への接続は「インターネット系」端末でのみ可能です。さらにそのインターネット系もプロキシ経由で、許可ドメインのホワイトリスト運用です。
...
技術的に動かせても、組織として使ってよい範囲を整理しないと「使用停止」の指示がいつ飛んでくるか分かりません。

「Claude API に何のデータを送るのか」「ログはどこに残るのか」「学習に使われないか」——**情シスが必ず聞く 3 点**に答えられないと止まります。これは [#03 自治体 IT 担当に渡せる Claude Code セキュリティ説明資料](../03-it-dept-security-doc/draft.md) で詳述します。
...
- ツール購入ではなく「会社 PC に既存ソフト（VS Code 等）を入れ、個人契約の API キーで動かす」構成として申請し、論点を「新規ソフト導入」から「既存環境の利用」に寄せます
- Anthropic 公式 Trust Center (https://trust.anthropic.com/) と Data Usage Policy の URL を最初の相談メールに添付します
- 「6 ヶ月試行 → 報告」のスコープを切ります
...
[https://console.anthropic.com](https://console.anthropic.com) で **個人メール (Gmail 等)** のアカウントを作成します。**職場メールアドレスは使わない**でください。理由は 2 つあります。

1. 退職・異動時にアカウントがロックされ、API キーごと使えなくなります
2. 職場メールでサインアップした履歴が監査で「業務利用」と判断される可能性があります

Claude Code 利用には **Pro プラン (月額 $20)** または **API クレジット** が必要です。月額の負担感は ¥3,000 前後から。Pro プランはブラウザ版 Claude も併用できるので、最初は Pro を推奨します。

### 1-2. API キーの取得と保管

API キーは `sk-ant-api03-` で始まる長い文字列です。以下は **絶対 NG** です。

- メモ帳に貼って庁内ファイルサーバに置く
- メール添付・Slack DM・チャットツール送信
- スクリーンショットを撮って画像保存
- git にコミット (`.env` を `.gitignore` し忘れる事故が頻発します)
...
ファイルに直書きする場合は **`chmod 600`** で他ユーザー読み取り不可にします。
...
**庁内 PC・庁内ファイルサーバ・共有フォルダ・メール添付は全て不可**で、これは情シスから「キーは個人 PC 内のみで保管」と明文で運用ルール化しているケースが多いためです。

万一の漏洩時に責任の所在が個人で明確化されることが、組織側の許容ポイントになっています。
...
`npm install -g` で **permission denied** が出る場合は、グローバルインストール先を `$HOME` 配下に変更します。
...
ここまでで Mac 環境の構築は完了です。所要時間は実質 15-20 分です。

## 手順 3: Windows での構築 (WSL2 推奨)

Windows ネイティブで動かすことも可能ですが、以下の理由で **WSL2 (Ubuntu 22.04)** を強く推奨します。

- ネイティブ Windows では `npm install` 時に **long path 問題** (260 文字制限) で詰まります
- 改行コード CRLF と LF の混在で `bash` スクリプトが動きません
- PowerShell 実行ポリシーの解除に管理者権限が必要です
- VS Code + WSL は **Remote-WSL 拡張** で快適に統合できます
...
再起動後、Ubuntu のセットアップが始まります。ユーザー名・パスワードを設定してください (このパスワードは sudo で頻繁に使うので忘れないようにしましょう)。

なお WSL2 の有効化には管理者権限が必要です。会社 PC で管理者権限がない場合は、次のいずれかの進め方になります。

- 情シスに「WSL2 の有効化」だけを依頼する（インストール作業そのものは数分で終わります）
...
WSL 内の `/home/<user>/work/` は Windows 側からは `\\wsl$\Ubuntu-22.04\home\<user>\work\` でアクセスできます。エクスプローラーで開くなら次のコマンドを使います。
...
逆に Windows のファイルを WSL から触る場合は `/mnt/c/Users/<user>/...` でアクセスできます。

Windows ネイティブで Claude Code を動かそうとした事例では、以下の事象が連鎖的に発生します。

- PowerShell の実行ポリシー (`Restricted` 既定) で `npm` 関連スクリプトが起動段階で弾かれます
- `node_modules` 配下のパスが Windows の 260 文字制限 (long path 問題) に引っかかって `ENAMETOOLONG` が出ます
- git bash と PowerShell で改行コードが CRLF と LF で混在しシェルスクリプトが動作しません
...
VS Code に **Remote-WSL 拡張** をインストール後、WSL ターミナルで次のコマンドを実行します。
...
これで VS Code が Ubuntu 内のファイルを直接編集できます。Windows と WSL を 1 つの IDE で扱える状態が完成します。
...
VS Code を入れれば、その**統合ターミナル**（VS Code の下部に開くパネル）の中で `claude` を起動でき、左側のファイルエクスプローラで生成・編集されたファイルをそのまま目視確認できます。

単独の真っ黒なターミナル画面と向き合う必要はなく、Word のような GUI に近い感覚で扱えます。
...
「ファイルを自動で整える・繰り返し作業を自動化する」という Claude Code の強みは Claude Desktop にはありませんが、「AI とのやり取り自体に慣れる」最初の一歩としては有効です。

Claude Desktop と Claude Code の役割の違いは [#00 Claude Code とは何か](../00-claude-code-intro-for-public-servants/draft.md) で整理しています。
...
npm の `unable to verify the first certificate` エラーは、社内プロキシが SSL を中間で復号 (SSL インスペクション) しているのが原因です。社内 CA 証明書を取得し、`NODE_EXTRA_CA_CERTS` に設定します。
...
1. ブラウザで `https://www.google.com` を開き、アドレスバーの鍵アイコンから「証明書を表示」します
2. 証明書チェーンの **ルート** が社内 CA 名になっていればそれを書き出します
3. 書き出した `.cer` または `.crt` を `.pem` 形式に変換します (`openssl x509 -inform DER -in ca.cer -out ca.pem`)

情シスから直接 `.pem` を受け取れる場合はその方が早いです。「Node.js (npm) のために CA 証明書が必要」と申請書に書きます。

社内 CA 証明書 (`.pem`) を情シスから受け取る際は、申請書に次の 3 点を書くと話が早く進む傾向があります。

- Node.js (npm) が HTTPS 経路で外部 registry に接続するために必要です
- 保管場所は本人 PC の `~/certs/` 配下、`chmod 600` で他ユーザー読み取り不可にします
- 用途は Claude Code 利用に限定します
...
`api.anthropic.com` がプロキシのホワイトリストに入っていない場合、ここで弾かれます。情シスに **追加申請** が必要です (これが時間かかります)。
...
Claude Code の環境構築は、自治体職員にとって「技術的な壁」よりも「組織的な壁」(管理者権限・ネットワーク制約・稟議) の方が大きいです。
...
私物 PC + iPhone テザリング運用での実測値の目安としては、Claude Code 1 セッション (1-2 時間の対話 + ファイル編集) で **20-50 MB 程度の通信量**が観測されます。

1 日 2-3 セッションを 20 営業日続けると月 3-5 GB 程度で、大手キャリアの 20 GB プラン (月額 ¥3,000-4,000) で十分賄えます。

推奨モバイル WiFi は楽天モバイル Rakuten Hand 5G または Pixel 8a のテザリング機能 (バッテリー持ち良好) が定番です。

コスト総額は Anthropic Pro $20 + テザリング ¥3,000-4,000 + 端末減価償却 ¥4,000 = **月 ¥10,000-11,000** が目安です。
...
フォントは MS 明朝 10.5pt、上下マージン 25mm が読みやすさの目安です。
...
`~/.claude/settings.json` の hooks 機能で全プロンプトを TSV に自動記録する設定例です。情シスへの「監査ログ取れます」回答に必須です。
...
- **情シスに止められた** — 許可ドメイン申請が 3 ヶ月以上滞留し、その間に申請者が異動になって自然消滅します
- **上司の理解が得られなかった** — 課長が「AI は危険」というイメージから入ってしまい、技術的説明が届きません
- **監査で指摘された** — 設定不備で監査ログが取れていなかった、または API キーが共有フォルダに置かれていました
