...
note_url: https://note.com/stats47/n/ne4e746d2b0cf
published: true
...
本記事では、Claude Code を使って 「攻め・守り・折衷の 3 案」 を短時間で出すための実用プロンプト集を公開します。

**こんな方に向けた記事です**

- 議会期のたびに答弁原稿の作成で土日が消える、議会事務局・答弁作成担当の職員
- 部長・首長レビューでの方向転換や過去答弁との整合確認に、繰り返し時間を奪われている方
- ゼロから書くのではなく、たたき台を AI に出させて人間が磨く作業に切り替えたい方

**この記事でわかること**

- 答弁案を「攻め・守り・折衷」の 3 パターンで短時間に出させる実用プロンプト
- CLAUDE.md に文体規約 (主語・敬体・字数・禁止語彙) を固定して AI 出力のブレを抑える方法
- Subagents で議員役・市民役から想定再質問を洗い出し、弱点を事前に潰す進め方

執筆者は元自治体職員で、議会事務局および答弁作成補佐の経験があります。現在は Claude Code を使い、47 都道府県の統計サイト stats47.jp（約 2,000 のランキングを毎日自動更新）を個人で開発・運用しています。
...
- **金曜夜の通告 → 月曜本会議**という日程で土日返上を強いられます
- 部長・副市長決裁段階で「過去答弁との整合性が取れていない」と差し戻され、深夜まで関連議事録を遡ります
- 首長レビューで「もっと攻めの姿勢を出せ」または逆に「踏み込みすぎだ」と方向転換を求められ、3 時間で全面書き直しになります
...
1. **政治的配慮** — 議員の質問意図を読み、与党・野党のバランスを取ります
2. **法令準拠** — 関連条例・要綱・要領の範囲内で答えます
3. **首長の方針との整合** — 過去発言・選挙公約・所信表明と矛盾しません
4. **文体の規範** — 自治体ごとの慣習文体 (主語の明示・受動態回避・決まり文句) を守ります
5. **時間制限** — 答弁時間 (1 分・3 分・5 分) に収まる文字数にします
6. **想定再質問対応** — 1 回目答弁の弱点を突く再質問への準備をします
7. **読み上げやすさ** — 首長が初見で読みやすい改行・振り仮名 (難読漢字対策) を施します

これらを同時に満たす原稿を、過去答弁を調べながら一から書くのは大変な作業です。

AI で 「たたき台 3 案」を出してから磨く 方が、ゼロから書くより速いです。これが本記事の核心です。
...
- **主語の選択** — 多くの自治体で「本市」「本県」が標準ですが、町村では「本町」「本村」と規模に応じて使い分けます
- **結びの定型句** — 「お答えいたします」「答弁といたします」「以上、お答え申し上げます」のいずれを採用するかが自治体・首長ごとに異なります
- **議員への呼びかけ方** — 「議員ご指摘の通り」「議員ご質問の」など慣習差があります
- **和暦と西暦の併記ルール** — 令和 8 年 / 2026 年の順序です
- **受動態回避の徹底度合い**
...
- **攻め**: 積極的推進、具体目標年次・施策明示。想定リスク: 後で達成できない場合に追及されます
- **守り**: 慎重姿勢、現状認識と課題整理にとどめます。想定リスク: 「やる気がない」と批判されます
- **折衷**: 推進方針は示すが時期は明言せず、先行事例研究に言及します。想定リスク: 中庸すぎて印象が薄いです
...
公開済みの本会議議事録 (自治体 HP 掲載) から、答弁の **文体パターン** だけを抽出します。固有名詞は全て `◯◯` に置換します:
...
- 自治体公式 HP の本会議録 (PDF) を過去 3-5 年分 (定例議会 12-20 回分) ダウンロードします
- 答弁部分だけ抽出します
- 固有名詞を `◯◯` に置換します
- 文末パターンを正規表現で機械抽出します
...
【この案の強み】
1. 具体数値 (50 手続き / 80%) で説得力があります
2. 法令引用で根拠を示しています
3. 進捗率の現状報告が信頼感を生みます

【この案の弱み】
1. 達成できなかった場合の批判リスクが大きいです
2. デジタルデバイドへの言及がありません
3. セキュリティ対策の言及がありません
...
<!-- SVG: terminal | Claude Code で /draft-3-options 実行後 -->
...
【出力形式】
修正前・修正後・修正理由・重要度 (高/中/低) の 4 項目を番号付きで列挙
...
```text
1. 修正前: これまで複数の取り組みが進められてきており
   修正後: 本市はこれまで複数の取り組みを進めてまいり
   修正理由: 受動態 → 能動態
   重要度: 高
...
長文の答弁案をレビューする際は、Claude Code の **Subagents 機能** で「議員役」「事務局役」「首長役」の 3 視点からそれぞれ再質問を出させると深掘りできます。
...
Subagents の 3 役プロンプトを比較した現場感では、最も実用性が高いと評価されるのは「野党議員役」と「市民傍聴者役」の 2 つです。

野党議員役は「数値の根拠」「過去発言との矛盾」「デメリットへの対応」を厳しく突くため、答弁の弱点を事前に潰すのに直接役立ちます。

市民傍聴者役は「専門用語が分からない」「自分の生活にどう関わるか不明」という素朴な視点を出し、答弁の分かりやすさ改善に寄与します。
...
4. **最終原稿は必ず人間が責任を持つ** (AI 案は素材であって完成品ではありません)
5. 個人名・実際の質問通告文・部内検討資料は絶対に投げません
...
議会期のたびに答弁原稿の作成で土日が消える議会事務局・答弁作成担当の人。

攻め・守り・折衷の 3 案を出すプロンプトを本数多く揃え、文体パターンや失敗事例まで踏まえて即戦力化したい自治体職員に向いた内容です。

**この続きで読めること**

> - 実用プロンプト 15 本完全版 (本記事の 5 本 + 追加 10 本、コピペで使えます)
...
- **教育分野** — いじめ対応・GIGA スクール関連で「子どもの安全配慮」と「家庭との連携」が必須論点です
- **福祉分野** — 「対象者数の現状」「他制度との連携」「予算規模」の 3 点が定型構造です
- **防災・危機管理** — 「想定被害」「過去事例」「マニュアル整備状況」の 3 段構成です
- **産業振興** — 「対象事業者数」「補助金実績」「他自治体ベンチマーク」が定番です
- **財政** — 「収支見通し」「基金残高」「中期財政計画との整合」が必須項目です
...
- **数値の誤記** — 過去答弁で「50 手続き」と言っていたものが新答弁で「45 手続き」になり決裁差し戻しになりました
- **過去発言との矛盾** — 首長の所信表明と方向性が逆になり首長レビューで指摘されました
- **文体の浮き上がり** — 民間ビジネス語彙が混入し部長から差し戻しになりました
- **ハルシネーション** — 存在しない条例名・統計名を AI が生成し、提出前チェックで発覚しました
- **政治的不適切** — 攻めすぎた表現が他会派批判と取られかねない内容になりました
...
本記事で紹介した 4 つのスキル (`draft-3-options` / `style-check` / `anticipate-rebuttal` / `integrity-check`) を提供します。

加えて 3 つの Subagents (`opposition-member` / `ruling-member` / `citizen-observer`) の完全ファイルも zip でダウンロード可能です。

自分の自治体に合わせて編集するベースとして利用してください。
