...
is_paid: true
price_jpy: 3980
tags: [Excel, 都道府県, 地図, データ可視化, コロプレス, 統計]
published: false
source: product-factory (deterministic draft — 公開前に critic/人間レビューが必要)
---

# 数値を貼るだけの47都道府県コロプレス地図Excel

## こんな作業をしていませんか
47都道府県の数値を貼るだけで、色分けされた日本地図をExcelで作りたい——そんなときに、そのまま使えるテンプレートと手順をまとめました。

## この記事で手に入るもの
- 数値を貼るだけの47都道府県コロプレス地図：数値貼付だけで都道府県地図を自動着色する
- Excel テンプレートなので、47 都道府県ぶんのデータを差し替えるだけで自分のテーマに応用できます。

## 完成イメージ
![完成イメージ：数値を貼るだけの47都道府県コロプレス地図Excel](images/completion.png)
...
- 対応環境: Excel 365 (Windows / Mac) / 地図の新規作成・データ追加にオンライン接続が必要
- 形式: xlsx
...
## 添付ファイル
- C-01 / SOURCES.csv（csv・約1KB）（実機検証前のため暫定）
- C-01 / LICENSE-ja.txt（plain・約1KB）（実機検証前のため暫定）
- C-01 / data.csv（csv・約2KB）（実機検証前のため暫定）
- C-01 / product.xlsx（vnd.openxmlformats-officedocument.spreadsheetml.sheet・約11KB）（実機検証前のため暫定）
- C-01 / manual.pdf（pdf・約38KB）（実機検証前のため暫定）
