...
is_paid: true
price_jpy: 3980
tags: [都道府県, PowerPoint, プレゼン資料, 統計, データ可視化]
published: false
...
## こんな作業をしていませんか
都道府県データを社内外にわかりやすく説明するスライド一式を用意したい——そんなときに、そのまま使えるテンプレートと手順をまとめました。

## この記事で手に入るもの
- 都道府県データ説明スライド20枚セット：都道府県データを社内外に説明する
- PowerPoint テンプレートなので、47 都道府県ぶんのデータを差し替えるだけで自分のテーマに応用できます。

## 完成イメージ
![完成イメージ：都道府県データ説明スライド20枚セット](images/completion.png)
...
## 添付ファイル
- B-01 / SOURCES.csv（csv・約1KB）（実機検証前のため暫定）
- B-01 / LICENSE-ja.txt（plain・約1KB）（実機検証前のため暫定）
- B-01 / data.csv（csv・約2KB）（実機検証前のため暫定）
- B-01 / product.pptx（vnd.openxmlformats-officedocument.presentationml.presentation・約6,259KB）（実機検証前のため暫定）
- B-01 / manual.pdf（pdf・約37KB）（実機検証前のため暫定）
