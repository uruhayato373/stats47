...
is_paid: true
price_jpy: 5980
tags: [Excel, 人口, 人口移動, 都道府県, データ分析, 統計]
published: false
source: product-factory (deterministic draft — 公開前に critic/人間レビューが必要)
---

# 人口構成・人口移動を分析するExcelテンプレート

## こんな作業をしていませんか
人口ピラミッドや転入転出の流れを都道府県別に分析したい——そんなときに、そのまま使えるテンプレートと手順をまとめました。

## この記事で手に入るもの
- 人口構成・人口ピラミッド：年齢別・男女別の人口構成を可視化する
- 転入転出・人口移動分析：純移動・流入元・流出先を集計分析する
- Excel テンプレートなので、47 都道府県ぶんのデータを差し替えるだけで自分のテーマに応用できます。

## 完成イメージ
![完成イメージ：人口構成・人口移動を分析するExcelテンプレート](images/completion.png)
（この記事のExcel テンプレートで作れる成果物のイメージです。表紙・図版は images-plan に沿って用意します。）

## 対象者・対象外
- 想定読者: 自治体・公務員・調査担当・移住支援
...
## 添付ファイル
- C-09 / SOURCES.csv（csv・約1KB）（実機検証前のため暫定）
- C-09 / LICENSE-ja.txt（plain・約1KB）（実機検証前のため暫定）
- C-09 / data.csv（csv・約2KB）（実機検証前のため暫定）
- C-09 / product.xlsx（vnd.openxmlformats-officedocument.spreadsheetml.sheet・約11KB）（実機検証前のため暫定）
- C-09 / manual.pdf（pdf・約37KB）（実機検証前のため暫定）
- C-10 / SOURCES.csv（csv・約1KB）（実機検証前のため暫定）
- C-10 / LICENSE-ja.txt（plain・約1KB）（実機検証前のため暫定）
- C-10 / data.csv（csv・約2KB）（実機検証前のため暫定）
- C-10 / product.xlsx（vnd.openxmlformats-officedocument.spreadsheetml.sheet・約11KB）（実機検証前のため暫定）
- C-10 / manual.pdf（pdf・約37KB）（実機検証前のため暫定）
