...
is_paid: true
price_jpy: 2980
tags: [都道府県, 統計データ, データパック, オープンデータ, eStat, 統計]
published: false
...
## こんな作業をしていませんか
都道府県の基礎統計を出典つきの一覧データとしてまとめて手に入れたい——そんなときに、そのまま使えるテンプレートと手順をまとめました。

## この記事で手に入るもの
- 47都道府県基礎統計データブック：人口・面積・世帯・所得を一冊で把握する
- データパックなので、47 都道府県ぶんのデータを差し替えるだけで自分のテーマに応用できます。

## 完成イメージ
![完成イメージ：47都道府県 基礎統計データブック](images/completion.png)
...
## 添付ファイル
- D-01 / SOURCES.csv（csv・約1KB）（実機検証前のため暫定）
- D-01 / LICENSE-ja.txt（plain・約1KB）（実機検証前のため暫定）
- D-01 / data.csv（csv・約2KB）（実機検証前のため暫定）
- D-01 / product.xlsx（vnd.openxmlformats-officedocument.spreadsheetml.sheet・約11KB）（実機検証前のため暫定）
- D-01 / manual.pdf（pdf・約38KB）（実機検証前のため暫定）
