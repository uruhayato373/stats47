...
- ドラフト群: `docs/31_note記事原稿/koumuin-claude-code/`
...
MVP 5 本公開後、`/fetch-note-metrics` で CVR と PV を測定し、未完了の次アクションだけを `.claude/todo/improvements.md` にID付きで記録する。
...
集計履歴は `/fetch-note-metrics` のreferenceへ保存し、未完了の次アクションだけを `.claude/todo/improvements.md` に記録する。
...
撤退判定は本書の条件と実測を突合し、「続行 / 縮小 / 撤退」を明示する。続行時の未完了策だけを `.claude/todo/improvements.md` に残し、撤退・完了履歴はGitに委ねる。
