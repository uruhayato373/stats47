...
MVP 5 本公開後、`/fetch-note-metrics` で CVR と PV を測定 → `docs/04_レビュー/2026-06-note-koumuin-mvp.md` に判定を記録する。
...
集計結果は `docs/04_レビュー/YYYY-MM-note-koumuin.md` に月次レビューとして記録する。
...
撤退判定はすべて `docs/04_レビュー/YYYY-MM-note-koumuin.md` の月次レビューで根拠とともに記録する。「なんとなく続けて気づいたら 6 ヶ月たっていた」を避けるため、各タイミングで「続行 / 縮小 / 撤退」のいずれかを明示する。
...
- 月次レビュー: `docs/04_レビュー/YYYY-MM-note-koumuin.md` (M1 / M3 / M6 で必須)
