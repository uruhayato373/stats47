...
| `.github/workflows/oauth-token-health-check.yml` | `0 0 * * 1` | 月曜 09:00 JST | YouTube/AdSense OAuth refresh token の健康確認、失効検知時 `[OAuth Alert]` Issue 起票 | GitHub Issue (label: `oauth-alert`) | 2026-05-03 追加 |
| `.github/workflows/snapshot-freshness-check.yml` | `0 0 * * 0` | 日曜 09:00 JST | R2 snapshot prefix の最終更新時刻と `snapshots.config.json` の `maxAgeDays` を比較、stale 検知で `[Snapshot Stale]` Issue 起票 | GitHub Issue (label: `snapshot-stale`) | 2026-05-03 追加 (PR-6) |
| `.github/workflows/post-instagram-scheduled.yml` | `3 0 * * *` | 毎日 09:03 JST | `.claude/state/instagram-w18-schedule.json` 参照、今日と一致するエントリがあれば IG 投稿（IG API 予約非対応の代替） | Instagram | #127 |
...
|---|---|---|---|---|
| ~~`com.stats47.backup-d1`~~ | ~~毎日 03:47~~ | **2026-05-03 廃止**。リモート D1 撤廃 (Phase 10) で対象が消失。`launchctl unload ~/Library/LaunchAgents/com.stats47.backup-d1.plist` で停止推奨 | — | — |
...
| ~~`com.stats47.post-instagram-w18`~~ | ~~毎日 09:03~~ | **2026-04-26 撤去**。GitHub Actions `post-instagram-scheduled.yml` に移行（Mac 起動依存解消） | — | #127 |
