...
## 正方形クロップ対策（OGP セーフゾーン規約）

### 背景

外部プラットフォーム（note.com のリンクカード等）が OGP 画像を**正方形にクロップ**して表示するケースがある。stats47 の OGP 画像は 1200x630 のため、正方形表示時に左右が大幅にカットされ、情報が欠落する。

### プラットフォーム別クロップ仕様（参考）

| プラットフォーム | OGP 画像の表示 | クロップ |
|---|---|---|
| X (Twitter) | `summary_large_image` で横長表示 | 上下が少しカット |
| Facebook | 横長表示 | ほぼそのまま |
| LINE | 横長表示 | ほぼそのまま |
| note.com リンクカード | **正方形表示** | **左右が大幅カット（中央 630x630）** |
| Discord | 横長表示 | ほぼそのまま |

### プラットフォーム別に別画像を出し分けられるか

**できない。** OGP 仕様（Open Graph Protocol）にはプラットフォーム別の画像指定機能がない。すべてのプラットフォームが同じ `og:image` を取得するため、note.com だけに正方形画像を返す標準的な方法は存在しない。

> User-Agent で note.com のクローラーを判定して出し分ける方法は技術的に可能だが、クローラー識別が不安定で保守コストが高いため非推奨。

### 対策：正方形セーフエリアの遵守

`OgpSafeZone` コンポーネント（`apps/remotion/src/shared/components/layouts/OgpSafeZone.tsx`）が**中央 630x630 の正方形セーフエリア**をガイド表示する。
...
2. **ロゴ・ウォーターマーク** → セーフエリア内に配置（左端 `x < 285` や右端 `x > 915` は正方形クロップで消える）
...
```tsx
// showGuides={true} で正方形クロップ境界を赤線で表示
<OgpSafeZone showGuides={true}>
  {/* OGP コンテンツ */}
</OgpSafeZone>
```

Remotion Studio で `showGuides` を有効にし、赤い境界線の内側にすべての重要要素が収まっているか目視確認する。

### 既知の問題

| コンポーネント | 問題 | 対応状況 |
|---|---|---|
| `RankingHeroOgp` | LogoWatermark が `left: 40`（セーフエリア外）| 未修正 |
| その他 OGP | 未監査 | 要確認 |
