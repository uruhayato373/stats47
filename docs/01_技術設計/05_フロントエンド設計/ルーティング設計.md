...
# 新機能（Phase 2）
/{category}/{subcategory}/timeseries/{rankingKey}  # 時系列分析
/{category}/{subcategory}/map/{rankingKey}         # 地図可視化（コロプレス地図）
/{category}/{subcategory}/correlation              # 相関分析
...
5. **新機能の追加**: `timeseries/`, `map/`, `correlation/` でデータ分析機能を提供
6. **e-Stat API 関連ページの移動**: 管理画面配下に移動してセキュリティ向上
7. **開発ツールセクションの追加**: 管理画面内に開発ツールを集約
...
| -                                                       | `/population/basic-population/timeseries/total-population` | 人口の時系列分析                 |
| -                                                       | `/population/basic-population/map/total-population`        | 人口のコロプレス地図             |
| -                                                       | `/population/basic-population/correlation`                 | 人口と GDP の相関分析            |
...
│           │       └── page.tsx       # エリア詳細 ✅
│           ├── timeseries/            # 時系列分析機能 ❌
│           │   ├── layout.tsx         # 時系列専用レイアウト ❌
│           │   ├── page.tsx           # 時系列分析一覧 ❌
│           │   └── [rankingKey]/
│           │       ├── page.tsx       # 時系列詳細 ❌
│           │       └── loading.tsx    # ローディング状態 ❌
│           ├── map/                   # 地図可視化機能 ❌
│           │   ├── layout.tsx         # 地図専用レイアウト ❌
│           │   ├── page.tsx           # 地図可視化一覧 ❌
│           │   └── [rankingKey]/
│           │       ├── page.tsx       # 地図詳細 ❌
│           │       └── loading.tsx    # ローディング状態 ❌
│           └── correlation/           # 相関分析機能 ❌
│               ├── layout.tsx         # 相関分析専用レイアウト ❌
│               ├── page.tsx           # 相関分析ページ ❌
│               └── loading.tsx        # ローディング状態 ❌
│
├── admin/                              # 管理画面（認証必須）✅
...
```typescript
import { Metadata } from "next";
import { validateSubcategoryOrThrow } from "@/infrastructure/category/subcategory-validator";
...
#### [category]/[subcategory]/timeseries/layout.tsx
...
export const metadata: Metadata = {
  title: { template: "%s 時系列分析 | 統計で見る都道府県" },
  openGraph: {
    type: "article",
  },
};
...
#### [category]/[subcategory]/map/layout.tsx
...
export const metadata: Metadata = {
  title: { template: "%s 地図可視化 | 統計で見る都道府県" },
  openGraph: {
    type: "article",
  },
};
...
#### [category]/[subcategory]/correlation/layout.tsx
...
export const metadata: Metadata = {
  title: { template: "%s 相関分析 | 統計で見る都道府県" },
  openGraph: {
    type: "article",
  },
};
...
```typescript
// components/atoms/DataAttribution.tsx
    ...
    <div className="data-attribution">
      <details className={showDetails ? "open" : ""}>
        ...
        <div className="attribution-list">
          {sources.map((source) => (
            <div key={source.id} className="attribution-item">
...
```typescript
// [category]/[subcategory]/map/[rankingKey]/page.tsx
import { DataAttribution } from "@/components/atoms/DataAttribution";

export default async function MapVisualizationPage({ params }: PageProps) {
  const { category, subcategory, rankingKey } = await params;
  ...
  return (
    <div className="map-visualization-page">
      <ChoroplethMap rankingKey={rankingKey} />
      <DataAttribution sources={dataSources} showDetails={true} />
    </div>
  );
}
```
...
2. **レイアウトファイル実装** ⚠️ 部分実装
   - 統計ページ共通レイアウト ❌
   - ランキング専用レイアウト ❌
   - エリア専用レイアウト ❌

### Phase 2: ページリファクタリング（2-3 週間）⚠️ 部分実装
...
### Phase 2.5: 新機能のルーティング実装（2-3 週間）❌ 未実装

5. **時系列分析機能のルーティング** ❌

   - `timeseries/` ディレクトリ構造の作成 ❌
   - 時系列専用レイアウトの実装 ❌
   - 時系列分析ページの実装 ❌
...
7. **相関分析機能のルーティング** ❌
   ...
   - `correlation/` ディレクトリ構造の作成 ❌
   - 相関分析専用レイアウトの実装 ❌
   - 相関分析ページの実装 ❌

### Phase 3: 最適化（1-2 週間）⚠️ 部分実装

8. **静的生成パラメータの実装** ⚠️

   - `generateStaticParams` 関数の実装 ⚠️
   - 再生成設定の最適化 ⚠️
   - 新機能の静的生成対応 ❌
...
- **レイアウト確認**: 各機能のレイアウト表示
- **新機能テスト**: 時系列分析、地図可視化、相関分析の動作確認
- **データソース表記確認**: 各ページでの適切な表記表示
...
### 10.1 カテゴリページ（サブカテゴリ一覧）

`[category]/page.tsx` では、shadcn/ui の`Item`コンポーネントを使用してサブカテゴリ一覧を表示します。

**特徴**:

- サブカテゴリをカード形式で一覧表示
- クリック可能なリンクとして実装（`asChild`プロパティ使用）
- アイコン、タイトル、説明文を含む
- レスポンシブデザイン対応
...
<Item variant="outline" asChild>
  <a href={`/${category}/${subcategory.id}`}>
    <ItemContent>
      ...
      <ItemDescription>{subcategory.description}</ItemDescription>
    </ItemContent>
    <ItemActions>
...
`[category]/[subcategory]/page.tsx` では、dashboard と ranking へのナビゲーションを提供します。

**表示項目**:

1. **ランキング**: 都道府県別のランキング表示
2. **地域別ダッシュボード**: 全国および各都道府県の詳細データ

**実装パターン**:

- `Item`コンポーネントでカード形式の一覧
- アイコン（`ItemMedia`）で機能を視覚的に表現
- `asChild`プロパティでリンクとして機能
...
```
Item (variant="outline", asChild)
├── ItemMedia (variant="icon")     # アイコン表示
├── ItemContent                    # コンテンツ領域
│   ├── ItemTitle                  # タイトル
│   └── ItemDescription            # 説明文
└── ItemActions                    # アクション領域（矢印アイコン）
```
...
- **カテゴリページ**: カテゴリ名をタイトルに設定
- **サブカテゴリページ**: サブカテゴリ名と説明を設定
- **SEO 最適化**: 検索エンジン向けの最適化
...
- [shadcn/ui Item コンポーネント](https://ui.shadcn.com/docs/components/item)
