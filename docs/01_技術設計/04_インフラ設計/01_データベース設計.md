...
| カラム名      | データ型 | 制約        | デフォルト値      | 説明       |
| ------------- | -------- | ----------- | ----------------- | ---------- |
| category_name | TEXT     | PRIMARY KEY | -                 | カテゴリ名 |
...
**設計ノート**: `category_name` を主キーとして使用（例: "population", "economy"）
...
| カラム名         | データ型 | 制約        | デフォルト値      | 説明           |
| ---------------- | -------- | ----------- | ----------------- | -------------- |
| subcategory_name | TEXT     | PRIMARY KEY | -                 | サブカテゴリ名 |
| name             | TEXT     | NOT NULL    | -                 | 表示名         |
| category_name    | TEXT     | NOT NULL FK | -                 | カテゴリ名     |
| display_order    | INTEGER  | -           | 0                 | 表示順         |
| created_at       | DATETIME | -           | CURRENT_TIMESTAMP | 作成日時       |
| updated_at       | DATETIME | -           | CURRENT_TIMESTAMP | 更新日時       |

**外部キー**: `category_name` → `categories(category_name)` ON DELETE CASCADE

**インデックス**:
- `idx_subcategories_category` ON subcategories(category_name)
- `idx_subcategories_display_order` ON subcategories(display_order)

**設計ノート**: `subcategory_name` を主キーとして使用（例: "basic-population", "labor-force"）
...
> **重要**: ランキング値データは **R2 ストレージ** に保存します。D1にはranking_valuesテーブルは作成しません。
>
> **保存先**: `ranking/{ranking_key}/{area_type}/{time_code}.json`
>
> **理由**:
> - D1の容量制限（10GB）を考慮
> - 大量データ（都道府県×市区町村×年度×項目）をR2で効率的に管理
> - R2の低コスト・大容量特性を活用
...
**設計のポイント**:
1. **categories, subcategories**: キーベース主キー（`category_name`, `subcategory_name`）
2. **ranking_items, ranking_groups**: キーベース主キー（`ranking_key`, `group_key`）
3. **ranking_values**: D1には保存せず、R2ストレージを使用
...
- **v3.1.0** (2025-10-29): ローカルデータベース(main.sql)に合わせて更新
  - categories/subcategoriesをキーベース主キーに変更
  - ranking_items/ranking_groupsをキーベース主キーに変更
  - geo_shapesテーブルを削除（現在未使用）
  - widget_templatesテーブルを追加
  - ER図を最新状態に更新
