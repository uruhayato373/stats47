...
├─────────────────────────────────────────────────────────────┤
│  Cloudflare D1 (SQLite — local 編集元のみ。production の    │
│                  remote D1 は 2026-04-29 削除済 Phase 10)   │
│  ├── 3 層モデル: sources / indicators / observations       │
│  ├── 派生: area_profiles / correlations / ai_content        │
│  │           indicator_tags                                  │
│  ├── マスタ: prefectures / cities / ports / fishing_ports  │
│  │           categories / tags / surveys                    │
│  ├── コンテンツ: articles / article_tags                    │
│  │               page_components / page_component_assignments│
│  ├── 運用: estat_metainfo (catalog + 運用、status 列で区別) │
│  │         affiliate_ads / sns_posts                        │
│  └── (計 22 テーブル、2026-05-04 時点)                      │
└─────────────────────────────────────────────────────────────┘
                                ↓
┌─────────────────────────────────────────────────────────────┐
│                    External Services                        │
...
| 環境            | データソース  | 接続方法                    | 用途                      |
| --------------- | ------------- | --------------------------- | ------------------------- |
| **development** | ローカル D1   | `.local/d1/v3/d1/miniflare-D1DatabaseObject/<hash>.sqlite` | ローカル開発・スクリプト |
| **production**  | Cloudflare R2 (snapshot JSON) | `https://storage.stats47.jp/app/...` | 本番配信 (D1 binding なし) |

**重要**: production app は 2026-04-29 (Phase 10) 以降、Cloudflare D1 binding を持たない。すべての app データ read は R2 snapshot 経由。D1 はローカルでバッチ・編集に利用するのみ (リモート D1 は削除済み)。
...
### 主要テーブル構成 (2026-05-04 時点、22 テーブル)

| 役割 | テーブル |
|---|---|
| 3 層モデル | `sources` / `indicators` / `observations` |
| 派生 (R2 export 経由で公開) | `area_profiles` / `correlations` / `ai_content` / `indicator_tags` |
| エンティティマスタ | `prefectures` / `cities` / `ports` / `fishing_ports` |
| 分類マスタ | `categories` / `tags` / `surveys` |
| コンテンツ | `articles` / `article_tags` / `page_components` / `page_component_assignments` |
| e-Stat 統合カタログ | `estat_metainfo` (status='candidate'/'registered' で区別) |
| 運用 | `affiliate_ads` / `sns_posts` |
...
**アーキテクチャ**:
- **データベース層**: `page_components` (component 定義) + `page_component_assignments` (page_type / page_key で割当)
  - page_type: `theme` / `area-category` / `area` / `ranking` / `city-category`
- **配信**: `snapshots/page-components/all.json` → R2 経由で production app に配信
- **編集**: ローカル D1 を直接 INSERT / UPDATE → `/sync-snapshots` で再生成
