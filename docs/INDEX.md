...
| 知りたいこと                                       | 場所                                                                                           |
| -------------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| **次に何をやるか (TODO)**                          | [`todo/`](todo/00_運用ガイド.md) — 連番7ファイル（運用、受信箱、月、週、改善、機能、指標）           |
| **前セッションからの残タスク**                     | [`todo/`](todo/00_運用ガイド.md) — 該当バックログを参照                                               |
| 今月・今週の計画                                   | `todo/02_今月の重点.md` `todo/03_今週の計画.md`                                                 |
...
| ディレクトリ           | 役割                                                                                                                                            | 運用                                                                                                |
| ---------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| `todo/`                | **TODO と現在計画の単一入口**                                                                                                                   | `00_運用ガイド.md` から連番順に辿る。完了は削除、月次・週次計画は上書き                           |
...
1. **やることを思いついた** → `todo/01_未整理タスク.md` に 1 行 append (triage で各バックログへ)
2. **セッションの残タスクを記録する** → `todo/` の該当バックログへ直接反映
3. **戦略・要件の変更** → `00_プロジェクト管理/` 該当ファイルを Edit (新規ファイル追加禁止)
4. **月次計画** → `/monthly-plan` スキルが `todo/02_今月の重点.md` を上書き（月初）
5. **週次計画・レビュー** → `/weekly-plan` は `todo/03_今週の計画.md` を上書き、`/weekly-review` は skill reference に保存
6. **改善施策の記録** → active 中だけ `todo/04_改善バックログ.md` の行を追加・更新し、効果判定後は詳細ログへ結果を残して行を削除
