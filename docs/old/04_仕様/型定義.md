...
2. **ドメイン固有の型定義**
   ...
   - `/src/lib/estat-api/types/` - e-Stat API 関連の型（statsdata.ts に統合）
   - `/src/lib/auth/` - 認証関連の型
...
### 例 1: e-Stat API 統計データ型定義（拡張版）

#### ファイル構成

```
src/lib/estat-api/types/
├── statsdata.ts                    # 統計データ型定義（統合版）
...
`src/lib/estat-api/types/statsdata.ts`

```typescript
/**
 * 整形されたe-STATデータ（拡張版）
 * CMS/Webサイト構築に必要な追加情報を含む
 */
...
このセクションでは、型定義管理戦略を実際のプロジェクトに適用するための具体的な作業手順を、チェックリスト形式で提供します。

### 作業の進め方

各フェーズは独立して実行可能ですが、順番に進めることを推奨します。各ステップの完了後にチェックマークを付けて進捗を管理してください。

---

### 【フェーズ 1】準備作業（所要時間: 1 日）

#### ステップ 1-1: プロジェクト全体の型定義を調査する

```bash
# 型定義ファイルの一覧を取得
find src -name "types.ts" -o -name "*.types.ts" -o -path "*/types/*" > docs/type-files-list.txt

# 型定義の総数をカウント
grep -r "export interface\|export type" src/ | wc -l > docs/type-count.txt

# 型定義のインポート状況を調査
grep -r "import.*from.*types" src/ > docs/type-imports-analysis.txt
```

**チェックリスト:**

- [ ] 型定義ファイルの一覧を作成した（`docs/type-files-list.txt`）
- [ ] 型定義の総数を確認した（`docs/type-count.txt`）
- [ ] インポート状況を分析した（`docs/type-imports-analysis.txt`）
- [ ] 重複している型定義を特定した
- [ ] 使用頻度の高い型を特定した

#### ステップ 1-2: 新しいディレクトリ構造を作成する
...
mkdir -p src/types/visualization
mkdir -p src/types/database
mkdir -p src/types/external

# ドメイン型定義ディレクトリの作成
mkdir -p src/domains/estat/types
mkdir -p src/domains/ranking/types
...
touch src/types/visualization/index.ts
touch src/types/database/index.ts
touch src/types/external/index.ts
touch src/domains/estat/types/index.ts
touch src/domains/ranking/types/index.ts
touch src/domains/auth/types/index.ts
```

**チェックリスト:**

- [ ] `src/types/` ディレクトリ構造を作成した
- [ ] `src/domains/*/types/` ディレクトリ構造を作成した
- [ ] 各ディレクトリに `index.ts` を作成した
- [ ] ディレクトリ構造を確認した（`tree src/types`）

#### ステップ 1-3: マイグレーション計画書を作成する

```bash
# マイグレーション計画書のテンプレートを作成
touch docs/type-migration-plan.md
```
...
- フェーズ 1: 準備作業（1 日）- [完了日: YYYY-MM-DD]
- フェーズ 2: グローバル型定義の移行（2-3 日）- [完了予定: YYYY-MM-DD]
- フェーズ 3: ドメイン型定義の移行（2-3 日）- [完了予定: YYYY-MM-DD]
  ...
...
| 現在のパス                          | 移行先のパス                            | 優先度 | 状態   |
| ----------------------------------- | --------------------------------------- | ------ | ------ |
| src/types/prefecture.ts             | src/types/models/prefecture.ts          | 高     | 未着手 |
| src/lib/estat/types/raw-response.ts | src/domains/estat/types/raw-response.ts | 高     | 未着手 |
...
- [ ] マイグレーション計画書を作成した（`docs/type-migration-plan.md`）
- [ ] 移行対象ファイル一覧を記載した
- [ ] 移行スケジュールを決定した
- [ ] リスク評価を実施した
- [ ] ロールバック計画を策定した
...
### 【フェーズ 2】グローバル型定義の移行（所要時間: 2-3 日）

#### ステップ 2-1: 共通型定義を作成する

```bash
# プリミティブ型の作成
touch src/types/common/primitives.ts
# ユーティリティ型の作成
touch src/types/common/utility.ts
# ページネーション型の作成
touch src/types/common/pagination.ts
```

**`src/types/common/primitives.ts` に追加:**
...
**`src/types/common/utility.ts` に追加:**
...
- [ ] `primitives.ts` を作成し、基本的な型エイリアスを定義した
- [ ] `utility.ts` を作成し、ユーティリティ型を定義した
- [ ] `pagination.ts` を作成し、ページネーション型を定義した
- [ ] `common/index.ts` で re-export を設定した
- [ ] TypeScript コンパイルエラーがないことを確認した（`npx tsc --noEmit`）

#### ステップ 2-2: モデル型定義を移行する

```bash
# ユーザーモデルの作成
touch src/types/models/user.ts
# 都道府県モデルの作成
touch src/types/models/prefecture.ts
# ランキングモデルの作成
touch src/types/models/ranking.ts
```

**`src/types/models/user.ts` に追加:**
...
**`src/types/models/prefecture.ts` に追加:**
...
- [ ] `user.ts` を作成し、ユーザー関連の型を定義した
- [ ] `prefecture.ts` を作成し、都道府県関連の型を定義した
- [ ] `ranking.ts` を作成し、ランキング関連の型を定義した
- [ ] 各型に JSDoc コメントを追加した
- [ ] `models/index.ts` で re-export を設定した
- [ ] TypeScript コンパイルエラーがないことを確認した

#### ステップ 2-3: API 型定義を作成する

```bash
touch src/types/api/request.ts
touch src/types/api/response.ts
touch src/types/api/error.ts
```

**`src/types/api/error.ts` に追加:**
...
- [ ] `error.ts` を作成し、エラー型を定義した
- [ ] `response.ts` を作成し、レスポンス型を定義した
- [ ] `request.ts` を作成し、リクエスト型を定義した
- [ ] `api/index.ts` で re-export を設定した
- [ ] TypeScript コンパイルエラーがないことを確認した

#### ステップ 2-4: 集約エクスポートファイルを作成する
...
- [ ] `src/types/index.ts` を作成した
- [ ] すべての型定義を re-export した
- [ ] インポートパスが正しいことを確認した
- [ ] TypeScript コンパイルエラーがないことを確認した
- [ ] テストコードを実行して動作確認した（`npm test`）

---

### 【フェーズ 3】ドメイン型定義の移行（所要時間: 2-3 日）

#### ステップ 3-1: e-Stat ドメインの型定義を移行する

```bash
# 既存の e-Stat 型定義をコピー（バックアップを兼ねる）
cp -r src/lib/estat/types src/domains/estat/types-backup

# 新しい場所に移動（または src/lib/estat が存在しない場合は新規作成）
mkdir -p src/domains/estat/types
```
...
```bash
# 例: raw-response.ts を移行
touch src/domains/estat/types/raw-response.ts
touch src/domains/estat/types/meta-response.ts
touch src/domains/estat/types/list-response.ts
touch src/domains/estat/types/parameters.ts
touch src/domains/estat/types/processed.ts
```

**`src/domains/estat/types/parameters.ts` に追加:**
...
**`src/domains/estat/types/index.ts` に追加:**
...
- [ ] e-Stat 型定義ファイルをバックアップした
- [ ] `src/domains/estat/types/` に型定義を移行した
- [ ] グローバル型定義を正しくインポートした（`@/types`）
- [ ] `domains/estat/types/index.ts` で re-export を設定した
- [ ] TypeScript コンパイルエラーがないことを確認した

#### ステップ 3-2: ranking ドメインの型定義を移行する

```bash
mkdir -p src/domains/ranking/types
touch src/domains/ranking/types/ranking-item.ts
touch src/domains/ranking/types/visualization.ts
touch src/domains/ranking/types/navigation.ts
touch src/domains/ranking/types/index.ts
```

**チェックリスト:**

- [ ] ranking 型定義ディレクトリを作成した
- [ ] ランキング関連の型定義を移行した
- [ ] `domains/ranking/types/index.ts` で re-export を設定した
- [ ] TypeScript コンパイルエラーがないことを確認した

#### ステップ 3-3: 既存コードのインポートパスを更新する

```bash
# インポートパスの一括置換スクリプトを作成
touch scripts/migrate-imports.sh
chmod +x scripts/migrate-imports.sh
```

**`scripts/migrate-imports.sh` の内容:**

```bash
#!/bin/bash

echo "インポートパスの一括置換を開始します..."

# e-Stat 型のインポートパスを置換
find src -type f \( -name "*.ts" -o -name "*.tsx" \) -exec sed -i.bak \
  's|@/lib/estat/types|@/domains/estat/types|g' {} \;

# Prefecture 型のインポートパスを置換
find src -type f \( -name "*.ts" -o -name "*.tsx" \) -exec sed -i.bak \
  's|@/types/prefecture|@/types/models/prefecture|g' {} \;
...
- [ ] インポートパス置換スクリプトを作成した
- [ ] スクリプトを実行してインポートパスを更新した
- [ ] TypeScript コンパイルエラーがないことを確認した（`npx tsc --noEmit`）
- [ ] すべてのテストが通ることを確認した（`npm test`）
- [ ] 変更をコミットした（`git add . && git commit -m "型定義のインポートパスを更新"`）

---

### 【フェーズ 4】コンポーネント型定義の整理（所要時間: 2-3 日）

#### ステップ 4-1: コンポーネント型定義ファイルをリネームする

```bash
# types.ts を ComponentName.types.ts にリネーム
mv src/components/ranking/RankingClient/types.ts \
   src/components/ranking/RankingClient/RankingClient.types.ts

# DataTable コンポーネント
mv src/components/common/DataTable/types.ts \
   src/components/common/DataTable/DataTable.types.ts
```

**チェックリスト:**

- [ ] すべてのコンポーネント型定義ファイルをリネームした
- [ ] リネームしたファイルの一覧を記録した
- [ ] 次のステップでインポートパスを更新する準備ができた

#### ステップ 4-2: コンポーネント内のインポートパスを更新する

**例: `src/components/ranking/RankingClient/RankingClient.tsx`**
...
```bash
# コンポーネントディレクトリ内の types インポートを更新
find src/components -type f \( -name "*.tsx" -o -name "*.ts" \) -exec sed -i.bak \
  "s|from './types'|from './\$(basename \$(dirname {}))).types'|g" {} \;
```

**チェックリスト:**

- [ ] すべてのコンポーネントのインポートパスを更新した
- [ ] TypeScript コンパイルエラーがないことを確認した
- [ ] テストが通ることを確認した

#### ステップ 4-3: index.ts に re-export を追加する

**例: `src/components/ranking/RankingClient/index.ts`**
...
- [ ] すべてのコンポーネントの `index.ts` に型の re-export を追加した
- [ ] 外部から型をインポートできることを確認した
- [ ] TypeScript コンパイルエラーがないことを確認した

---

### 【フェーズ 5】インポートパスの一括更新と検証（所要時間: 1 日）

#### ステップ 5-1: TypeScript コンパイラで型エラーを確認する

```bash
# 型エラーがないか確認
npx tsc --noEmit

# エラーがある場合は、エラーメッセージを保存
npx tsc --noEmit 2>&1 | tee docs/type-errors.log
```
...
- [ ] TypeScript コンパイルエラーがないことを確認した
- [ ] エラーがある場合は、すべて修正した
- [ ] エラーログを保存した（`docs/type-errors.log`）
...
- [ ] ビルドが成功することを確認した
- [ ] ビルド時間が大幅に増加していないことを確認した
- [ ] バンドルサイズが大幅に増加していないことを確認した

---

### 【フェーズ 6】古いファイルの削除とクリーンアップ（所要時間: 1 日）

#### ステップ 6-1: 古い型定義ファイルをバックアップする

```bash
# バックアップディレクトリを作成
mkdir -p backup/old-types-$(date +%Y%m%d)

# 古いファイルをバックアップ
cp -r src/types/prefecture.ts backup/old-types-$(date +%Y%m%d)/ 2>/dev/null || true
cp -r src/lib/estat/types backup/old-types-$(date +%Y%m%d)/ 2>/dev/null || true
```

**チェックリスト:**

- [ ] 古い型定義ファイルをバックアップした
- [ ] バックアップが正しく作成されたことを確認した
- [ ] バックアップの場所を記録した

#### ステップ 6-2: 未使用の型定義ファイルを削除する
...
- [ ] ビルドが成功する
- [ ] 未使用の依存関係がない
- [ ] 循環参照がない（`npx madge --circular --extensions ts,tsx src/`）

---

### 【フェーズ 7】ドキュメント化と最終レビュー（所要時間: 1 日）

#### ステップ 7-1: 型定義ドキュメントを作成する

```bash
touch docs/types-documentation.md
```

**`docs/types-documentation.md` の内容:**
...
src/domains/_/types/ # ドメイン固有の型定義
src/components/_/Component.types.ts # コンポーネント固有の型定義
\`\`\`

## 型定義の配置ルール

1. グローバル型定義（`/src/types/`）

   - 複数のドメインで使用される型

2. ドメイン固有の型定義（`/src/domains/*/types/`）
...
```markdown
## [型定義リファクタリング] - YYYY-MM-DD

### Added

- 新しい型定義ディレクトリ構造を導入
- グローバル型定義（`/src/types/`）
- ドメイン固有の型定義（`/src/domains/*/types/`）
- コンポーネント固有の型定義
...
# コミット
git commit -m "型定義管理戦略の実装

- 新しい型定義ディレクトリ構造を導入
- グローバル型、ドメイン型、コンポーネント型を分離
- 命名規則を統一
- ドキュメントを更新
"

# プッシュ
git push origin feature/type-definition-refactoring
```
...
```bash
# 未使用の型定義を検出
npx ts-prune > docs/unused-types-$(date +%Y%m).txt

# 循環参照を検出
npx madge --circular --extensions ts,tsx src/ > docs/circular-deps-$(date +%Y%m).txt

# 依存関係をチェック
npx depcheck > docs/depcheck-$(date +%Y%m).txt
```
